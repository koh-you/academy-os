...
const { dispatch: dispatchExamAnalysisQuestionCountRoute } = createExamAnalysisQuestionCountRouteRegistry({
  confirmExamAnalysisQuestionCount,
  parseExamAnalysisQuestionCountConfirmRequest,
  readJsonBody,
  sendJson
...
const { dispatch: dispatchSpecialLectureApplicationRoute } = createSpecialLectureApplicationRouteRegistry({
  deleteSpecialLectureApplication,
  listSpecialLectureApplications,
  readJsonBody,
  sendJson,
...
// MV-2b: orchestration body lives in notificationSolapiReserveService.js. Kept as
// hoisted `function` wrappers (not `const` from destructuring) because
// createNotificationJobRouteRegistry({ reserveNotificationJobInSolapi, ... }) at
// line ~489 runs before this line executes -- function declarations are hoisted
// so the reference resolves; the wrapper body only runs later once
// notificationSolapiReserveService below has been initialized.
function reserveNotificationJobInSolapi(job, options) {
  return notificationSolapiReserveService.reserveNotificationJobInSolapi(job, options);
}

function reserveNotificationJobsInSolapi(jobs, options) {
  return notificationSolapiReserveService.reserveNotificationJobsInSolapi(jobs, options);
}

const notificationSolapiReserveService = createNotificationSolapiReserveService({
  cancelSolapiReservationGroup,
...
// MV-2c: getNotificationJobSolapiGroupId, normalizeSolapiStatus,
// isSolapiCanceledStatus, isSolapiFailedStatus, isSolapiCompleteStatus,
// getSolapiPrimaryMessage, getSolapiStatusDetail, getReconciledSolapiJobState,
// getLessonRecordStatusForSolapiResult, wait, and withSolapiRetry moved into
// notificationSolapiReconcileService.js as private (non-exported) helpers --
// each was used exclusively by the reconcile cluster (verified via grep before
// moving), so they co-locate with it rather than staying injected here.
...
// MV-2c: orchestration body lives in notificationSolapiReconcileService.js.
// Hoisted `function` wrappers, same reasoning as the MV-2b reserve wrappers
// above -- callers earlier in file order (route registry wiring, and
// dispatchDueNotificationJobs's internal call to reconcileDueSolapiNotificationJobs)
// resolve the hoisted name; the body only runs once notificationSolapiReconcileService
// has been constructed below.
function reconcileSolapiNotificationJobs(options) {
  return notificationSolapiReconcileService.reconcileSolapiNotificationJobs(options);
}

function reconcileDueSolapiNotificationJobs(options) {
  return notificationSolapiReconcileService.reconcileDueSolapiNotificationJobs(options);
}
...
const notificationSolapiReconcileService = createNotificationSolapiReconcileService({
  defaultSolapiAutoReconcileGraceMs,
  defaultSolapiAutoReconcileLookbackMs,
  defaultSolapiAutoReconcileRetryMs,
  getKoreaDayUtcRange,
...
// MV-2d: orchestration body lives in notificationSolapiDispatchService.js.
// Hoisted `function` wrapper for the same reason as MV-2b/2c: the route
...
const server = http.createServer(async (request, response) => {
  ...
  const effectiveTenantId = requestedTenantId
    ...
    : (auth.tenantId ?? null);
  request.__auth = { ...auth, tenantId: effectiveTenantId };
  ...
  if (await dispatchAttendanceRoute({ request, response, requestUrl })) return; if (await dispatchAttendanceRealtimeRoute({ request, response, requestUrl })) return;
