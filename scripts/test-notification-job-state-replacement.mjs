...
const emptyReplacementResult = replaceNotificationJobListRows(currentJobs, []);
assert.deepEqual(emptyReplacementResult, currentJobs);
assert.notEqual(emptyReplacementResult, currentJobs);
assert.deepEqual(replaceNotificationJobListRows([], replacementJobs), []);
...
for (const forbiddenHelperEffect of [
  ...
  assert.ok(
    ...
    `notification job replacement helper crossed a side effect: ${forbiddenHelperEffect}`
  );
}

console.log("notification job state replacement TARGET/CONTROL fixtures passed");
