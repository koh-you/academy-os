...
const dependencies = {
  ...
  getDateString: (value) => value.toISOString().slice(0, 10),
...
const datedModel = createNotificationHistoryViewModel({
  ...dependencies,
  historyDate: "2026-08-01",
  localNoticeJobs,
  notificationJobs
});

assert.deepEqual(datedModel.filteredNotificationJobs.map((job) => job.notificationJobId), ["future"]);
assert.deepEqual(datedModel.solapiResultTargets.map((job) => job.notificationJobId), ["confirm"]);
