...
check("64g early Sunday makeup lessons are not auto-deleted or hidden", !app.includes("/api/lessons?before=") && hasAll(app, ["isGeneratedExamLesson", '["preExam", "examSundayMakeup"].includes(lesson?.lessonType)', "lesson?.date >= academyOperationalStartDate || isGeneratedExamLesson"]));
