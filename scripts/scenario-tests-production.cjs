...
check("41b comment Alimtalk shows assignment status after attendance for parent and student", hasAll(app, ['createMessageLine("🏫 출결", attendance)', 'createMessageLine("✅ 과제 상태", getAssignmentStatusParentMessage(assignmentStatus))', 'createMessageLine("✅ 과제 상태", getAssignmentStatusParentMessage(base.assignmentStatus))']) && hasAll(notificationRoute, ['messageLine("🏫 출결", attendanceLabel(attendanceStatus))', 'messageLine("✅ 과제 상태", assignmentStatusMessage)', "assignmentStatus: payload.assignmentStatusMessage || payload.assignmentStatus"]) && !notificationRoute.includes('audience === "parent" ? payload.assignmentStatusMessage'));
