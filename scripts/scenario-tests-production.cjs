...
check("84d lesson notification plan controls default delay and no-send modes", hasAll(app, ["lessonNotificationPlans", "handleUpdateLessonNotificationPlan", "lessonNotificationPlan={lessonNotificationPlans[selectedLesson.lessonId] ?? { mode: \"default\" }}", "기본 예약", "30분 지연", "알림톡 없음", "initialSendTiming", "sendTiming === \"none\"", "즉시 발송", "const baseTime = getDayKey(lesson?.date) === \"sat\" ? \"18:30\" : \"22:30\""]) && hasAll(css, [".defaultScheduleHint", ".schedulePlanButton", "grid-template-columns: repeat(4, minmax(0, 1fr))"]));
