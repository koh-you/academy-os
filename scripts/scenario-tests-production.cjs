...
check("03b teacher account can be changed from settings", hasAll(app, ["teacherAccountSettings", "defaultTeacherAccountSettings", "onUpdateTeacherAccountSettings", "function saveTeacherAccount(event)", "currentPassword !== account.password", "password: nextPassword || currentPassword || current?.password || defaultTeacherAccountSettings.password", "계정 설정", "계정 저장"]) && hasAll(css, [".accountSettingsGrid", ".accountSettingsActions"]));
check("03b-2 teacher login falls back to local account when server rejects during migration", hasAll(app, ["const loginWithLocalTeacherAccount", "if (loginWithLocalTeacherAccount())", "Server teacher auth failed; falling back to local settings."]));
...
check("22j exam post target matching is tolerant and pre-opens upcoming exams", hasAll(app, ["function schoolMatchesStudent", "daysFromTodayToExam <= 7", "isOpen: daysFromTodayToExam <= 0", "시험 후 제출 예정"]));
