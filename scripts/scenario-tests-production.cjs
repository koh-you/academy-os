...
check("88 supplement save combines notification draft and lesson sync", hasAll(app, ["function handleSaveTask(task)", "const taskWithDraft = { ...task, notificationDraft: draft }", "onUpdateTask(task.makeupTaskId, \"notificationDraft\", draft)", "onScheduleTask(taskWithDraft)", "저장 및 수업일지 반영", "보충 내용 저장"]));
