...
// 같은 고사·학교·학년 행이 둘 이상이면 중복(제외 행·다른 고사는 세지 않는다) — 사람이 골라 지운다.
assert.deepEqual([...model.duplicateExamPrepIds].sort(), ["row-fallback-cycle", "row-visible"]);
...
// 2026-09-17: 학생의 학교·학년을 고쳐 연결이 끊긴 자동 생성 행은 지울 수 있어야 한다.
// 2026-09-19 · UI U3: 삭제는 푸터 왼쪽 끝 ⋯ 메뉴(tone danger)로 옮겼다. 연결 학생이 남은 자동 생성 행은
// 비활성 버튼 대신 같은 이유 문구를 그 자리에 보여준다(가드 조건 isAutoGenerated && !isOrphaned 는 그대로).
assert.match(editModalSource, /const isDeleteBlocked = isAutoGenerated && !isOrphaned;/);
assert.match(editModalSource, /isDeleteBlocked \? \([\s\S]*?연결된 학생이 있는 동안은 삭제해도 다시 생성됩니다[\s\S]*?<OverflowMenu[\s\S]*?label: "시험정보 삭제", onSelect: \(\) => onDeleteRow\?\.\(row\.examPrepId\), tone: "danger"/);
assert.doesNotMatch(editModalSource, /row\.source !== "학생DB 자동생성" \? \(/);
assert.match(centerSource, /isOrphaned=\{orphanedExamPrepIds\.has\(editingExamPrepRow\.examPrepId\) \|\| duplicateExamPrepIds\.has\(editingExamPrepRow\.examPrepId\)\}/);
assert.match(centerSource, /같은 학교·학년 행이 또 있음/);
