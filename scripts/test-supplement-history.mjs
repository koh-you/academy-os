...
const tasks = [
  ...
  { makeupTaskId: "canceled-1", studentId: "student-1", taskType: "absence_makeup", status: "canceled", cancellationMode: "keep_source_absence", canceledAt: "2026-07-20T13:00:00.000Z", sourceLabel: "7월 19일 결석" },
...
assert.deepEqual(allItems.map((item) => item.task.makeupTaskId), ["done-1", "canceled-1", "scheduled-1", "draft-1"]);
assert.deepEqual(allItems.map((item) => item.status), [
  { label: "보충 완료", tone: "done" },
  { label: "보강 취소 · 결석 유지", tone: "canceled" },
  { label: "일정 확정", tone: "scheduled" },
  { label: "진행 중", tone: "draft" }
]);
assert.equal(allItems[0].historyDate, "2026-07-21");
assert.equal(allItems[1].historyDate, "2026-07-20");
assert.equal(allItems[2].sourceLabel, "숙제 미완료");
assert.equal(allItems[3].sourceLabel, "보충 항목");
assert.equal(allItems[3].student, undefined);

assert.deepEqual(createSupplementHistoryItems({ ...dependencies, query: " 창동고 " }).map((item) => item.task.makeupTaskId), ["done-1", "canceled-1"]);
...
const recentTasks = selectRecentSupplementTasks({
  ...
  tasks: [
    ...
    {
      canceledAt: "2026-07-21T16:00:00.000Z",
      makeupTaskId: "canceled",
...
assert.deepEqual(
  ...
  ["canceled", "touched", "completed", "scheduled"]
);
assert.equal(getSupplementHistoryTimestamp(recentTasks[0]), "2026-07-21T16:00:00.000Z");
assert.equal(getSupplementHistoryDate(recentTasks[2]), "2026-07-21");
