...
export async function drawQa(canvas, pageWidth, pageHeight, boxes, filePath) {
  const scale = 100 / 72;
...
export function cropToCanvas(canvas, renderScale, box) {
  const sx = Math.round(box.x0 * renderScale);
  const sy = Math.round(box.y0 * renderScale);
  const sw = Math.max(1, Math.round((box.x1 - box.x0) * renderScale));
  const sh = Math.max(1, Math.round((box.y1 - box.y0) * renderScale));
...
export async function writeStacked(parts, filePath, gapPx) {
