...
const backendPlanPath = path.join(root, "docs", "archive", "backend-db-deployment-plan.md");
const productionChecklistPath = path.join(root, "docs", "archive", "first-time-production-checklist.md");
...
results.push(assert("19. test paper manager removes PageSnap import UI and keeps manual shelf controls", includesAll(app, ["function createDefaultProblemBooks", "function normalizeProblemBooks", "시험지 보관함", "유형트리", "testPaperKindTabs", "onDeleteBook", "dangerTextButton", "function buildSsenTypeCatalog", "ssenTypeIndex", "testProgressHeaderCell", "<details className=\"ssenUnitNode\" key={unit.id}>"]) && !app.includes("쎈 유형트리") && !app.includes("문제수 동기화") && !app.includes("onSyncProblemCounts") && !app.includes("PageSnap JSON") && !app.includes("createProblemBooksFromPageSnapJson") && !app.includes("createSsenCommonMath1PageSnapExample")));
...
results.push(assert("25. frontend API base URL is configurable for hosting", includesAll(apiClient, ["VITE_API_BASE_URL", "apiUrl(", "productionApiBaseUrl", "configuredApiBaseUrl", "getApiBaseUrl"]) && !app.includes('fetch("http://127.0.0.1:8787')));
