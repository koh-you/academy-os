...
- 시험관리 `시험정보`는 반별 필터와 별도로 `전체 반` 조회를 지원한다. 반·고사 필터 변경은 조회만 수행하며 미확인 시험행을 자동생성하지 않는다.
- 2026년 2학기 운영 시험정보에서 확인되지 않은 `학생DB 자동생성` 빈 행 12건을 감사 삭제했다. Supabase 재조회에서 대상 잔존 0건, 확인된 일정 11건 보존, 연결 시험대비 수업 0건을 확인했다.
