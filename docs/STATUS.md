...
- RPM 기하(806문): 검수 11 완료(내용 불일치 0 · yellow 52 · 조사 「를」 1 반영) → 조판 반영 19건 재렌더 중 → export 예정. RPM 미적분Ⅱ(1177문): review 조판 완료 · 검수 16 착수.
...
- 개념원리 공통수학1(797문 = 통번호 654 · 예제 143): `ingest-gn-hs.mjs` 보완 — 특강 뒤 연습문제 쪽이 특강 범위에 먹히던 것(차례 항목 전부를 경계로) · 특강 쪽의 확인체크(통번호 21)도 문항 · 예제 크롭 폭을 상자 오른쪽 테두리(열 밀도 · 87~89%W)까지(그래프가 테두리까지 그려짐 · 그림 66→107) · `pdfVectorFigures` 클립 사각형 반영 + 상자 조각/제목 띠 제외. 배치 35(단원별 · `gn-plan.mjs`) → 전사 진행 중(주간 한도 429 로 20개 중단 후 재실행).
