...
## 2026-09-15 베이직쎈 2·3단원 원천화 + 시험지 제작 탭

- PR #332(스캔 경로·LaTeX 오답은행 시범)를 main 에 병합했다(`41823fb1`). 이어서 branch `codex/latex-bank-ssen-cm2-u2u3-20260915`.
- 스캔 원천화: `ingest-scan-badges.mjs --pages 2-54 --unit-pages "2-15,16-32,33-54"` 로 1~3단원을 한 패키지(`output/problem-bank/ssen-basic-cm2`)로 다시 잘랐다 — 문항 420(01:104 · 02:143 · 03:173), 쪽마다 번호 연속(빠짐 0), 1단원 104문 id·상자 그대로. 고친 것: 단원 경계를 쪽 범위로 못 박는 `--unit-pages`, 유형 라벨 「018」이 「O18」로 읽혀 배지로 잡히던 것 제외, 배지 x 창(±8pt) 선택, 쪽 OCR 이 놓친 배지를 배지 띠 2배 재판독(psm 11·6)으로 되찾기(신뢰도 75 미만은 앞뒤 번호와 이어질 때만). 답지 2~37쪽 → 해설 372/420 · 답 줄 270 · 빠진 문항 48(`검수-필요.md`).
- LaTeX 오답은행: `items.json` 이 여러 단원(`units[]`)을 받고 `build.mjs` 가 단원별 장 머리·정답을 만든다(1단원 13쪽 재빌드 통과). 전사 규약 `latex-bank/ssen-basic-cm2/전사-가이드.md`, 배치 검사 `scripts/latex-bank/check-batch.mjs`, 그림 크롭 `crop-figure.mjs`. **2·3단원 316문 전사는 사람 결정 대기** — 전사 에이전트 8개가 모두 「시판 교재 원문 전면 복제」를 이유로 거부했다(맥락 추가 후 재시도도 동일). 본 세션은 저작권 위험을 사용자에게 알리고 확인을 받은 뒤 진행하기로 했다. 대안(오답지 = 원문 크롭 그대로 · 시험지 = 동형 변형 문항 LaTeX · 정독 자산 = 발문 요약)을 함께 제시했다.
- 시험지 제작: 오답관리 › 「시험지 제작」 탭(`BookWrongAnswerBoard mode="exam"`). 바구니(교재 바꿔도 유지·여러 교재 혼합) · 구역·유형 필터 · 제목·문항 순서(교재순/담은순/섞기)·총점 배점 분배·문항별 배점·출처 on/off·수험자 칸·정답표. `test:problem-bank-exam-model`, browser `문제은행 › 시험지 제작` 통과. `check:fast` 통과. 바구니 서버 저장(시험지 목록)은 다음 작업.
