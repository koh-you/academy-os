...
- **가로 2열**: 모달 640 → 1040px. 왼쪽은 수업 유형·유형별 패널·달력 색상·수업명/날짜/시작·종료, 오른쪽은 포함 학생(명단만 자체 스크롤). 유형 탭 8개를 3줄 176px → 2줄 108px 로 줄였다(버튼은 8개 그대로). 1100px 이하는 1열, 640px 이하는 기존대로.
- **고정 푸터**: 공용 `Modal` 에 `footer` 슬롯을 더해(`scrollable` + `modalHasFooter`) 본문만 스크롤하고 `[수업 취소][저장 경계 상태바][취소][✅ 수업 등록]` 이 항상 보인다. 확정 버튼 180px ≥ 취소 108px, 세 버튼 모두 44px 이상.
- **「큰 수업 틀」**: 폼에서 뺐다. 기능은 그대로 살아 있고(반을 고르면 수업명·시간·색상·명단을 한 번에 채운다) 포함 학생 헤더의 「반 불러오기」 + HelpTip 으로 자리만 옮겼다. 패치 함수·저장 payload 무변경.
- **시간·명단**: 시작 시간을 사람이 바꿀 때만 종료가 +3시간으로 따라간다(자정을 넘기면 23:59 로 묶음 — 저장 계약이 종료>시작을 요구). 학년 그룹별 전체 선택/해제 2버튼×학년수 → 헤더 토글 하나. **신규 등록 기본 선택은 전원 → 0명**(기존 수업 편집은 그 명단 유지).
- **실측(1440×950 / 1280×800 / 1024×768 / 390×844, 학생 27명)**: 모달 높이 771/688/656/732px · 모달 레이어 스크롤 없음 · 저장 버튼·상태바 전부 뷰포트 안 · 본문 스크롤 시 푸터 이동 0px · 가로 넘침 0.

## 2026-09-26 수업 등록 모달 개편 — 검증 지적 9건 반영 (PR #425)

- **죽은 컨트롤 제거**: 신규 등록의 `classTemplateId` 초기값을 templates[0] → `""`(직접 입력 일정). 목록 첫 반이 미리 선택돼 있어 그 항목을 다시 골라도 `<select>` 가 change 를 쏘지 않았고, 저장 payload 가 고르지 않은 반을 참칭했다. 수업명·시간 시드는 그대로(`activeTemplate ?? fallbackTemplate`), 기존 수업 편집은 그 수업의 반 유지.
- **푸터 문구 모순 제거**: `getLessonModalRosterNotice`(`lessonModalDraftModel.js`)를 `getLessonModalValidationError` 와 같은 `lessonModalRosterRequiredTypes` 에서 뽑는다. 신입생 보강·휴강 보충은 「학생을 1명 이상 선택해야 저장됩니다.」, 나머지는 기존 「포함 학생 0명으로 저장됩니다.」. 저장 중·완료·실패에는 그리지 않는다.
- **가로 넘침 0**: `.lessonStudentPicker .sectionHeader` 를 1열 grid 로. flex-wrap + 퍼센트 `flex-basis` 는 컨테이너 main 크기가 미정일 때 content 로 되돌아가 390px 에서 283px 넘쳤다(실측). 넘침 검사도 `document.documentElement` 대신 `.modalScrollBody` 의 `scrollWidth - clientWidth` 로 바꿨다.
- **그 외**: 390px 수업 취소 322px 전폭·저장 행 아래 → 112px·저장 행 위 / 기존 수업 편집은 사람이 정한 종료 유지(16:00-17:45 에서 시작만 옮겨도 3시간으로 덮이지 않음) / 시작 23:59 클램프 때 종료 미변경 / 검색 중 헤더 토글 숨김 + 라벨에 대상 수 / `.lessonModalSection`·`.lessonModalFields` 를 `.lessonModal` 로 한정하고 반 개설 모달 원래 값 복구 / 1열 구간 카드 760px + 2열 필드로 입력 폭 970 → 347px.
- **실측(학생 27명 · 5폭)**: `.modalScrollBody` 가로 넘침 0/0/0/0/0 · 2열 유지 1440·1280·1101 · 1열 1024·390 · 스크롤해도 저장 버튼 y 이동 0 · 유형 탭 108px(≥1024) / 212px(390).

## 2026-09-26 화면 상시 설명 문구 정리 — 275건 조사 → 9 PR (`docs/ui-helptext-cleanup-2026-09-26.md`)

- 사용자 요청("설명글이 늘 떠 있어 화면이 정리가 안 된다. 물음표 뒤로 넣고 싶다")에 따라 `src/**` 의 상시 노출 한국어 설명 **275건/10,728자**를 전수 조사하고 화면 단위 8 PR + 공용 1 PR 로 정리했다(#414 #415 #416 #417 #418 #419 #420 #421 #422).
- **공용 `HelpTip` 신설**: 물음표 트리거 + hover·키보드 focus·터치 클릭, Esc·바깥 클릭·포커스 이탈로 닫힘, `role="tooltip"`+`aria-describedby`(닫혀도 DOM 유지). 말풍선은 `position: fixed` 로 `overflow: clip` 조상을 벗어나고 아래가 좁으면 위로 뒤집으며 scroll·resize 에 재계산(#417). 640px 이하 전역 44px 규칙을 되돌려 트리거는 24×24, 히트 영역은 `::before` 44px.
- **결과**: 상시 노출 문장 **330 → 246건**(같은 스캔 기준), 물음표 뒤 **76곳**, 삭제 18건. 나머지는 의도적으로 남겼다 — 빈 상태·진행 상태·실행 직전 경고·확인 모달 부제·필수 입력/형식 안내·숫자가 들어간 동적 문구·오류 복구 안내, 그리고 `<label>`/`<button>` 안처럼 물음표를 붙이면 접근성이 깨지는 자리.
- **전체 회귀 점검**: 사이드바 18개 화면 × 1440·390 에서 물음표를 하나씩 열어 말풍선 9지점 hit test — 잘림 0 · 트리거 24×24 유지 · 뷰포트 이탈 0 · pageerror 0, 앞 단위가 심은 물음표 소실 0(`origin/main` 기준선 대조).
- **남은 것**: 오답관리 390px 가로 넘침(기존 문제 · 별건), 시험분석 산출물 패널의 `작성 도움말 보기` 토글과 물음표 이원화, 학사일정 수정 모드 물음표 브라우저 커버리지.

## 2026-09-19 전체 UI 구조 정돈 — 감사 277건 → 14 PR (`docs/ui-audit-2026-09-19.md` · 원칙 `docs/ui-principles.md`)

- **조사**: 13개 영역(코드 + 실제 스크린샷 119장) 감사로 발견 277건(P0 2 · P1 52 · P2 151 · P3 72), 78% 가 로직 무변경 UI 수정. 테마별 개선 단위 16개 중 15개를 PR 1개씩으로 처리(#378 #383 #389 #394 #397 #400 #401 #402 #403 #404 #405 #406 #407 #408), 98건 해소(P0 2/2 · P1 22/52).
- **위험 제거(P0·P1)**: 알림관리 공지 즉시/예약 발송에 **확인 모달**(수신 N건·미리보기, #383). 저장 안 한 초안의 조용한 소실 — 학생 프로필·시험정보 수정·학사일정·수업연구·수업일지 편집에 dirty 확인과 [편집 취소] 출구(#400 #402 #403 #404 #397). 파괴 행동 확인 통일 — 공용 `ConfirmDialog` 신설 + 교재/교안 삭제·예약 취소·자동 생성 수업 취소·특강 안내문 보관/삭제(#403 #406). 저장 실패·부분 저장 메시지가 하단바 `display:none`·모달 뒤에 숨던 것 표시(#397 #402 #400).
- **일관성**: 버튼 표준 6종 잔재 정리·죽은 버튼 CSS 10종 삭제(#378), 푸터 순서 `[파괴 ⋯][취소][확정]`(#389), 화면·모달당 primary 1개 — 반복 행 primary→soft·비활성 반복 버튼 렌더 생략(#394), 모달 저장 중 닫기 `closeDisabled` 15곳 통일(#405), 1차 탭 variant 통일·EmptyState 상자 소유·로그인 역할 기억(#408), 숙제현황·시험지관리 PageHeader·영문 eyebrow 제거(#404), 특강 안내문 저장 바·상태 알림 AsyncOperationStatus(#406), SNS 스튜디오 버튼 위계(#405).
- **반응형**: 641~1100px(iPad 가로) 사이드바가 865px 블록으로 본문을 밀어내던 것 → 아이콘 레일 + 오버레이 드로어, 390 가로 넘침 3뷰→0, 학생/학부모 포털 문서 폭 547/396→390, 학생 포털 터치 44px 미만 20→0, 키오스크 세로 잘림 해소(#407, `responsiveShell.css`).
- **유지**: 저장·삭제·발송·예약 함수와 인자·API·권한 무변경(진입점만 이동). 수업일지 헤더 pill·하단바·예약 업데이트 danger 톤은 사용자 확정 구조 그대로.
- **보류(이유는 보고서 §9)**: 전역 뷰 전환 dirty 가드·시험분석/총평 자동저장 혼재·설정 기본값 복원·키오스크 3초 자동 확정 등 로직 경계 P1 30건, 수업일지/프로필 모달 중첩·URL 라우팅·달력 상단 필터 등 제품 결정, window.confirm 잔여 25곳, App.css 죽은 클래스 ~240개.
- Vercel 미리보기 배포가 19일 오후 일일 빌드 한도(24h)에 걸림 — CI 와 무관. 한도 해제 뒤 main 자동 배포 안 되면 빈 커밋 push 또는 재배포 필요.

## 2026-09-19 유지보수·성능 리팩터링 — 측정 기반 17 PR

- **조사**: 9차원(진입·초기 데이터 흐름·API 서버·렌더·CSS·중복·테스트 가드·문서 계약·의존성) 감사 → 판정 3인 검증으로 39건 확정(P0 1 · P1 16 · P2 22). 기준선(main `fc63a738`): build 5.8 s · main-*.js 506,644 B(gz 153,849) · main-*.css 352,376 B · App.jsx 173 KB(main 의 37%) · 교사 부트스트랩 16 요청 · 출결 폴링 7 s 당 Supabase 3왕복.
- **정확성(P0·P1, 먼저)**: 알림톡 원천 줄(보충 일정·테스트 통과/재시험·교재/진도)을 화면과 서버가 같은 모듈에서 쓰도록 통합해 **실제 발송 문구가 미리보기와 같아짐**(#381 · 통과/재시험 접미·시험대비 테스트 줄이 발송에도 붙는다). 세션 갱신 타이머가 매 렌더 리셋돼 8시간 토큰이 연장되지 않던 것·원장 「보는 중」 전환 시 재조회 안 되던 것·매 로드마다 화면 캐시가 지워지던 것(main.jsx vs App.jsx 주인 키 불일치)을 자료 정체성 하나로 통합(#371). 같은 탭 로그인 뒤 발송 설정이 null 이던 것(#374). listRows offset 페이지네이션에 기본키 tiebreaker(1,200행 중 1 누락·1 중복 → 0, #372). 시험대비 중복 정리의 서버 대표 행 사본이 isExcluded 규칙을 빠뜨려 사용자가 제외한 행을 지우던 것 → 화면과 같은 모듈(#398).
- **API 왕복**: 취소 수업 7일 정리를 `listLessons` 읽기에서 떼어 기동+매시간 sweep 으로(출결 폴링 3→2 왕복, #373). app_state 저장 뒤 검증 재조회가 전체 테이블×2 직렬화 → `?keys=` 한 키(#379). 수업 일괄 저장 명단 정리 12수업 37→5 왕복(#380).
- **렌더**: 수업일지 학생별 직전수업 파생값 useMemo(렌더당 16.8/31.7/114.8 ms → 입력 불변 시 0, #375) · 달력 pill 인원수 view model useMemo(3.32/6.56/31.33 → 0.57/1.54/9.46 ms, #377).
- **번들·캐시**: vendor-react 청크 분리(192.6 KB, 배포 간 해시 유지) + API preconnect(#382). App.jsx 미사용 import 149개(문장 42개, lazy 모듈 36개) 제거 — 죽은 import 가 끌어오던 lazy 화면 CSS 6개가 main.css 에서 빠짐(−9.9 KB, #392). 화면 전용 CSS 를 lazy 청크로: LessonNestedPanels 106 셀렉터(−12 KB, #388) · StudentManager 232 셀렉터(−23.2 KB, #396) — 둘 다 이동 전후 computed style 대조 0 차이(후자 13상태×3뷰포트 2.2M 속성). **main CSS 352,376 → 301,787 B(gz 48,197 → 41,384)**, 예산 가드 신설(`check-teacher-view-chunks`).
- **중복 제거**: App.jsx 의 수업·기록 헬퍼 사본 6개 → `lessonRecordCollections.js` import(#393, kiosk 와 규칙 단일화).
- **테스트 인프라**: test:production 109구간 npm 체인 → 평면 목록 실행기 하나(`scripts/production-test-file-list.json`, 로컬 117 s → 40 s, #385). CI 가 실행하지 않던 검사 24개 편입 + 죽은 4개 삭제 + 도달 가능성 가드(#390). 새 가드: 미사용 import(`test-unused-imports`), CSS 도메인 분리 2개, exam-prep 대표 행 parity, 페이지네이션·sweep·keyed read·bulk roster fixture.
- **하지 않은 것(이유)**: 저장 뒤 재SELECT 를 `return=representation` 으로 대체(18곳) — "저장 후 서버 재조회 계약 약화 금지" 규칙과 충돌 가능해 사용자 결정 대기. 부트스트랩 전체 이력 읽기(lessons·records·homeworks `select=*`) — 기간 계약이 업무 결정. 수업 허브 청크 사전 import — 운영 빌드 실측 main.js 응답 뒤 청크 요청까지 24 → 17 ms(중앙값)로 효과 없어 폐기. 죽은 CSS 240 클래스(51 KB) — 동적 클래스명 확인 필요.
