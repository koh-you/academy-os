...
- 4-2b는 Student/ClassTemplate/Lesson 6개 mapper와 공유 pure normalize helper를 `src/shared/data`로 추출했다. null/default, schema fallback, special lecture legacy alias, `updated_at` token과 unknown-field drop 정책을 동작 fixture로 고정하며 DB/API/provider 동작은 바꾸지 않는다.
- 다음 4-2c는 student intake와 special lecture application/enrollment 6개 mapper만 같은 방식으로 분리한다.
...
- 학생 프로파일의 선택 월에서 `월간 제출 미리보기`를 열 수 있다. 예정 수업·실제 출결·휴강/보강/특강/취소/개별시간 변동을 분리하고, 학부모용 간단본과 원장님용 상세본을 전환해 확인한 뒤 제출 문구를 복사하거나 PDF로 인쇄한다. 전달 메모는 이번 미리보기와 출력에만 포함되고 저장되지 않으며 실제 발송은 하지 않는다.
