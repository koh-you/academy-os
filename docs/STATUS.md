...
- 3-2 두 번째 단위에서 수업일지의 persistence callback 11개와 provider/transport callback 6개를 `lessonJournalEffectAdapter` 계약으로 묶었다. App의 실제 저장·삭제·AI·Solapi·재대조 함수는 이동하지 않았고 Teacher Lesson Hub는 개별 고위험 callback 대신 adapter 하나를 상세 화면에 전달한다.
