...
1. App 2차 Phase 1~5와 3차 3-0~3-8, P1 운영 저장 신뢰성, P2 modal 통일 후속은 완료됐다. P3-0~P3-2에서 알림 제품 경로 9개, 설정 key 6개, Solapi provider template 4개를 고정하고 출결·수업일지 client preview/server live body를 같은 pure renderer에 연결했다. P3-3a는 일반 공지·특강의 seed→교사 최종 job→provider 경로와 재시험의 독립 11시 transport 부재를 고정했다. 다음 P3-3b는 일반 공지 preset 3개와 특강 guide seed만 설정 원천에 연결한다.
