...
- 시험대비 상세에 날짜별 `오늘 진행한 내용`을 추가했다. 기존 lesson 저장·CAS·Supabase 재조회 계약으로 저장하며 자동 일정 재생성이나 이후 일정 시간 수정에도 사람 작성 내용을 보존한다.
- 390px safe browser에서 체크박스 20×20px, 가로 overflow 0, 진행 내용 저장→일정 수정→API 재조회→새로고침 지속성을 확인했다.
