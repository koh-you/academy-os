...
## 2026-09-21 개념원리 9권 완료(OCR 6권 4154문 포함) · RPM 윤곽선 5권 완료

- **개념원리 윤곽선 6권(OCR) 완료** — 공통수학2 839(예제 149 · 33단원) · 대수 885(186 · 32) · 기하 624(97 · 31) · 미적분Ⅰ 591(115 · 25) · 미적분Ⅱ 719(137 · 35) · 확률과 통계 496(84 · 22) = 4154문. 경로: OCR ingest(색 배지) → `prepare-text-pdf-bank --whiten-gray` → 단원별 배치 전사(178배치 · 예제는 발문만 · 답은 빠른정답) → 병합 → 그림 크롭 정비 → review 조판 → 검수(68문항씩 65개 목록) → 조판 반영 → export(`--answers-from-bank`) → 바탕화면 `개념원리-<과목>-라텍스-패키지`(공통수학2 · 대수 · 기하 · 미적분1 · 미적분2 · 확률과통계).
- 검수 결과(내용 불일치 = red): 공통수학2 red 0 · yellow 99(반영 75 · 유지 24) / 대수 **red 1**(291-686 크롭 잡글자·134pt 그림이 옆 단 넘침 → 재크롭·`\linewidth`) · yellow 96(반영 68 · 유지 28) / 기하 **red 1**(158-314 소문항별 답안용 빈 격자 4개 누락 → subs 끝 TikZ 격자) · yellow 68(반영 49 · 유지 19 · 재크롭 2) / 미적분Ⅰ red 0 · yellow 105(반영 49 · 유지 56) / 미적분Ⅱ red 0 · yellow 70(반영 57 · 유지 15 · 낱장 폭 초과 8장 → 0) / 확통 red 0 · yellow 44(반영 43 · 유지 1). 상세 `latex-bank/<bank>/검수/2026-09-21/00-검수-요약.md`(gn-cm2 · gn-alg · gn-geo · gn-calc1 · gn-calc2 · gn-prob).
- 윤곽선 PDF 그림 크롭(`pdfVectorFigures` · `prepare-text-pdf-bank`): 좌표축처럼 높이 0 인 두 점 선에 0.6pt 두께를 줘 축·화살표가 크롭에 들어가게 함 · 그림 옆 고립 라벨(y·O·x)은 8pt 까지 붙이되 본문 글줄에 있는 토큰은 3pt(라벨 사슬로 본문이 딸려오던 것) · 보기 상자처럼 글자 3개 이상 든 넓은 패널은 그림에서 제외 · OCR 토큰 사이에 라벨(ㄱ·①·⑴) 아닌 글자 6자 이상이면 두 그림으로 나눔. `latex-bank/tools/compare-crops.mjs`(새 크롭 ↔ 기존 IoU 대조 · `--apply`) 로 자동 재크롭을 반영하고 `crop-fix.mjs`(pt 상자 → 300dpi) 로 사람 재크롭 · `crop-page.mjs` 로 위치 확인(같은 폴더 · 검수 목록은 `make-review-ids.mjs`). 은행별 사람 재크롭 32 · 28 · 42+9 · 12+1 · 5+1 · 15+3 건은 요약의 「전사 단계 후처리」에 기록.
- 에이전트 지시문 추가: `latex-bank/agents/figure-fix-prompt-gn-hs.md`(크롭 수정) · `review-fix-prompt-gn-hs.md`(검수 반영 · yellow 규약 · 유지 분류 · 요약 형식). 검수는 `reviewer-prompt-scan-book.md` + 개념원리 고등 정상 사항 문구 공통.

- RPM 대수(22개정 · 1126문): 검수 15(내용 불일치 0 · yellow 98 중 조판 반영 40여 건) → export(`--bank-only --answers-from-bank` · 답 1126 · 해설 1110) → 바탕화면 `rpm-대수-라텍스-패키지`. 상세 `latex-bank/rpm-alg/검수/2026-09-21/00-검수-요약.md`.
- RPM 미적분Ⅰ(22개정 · 885문): 검수 12(내용 불일치 0 · yellow 89 중 조판 반영 30여 건 — ∫·lim 등식을 원문처럼 별도 줄로) → export(답 885 · 해설 833) → 바탕화면 `rpm-미적분1-라텍스-패키지`. 상세 `latex-bank/rpm-calc1/검수/2026-09-21/00-검수-요약.md`.
- 개념원리 중3-2(472문): 검수 7(**red 6** = 자료 상자 값 잘림 3 · 「㉠~㉣」 물결 누락 1 · 과정 상자 안 둘째 그림 누락 2 — 전부 반영 · yellow 24) → export(`--answers-from-bank` · 답 472) → 바탕화면 `개념원리-중3-2-라텍스-패키지`. 상세 `latex-bank/gn-m32/검수/2026-09-21/00-검수-요약.md`.
- RPM 기하(806문): 검수 11(내용 불일치 1 = 조사 「를」 · yellow 52 중 조판 반영 19) → export(답 806 · 해설 500) → 바탕화면 `rpm-기하-라텍스-패키지`. RPM 미적분Ⅱ(1177문): 검수 16(내용 불일치 0 · yellow 69 중 반영 4) → export(답 1177 · 해설 774) → 바탕화면 `rpm-미적분2-라텍스-패키지`. **RPM 윤곽선 5권 완료.** 상세 `latex-bank/rpm-geo/검수/…`, `latex-bank/rpm-calc2/검수/2026-09-21/00-검수-요약.md`.
- build.mjs: 낱장 렌더가 200mm 를 넘는 문항(RPM 대수 1117 증명 상자 + 세로 보기)은 300·420mm 낱장으로 다시 조판(첫 쪽만 쓰면 보기가 잘리던 것) · 배지 올림 상향(cases 6.5mm · displaystyle/lim/∑/∫ 4.5mm).
- 개념원리 공통수학1(797문 = 통번호 654 · 예제 143): `ingest-gn-hs.mjs` 보완 — 특강 뒤 연습문제 쪽이 특강 범위에 먹히던 것(차례 항목 전부를 경계로) · 특강 쪽의 확인체크(통번호 21)도 문항 · 예제 크롭 폭을 상자 오른쪽 테두리(열 밀도 · 87~89%W)까지(그래프가 테두리까지 그려짐 · 그림 66→107) · `pdfVectorFigures` 클립 사각형 반영 + 상자 조각/제목 띠 제외. 배치 35(단원별) 전사 완료 → 특강 쪽의 「특강 01」 예제(초록 번호 · 발문만 · tag 특강)와 292쪽 특강 뒤 293~295쪽 예제(총 13문)를 추가 ingest 해 810문 병합 → 검수 12(내용 불일치 0 · yellow 66 중 조판 반영 30여 건) → export(`--answers-from-bank` · 답 810) → 바탕화면 `개념원리-공통수학1-라텍스-패키지`. 번호 글자 색으로 예제 종류(필수 청록 · 발전 자주 · 특강 초록)를 태그. 상세 `latex-bank/gn-cm1/검수/2026-09-21/00-검수-요약.md`.
- 개념원리 윤곽선 6권용 OCR 경로(`colorBadges.mjs`): 배지를 전체 쪽 OCR 대신 **색 덩어리**(통번호 파랑·자주 · 예제 청록·빨강·초록)로 찾고 덩어리만 3배 키워 숫자 OCR → 순서 검증(통번호 +1 · 예제 중단원별 · 특강별). 빠른 정답도 같은 방식(열 안 단조 증가로 오독 보정). 공통수학2 10~35쪽 시험: 통번호 1~60 전부 · 예제 14 · 답 60/60. 바닥글 쪽 번호 OCR 오독(「90」→「20」)은 pdf 쪽 + offset 으로 대체 · 라벨 상자에 붙은 예제 번호(기하)는 가장 넓은 빈 열에서 분리. 차례 6권은 차례 쪽을 읽어 `latex-bank/<bank>/toc-manual.json` 으로 고정. 6권 전체 ingest(공통수학2·대수·기하·미적분Ⅰ·Ⅱ·확통) 진행 중 → 배치 생성·전사 예정.
- RPM 확률과 통계(22개정 · 596문): 검수 8(**내용 불일치 1** = 0487 보기 ⑤ 「ㄴ, ㄷ」→「ㄱ, ㄷ」 반영 · red 2·yellow 36 중 조판 반영 3) → export(`--bank-only --answers-from-bank` · 답 596 · 해설 539) → 바탕화면 `rpm-확률과통계-라텍스-패키지`. 상세 `latex-bank/rpm-prob/검수/2026-09-21/00-검수-요약.md`.
- 개념원리 중3-2(472문 · 14단원): ingest(472 · 답 471) → 전사 14배치(472/472) → 병합 → 재크롭 40여 건(⑴ 라벨 누락 합친 크롭은 `recrop-margin --dl 22`) → review 조판 → 검수 7 진행 중.
- 개념원리 고등 프로필: `ingest-gn-hs.mjs`(차례로 소단원·연습문제·특강 쪽 판독 · 통번호 h15/예제 h19 배지 · 예제는 「풀이」 앞까지 발문 + 풀이 블록 답 크롭 · 확인체크·교육청 기출·발전 태그) + `ingest-gn-hs-answers.mjs`(본책 끝 빠른 정답 찾기 · 번호 열 군집). 공통수학1 시험: 709문항(통번호 564 · 예제 145) · 빠른정답 564/564. 나머지 6권은 윤곽선(OCR 경로 · 차례도 OCR).
- 개념원리 중3-1(새 프로필 · id 「쪽-번호」): `ingest-gn.mjs`(788문항: 큰 번호 458 · 핵심문제 127 · 확인 139 · 예제 16 · 유제 48) + `ingest-gn-answers.mjs`(빠른정답 787) → `prepare-text-pdf-bank --whiten-gray` → 전사 24배치(788/788 · 예제·핵심문제는 발문만 · 여백 풀이 팁 제외) → 재크롭 12 → 검수 11(**내용 불일치 0** · red 3(3단 보기 잘림)·yellow 48 전부 조판 배치 → 반영) → export(`--answers-from-bank` · 답 788) → 바탕화면 `개념원리-중3-1-라텍스-패키지`(문항·답 788). 상세 `latex-bank/gn-m31/검수/2026-09-21/00-검수-요약.md`.
- build.mjs: 첨자 자리 빈칸(`2^{\blank}`)을 fboxsep 1.5pt 작은 정사각형으로 · `\vecAB` 매크로 · reviver `,\,` 뒤 `\mathopen{}`(「(1, − 4)」 벌어짐) · `[N점]`·「(정답 N개)」 `\mbox`. `recrop-margin.mjs`: 한쪽만 넓히는 `--dl/--dr/--dt/--db`.
- RPM 윤곽선 5권(OCR 원천): 대수 28배치(1126문)·기하 19배치(806문)·미적분Ⅰ 22배치(885문) 전사 완료·병합·재크롭까지 — 검수 대기. 확통 596문 재크롭 완료 → review 조판 중. 미적분Ⅱ 29배치 전사 중(A~S·AC 진행 · 완료 20).
- 다음: 바탕화면 개념원리 6권 패키지 등록(사람) → 남은 후속(공통수학2 267-621 재크롭 · 262-609/281-655 답안용 빈 좌표축 정책 · 확통 73-151 그림 「문항 맨 아래」 배치값 · build.mjs 배지 겹침·cond 템플릿 · 1행 5지선다 배치).
...
- build.mjs: `\emergencystretch=2em`(긴 수식 앞 overfull) · aligned 첫 줄 배지 5.5mm · `--only <id,…>`(review 는 그 id 만, export 는 export 패키지 패치) · xelatex 3분 timeout+재시도(0522 가 2시간 멈춘 사례). make-batches 문항 줄 「인쇄 쪽」= pdf+offset(전 배치가 지적한 오기). crop-figure `--whiten-gray`.
- RPM 중3-2(22개정 · 673문 · 7단원: 삼각비/삼각비의 활용/원과 직선/원주각/원주각의 활용/산포도/상자그림과 산점도): ingest·답지(673)·크롭 570·전사 17배치(673/673)·병합·section 통일 17·상자그림 재크롭 12 완료. 다음: build --review → 검수 → export → 바탕화면 `rpm-중3-2-22개정-라텍스-패키지`.
- 남은 순서: RPM 윤곽선 5권(대수·기하·미적분Ⅰ·Ⅱ·확통 · 배지 이미지 검출 템플릿) → 개념원리 9권(예제 포함 · 새 프로필).

## 2026-09-19 100발100중 3권 완료 — 3-2 중간 660 · 3-2 기말 490 · 3-1 기말 723 (합 1,873문)

- 세 권 모두 `ingest-print-pdf.mjs --mode book` → 전사(16·12·18배치) → 사람 지정 크롭(49·28·18) → build → 검수(9·7·10 · **내용 불일치 0 · 그림 누락 2(77-02·130-71) 보완**) → export → 바탕화면 `100발100중-3-2-중간/3-2-기말/3-1-기말-라텍스-패키지`(문항·답 100% · 해설 미포함).
- 다음: 22개정 RPM 중3-1·3-2(텍스트 PDF · 기존 RPM 경로) → RPM 윤곽선 5권(배지 이미지 검출 템플릿) → 개념원리 9권(예제 포함 · 새 프로필).

## 2026-09-19 100발100중 3-2 중간 660문 LaTeX 조판 (기출 모음 · 구역별 번호 재시작)

- 텍스트 PDF 기출 모음이라 `ingest-print-pdf.mjs --mode book`(큰 글꼴 1~2자리 번호 · 「인쇄 쪽-번호」 id · 예제 「쪽-e번호」 · 1단/2단 자동 · 번호 빈틈 flag)으로 원천화 → 전사 16배치(목록 650 → 실제 660: 집중공략 그래픽 번호·최다 오답문제 34 추가 · 머리띠·QR 조각 24 제외) → 사람(에이전트) 지정 크롭 49(`crop-figure.mjs` · crops.json 크기 기록) → 검수 9(660/660 · 내용 불일치 0 · 🟡 6) → export → 바탕화면 `100발100중-3-2-중간-라텍스-패키지`(문항·답 660 · 해설 미포함).
- 답은 스캔 답지의 빠른정답 쪽 렌더로 확정(답지 632 · 예제 모범답안 28). 예제는 발문만 전사(모범답안 제외 · 사용자 결정 · 개념원리도 동일). build.mjs: `[N점]` 배점 줄바꿈 방지 · 낱장 위 여백(배지 잘림) · 예제 id 배지 「N쪽 예제 1」.
- 이어서 3-2 기말(487 목록 · 12배치 전사 중) · 3-1 기말.

## 2026-09-19 프린트(정의여고 1-2 중간) 정독 자산 + 숫자·창의 변형 시범

- 교재가 아닌 낱장 프린트(HWP 출력 텍스트 PDF · 27쪽 · 41문)를 `ingest-print-pdf.mjs`(2단 · 「N.」 번호 · 출처 태그·Lv 메타 · 답/해설 쪽) → `prepare-text-pdf-bank`(래스터 그림도 상자로 검출) → 전사 2배치 → 검수 41/41 로 조판했다.
- **정독 자산**(`latex-bank/jeongui-1-2-mid/정독/…CM2-GM.md` · 스키마 v2.0 + 변형용 `variation_notes`): ★1 2·★2 14·★3 13·★4 11·★5 1 · 통찰형 23 · type_id 20종 · 분류 이슈 10(카탈로그 M06/M07 base ★ 과대 · 접선-판별식·원주각 유형 부재).
- **숫자 변형 1세트**(level 1 · 38/41 변형 · 그림 고정 3)와 **창의 변형 1세트**(level 2 · 41/41 · TikZ 8 · similarity 0.6~0.75 · ★ 변동 8)를 만들고 변형 검수(독립 풀이 · 41/41 답 정합 · 골조 이탈 1건 재설계)까지 마쳤다. 바탕화면 `문제은행-패키지\정의여고-1-2-중간-프린트\`(원본·숫자·창의 PDF + 변형-노트.md + 정독자산.md). 지시문: `latex-bank/agents/{dokdu-prompt-jeongui,variant-numeric-prompt,variant-creative-prompt,variant-reviewer-prompt}.md`.
- 다른 교재(쎈·라이트쎈·RPM·100발100중)는 정독 없이 전사→검수→export 까지만 한다(사용자 결정 2026-09-19).

## 2026-09-19 쎈 중3-2 753 · 라이트쎈 공통수학1 1414 · 공통수학2 1230 LaTeX 조판(책 전체 4자리 번호 스캔 판)

- 세 권 모두 배치 전사(15·30·28) → 병합 → 병렬 낱장 조판 → 검수(에이전트 10·18·16 · **내용 불일치: 라이트쎈1 0154 차수 누락 1건뿐** · 🟡 조판 24건 반영) → 전사본 기준 export(`--bank-only --answers-from-bank`) → 바탕화면 `문제은행-패키지\쎈-중3-2-수학-라텍스-패키지`·`라이트쎈-공통수학1-라텍스-패키지`·`라이트쎈-공통수학2-라텍스-패키지`. 상세 `latex-bank/<책>/검수/2026-09-19/00-검수-요약.md`.
- **쪽 번호 드리프트**: 스캔 manifest 의 인쇄 쪽은 pdf 쪽 + 고정 offset 이라 간지가 빠진 스캔에서 뒤로 갈수록 어긋난다(쎈 중3-2 최대 10쪽 · 라이트쎈2 는 05 집합 단원이 없어 0635 부터 16쪽). 전사본 `page` 가 원천이며 `build.mjs --export`·`sync-manifest-to-bank.mjs` 가 manifest 를 그 값으로 덮어쓴다.
- 도구: 낱장 조판을 worker 8개로 병렬화(`--jobs` · 1414문 1시간 45분 → 24분) · 수식 안 `\,` 를 kern 으로 바꿔 좁은 낱장의 줄바꿈 잘림 방지 · 긴 보기 판정에 수식 폭 반영 · 그룹 표·지시문 뒤 배지 간격. 답지: 별도 답지 PDF 는 `ingest-scan-answers --pages` 필수(없으면 0건).
- 해설 크롭 누락(쎈 중3-2 57 · 라이트쎈1 33 · 라이트쎈2 70)은 사람 지정 상자 작업을 미뤘다(답 이미지는 전사본 answer 로 100%).

## 2026-09-18 교재관리: 정보 표시·검토 목록·바뀐 파일만 올리기

- 교재 상세를 정보 표시(제목·폴더·학년·과목·문항/해설/답 수)로 바꾸고 원천·다시 등록 안내 줄을 없앴다. 도구는 「이미지 누락 검사 · 정답·해설만 다시 올리기 · 교재 삭제」 한 줄.
- 「경계 확인 필요」 → **「검토 필요」**: 패키지 manifest 의 review_note(🔴 내용 확인 · 🟡 조판 · 📝 전사 메모 · ✂ 경계)를 문항 이미지와 함께 종류별로 본다. `scripts/latex-bank/annotate-package.mjs` 가 검수 JSON(🔴)·전사 메모(사람 판단 필요한 것만)를 manifest 에 넣고 파일 md5 를 적는다(export 끝에 자동 실행). 바탕화면 5개 패키지에 적용(rpm-cm1 2 · 쎈 cm1 2 · rpm-cm2 15 · 중3-2 1 · 쎈 cm2 0).
- **다시 등록 시 바뀐 파일만 올리기**: `book-audit` 가 Storage 객체의 md5(eTag)·크기를 돌려주고, 교재관리가 manifest 의 md5 와 대조해 같은 파일은 건너뛴다. 업로드도 40장 묶음 3개 동시 · 서버는 8장 동시 Storage 업로드(한 권 4천 장 십여 분 → 수 분).
- 실측(9-18 등록 상태): RPM 공통수학2·중3-2 는 누락 없음. RPM 공통수학1(이미지 1305 누락)·베이직쎈 공통수학1(1517)·베이직쎈 공통수학2(105)는 등록 중 화면을 벗어나 끊김 → 같은 폴더 다시 등록(사람 Gate).

## 2026-09-18 RPM 공통수학1 1184문 + 베이직쎈 공통수학1 1570문 LaTeX 조판 · 「공통수학1 파일 문제」 정정

- **정정**: 9-17 에 「RPM 공통수학1 PDF 는 한글 글꼴이 제거되어 복원 불가」라고 적은 것은 틀렸다. 원인은 pdfjs 에 CMap(`cMapUrl`·`standardFontDataUrl`)을 안 넘겨 CID 한글 글꼴을 못 그린 우리 쪽 설정이었고(`scripts/problem-bank/pdfTools.mjs` 에서 기본 지정), 고친 뒤 본문·답지 모두 한글이 온전히 읽혔다. 공통수학2 답지도 같은 원인이라 한글 포함으로 다시 만들었다(9-17 후반).
- **RPM 공통수학1(22개정 학생용) 1184문항 LaTeX 조판 완료**: 배치 26 전사 · 검수 15(1184/1184 · **내용 불일치 0** · 🔴 3 반영 · 🟡 126 조판 규칙) · 그림 크롭 114(사람 지정 재크롭 11: 조립제법·정사각형·라벨 잘림·점 그림·워터마크 `whiten`). book.pdf 158쪽. 배지 겹침은 첫 줄 내용별 올림(cases·pmatrix 5.5mm · 분수 3.5mm)으로 해결. 상세 `latex-bank/rpm-cm1/검수/2026-09-17/00-검수-요약.md`.
- **베이직쎈 공통수학1(스캔) 1570문항 LaTeX 조판 완료**: 스캔 크롭 1520 → 전사 에이전트 35개가 실제 문항 1570 확정(OCR 이 놓친 76 추가 · 라벨 상자·간지·오번호 26 제외), TikZ·표 그림 80. 검수 20(1570/1570 · **내용 불일치 0** · 🔴 1 = 원문 오식 의심 186-11 · 🟡 51 조판 규칙). 답지: 반 컬럼 격자·색 배지 보완으로 해설 1276→1454, 사람 지정 상자(`latex-bank/ssen-basic-cm1/answer-overrides.json` · 놓친 116 + 이웃 오배정 18)로 **1570/1570**. 등록 패키지의 답 이미지는 전사본 answer 조판(`--answers-from-bank`) 1570/1570. 상세 `latex-bank/ssen-basic-cm1/검수/2026-09-18/00-검수-요약.md`.
- **사람 Gate**: 바탕화면 `문제은행-패키지` 의 `rpm-공통수학1-라텍스-패키지`(문항 1184·해설 1184·답 1183)와 `베이직쎈-공통수학1-라텍스-패키지`(문항 1570·해설 1570·답 1570)를 교재관리 › 패키지 등록. 베이직쎈 186-11 원문 오식 여부는 실물 교재로 확인.

## 2026-09-17 RPM 공통수학2 LaTeX 조판 1132문 · (공통수학1 보류는 9-18 정정 — 파일 문제 아님)

- **RPM 공통수학2(22개정 학생용) 1132문항 LaTeX 조판 완료**: 텍스트 PDF 원천화(컬럼 경계를 배지 x 최대 틈으로 · 바닥글 줄로 단원 읽기 · 「04|2」 소단원 헤더) 1132문 · 10단원 · flagged 0, 답지(벡터 「답」 아이콘 감지) 해설 1132/1132 · 답 줄 1125. 그림 208장 자동 크롭(얇은 선 십자 축 검출 · `--whiten-gray` 워터마크 제거 · override 6건). 배치 27개를 전사 에이전트 27개가 전사(거부 0), 검수 에이전트 14개가 1132문 대조: **내용 불일치 0**, 🔴 2 반영, 🟡 158 은 조판 규칙으로 반영(배지 1.5mm 올림 · `\cond` 줄바꿈 허용 · `\dispeq` 폭 축소). book.pdf 167쪽. 상세 `latex-bank/rpm-cm2/검수/2026-09-17/00-검수-요약.md`.
- 원본 파일 한계(사용자 확인 필요): 이 학생용 PDF 는 한글 일부·①~⑤·⊂·√ 글리프가 빠져 있고 답지 PDF 는 한글이 전부 빠져 있어 해설 이미지가 수식만 남는다. 조판본 문항은 글자 레이어·해설·숨은 답으로 보완해 완전하지만 **해설 크롭은 불완전**하다. 완전한 답지 PDF 가 있으면 `ingest-answers.mjs` 로 다시 만들어 export 만 다시 하면 된다.
- **RPM 공통수학1 은 보류**: 본문·답지 PDF 모두 한글 글꼴이 제거되어(렌더·글자 레이어 어디에도 한글 없음 · 수식만 남음) 문항을 복원할 수 없다. 한글이 살아 있는 PDF(교사용·원본 출력본)가 필요하다.
- **사람 Gate**: 바탕화면 `문제은행-패키지\rpm-공통수학2-라텍스-패키지` 를 교재관리 › 패키지 등록(문항 1132·해설 1132·답 1125 재조회).

## 2026-09-16 RPM 중3-2 LaTeX 조판 644문 + 베이직쎈 답지 누락 48건 채움

- 베이직쎈 공통수학2 답지: `ingest-scan-answers.mjs --overrides`(사람 지정 상자 122개 · `latex-bank/ssen-basic-cm2/answer-overrides.json`)로 해설 누락 48 → 0(420/420), 답 줄 270 → 334, 번호 불일치 17 → 0. PR #350 병합. 바탕화면 `베이직쎈-공통수학2-1~3단원-라텍스-패키지` 의 answers/solutions/manifest-answers 갱신. 바탕화면의 1단원 이전 폴더 3개 삭제(저장소에 검수·정독 원천 있음).
- **RPM 중3-2 수학 644문항 LaTeX 조판 완료**(텍스트 PDF 경로 신설): `prepare-text-pdf-bank.mjs` 가 벡터 그림 521장을 자동 크롭(TikZ 없음)하고 글자 레이어 힌트를 만들고, 배치 16개(22~53문)를 전사 에이전트 16개가 전사(거부 0 · 답 644/644 답지 크롭 일치). 검수 에이전트 8개가 642문 원본↔조판 대조: 내용 불일치 0, 🔴 22(그림 크롭 보기 글자 혼입·참조 표 누락·백슬래시 소실) 전부 반영, 🟡 39 중 조판 규칙으로 반영(분수 보기 행 지주 · 그림 옆 raggedright · 긴 세로 보기 문단형). book.pdf 130쪽 · Overfull 1. 상세 `latex-bank/rpm-m3-2/검수/2026-09-16/00-검수-요약.md`.
- **사람 Gate**: 바탕화면 `문제은행-패키지\rpm-중3-2-수학-라텍스-패키지`(export · 조판본 PNG 644 + 원본 정답·해설 644) 를 교재관리 › 패키지 등록(기존 RPM 문항이 없어 새로 등록됨 · 문항 644·해설 644·답 644 재조회). 등록 뒤 교재별 오답에서 문항 이미지가 조판본으로 나오는지 확인.
...
- 공통수학1 178쪽 → 313문항 · 8소단원 + 종합문제 묶음 4 · 번호 연속 · 버린 코드 0 · flagged 1(8분). 구역·구역 내 순번은 `type_label`(「서술형 연습장 02」). 오답지 2단 인쇄에서 전폭 문항은 두 열을 차지한다(`assignPrintColumns`).
...
- Realtime 실패 시에도 7초 polling은 계속되는 안전망을 유지한다. 실제 Slack/알림톡 발송·예약·취소는 실행하지 않았다. 사용자 승인 뒤 테스트 수업 `Realtime 검증 0915`는 7일 복구 가능한 취소 상태, 학생 `리얼타임검증0915`는 기록을 보존하는 퇴원 상태로 정리하고 목록 재조회까지 확인했다.
...
- 수업일지 하단의 `수업 취소`를 제거하고 수업 수정 모달 왼쪽 위험 버튼으로 옮겼다. 일반 수업은 기존 확인 모달을 유지하고, 취소 성공 뒤 수업 수정 모달과 수업일지를 함께 닫는다.
- lesson domain 22/22, focused browser, `check:duplication`, `check:fast`(scenario 828/828·lint·build), `test:production`(308/308), 격리 safe browser 97/97과 실제 안전 미리보기의 읽기·편집·취소 확인 화면을 통과했다. API·DB 계약과 운영 데이터·알림에는 변경이 없다.
## 2026-09-13 문제은행 피드백 7건 (반/학생 분리 · 학생 앱 · PPT)

- 교재별 오답 = 반 전체 집계(학생 선택 없음, 오답률 N% 이상 선택, 인쇄·PPT), 학생별 오답 = FilterBar 학생 선택 + 같은 보드(기록 입력). 학생 앱에 「오답」 탭(`/api/portal-problem-bank`, 읽기 전용).
- 재풀이 정답은 흰 바탕 + 초록 점선 + 「재」로 정답과 구분. 2단 인쇄는 실제 2열 grid. 미리보기 넓힘 + 확대 보기. 「PPT 저장」(pptxgenjs, 별도 청크).
- 등록 실패(Failed to fetch) 대응: 등록 전 `/health` 깨우기·네트워크 오류 재시도·목록 45초. 교재관리 「이미지 누락 검사」. **사람 Gate**: 운영 DB 에 교재가 없으므로(교재 삭제 뒤 등록 실패 상태) 바탕화면 `문제은행-패키지\rpm-중3-2-수학` 을 다시 등록.
- 다음: EBS 올림포스 6권(스캔 PDF · 회전·기울기) 스캔 경로 — `docs/problem-bank/README.md` 남은 일 3.

## 2026-09-13 문제은행 정답·해설 연결

- 「정답과 풀이」 PDF 를 문항과 같은 세그먼터로 잘라(`scripts/problem-bank/ingest-answers.mjs`) 해설 이미지와 「답」 줄 이미지를 문항 번호에 붙였다. RPM 3-2: 해설 644/644 · 답 644/644, 컬럼·쪽을 넘는 풀이 44건 이어 붙임, 비전 토큰 0. 별도 「빠른 정답」 PDF 는 판이 달라(673 vs 644문항) 쓰지 않았다.
- 교재관리 › 교재 상세에 「정답·해설 패키지 등록」, 오답지 인쇄에 빠른정답 격자·해설 쪽, 미리보기에 정답·해설이 붙었다. **사람 Gate**: `supabase/20260913_problem_bank_answers.sql` 적용 → 바탕화면 `문제은행-패키지\rpm-중3-2-수학` 폴더(문항+정답·해설이 한 폴더) 등록.
- 검증: store·route·segmenter 단위 검사, safe browser problem-bank 2/2, `check:fast` 828/828·lint·build.

## 2026-09-13 문제은행 사용자 피드백 9건 반영

- 교재별 오답: 번호 색을 **고른 학생 기준**(기록 없음·오답·정답·재풀이 정답)으로 바꾸고 범례에 클릭 순서를 적었다. 클릭 즉시 색이 바뀌고 서버 응답으로 확정한다(실패 시 되돌림). 퇴원생은 학생 목록에서 뺐다. 현행·추가1·추가2 탭과 옛 PDF 빈 문항 보드는 제거했다.
- 공통 지시문 문항(교과서문제 정복하기 [0001~0006] 등)은 지시문 블록을 한 번 싣고 고른 번호에 검정 강조 상자를 얹는다(비전 토큰 0). 오답지 1단/2단 선택, 워터마크를 이미지 위(multiply)로 올렸다. 교재관리에서 교재 정보 수정·삭제(문항·기록·이미지 정리)가 된다. 재등록은 문항을 지우지 않아 학생 기록이 남는다.
- CLI: 다음 번호 위 「중요」 아이콘 때문에 하단이 길어지던 0126번류 17건을 아이콘 영역 규칙으로 고쳤다(소문항 (2)처럼 빈 줄 뒤 진짜 내용은 유지). 검증: `test:production` 308/308·scenario 828/828, safe browser problem-bank 2/2 + lesson-journal 40/40.
...
- 상단 상태, 예약 확인 모달, 하단 고정 바가 같은 동기화 판정을 사용한다. 모달은 내용이 달라진 예약을 행 단위로 표시하고, 하단 바는 현재 상태가 `needs`일 때 과거의 “Solapi 예약 반영 완료”를 계속 노출하지 않는다.
- 관련 lesson/notification fixture, `check:fast`(scenario 828/828·lint·build), `test:production`(308/308), 격리 safe browser 96/96을 통과했다. 운영 데이터는 조회만 했고 기존 2건의 실제 Solapi 취소·재예약은 사람 Gate로 분리했다.

## 2026-09-09 시험지관리 중학 3학년 과목 추가

- 시험지관리의 응시 기록 과목 드롭다운에 `중학 3-1`, `중학 3-2`를 추가했다. 쎈 단원 카탈로그 6과목은 별도 목록으로 유지해 시험지 목록의 단원·커버리지 계산을 바꾸지 않았다.
- 가상 데이터에서 두 과목 선택 가능 여부와 `중학 3-1` 저장·서버 재조회·회차 재열기 지속성을 확인했다. `check:fast` 828/828, 시험지 모델·쎈 과목 parity, focused browser 1/1 통과. API·DB 계약 변경 없음. 병합·배포 전.

## 2026-09-07 넓은 화면의 스튜디오 공간 활용

- 사용자 요청에 따라 시험분석·SNS 스튜디오는 데스크톱 기준으로 설계·검증한다. AGENTS.md/testing-policy에 범위를 명시했으며 모바일 최적화를 별도 필수 완료 조건으로 추가하지 않는다. 기존 학생·학부모 기기 원칙은 유지한다.

- 사용자 Chrome 화면(2844×1038)에서 1440px 최대 폭과 세로로 쌓인 기본정보/PDF, SNS 빈 작업 영역을 직접 확인했다.
- 공통 최대 폭·중복 바깥 여백을 제거했다. 본문 가용 폭 1100px 이상에서는 시험 기본정보/PDF, SNS 기본자료/메모·공개 확인, Chat 안내/전달 자료를 나란히 배치한다. 콘텐츠 미리보기는 읽기 폭을 제한하고 빈 SNS에는 시작 안내를 제공한다.
- 검증: 2844/1920/1024/390 폭·공통 정렬·병렬 배치·넘침 focused browser 1개, SNS 저장/실패/재조회 3개, 시험분석 2개 통과. check:fast 828/828·lint·build, CSS hygiene 통과. API·DB·저장/AI 실행 계약 변경 없음. 병합·배포 전.

## 2026-09-07 시험분석·SNS 스튜디오 공통 레이아웃

- 병합 요청 후 CI에서 긴 시험분석 시나리오의 전체 45초 제한 소진을 확인했다. 개별 저장·재조회 assertion은 유지하고 해당 시나리오만 90초로 조정했다.

- StudioWorkspace를 공유하여 자료 요약 → 왼쪽 자료 선택 / 오른쪽 단계·편집 영역으로 통일했다. 모바일은 같은 단계 선택 메뉴를 사용한다. 표시명은 SNS 스튜디오다.
- 최종 미리보기는 출제 비중 / 난이도·문항 흐름 / 주요문항 / 카드 제작으로 분리했다. 비선택 편집기는 mounted 상태로 유지하여 입력을 보존한다. 학교·학년·고사는 선택 메뉴, 분석본은 기존 선택 카드다.
- 검증: 관련 safe browser 5/5, 최종 시험분석 집중 재검사 2/2, 828/828 scenario·runtime lint·build·6개 관련 fixture 통과. 운영 쓰기·유료 AI 호출 없음. 기존 API/DB/저장·재조회 계약 유지. 병합·배포 전.
- 기능 위치 및 제한: docs/studio-layout-20260907.md.

## 2026-09-06 시험분석 작업 흐름 간소화

- 사용자 병합 승인 후 이전 defaultOpen 기대값을 새 접힘 UI에 맞게 수정하고 test:production 전체 재검증을 통과했다.
...
- 수업 달력의 `삭제한 수업 복구`에서 서버의 취소 수업을 조회하고 최근 7일 항목만 표시한다. 새로고침·재로그인 뒤에도 수업 날짜·시간·반·원천·학생 수·남은 기간을 확인해 복구하며, 기존 수업 상태 CAS와 관련 기록·숙제 재조회를 그대로 사용한다.
- 시험대비 자동 생성 수업은 복구 뒤 남을 수 있는 달력 숨김 키도 같은 창에서 `달력 표시 복구`로 해제한다. 숨김 해제는 기존 app_state 자동저장·CAS·재조회 경로를 사용한다.

## 2026-09-04 운영 API 인증 인계 정정

- 인증 코드의 OFF 기본값과 실제 운영 설정을 분리했다. 별도 작업은 운영 무인증 거부·허가된 ops 인증 통과를 확인했으며, 사용자 의도는 허가받은 AI의 접근·수정 권한 유지다. [현재 인증 인계](security/ops-api-auth-current-handoff.md)를 우선하며 오래된 구현 프롬프트를 재실행하지 않는다. 이번 변경은 문서만이며 운영 설정·데이터·권한 변경은 없다.

업데이트: 2026-08-31

## 2026-08-31 개인 요일별 시간표 수업 연속성 교정

- 수업일지의 직전 수업은 반·요일 조합이나 개별 시간표 문자열이 아니라 학생이 실제 출석·지각 또는 체크인한 전체 일반 수업 중 가장 최근 1건을 사용한다. 해당 수업 값이 비어 있어도 더 오래된 비어 있지 않은 값으로 대체하지 않으며 특강 트랙 격리는 유지한다.
- 직전 등원 수업에 다음 숙제가 비어 있으면 더 오래된 정규수업 숙제로 건너뛰지 않는다. 이진후의 2026-08-31 수업에 잘못 연결된 2026-08-24 숙제는 배포 후 기존 수업일지 CAS 저장 경로로 제목·원수업 연결을 비우고 Supabase 재조회까지 확인했다.
- lesson domain 22/22, runtime lint, 임의 요일·시간·반의 등원 회귀 fixture, 휴강보충 알림 focused safe browser, production build와 exact-head 원격 전체검사가 통과했다.

## 2026-08-29 휴강보충 분할 등록·그룹 알림 예약

- 수업 등록에 `휴강 보충` 유형을 추가해 반 전체 대신 해당 시간 참석 학생만 골라 별도 수업을 만들 수 있다.
- 수업 수정의 `휴강 보충 알림 관리` 버튼이 상세 모달을 열고, 학생·학부모·당일 11시 문구 3종을 한 화면에서 확인·수정한 뒤 일정 저장·Supabase 재확인·알림 예약을 한 번에 실행한다. 그룹 수업은 `{{학생명}}`을 각 학생 이름으로 치환하며 안정적인 수업·학생·대상 ID로 재시도 중복을 막는다.
- 기존 `휴강 보충` 수업과 sourceLabel에 휴강보충 원천이 있는 과거 별도 일정도 같은 수정 화면과 알림 예약 기능을 사용한다.

## 2026-08-26 JSON 요청 한글 청크 손상 수정

- 공용 `readJsonBody`가 네트워크 Buffer 청크를 각각 문자열로 바꿔 이어 붙이면서, UTF-8 한글 바이트가 청크 경계에서 나뉠 때 `번 → ���`처럼 손상되던 원인을 수정했다.
- 요청 본문은 바이트 배열로 누적하고 제한도 실제 바이트 수로 검사한 뒤, 수신 완료 시 한 번만 UTF-8로 디코딩한다. 분할 한글 fixture와 수업일지 저장·재조회 focused browser가 통과했다.
- 운영 원천의 기존 손상 문자열은 자동 변경하지 않았다. 별도 사람 Gate로 대조·보정해야 한다.

## 2026-08-25 월 정규수업 열기 요일별 명단 오류 수정

- 사용자가 9월 수업일지 명단이 8월과 다르다고 보고했다. 박지현(월수금 7-10반, 개별시간표로 수요일만 출석) 사례를 운영 API read-only 조회로 대조한 결과, 8월엔 수요일 회차에 정상 포함됐지만 9월엔 월/수/금 전 회차에서 빠져 있었다.
- 원인은 `monthlyRegularLessonOpen.js`의 "월 정규수업 열기"가 반(class template) 전체의 "지난달 마지막 실제 수업" 단 1건(박지현 케이스에서는 월요일 회차, 원래 결석)의 명단을 새 달의 모든 요일 회차에 그대로 복사하던 것이었다. 요일별 최신 수업(`latestByTemplateDay`)은 이미 계산돼 있었고 시작/종료 시간에는 요일별로 정확히 쓰이고 있었는데, 명단(`studentIds`)만 반 전체 단일 스냅샷을 썼다.
- 각 신규 회차의 명단을 그 요일의 마지막 수업 명단에서 가져오도록 수정했다. 저장·삭제·알림 로직은 건드리지 않았다. 요일별로 다른 명단(예: 월수금 중 수요일만 다니는 학생)이 새 달에도 요일별로 정확히 이어지는지 확인하는 회귀 fixture를 추가했다(수정 전 코드로 실패 확인).
- 운영 API read-only 조회로 잘못 생성된 9월 정규수업 52건(전부 자동생성, 연결된 수업기록·숙제·알림 예약 0건)을 확인해 사용자가 Supabase SQL Editor에서 직접 삭제하도록 안내했다. 삭제 후 "월 정규수업 열기"를 다시 실행하면 고친 코드로 재생성된다.
- runtime lint, production build, `npm run test:production`(305/305, scenario 827/827), `tests/browser/lesson-journal.spec.js` 35/35를 통과했다.
...
- 입력 draft와 3개 저장 필요 상태를 유지하며 실제 저장 API가 호출되지 않는 것을 safe browser로 확인했다. API·DB·CAS·알림 provider 계약은 변경하지 않았다.
...
- 시험관리 첫 화면은 `전체 반`을 기본 선택해 현재 고사의 모든 학교를 바로 표시한다. 정렬은 기본 `학교명 가나다순`이며 시험기간 빠른순·학년순·학생 많은 순으로 바꿀 수 있다.

## 2026-08-13 개별시간표 배지 시간 표시

- 수업일지의 개별시간 배지를 문구 대신 `4-7`, `5-8`, `1-4`처럼 실제 수정 시간으로 표시한다. 시험대비는 이미 실제 시간이 표시되므로 중복 배지를 제거했다. 명단·시간·저장 판정은 변경하지 않았다.

## 2026-08-13 화목토 앞반 개별시간 표시 회귀 수정

- 기본 화목토 앞반 학생의 개별시간 `17:00-20:00`이 반 행 `16:00-19:00`과 일부 겹치면 기존 소속 명단을 유지한다. 저장 전 확인 모달에서 반·학생 시간을 대조하고 `기본 반 명단에 표시하고 저장`을 선택해야 하며, 다른 반 배치는 기존처럼 개별시간 전체가 대상 수업시간에 포함돼야 한다.
- 운영 원천 확인 결과 서빈·김보현은 학생 DB와 수업 `studentIds`에 그대로 있었으며 화면 필터만 숨겼다. 운영 데이터 수정은 하지 않았다.

## 2026-08-13 명단 재계산 적용일 선택

- 학생 프로필에서 `현재 시간표로 명단 재계산`을 실행할 수 있다. 오늘 수업 행·기록이 있으면 `오늘 명단 유지 · 내일부터 적용` 또는 `오늘 명단 포함 · 오늘부터 적용`을 명시적으로 선택한다.
- 선택일부터 현재 반·개별 시간표에 맞는 정규수업 명단을 기존 CAS·서버 재조회 계약으로 보정한다. 보강·특강 등 수동 일정은 재계산하지 않는다.

## 2026-08-13 개별 시간표 기반 수업일지 명단

- 학생 프로필의 요일별 개별 시간이 실제 정규반 시간 범위에 포함되면 소속 반과 달라도 해당 수업일지 명단에 배치한다. 이진후의 월요일 4-7, 박지현의 토요일 1-4 사례를 fixture로 고정했다.
- 변경 전·후 계산 결과가 달라진 미래 정규수업 명단만 기존 CAS·Supabase 재조회 계약으로 저장한다. 수동 보강 명단과 과거 수업은 보존한다.
- 수업일지의 개별 시간표 표시는 긴 시간 문자열 대신 짧은 `개별` 배지를 사용한다.

## 2026-08-12 시험대비 체크박스·당일 진행 내용

- 공용 checkbox 기본 크기를 정규화하고 시험대비 일정 대상 선택은 20×20px로 고정해 일반 input 높이 규칙이 체크박스에 번지지 않게 했다.
- 시험대비 상세에 학생별 `오늘 진행한 내용` 입력칸을 추가했다. 각 학생의 기존 수업기록 저장·Supabase 재조회 계약을 사용하며 공통 입력·일괄 복사 기능은 두지 않았다.
- 390px safe browser에서 체크박스 20×20px, 가로 overflow 0, 학생별 진행 내용 저장→일정 수정→API 재조회→새로고침 지속성을 확인했다.
...
- 현재 고사에 저장됐지만 활성 학생이 없는 시험정보도 숨기지 않고 모든 반 화면에 `연결된 활성 학생 없음`으로 표시해 제외·복원·상세관리가 가능하다.
- 대표 행의 포함·제외 상태는 같은 고사·학교·학년·과목의 저장 중복 전체에 함께 적용한다. 학사일정 카드·달력·학교 필터는 포함 행만 원천으로 사용한다.
- 기존 exam-prep bulk API·CAS·서버 재조회 계약은 유지하며 운영 행을 자동 삭제하거나 상태 변경하지 않는다.

## 2026-08-11 보충관리 정렬 표시·수업일지 날짜순

- 보충관리 정렬에 `수업일지 날짜순`을 추가했다. 연결된 수업일지 날짜를 오래된 순으로 정렬하고 날짜가 없는 항목은 마지막에 둔다.
- `정렬` 라벨과 선택창이 축소되며 세로로 깨지지 않도록 한 줄 배치를 고정했다. API·저장·알림 계약은 변경하지 않았다.

## 2026-08-11 시험관리 학교·학년별 학생 표시

- 시험정보 목록을 `학교명 → 학년 → 해당 학생` 순서로 바꾸고, 선택한 반 범위에서 학교·학년이 일치하는 활성 학생 이름과 인원수를 표시한다. 퇴원생은 제외하며 학생 이름으로도 검색할 수 있다.
- 기존 학생·시험정보 API와 저장 계약은 변경하지 않았다. 표는 자체 가로 스크롤을 유지하고 390px 문서 폭은 넘지 않는다.

## 2026-08-11 시험대비 자동 명단·이후 일정 전파

- 시험관리의 현재 포함 학교·학년과 재학생을 날짜별 시험대비 명단의 원천으로 사용한다. 시험관리에서 빠진 `상계고` 같은 과거 생성 문구·학생은 달력에서 즉시 제외한다.
- 일정 수정은 `반별 / 학교별 / 학생별` 대상을 고르고, 선택 날짜부터 각 학생의 마지막 시험대비까지 같은 시간을 적용한다. 중간 날짜 수정은 이전 날짜를 보존한다.
- 다중 lesson 저장은 `updatedAt` CAS, Supabase 재조회 대조, 실패 시 역순 rollback을 사용하며 제거 학생에게 기록·알림 작업이 있으면 중단한다. 알림 발송·예약은 하지 않는다.

## 2026-08-11 시험대비 학교·학생 시간 가시성

- 수업 달력의 시험대비 일정은 학교명을 한 줄로 줄이지 않고 학교별 배지로 모두 표시하며 `학교 수·학생 수`를 함께 요약한다.
- 상세 모달은 학생을 기본 `시간순`으로 묶고 `학교별` 보기로 전환한다. 저장된 개인별 시간이 있으면 우선 표시하고, 없으면 수업 공통 시간을 사용한다.
- Supabase 제외 SQL 적용 완료를 사용자에게 확인받았으며 새 SQL·API·DB 계약 없이 기존 lesson 원천만 표시한다.

## 2026-08-11 학사 달력 시험기간·수학시험 표시

- 월간 학사 개요의 학교별 전체 시험기간을 하단 달력 각 날짜에 옅은 학교색 띠로 표시하고, 기간 안의 수학시험일은 같은 학교색의 진한 탭으로 구분했다.
- 시험기간 필터와 범례를 추가했으며 절대 위치 레이어를 제거해 여러 일정이 겹쳐도 셀 내용 흐름대로 높이가 늘어난다.
- 1280px에서는 달력 내부 overflow가 없고, 390px에서는 달력만 가로 스크롤되며 페이지 전체 폭은 유지됨을 focused safe browser로 확인했다.

## 2026-08-11 시험정보 하단 작업 영역 분리

- 시험정보 상세의 저장 상태를 별도 행으로 올리고, 제외·복원·삭제는 왼쪽 `관리 범위 변경`, 총평·닫기·저장은 오른쪽 주요 작업으로 분리했다.
- 기존 local draft·명시 저장·Supabase 재조회 계약은 유지하며 작은 화면에서는 상태→관리 범위→주요 작업 순서로 쌓인다.
- 시험관리 fixture, runtime lint, build, focused safe browser와 실제 safe 화면을 확인했다.

## 2026-08-11 시험정보 내신 준비 제외

- 활성 학생의 학교·학년 행은 삭제 후 자동복구되므로, 행을 지우지 않고 고사별 `is_excluded` 상태로 보존하는 기능을 준비했다.
- 상세관리에서 제외·복원 초안을 선택하고 `변경 저장`으로 확정한다. 제외 행은 기본 시험관리·학사일정·시험대비 자동계획·시험분석에서 빠지며 `내신 제외 보기`에서 복원할 수 있다.
- 운영 DB의 `supabase/20260811_exam_prep_exclusion.sql` 적용을 사용자가 완료했다. production `827/827`, 집중 safe browser `1/1`, build를 통과했다.

## 2026-08-10 시험정보 명시 저장·날짜 표시

- 시험정보 표의 기간 열을 넓히고 줄바꿈을 허용해 `2026-10-13 ~ 2026-10-19` 전체가 보이게 했다.
- 시험정보는 모달 local draft에서 편집하고 `변경 저장` 때만 변경 행을 한 번 저장·재조회한다. 표의 범위·부교재 즉시 저장 입력도 제거했다.
- runtime lint, 관련 fixture/scenario, 집중 safe browser `3/3`, build를 통과했다.
...
- 시작일·종료일 어느 쪽을 먼저 선택해도 중간값을 보존하고, 두 날짜가 완성되면 기존 `시작일 ~ 종료일` 원천으로 저장·Supabase 재조회한다.
- 시험관리 전용 fixture, 저장 controller/API/reconcile, 집중 safe browser, runtime lint, production `827/827`, build `424 modules`를 통과했다.

## 2026-08-10 반 이동·퇴원 적용일 선택

- 반 이동 또는 퇴원 대상 학생에게 오늘 수업일지 명단이나 저장 기록이 있으면 `오늘부터 / 내일부터` 적용 시점을 선택한다. 기본값은 오늘 행을 보존하는 `내일부터`다.
- 학생 원천은 즉시 저장하되 수업 명단은 선택 날짜부터만 변경하며, 기존 CAS·Supabase 재조회 대조와 과거 수업기록 보존을 유지한다.
- student domain `16/16`, 이채빈형 날짜 fixture, 집중 safe browser `2/2`, runtime lint, production `827/827`, build `424 modules`를 통과했다.

## 2026-08-10 수업일지 비중첩 저장 충돌 복구

- 수업일지 편집 중 다른 화면의 출결·숙제 상태 갱신이 같은 행의 버전을 바꾸면 문구·수업내용처럼 겹치지 않는 수정까지 막던 행 전체 CAS 충돌을 수정했다.
- `이전 원천 / 사람 수정본 / 최신 Supabase`를 필드별 비교해 비중첩 변경만 자동 병합하고, 같은 필드의 서로 다른 변경은 계속 차단한다. 저장·rollback·재조회 대조와 알림 분리는 유지한다.
- 후속 운영 재현에서 상태가 바뀌지 않은 12개 숙제도 매 저장마다 새 `checkedAt`을 만들어 영구 충돌하는 원인을 확인했다. 상태가 이미 같으면 현재 명시 저장 시각을 충돌 없이 적용하고, 새 화면은 의미 변화가 없는 숙제를 저장 계획에서 제외한다.

## 2026-08-10 지난 숙제 미진행 일정 건너뛰기

- 같은 반의 직전 일정에 수업기록과 새 숙제가 모두 없으면 실제 진행하지 않은 일정으로 보고, 더 이전에 저장된 `다음 숙제`를 현재 수업의 `지난 숙제`로 이어서 표시한다.
- 중간 일정에 출결·수업내용·과제상태 등 진행 흔적이나 새 `다음 숙제`가 있으면 그 수업을 경계로 유지한다. 실제 진행한 수업의 빈 숙제를 오래된 값으로 덮지 않으며 수업일지 저장·알림 미리보기는 같은 원천을 사용한다.

## 2026-08-10 Supabase 알림 조회 egress 긴급 축소

- Render의 1분 알림 dispatch가 매번 최근 알림 1,000건 전체를 읽던 경로를 `scheduled`이면서 현재 시각까지 도래한 후보만 Supabase에서 조회하도록 좁혔다.
- 1분 dispatch, GitHub fallback, 원자적 claim, stale retry, Solapi 결과 대조는 유지한다. 로컬 production `827/827`과 알림 계약·도메인·build가 통과했으며 exact-head CI와 운영 로그 확인을 진행한다.
...
- 수동 보충 safe browser의 고정 날짜를 서울 기준 미래 날짜로 바꿔 당일 11시가 지난 실행에서도 학생 reminder 3종 계약을 검증한다.

## 2026-08-08 퇴원 최신 학생 원천 재기준화

- 퇴원 실행 직전에 학생·수업 원천을 Supabase에서 다시 읽고 최신 학생 버전으로 퇴원 상태와 다음 날 이후 수업 명단을 함께 저장한다. 다른 화면의 학생 정보 변경으로 오래된 `updatedAt` 충돌이 반복되던 경로를 복구했다.

## 2026-08-08 출결 태블릿 자동 확인 3초

- 출결 태블릿의 수업 선택 완료 preview는 버튼을 누르지 않아도 3초 뒤 자동 저장을 시작한다. 저장 성공 응답 뒤 확인창을 닫고 번호 입력으로 복귀하며, 여러 수업 중 선택 전에는 타이머를 시작하지 않는다.
...
- 이후 수업일지의 과제 상태 셀에 계속 남던 `확인할 숙제`를 학생별 수업메모로 옮겼다. 수업메모의 `확인 완료` 체크는 원본 수업기록 follow-up을 저장·Supabase 재조회한 뒤 이후 수업에서 제거한다.
- 숙제 완료/미완료 판정과 기존 알림 예약 본문은 바꾸지 않는다. 수업메모 확인 체크박스는 `16×16px`로 통일했다.

## 2026-08-08 수동 보충 입력 검증 회귀

- 수동 보충 작성에서 시작 시간이 빠진 검증 실패 뒤에도 학생·제목·내용·날짜 초안과 작성 모달이 유지되는 안전 브라우저 회귀를 추가했다.
- 실패 동선에서 `makeup_tasks`·`lessons`·`notification_jobs` 쓰기 요청이 0건임을 고정했다. 제품 runtime과 운영 데이터는 변경하지 않았다.

## 2026-08-07 수동 보충 수업일지·알림 연결

- 보충관리에 자동 후보와 별개인 `수동 보충 등록`을 추가했다. 재원생, 제목, 사유·내용, 날짜·시간을 직접 입력해 기존 versioned 보충 원천과 1인 수업일지를 함께 저장한다.
- 수동 보충도 학생·학부모 다음 정각 일정 안내와 보충 당일 학생 11시 알림 3종에 같은 보충 ID로 연결된다. 운영 데이터·실제 알림은 실행하지 않고 safe fixture에서 저장·재조회·달력 진입을 확인했다.

## 2026-08-07 중첩 모달 ESC 닫기 복구

- 수업일지 위에 알림톡 작성 모달을 연 상태에서 ESC 한 번에 두 창이 모두 닫히던 공용 모달 이벤트 전파 오류를 수정했다.
- DOM상 최상위 모달만 ESC를 처리하며 첫 ESC는 알림톡, 다음 ESC는 수업일지를 닫는다. close-disabled 모달은 ESC를 소비하되 상위 창을 닫지 않는다.

## 2026-08-07 수업일지 표 레이아웃 복구

- 데스크톱 수업일지의 10개 고정 열 실제 합계가 표 최소 폭보다 커 오른쪽 알림톡 열이 잘리고, 긴 `확인할 숙제` 문항 목록이 과제 상태 셀 밖으로 침범하던 표시 오류를 수정했다.
- 열 최소 폭을 데스크톱 모달에 맞게 재배치하고 좁은 화면의 기존 가로 스크롤은 유지했다. 긴 문구는 과제 상태 셀 안에서 줄바꿈하며 1417×945 집중 safe browser를 통과했다.
...
- 수업일정표 PDF 새 창은 열려도 문서 안의 지연된 인쇄 호출에 의존해 브라우저에서 인쇄창이 열리지 않는 오류를 수정했다.
- 사용자 클릭 흐름에서 문서 생성 직후 인쇄를 직접 호출하고, 새 창에도 다시 누를 수 있는 `인쇄하기` 버튼을 추가했다. student `15/15`, runtime lint, build, 집중 safe browser `1/1`을 통과했다.

## 2026-08-06 수업일지 숙제 동시 생성 충돌 복구

- 다른 화면에서 같은 숙제가 먼저 생성되면 수업일지 저장이 충돌 보호로 중단되지만 최신 숙제 원천이 화면에 반영되지 않아 재시도도 계속 막히는 오류를 수정했다.
- 서버가 충돌한 최신 숙제를 구조화해 반환하고, 화면은 해당 ID가 로컬에 없을 때만 원천을 병합한다. 교사 입력은 유지하며 명시적인 두 번째 `변경 저장`에서 CAS 갱신한다. lesson `21/21`, production `827/827`, build, 집중 safe browser `1/1`을 통과했다.

## 2026-08-06 보강 수업일정표 시간 원천 수정

- 강민준 8월 7일 보강이 수업일정표에서 17시로 표시되는 운영 오류를 재현했다. 학생 프로필의 금요일 정규 시간표가 13시 보강 자체 시간을 덮어쓴 것이 원인이다.
- 프로필 개별 시간은 기본 정규수업·휴강에만 적용하고 보강은 수업일지 원천 시간을 유지한다. student `15/15`, lesson `21/21`, production `827/827`, build, 집중 safe browser `1/1`을 통과했다.

## 2026-08-06 수업메모 배포 교체 복구

- 배포 교체 뒤 오래 열린 탭이 제거된 이전 `LessonNestedPanels` 자산을 요청하면 수업메모가 열리지 않는 원인을 운영 화면에서 확인했다.
- 저장 원천은 건드리지 않고 수업일지 오류 화면에 원인 안내와 `최신 화면으로 새로고침` 복구 버튼을 추가했다. lesson `21/21`, runtime lint, build, 집중 safe browser `2/2`를 통과했다.

## 2026-08-06 출결 알림 예약 취소 범위 수정

- 최지민 결석보충 운영 기록을 읽기 전용 대조해 18시 출결 알림 예약이 17:54에 생성된 뒤, 수업일지 Solapi 예약 업데이트에서 `알림 제외`로 취소된 것을 확인했다.
- 수업일지 학부모·학생 코멘트 예약 갱신이 취소할 수 있는 대상을 `parent_comment`·`student_comment`로 제한해 별도 출결 알림 예약을 보존한다.

## 2026-08-06 Tally 기존 학생 반영 문구 단순화

- 기존 학생 선택 시 안내를 `이미 등록되어 있습니다` 질문형으로 바꾸고, 버튼을 `기존 정보에 Tally 내용 추가`와 `Tally 내용으로 기본정보 교체`로 명확히 구분했다.
- 저장·학생 원천·수업 명단 반영 동작은 바꾸지 않았다.

## 2026-08-06 수업일정표 미입력 출결·상단 요약 제거

- 수업일정표 미리보기·복사 문구·PDF에서 `출결 미입력`을 표시하지 않고, 상단 `예정 수업 / 출결 확인 / 변동사항` 3칸 요약을 제거했다.
- 출석·지각·결석 등 입력된 출결과 수업·변동 원천은 그대로 유지한다. student `15/15`, runtime lint, build, 집중 safe browser `1/1`을 통과했다.
