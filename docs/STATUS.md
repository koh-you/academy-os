...
- 개념원리 공통수학1(797문 = 통번호 654 · 예제 143): `ingest-gn-hs.mjs` 보완 — 특강 뒤 연습문제 쪽이 특강 범위에 먹히던 것(차례 항목 전부를 경계로) · 특강 쪽의 확인체크(통번호 21)도 문항 · 예제 크롭 폭을 상자 오른쪽 테두리(열 밀도 · 87~89%W)까지(그래프가 테두리까지 그려짐 · 그림 66→107) · `pdfVectorFigures` 클립 사각형 반영 + 상자 조각/제목 띠 제외. 배치 35(단원별) 전사 완료 → 특강 쪽의 「특강 01」 예제(초록 번호 · 발문만 · tag 특강)와 292쪽 특강 뒤 293~295쪽 예제(총 13문)를 추가 ingest 해 810문 병합 → review 조판 → 검수 예정. 번호 글자 색으로 예제 종류(필수 청록 · 발전 자주 · 특강 초록)를 태그.
- 개념원리 윤곽선 6권용 OCR 경로(`colorBadges.mjs`): 배지를 전체 쪽 OCR 대신 **색 덩어리**(통번호 파랑·자주 · 예제 청록·빨강·초록)로 찾고 덩어리만 3배 키워 숫자 OCR → 순서 검증(통번호 +1 · 예제 중단원별 · 특강별). 빠른 정답도 같은 방식(열 안 단조 증가로 오독 보정). 공통수학2 10~35쪽 시험: 통번호 1~60 전부 · 예제 14 · 답 60/60. 전체 ingest 진행 중.
