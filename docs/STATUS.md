...
- 월별 정산의 신규생 회차제는 예정 시간표가 아니라 실제 수업일지의 이행 정규 회차를 사용하고, 원 휴강과 연결된 `휴강 보충`은 정규 이행으로 센다. 퇴원일이 해당 월에 있으면 교사 고정 override가 없는 한 자동 퇴원 정산하며, 출결 달력은 출석·결석·지각·대기를 색으로 구분한다.
- 2026년 7월 운영 원천을 읽기 전용으로 재계산한 교정 예상은 이채빈 정규 5회, 김형준 10회, 이윤채 11회, 김예나 퇴원 비례 13/14다. 운영 정산 snapshot 저장은 사람 확인 뒤 실행한다.
- 전역 사람 gate는 0건이다.
