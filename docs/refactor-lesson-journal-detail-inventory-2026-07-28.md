...
70. 다음 `17J-9`: comment/job 표시·payload 순수 경계와 App-owned provider action을 closeout audit한다.
71. `notification_jobs` 조회·OS job 취소·provider 예약 반영·발송결과 reconcile orchestration은 App callback에 남기고 순수 표시·판정만 분리
