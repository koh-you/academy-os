...
67. 다음 `17J-6`: lesson 예약 payload fingerprint 정규화·직렬화를 순수 모델로 분리한다.
68. `notification_jobs` 조회·OS job 취소·provider 예약 반영·발송결과 reconcile orchestration은 App callback에 남기고 순수 표시·판정만 분리
