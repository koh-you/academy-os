...
52. 다음 `17H-4`: 예약 audit 조회 결과의 성공·부분 실패·전체 실패를 React 상태로 반영하기 전 순수 응답 정규화 경계를 inventory한다.
53. `notification_jobs` 조회·OS job 취소·provider 예약 반영·발송결과 reconcile orchestration은 App callback에 남기고 순수 표시·판정만 분리
