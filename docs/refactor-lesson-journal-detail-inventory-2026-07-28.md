...
57. 다음 `17I-3`: 등원보충/다음 정규수업 확인/수업 후 보충 선택의 record patch·makeup task·안내 메시지 plan을 순수 모델로 분리한다.
58. `notification_jobs` 조회·OS job 취소·provider 예약 반영·발송결과 reconcile orchestration은 App callback에 남기고 순수 표시·판정만 분리
