...
56. 다음 `17I-2`: homework draft key·표시 title fallback·업데이트 row 조합을 순수 모델로 분리한다.
57. `notification_jobs` 조회·OS job 취소·provider 예약 반영·발송결과 reconcile orchestration은 App callback에 남기고 순수 표시·판정만 분리
