...
75. 다음 `17L-2`: 학생/학부모 대상별 record 알림 제외 판정을 순수 selector로 분리한다.
76. `notification_jobs` 조회·OS job 취소·provider 예약 반영·발송결과 reconcile orchestration은 App callback에 남기고 순수 표시·판정만 분리
