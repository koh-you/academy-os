...
81. 다음 `17M-1`: `saving`/`saved`/`failed` record 상태 map 반복 계산의 순수 분리 범위를 inventory한다.
82. `notification_jobs` 조회·OS job 취소·provider 예약 반영·발송결과 reconcile orchestration은 App callback에 남기고 순수 표시·판정만 분리
