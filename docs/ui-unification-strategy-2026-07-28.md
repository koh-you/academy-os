...
UI-0~UI-7, UI-8A·UI-8B·UI-8C와 UI-8D-0~2를 완료했다. UI-7·8A·8B·8C 사람 시각 확인은 `docs/next-session/README.md`의 독립 절차로 누적되어 다음 화면군을 막지 않는다. 다음 UI-8D-3은 작성·대상·학부모 채널 찾기·예약/결과 기록과 발송/취소/삭제 callback 분리를 통합 회귀하고 독립 사람 시각 절차를 누적한다. 실제 저장·발송·예약·삭제는 실행하지 않는다.
