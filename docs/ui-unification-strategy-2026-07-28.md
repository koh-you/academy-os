...
UI-0~UI-8J와 UI-9A~C를 완료했다. 공개 화면 세 폭 렌더와 전체 기능 회귀 기준을 닫았고, CSS 13개 파일의 완전 중복 selector를 8종에서 0으로 줄였다. 단순 문자열 미사용 후보는 동적 class 오탐을 피하려고 일괄 삭제하지 않았으며 실제 runtime 0건인 폐기 특강 매칭 UI만 제거했다. 인증 이후 UI-7·UI-8 화면군의 사람 시각 확인은 다음 UI-9D에서 하나의 실행 가능한 체크리스트로 통합한다.
