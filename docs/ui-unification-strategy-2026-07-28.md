...
UI-0~UI-7, UI-8A와 UI-8B를 완료했다. UI-7·8A·8B 사람 시각 확인은 `docs/next-session/README.md`의 독립 절차로 누적되어 다음 화면군을 막지 않는다. 다음 UI-8C-0은 보충관리의 원 결석/숙제, 일정, 알림 원천, 취소 의미와 모바일 예외를 읽기 전용 inventory로 정리한다. 실제 저장·발송·예약·삭제·출결은 실행하지 않는다.
