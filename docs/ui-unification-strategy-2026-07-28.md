...
UI-0~UI-7, UI-8A와 UI-8B를 완료했고 UI-8C-0 보충관리 inventory를 확정했다. UI-7·8A·8B 사람 시각 확인은 `docs/next-session/README.md`의 독립 절차로 누적되어 다음 화면군을 막지 않는다. 다음 UI-8C-1은 학생 상세를 공통 scrollable modal로 전환하고 저장 상태 3개를 정확한 3열로 맞춘다. 실제 저장·발송·예약·삭제·출결은 실행하지 않는다.
