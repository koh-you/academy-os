...
UI-0~UI-7과 UI-8A를 완료했고 UI-8B-0·1 수업일지 inventory와 모바일 viewport·safe-area를 완료했다. UI-7·8A 사람 시각 확인은 `docs/next-session/README.md`의 독립 절차로 누적되어 다음 화면군을 막지 않는다. 다음 UI-8B-2는 학생 기록의 중복 scroll 소유자만 제거한다. 실제 저장·발송·예약·삭제·출결은 실행하지 않는다.
