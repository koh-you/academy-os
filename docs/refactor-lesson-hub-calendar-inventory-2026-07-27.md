...
4. **15D — `LessonModal` callback-only UI — 진행 중**
   ...
   - 다음 15D-6: 학생 검색·학년 그룹·선택 chip JSX를 controlled `LessonModalStudentPicker`로 옮기고 state와 선택 patch는 App에 둔다.
...
- 다음 단계: 15D-5 완료 뒤 최신 `origin/main`을 다시 rebase하고, 15D-6 학생 선택 표시를 controlled component로 한 의미 단위만 분리한다. 실제 `onSubmit`, API·Supabase 재조회는 이동하지 않는다.
