...
- 다음 단계: 15F-4에서 `saveLessonModalLessons`의 POST/GET orchestration을 이동하지 않은 채 expected/persisted lesson 대조 결과만 순수 모델로 분리할 수 있는지 검토한다. 실제 요청·React state·서버 side effect는 이동하지 않는다.
