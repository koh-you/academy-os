...
- 4-3o: integrations status GET의 AI·알림 상태 selector→응답 조립을 frozen registry로 이동했다. provider 상태 계산과 환경 설정 owner는 기존 모듈에 유지한다.
- 다음 4-3p는 exam-analysis read route 경계를 작은 단위로 분리한다.
