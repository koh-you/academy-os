...
## 13D-8 수신 audience 번호 accessor 단위

- student/parent audience에 따른 전화 필드 선택을 `resolveNotificationAudiencePhone`으로 이동했다.
...
## 13F-4 persist/reserve request binding 단위

- App의 persist/reserve wrapper를 `notificationNoticeApi.js`의 factory callback으로 이동했다.
...
## 13F-7 예약발송 action binding 단위

- App의 예약발송 wrapper를 composer hook으로 이동했다.
...
## 13F-10 알림 이력 삭제 action binding 단위

- App의 알림 이력 삭제 wrapper를 history hook으로 이동했다.
