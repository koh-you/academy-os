...
- 블록 날짜/시간/메모 수정값은 `generatedLessonControls.sundayMakeupBlocks`에 저장되고, `/api/app-state`를 통해 Supabase `app_state`에 저장된다. 로컬 전용이 아니다.
- 블록을 수정하면 하단 저장 바에 `변경됨 · 블록 저장 필요`가 표시되고, `블록 저장` 후 `저장됨 · Supabase 자동 반영`으로 바뀐다.
