...
| UI-8B-4 | 완료 | 수업 맥락·10열 기록·저장·알림·modal 통합 회귀 |
| UI-8C-0 | 완료 | 보충관리 원천·일정·알림 3종·두 취소·모바일 예외 inventory |
...
| UI-8E-0 | 완료 | 특강 안내·신청·명단·회차·수업일지 반영·모바일 예외 inventory |
| UI-8E-1 | 완료 | 특강 탭 touch 경계·명단/수업일지 gate 모바일 header |
| UI-8E-2 | 완료 | 학생 연결/수동 접수/회차 modal·학생 목록·실제 저장 bar |
...
| UI-8I-2 | 완료 | AI 입력/결과·자료함 등록/목록 모바일 한 열과 긴 내용 |
| UI-8I-3 | 완료 | 세 화면 callback/source 통합 회귀와 시각 절차 |
| UI-8J-0 | 완료 | 학생·학부모 포털 account/read/write/Storage 경계 inventory |
| UI-8J-1 | 완료 | 계정 머리말·수업 기록 달력·읽기 카드 맥락과 긴 내용 |
| UI-8J-2 | 완료 | 숙제·질문·시험 후 제출 모바일 행동·상태·파일명 |
| UI-8J-3 | 완료 | 포털 callback/source 통합 회귀와 시각 절차 |
| UI-8J-0 | 다음 | 학생·학부모 포털 읽기·쓰기·계정·모바일 예외 inventory |
...
> 최신 상태: `UI-0`~`UI-8J`를 완료했다. 현재 다음 작은 목표는 `UI-9A 시각 회귀 매트릭스`이며, 실제 저장·삭제·발송·출결·Storage·AI 호출은 하지 않는다.

누적된 desktop·tablet·390px 시각 절차와 코드 기반 responsive 계약을 화면군별로 대조하고, AI가 재현 가능한 캡처·렌더 기준과 사람에게 남길 최소 시각 항목을 분리한다. 별도 App.jsx 리팩터링은 UI-9 완료까지 중단한다.
