...
- `wrongProblems`와 `lessonResearchItems` 명시 저장 분리는 완료. local draft 변경만으로 API를 호출하지 않고 화면별 저장 버튼에서 해당 key의 CAS·Supabase 재조회를 수행하며, 저장 중 후속 수정은 `변경됨`으로 남긴다.
...
- 2026-08-16 병합·검수 요청으로 Claude Code의 4-5a~e 통합분을 검토했다. Storage primitive 이동, 시험분석 download/delete operations 주입, vision-check·boundary-detect PDF transport 분리와 provider result envelope 정의까지 완료했다. envelope의 기존 orchestrator 적용과 provider route registry는 남아 있으며 4-5 전체 종료로 표시하거나 자동 진행하지 않는다.
