...
- 4-1 API payload 24개 contract와 4-2 DB/domain row mapper 45/45 분리·종료 감사는 완료했다. 4-3n까지 system/auth/portal/교사 제출/app-core·test session read/write와 app-state/report snapshot registry를 분리했다. Vercel rate-limit 해소 뒤 4-3n main 통합, integrations status와 domain·provider route 경계를 잇는다.
