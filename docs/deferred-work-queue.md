...
- 4-1 API payload 24개 contract와 4-2 DB/domain row mapper 45/45 분리·종료 감사는 완료했다. 4-3m까지 system/auth/portal/교사 제출/app-core·test session read와 app-state/report snapshot write registry를 분리했고 다음 활성 단위는 test session write/delete 뒤 domain·provider route 경계다.
