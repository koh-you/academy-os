...
- `ReportModal` footer와 Solapi 제어창 shell 공통화는 완료했다. dialog 41/41이 공통 shell을 사용하며 공통 footer 11개·기존 action wrapper 6개다. 다음은 다중 원천 확인 footer를 한 화면씩 검토하되 저장·provider callback identity와 오류 복구를 먼저 고정한다.
