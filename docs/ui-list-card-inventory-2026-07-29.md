...
3. `UI-4B-4A` 완료: 학생 포털 숙제 완료 카드와 질문 행의 draft, disabled 조건, 저장 feedback, 완료/수정/삭제 callback을 보존해 공통 body/action 슬롯을 적용했다.
4. `UI-4B-4B` 완료: 보충 후보 행의 상세·생성·완료 확인 callback과 진행 상태를 고정하고 shell을 이관했다.
5. `UI-4B` 완료: 알림 발송 기록·학생 편집 행처럼 표 구조에 가까운 목록은 카드로 바꾸지 않고 UI-4C 표 규칙과 함께 처리한다.
