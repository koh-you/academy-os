...
1. `UI-4B-2`: 학생·학부모 포털의 읽기 전용 공지·자료·보고서·수업기록 카드에 제목/메타/상태/행동 슬롯을 적용한다.
2. `UI-4B-3`: 알림 대상·시험 후 대상처럼 체크 선택만 하는 행의 선택 표현과 모바일 터치 영역을 통일한다.
3. `UI-4B-4`: 저장·완료·복귀·취소·삭제가 있는 운영 행은 callback·원천·실패 draft 유지 계약을 목록별로 고정한 뒤 하나씩 이관한다.

## 사람 검수

필수 중단 gate는 없다. 배포 후 학생 프로필 운영 알림, 날짜별 수업 카드, 학부모 응대, 보충 최근 이력에서 제목·메타·상태·행동 위치와 390px 줄바꿈만 확인한다. 날짜별 수업 선택과 학부모 확인 정보 복사는 눌러볼 수 있다. 운영 알림 완료/삭제와 보충관리 복귀는 누르지 않는다.
