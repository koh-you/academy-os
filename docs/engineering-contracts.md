...
- 표의 행·목록의 카드처럼 같은 모양이 N번 반복되는 자리의 액션 배치는 `docs/ui-row-actions.md`를 따른다. 반복 렌더 안에 비활성 버튼을 깔지 않고, 행별 저장은 `StickySaveBar` 하나로 모으며, 퇴원·삭제 같은 파괴적 저빈도 액션은 `OverflowMenu`(⋯)로 접는다. 리피터(폼 배열)의 행 제거는 예외로 아이콘 버튼을 쓴다.
