...
- UI-7D-4: 학생관리 41개, 월별 정산 5개, 테스트관리 3개, 특강 정산 2개, 학생 질문 1개도 같은 원칙으로 닫았다.
- JSX 48개, form control 378개의 이름 미지정은 현재 0개다.
- UI-7D-5: 실제 control 단위 메시지가 있는 로그인 도움말·오류, 월별 정산 스케줄 도움말·오류, 특강 정산 조정 사유 오류만 `aria-describedby`·`aria-invalid`로 연결했다. 영역 전체 저장 상태는 개별 control에 연결하지 않았다.
