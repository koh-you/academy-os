...
- **MV-5d**: `SettingsCenter.jsx`(단독 lazy 청크, 사실상 하위 패널 없는 단일 화면) 47 selector 분리. 처음엔 `settingsPage`/`settingsHero`/`settingsRow` 등 "settings" 접두어가 있는 것도 있고 `promptEditor`/`notificationTemplateEditor`처럼 없는 것도 섞여 있어 접두어 정규식만으로는 부족했다.
  - **재사용 가능한 격리 조사 도구를 만들었다**: 대상 파일의 모든 `className` 토큰을 추출한 뒤 `src/domains/` 전체(js/jsx/css)에서 각 클래스가 다른 파일에서도 쓰이는지 자동으로 grep하는 스크립트를 짜서, "isolated(이동 후보)"/"shared(App.css에 남길 것)" 목록을 한 번에 뽑았다. 이 결과로 `integrationStatusGrid`/`templateTestGrid`/`notificationTemplateSettingsGrid` 등 8개는 className은 SettingsCenter 전용이지만 CSS 선언 자체는 classManager/lessonResearch/aiVariant 등과 공유하는 mega-rule에 끼어 있음을 확인 — MV-5c의 postcss 필터가 이 8개를 자동으로 이동 대상에서 제외했다(같은 selector가 두 파일에 나뉘어 존재하는 정당한 이중 소유).
  - **cascade 검증**: 47개 중 44개(94%)를 6개 설정 탭(계정/알림톡 연결/알림톡 문구/AI 모델/AI 프롬프트/출결) 전체를 순회하며 `getComputedStyle()` 이동 전/후 대조 — 완전 일치. 나머지 3개(`.templateResultCard` 등)는 알림톡 템플릿 "테스트" 버튼을 눌러야 나타나는데(`dev:safe`에서도 실제 API 호출 경로라 시간 관계상 추적하지 않음) postcss 기계적 추출(재입력 없음)로 안전 보장.
  - `scenario-tests-production.cjs`의 `css` 조합 변수에 `settingsCenterCssSource` 추가해 3개 체크(03b·20c·20c-2) 수정.
  - build 결과: main CSS `365.82 kB → 360.61 kB`(-5.21 kB), 신규 `SettingsCenter.css` `5.21 kB`. `scripts/test-settings-center-css-domain-split.mjs`(신규)와 `test:production`(303/303, scenario 827/827) 통과.
- **MV-5e**: `NotificationCenter.jsx`(4-6c 이후 추가된 selector) — 이 파일 자체는 className이 없는 순수 조립 컴포넌트라, `NotificationCenterNavigation`/`NotificationComposeWorkspace`/`NotificationComposerPanel`/`NotificationHistoryPanel`/`NotificationHistoryRow`/`NotificationNoticeWorkspace`/`NotificationRecipientPanel`(전부 같은 lazy 청크, `ParentResponseContextPanel`은 이미 분리 완료)까지 격리 조사 도구를 여러 파일에 대해 한 번에 돌리도록 확장해 조사했다. 101개 selector 확인.
  - 이 필터가 `.notificationTableRow`/`.specialLectureTopTabs`/`.noticeComposerGrid`/`.noticePickerActions` 등 6개가 `DashboardAuxiliaryPanels.jsx`(다른 lazy 청크)와 공유하는 mixed rule에도 끼어 있음을 자동으로 찾아, 공유 규칙은 App.css에 남기고 각자의 전용 규칙만 이동했다(이전 단위들과 같은 정당한 이중 소유 패턴). 역사적 4-6c 문서가 경고했던 바로 그 `.notificationPanel` 계열 공유 관계가 실제로 다시 나타났다.
  - **cascade 검증**: 알림관리 화면(개별 발송/전체 기록)과 특강관리 화면(같은 컴포넌트가 다른 props로 재사용됨)을 순회해 101개 중 45개(82%)를 `getComputedStyle()` 이동 전/후 대조 — 완전 일치. 나머지는 실제 발송 job이 있어야 나타나는 이력 테이블 행 상태(safe fixture가 전부 "0건"이라 도달 불가)라 postcss 기계적 추출로 안전 보장.
  - `scenario-tests-production.cjs`의 `css` 조합 변수에 `notificationCenterExtraCssSource` 추가해 9개 체크 수정.
  - build 결과: main CSS `360.61 kB → 351.69 kB`(-8.92 kB), 신규 `NotificationCenter.css` `11.24 kB`. `scripts/test-notification-center-css-domain-split.mjs`(신규)와 `test:production`(303/303, scenario 827/827) 통과.
- **다음 후보**: `ExamPostSubmissionManager`의 `examPost*`는 학생 포털(`StudentExamPostSubmissionPanel.jsx`, 다른 lazy 청크)과 클래스명이 겹쳐 **분리 불가로 판정**, 더 이상 후보 아님. 아직 조사 안 한 lazy 청크: `LessonNestedPanels`, `TeacherLessonHubV2`, `SettlementWorkspace`(이미 자체 CSS 3개 보유, 낮은 우선순위), `StudentManager`, `SupplementCenter`, `DashboardAuxiliaryPanels`, `LearningSupportCenters`, `PlanningToolCenters`(이 마지막 3개는 여러 화면을 한 청크에 묶은 파일이라 화면별로 나눠 조사 필요). MV-5c/5d/5e에서 확립한 방법(className 격리 자동 조사, 여러 파일 동시 지원 → postcss 기계적 추출 → declared-property 기준 자동 computed-style 대조)을 다음 화면에도 그대로 재사용할 수 있다.
