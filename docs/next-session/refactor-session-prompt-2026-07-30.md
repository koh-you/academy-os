...
> 2026-07-31 완료 메모: 이 프롬프트의 17BA-2 재개점은 전용 브랜치에서 완료됐다. 이후 로드맵 18의 안전한 순수 shell 분리까지 닫혔으므로 새 세션 시작용으로 그대로 사용하지 않는다. 남은 경계는 `docs/current-worklog.md` 최신 18G closeout을 기준으로 별도 고위험 inventory를 먼저 설계한다.
