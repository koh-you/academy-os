...
이 handoff 작성 직전 기능 기준 최신 커밋은 `6564db5d Clarify special lecture notice actions`입니다.
...
1. 특강 안내문 구조 정리
   ...
   - 특강관리라는 이름은 아직 쓰지 않습니다. 신청자 관리까지 붙일 때 별도 `특강관리` 화면으로 확장하는 방향입니다.
...
- 하지만 OS 안에 `특강 신청자 목록`으로 따로 저장하는 구조는 아직 없습니다.
- 현재 `/api/intake/tally` 웹훅은 Supabase `student_intake_applicants`에 저장하는 신입생/상담 접수 후보 흐름입니다.
- 특강 신청자까지 여기에 섞으면 신입생 접수와 재원생 특강 신청이 혼동될 수 있습니다.
- 다음 구현 방향은 별도 Supabase 원천 `special_lecture_applications` 또는 동등한 저장소와 `특강관리 > 신청자` 화면을 만드는 것입니다.
...
2. 특강 안내문 사람 검수
   - `알림관리 > 특강 안내문` 진입
   - 버튼이 `안내문 저장`, `알림톡 발송 준비`, `링크 복사`만 보이는지 확인
   - `복사 옵션`, `알림톡 본문 복사`가 보이면 중단
   - `학습 목표`만 남고 `안내문 요약`, `핵심 안내`가 보이지 않는지 확인
...
4. 특강 신청자 관리 설계
   - 구현 전 먼저 저장 원천을 정합니다.
   - 추천 원천: `special_lecture_applications`
   - 필요한 필드 예시:
     - `application_id`
     ...
     - `status`
     - `created_at`
   - Tally hidden fields 또는 URL query에서 `specialLectureId/guideId/source/campaign`을 받아 특정 특강 안내문과 연결합니다.
   - 신입생 접수 후보 `student_intake_applicants`와 섞지 않습니다.
...
- 작성일: 2026-07-15
- handoff 작성 직전 기능 기준 최신 커밋: `6564db5d Clarify special lecture notice actions`
- 실제 최신 커밋: `git log -1 --oneline` 결과를 우선
- 브랜치: `main`
- 운영 프론트: https://academy-os-blue.vercel.app
- 운영 API: https://koh-you-math-academy-os-api.onrender.com
- 최근 검증: `node --check api/server.js`, `node --check api/routes/coreData.js`, `node --check api/routes/notifications.js`, `node --check scripts/scenario-tests-production.cjs`, `npm run test:production` 288개 통과, `npm run build`, `git diff --check`
...
- Tally 특강 신청자 관리는 아직 별도 원천이 없음을 확인했고, 다음 구현은 별도 신청자 저장소 설계가 먼저입니다.
