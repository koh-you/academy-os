...
App.jsx 보충관리 리팩터링은 `12R-37`에서 `SupplementCenter` 본체를 전용 파일로 이동해 로드맵 12 `supplement center/modals`의 App 본체 분리를 완료했다. 알림센터는 13A-1~5 표시, 13B-1 기록·13B-2 수신자 모델, 13C-1 builder, 13C-2 request adapter를 분리했다. 13C-3 즉시발송, 13C-4 예약, 13C-5 Solapi 결과 대조, 13C-6~7 AI 수정, 13C-8 삭제 request adapter, 13C-9 삭제 action, 13C-10 Solapi 취소 UI action, 13C-11 background refresh, 13D-1 template/display config, 13D-2 학생 선택 model, 13D-3 template local action, 13D-4 history filter action, 13D-5 local job upsert, 13D-6 composer model, 13D-7 학생 이름 fallback, 13D-8 audience 번호 accessor, 13D-9 compose workspace, 13D-10 notice shell, 13E-1 navigation hook, 13E-2 recipient hook, 13E-3 history hook, 13E-4 composer hook, 13F-1 template adapter, 13F-2 identity callback, 13F-3 job builder binding, 13F-4 request binding, 13F-5 background refresh binding, 13F-6 즉시발송 binding, 13F-7 예약발송 binding, 13F-8 결과 대조 binding, 13F-9 AI 문구 수정 binding, 13F-10 알림 이력 삭제 binding에 이어 13F-11에서 Solapi 예약 취소 wrapper를 hook 순환 없는 독립 binding으로 옮기고 production scenario 466개로 AI gate를 통과했다. 실제 취소/DELETE/Supabase/Solapi request는 callback 경계에 남고, 특강 안내문 적용 handler는 App이 소유한다. 다음은 `NotificationCenter`의 남은 App 소유 로직을 inventory하고 특강 gate 또는 화면 전체 조립만 남았으면 로드맵 13의 안전 경계를 닫는다. fixture로 검증 가능한 단위에는 재시험 등 운영 데이터를 만들지 않는다.
...
- 12번 `supplement center/modals` App 본체 분리 완료. 13번 `notification center`는 13A-1~5 표시, 13B-1 기록·13B-2 수신자 모델, 13C-1 builder·13C-2 request adapter·13C-3 즉시발송 action·13C-4 예약 action·13C-5 결과 reconcile action·13C-6~7 AI 수정·13C-8 삭제 request adapter·13C-9 삭제 action·13C-10 Solapi 취소 UI action·13C-11 background refresh action·13D-1 template/display config·13D-2 local selection model·13D-3 template local action·13D-4 history filter action·13D-5 local job upsert model·13D-6 composer model·13D-7 학생 이름 fallback model·13D-8 audience 번호 accessor·13D-9 compose workspace·13D-10 notice shell·13E-1 navigation hook·13E-2 recipient hook·13E-3 history hook·13E-4 composer hook·13F-1 template adapter·13F-2 identity callback·13F-3 job builder binding·13F-4 request binding·13F-5 background refresh binding·13F-6 즉시발송 binding·13F-7 예약발송 binding·13F-8 결과 대조 binding·13F-9 AI 문구 수정 binding·13F-10 알림 이력 삭제 binding·13F-11 Solapi 예약 취소 binding까지 완료했고, 다음은 NotificationCenter 남은 경계 inventory입니다.
