...
> **UI 배포 후 누적 시각 검수 — 작업 중단 아님:** UI-2C는 수업일지 탐색/행동과 숨긴 달력 상단바, UI-2D는 학사일정·학생관리·시험관리·정산 페이지 머리말, UI-2E-1은 학부모 응대·학사 개요·시험 미리보기·학생 포털·최근 응시 회차의 제목/건수, UI-2E-2는 테스트 학생·정산월·특강 접힘·학생 프로파일·문항/숙제 보기 행동을 확인한다. UI-2E-3A는 보충 처리·수업 운영 알림·포함 학생·시험 후 제출·수업연구 필터·HWPX 입력·학사일정 시험 초안·정산/응시 저장 상태의 줄바꿈과 모바일 폭만 확인한다. 저장·삭제·취소·알림 발송·AI 생성·다운로드 버튼은 누르지 않는다. 이 시각 검수는 뒤 UI 단위와 독립적이므로 다음 UI 작업을 계속 진행한다.
