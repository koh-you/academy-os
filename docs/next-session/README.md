...
4. App.jsx 리팩터링 연속 작업
   ...
   - 9번 `test manager`는 완료됐습니다. 10번 `student-parent portals` inventory와 자료함/정적 탭/공용 탭 내비게이션/리포트 카드/학생 지표 행 분리를 완료했으며, 다음은 저장·인증과 무관한 학부모 알림 카드 목록부터 이어갑니다.
...
현재 리팩터링 이어받을 지점:
- 9번 `test manager`는 완료됐습니다. 10번 `student-parent portals` inventory와 자료함/정적 탭/공용 탭 내비게이션/리포트 카드/학생 지표 행 분리도 완료했고, 다음 시작점은 학부모 알림 카드 목록입니다.
...
- 다음 후보: 10번 `student-parent portals`의 학부모 알림 탭 읽기 전용 카드 목록. 필터·정렬은 상위에 유지하고 포털 shell·인증·학생 쓰기 기능은 뒤로 둡니다.
...
- 다음 리팩터링 시작점은 10번 `student-parent portals`의 학부모 알림 카드 목록입니다. 포털 공개 링크·인증·저장 원천·모바일 표시 inventory와 자료함/정적 탭/공용 탭 내비게이션/리포트 카드/학생 지표 행 분리는 완료됐습니다.
