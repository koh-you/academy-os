...
> **2026-07-31 퇴원생 결석보강·보강만 취소 복구 — 사람 gate 1건:** 퇴원생도 과거 수업일지의 결석기록이 있으면 보충관리 후보에 유지된다. 최종 `보강만 취소`에서 발생한 `createPersistableSupplementTask is not defined`는 누락 import를 복구했고, 퇴원생 회귀 fixture·정적 시나리오 809/809·전체 production 테스트·build를 통과했다. 배포 후 실제로 취소하려는 조소현 항목에서만 `보강만 취소`를 최종 확인한다. 활성 후보 제거, 최근 이력의 `보강 취소 · 결석 유지`, 2026-07-21 원 수업일지 결석·병결 보존, 새 알림톡 예약·발송 없음이 성공 기준이다. `결석 기록 취소`는 누르지 않는다.
