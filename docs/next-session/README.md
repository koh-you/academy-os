...
> **UI 배포 후 누적 시각 검수 — 작업 중단 아님:** UI-2C는 수업일지의 뒤로가기·제목·수업 주제·수정·취소 버튼 배치와 숨긴 달력 상단바가 다시 나타나지 않는지 확인한다. UI-2D는 학사일정·학생관리·시험관리·정산을 데스크톱과 모바일에서 각각 열어 제목→설명→필터/검색/주요 행동 순서, 버튼 줄바꿈, 가로 넘침이 없는지 확인한다. UI-2E-1은 학부모 응대·월간 학사 개요·시험분석 최종 미리보기·학생 포털 오늘 숙제/수업 기록·최근 응시 회차에서 제목·설명·건수 badge가 겹치지 않는지 확인한다. 저장·삭제·취소·알림 발송 버튼은 누르지 않는다. 이 시각 검수는 뒤 UI 단위와 독립적이므로 다음 UI 작업을 계속 진행한다.
