...
App.jsx 보충관리 리팩터링은 최신 `origin/main`에 rebase한 뒤 `12R-10` 저장상태 patch 계산, `12R-11` task별 상태 map 병합, `12R-12` 알림톡 제어창 local state hook, `12R-13` 완료·일정 확인창 local state와 일정 확인 payload 조립까지 분리했다. API·Supabase·Solapi와 실제 완료·일정 action은 기존 App 경계에 둔다. 12R-13은 production 394/394와 build를 통과했고, 재시험 독립 탭이 없는 화면 구조 때문에 사용자 지시에 따라 AI gate로 통과했다. 검수용 task·lesson과 관련 job prefix는 모두 0건으로 정리했으며 Solapi 호출은 없었다.
