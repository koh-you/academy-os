...
4. App.jsx 리팩터링 연속 작업
   ...
   - 9번 `test manager`는 완료됐습니다. 10번 `student-parent portals` inventory와 공용 읽기 전용 `PortalMaterialsTab` 분리를 완료했으며, 다음은 낮은 위험의 빈 상태/평가 표시 컴포넌트부터 이어갑니다.
...
현재 리팩터링 이어받을 지점:
- 9번 `test manager`는 완료됐습니다. 10번 `student-parent portals` inventory와 공용 자료함 tab 분리도 완료했고, 다음 시작점은 빈 상태/평가 표시 컴포넌트입니다.
...
- 다음 후보: 10번 `student-parent portals`의 빈 상태/평가 같은 낮은 위험 표시 전용 컴포넌트. 포털 shell·인증·학생 쓰기 기능은 뒤로 둡니다.
...
- 다음 리팩터링 시작점은 10번 `student-parent portals`의 빈 상태/평가 표시 컴포넌트입니다. 포털 공개 링크·인증·저장 원천·모바일 표시 inventory와 공용 자료함 tab 분리는 완료됐습니다.
