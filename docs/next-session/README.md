...
> **UI 배포 후 누적 시각 검수 — 작업 중단 아님:** UI-2C는 수업일지 탐색/행동과 숨긴 달력 상단바, UI-2D는 학사일정·학생관리·시험관리·정산 페이지 머리말, UI-2E는 모든 섹션 머리말의 제목/설명·건수/actions 줄바꿈과 모바일 폭을 확인한다. UI-3A/3B는 상위·보조·중첩 탭 전환과 위계, UI-3C는 학사일정 표시·학교·월, 수업일지 종류·월, 수업연구 과목, 알림 대상 반, 시험·학생 반, 시험 고사, 오답 학년·학생, 월별 정산월 필터의 전환 결과와 건수·월 경계를 확인한다. UI-3D는 알림 발송·학부모 응대·수업 명단·시험·특강·보충 이력의 검색, 결과 건수, `×` 지우기, 검색/원천 0건 구분을 확인한다. UI-3E는 알림·퇴원생·시험 후 대상·AI 변형문항·자료 공개 학생·특강 회차를 체크만 해 선택 수와 안전/danger actions 분리가 맞는지 확인한다. UI-4A-1은 알림 대상·시험대비 수업·시험 후 제출·보고서 원천·월간 학사의 16개 요약 카드 값과 label·hint를 확인한다. 390px에서는 탭/필터 내부 스크롤, 검색 입력·지우기, 선택 작업 바 줄바꿈, 요약 카드 한 열 배치와 키보드 `Tab` focus가 보여야 한다. 저장·삭제·취소·알림 발송·AI 생성·업로드·다운로드·일정 등록·질문 변경·보충 되돌리기·HWPX·다시 생성 버튼은 누르지 않는다. 이 시각 검수는 뒤 UI 단위와 독립적이므로 다음 UI 작업을 계속 진행한다.
