...
4. App.jsx 리팩터링 연속 작업
   ...
   - 9번 `test manager`는 완료됐습니다. 10번 `student-parent portals`의 읽기 전용 표시 영역 분리를 완료했습니다. 다음 경계는 `StudentTodayTab`의 시험 후 제출, 질문 추가·상태변경·삭제, 숙제 완료 체크이며 저장 원천/실패 UI 사람 gate 전에는 이동하지 않습니다.
...
- 다음 리팩터링 시작점은 `StudentTodayTab` 쓰기 기능의 저장 원천/실패 UI gate입니다. 수업 준비 안내의 `prepStudentVisible` 미사용은 별도 기능 이슈로 남깁니다.
