...
- 화면 `최종 미리보기` 패널은 도넛 차트, 막대 차트, 문항 흐름, 주요문항 후보, 상세 표를 보여준다. 난이도는 AI가 틀릴 수 있으므로 상세표에서 바로 수정하고 `난이도 수정 저장`으로 기존 검수 저장 API에 저장한다. 앱 안 미리보기 1차는 수학 수식/그래프/시험지 원문을 재렌더링하지 않는다.
- 팔레트는 더 전문가적인 톤으로 조정했다. 단원 색상과 난이도 색상은 `src/domains/exams/finalPreview.js`의 `examAnalysisPreviewPalette`에 모여 있으며, 테스트 후 고정 색상코드로 확정한다.
