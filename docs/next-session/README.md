...
1. 특강 선개설·Tally/수동 명단·학생별 회차/진행 모달
   - 특강관리 콘텐츠 최상단을 `특강 수업`과 `특강 안내문` 탭으로 분리했고, `특강 수업`을 첫 탭이자 기본 진입 화면으로 둬 안내문 편집기와 운영 명단이 한 페이지에 길게 이어지지 않음.
   ...
   - 새 SQL 없음. 검증은 `npm run test:production` 318개, `npm run build`, Node check, `git diff --check` 통과.
