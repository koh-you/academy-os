...
> **UI-7F-2 누적 disclosure 검수 — 작업 중단 아님:** 390px에서 특강 취소 action/취소 명단, 수업연구 chapter/unit, 시험 후 `전체 원문 보기/직접 수정`, 월별 정산 `삭제한 정산 행`, 시험분석 프롬프트의 역할/출력/검수 단계 summary를 각각 한 번 열고 닫는다. 터치 영역이 최소 44px이고 키보드 focus outline이 잘리지 않으며 열림 상태와 내부 내용이 기존대로여야 한다. 저장·복사·삭제 버튼은 누르지 않는다.

> **UI-7E-4 누적 오작동 action 검수 — 작업 중단 아님:** 로그인 카드의 의미 없는 X, 반관리 `+ 반 추가`, 오답 교재의 `전부 맞음/오답수정/기록`, 학생 마이페이지의 작동하지 않는 비밀번호 입력/변경, 학생 포털 말풍선이 보이지 않는지 확인한다. 실제 callback이 있는 수업 등록·월 이동·로그아웃은 그대로 보여야 한다. 어떤 입력·저장·발송·삭제도 실행하지 않는다.

> **UI-7E-3 누적 모바일 검수 — 작업 중단 아님:** 390px iPhone Safari에서 사이드바의 작은 메뉴, 탭·필터·표 안 compact 버튼, 자료/시험분석의 `열기` 링크, PDF 업로드 drop 영역을 차례로 본다. 각 행동 영역이 최소 44px 높이여야 하고 키보드 focus outline이 잘리지 않아야 한다. 버튼 동작과 파일 선택은 실행하지 않으며 저장·발송·예약·삭제·출결도 누르지 않는다.

> **UI-7E-2 누적 접근성 검수 — 작업 중단 아님:** 모바일 VoiceOver/키보드로 월 이동 화살표, 수업일지 뒤로, 출결 닫기·한 자리 지우기, 보충 완료 알림·알림 제어 닫기, 공통 모달 닫기를 탐색한다. 기호만 읽히지 않고 각각 `이전 달`, `다음 달`, `수업 목록으로 돌아가기`, `출결 화면 닫기`, `한 자리 지우기`, 해당 닫기 이름으로 읽혀야 한다. 수학시험 삭제와 실제 저장·발송·삭제 action은 실행하지 않는다.
