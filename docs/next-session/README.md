...
App.jsx 보충관리 리팩터링은 `12R-37`에서 `SupplementCenter` 본체를 전용 파일로 이동해 로드맵 12 `supplement center/modals`의 App 본체 분리를 완료했다. 13A-1에서는 알림센터 원천·side effect inventory와 header/status/navigation shell을 분리했고, 13A-2에서는 수신 모드·반/검색·건수·학생 체크 목록을 `NotificationRecipientPanel.jsx`로 이동했다. 실제 학생/반 원천, 선택/초안 state, 수신 대상 계산, 즉시발송·예약·취소·삭제·결과 대조·AI 수정·특강관리 callback은 App에 남는다. production 422/422와 build를 통과했다. 다음은 공지 작성/미리보기 UI를 controlled 컴포넌트로 분리하되 AI 수정·예약·즉시발송 handler는 App에 남긴다.
...
- 12번 `supplement center/modals` App 본체 분리 완료. 13번 `notification center`는 13A-1 navigation shell과 13A-2 수신자 선택 panel까지 완료했고, 다음은 공지 작성/미리보기 controlled UI입니다.
