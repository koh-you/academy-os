...
> **2026-07-31 퇴원생 결석보강·보강만 취소 복구 — 사람 gate 완료:** 퇴원생도 과거 수업일지의 결석기록이 있으면 보충관리 후보에 유지된다. 최종 `보강만 취소`에서 발생한 `createPersistableSupplementTask is not defined`는 누락 import를 복구했고, 퇴원생 회귀 fixture·정적 시나리오 809/809·전체 production 테스트·build를 통과했다. 사용자가 Production에서 실제 조소현 항목의 `보강만 취소` 완료를 확인했으며 서버 재조회·원 결석 snapshot 보존 계약까지 완료 응답에 포함된다. 다음 세션에서 다시 요구하지 않는다.
