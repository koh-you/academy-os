...
7. 작업 완료 답변에는 짧은 AI 자기검토를 포함한다. 전체 맥락, 사용자 의도, 변경 이유, 저장 원천, 사용자 편집본 보호, 검토 가능성, 중단 조건에 답할 수 있어야 한다.
8. 시험분석을 건드린 작업은 시험분석 탭 전용 검토 절차를 추가로 포함한다.
9. 기존 시험분석 기능을 복구하지 않는다.
10. 운영 Supabase에서 새 v2 테이블과 `exam-analysis-pipeline-sources` Storage bucket이 정상인지 확인한다.
