...
> **UI-7D-5 누적 접근성 검수 — 작업 중단 아님:** 브라우저 접근성 트리에서 로그인 아이디·비밀번호가 현재 역할 안내를 설명으로 참조하고, 월별 정산 스케줄이 같은 학생 행의 형식/시수 도움말을 참조하는지 확인한다. 이미 화면에 `스케줄 수정 필요` 또는 특강 `조정 사유를 입력해 주세요`가 보이는 행이 있다면 해당 control만 invalid이고 해당 문구만 참조해야 한다. 오류를 새로 만들기 위해 로그인 실패나 정산 값 변경·저장을 실행하지 않는다.

> **UI-7D-4 누적 접근성 검수 — 작업 중단 아님:** 모바일 VoiceOver/접근성 트리에서 학생 프로필의 새 상담·성적·테스트와 기존 행, 월별/특강 정산의 학생별 방식·금액·스케줄·사유, 테스트관리의 학생별 응시 상태·정답 수·미응시 사유, 학생 포털 질문 입력을 순서대로 탐색한다. 각 이름에 학생명과 열 의미가 있어야 하며 같은 `점수`, `메모`만 반복되면 실패다. 값을 바꾸거나 저장·발송·삭제하지 않는다.

> **UI-7D-3 누적 접근성 검수 — 작업 중단 아님:** 모바일 VoiceOver 또는 브라우저 접근성 트리에서 로그인 아이디·비밀번호, 운영 알림 종류/대상/날짜/시간/중요도/상태, 수업일지의 학생별 교재·진도·숙제 상태, 설정의 AI 제공자·모델, 학생별 오답의 출처/범위/상태/메모가 각각 화면 맥락과 학생·문항을 포함한 이름으로 읽히는지 확인한다. placeholder만 읽거나 같은 이름이 반복되면 실패다. 값을 바꾸거나 저장·발송·삭제하지 않는다.

> **UI-7D-2 누적 모바일 검수 — 작업 중단 아님:** 390px iPhone Safari에서 로그인 input, 학생 프로필의 text/date/time/select/textarea, 시험관리의 number input을 차례로 focus한다. 각 control이 읽을 수 있는 16px 글꼴과 최소 44px 한 줄 높이를 유지하고 브라우저가 화면을 확대하거나 페이지 전체 가로 스크롤을 만들지 않아야 한다. checkbox/radio/file은 기존 native 크기·동작이어야 한다. 값을 바꾸거나 저장·발송·삭제하지 않는다.

> **UI-7C-2 누적 모바일 검수 — 작업 중단 아님:** 390px에서 `알림관리`의 상단 탭을 오른쪽 끝까지 이동해 `학부모 응대`처럼 뒤쪽 탭을 선택하고 다른 상위 탭을 왕복했을 때 현재 선택 tab이 가로 영역 안에 보이는지 확인한다. 알림 대상 필터는 좌우로 밀어 option·검색 field·결과 건수에 모두 접근할 수 있어야 한다. 학생 검색어를 입력한 뒤 `지우기`를 눌러 값과 결과가 초기화되고, 버튼이 44px 높이 안에서 쉽게 눌리는지 확인한다. 페이지 자체가 좌우나 위아래로 갑자기 이동하면 실패다. 저장·발송·삭제는 누르지 않는다.

> **UI-7B-2 누적 모바일 검수 — 작업 중단 아님:** 390px에서 알림관리 PageHeader, 수업일지 NavigationHeader, 학생관리의 대표 SectionHeader를 열어 긴 제목·설명이 화면 밖으로 넘치지 않고 leading/title/actions가 자연스럽게 줄바꿈되는지 확인한다. header의 작은 버튼도 손가락으로 누를 높이가 확보돼야 한다. 화면 이동 외 저장·발송·삭제 버튼은 누르지 않는다.
