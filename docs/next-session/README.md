...
> **UI-8A-1 누적 월간 달력 검수 — 작업 중단 아님:** 390px에서 수업일지 월간 달력 영역에 focus한 뒤 좌우로 밀어 일~토 7열을 확인한다. 달력 안에서만 움직이고 페이지·sidebar 전체가 좌우로 밀리지 않아야 하며 날짜·수업 시간·반 이름이 이전보다 읽혀야 한다. 사용자 요청으로 숨긴 달력 상단바는 계속 보이지 않아야 한다. 수업 pill·수업 등록·월 정규수업 열기·출결·저장·삭제는 누르지 않는다.
