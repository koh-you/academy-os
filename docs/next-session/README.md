...
4. App.jsx 리팩터링 연속 작업
   ...
   - 9번 `test manager`는 완료됐습니다. 10번 `student-parent portals` inventory, 공용 읽기 전용 `PortalMaterialsTab`, 정적 빈 상태/평가 탭 분리를 완료했으며, 다음은 저장·인증과 무관한 공용 탭 내비게이션부터 이어갑니다.
...
현재 리팩터링 이어받을 지점:
- 9번 `test manager`는 완료됐습니다. 10번 `student-parent portals` inventory, 공용 자료함 tab, 정적 빈 상태/평가 tab 분리도 완료했고, 다음 시작점은 공용 탭 내비게이션입니다.
...
- 다음 후보: 10번 `student-parent portals`의 학생·학부모 공용 탭 내비게이션. active state는 상위에 유지하고 포털 shell·인증·학생 쓰기 기능은 뒤로 둡니다.
...
- 다음 리팩터링 시작점은 10번 `student-parent portals`의 공용 탭 내비게이션입니다. 포털 공개 링크·인증·저장 원천·모바일 표시 inventory, 공용 자료함 tab, 정적 빈 상태/평가 tab 분리는 완료됐습니다.
