...
현재 새 v2 구조의 SQL, 백엔드 run/PDF 업로드/삭제 API, 첫 UI(학교/학년/고사/분석 카드형 목록, 기본정보 저장, PDF 업로드, 상태 확인), PDF 텍스트 후보 추출, Claude 우선 원본 검증, 문항 수 선생님 확인, 1~N 빈 행 생성, 문항 경계 탐지 1단계, AI 행 채움 1단계, 엑셀식 AI 결과 검수/선생님 확정 UI, 쎈 기준표 연결, 재확인 문항 AI 2차 수정까지 들어가 있습니다. 다음 작은 단계는 AI 2차 수정 결과를 실제 화면에서 검증하고, 남은 미확정 문항 처리/최종 산출물 생성 흐름을 설계하는 것입니다.
```
...
1. 시험분석 다음 작업은 실제 운영 PDF에서 `AI 2차 수정(과금)`을 눌러 17, 21, 24번 같은 재확인 문항이 쎈 유형명 수준으로 개선되는지 검증하는 것이다.
2. 2차 수정 후에도 대분류가 나오면 프롬프트를 덧대기보다 원인부터 확인한다. 쎈 후보 과목이 공통수학1로 잡혔는지, 프롬프트에 후보 목록이 들어갔는지, AI 응답이 후보 밖 이름을 냈는지 구분한다.
3. 그 다음은 남은 미확정 문항 처리와 최종 산출물 생성 흐름이다. 선생님이 이미 수정/확정한 `teacher_fields`/`final_fields`는 덮어쓰지 않는다.
