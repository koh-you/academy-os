...
App.jsx 보충관리 리팩터링은 최신 `origin/main`에 rebase한 뒤 `12R-10~20` local state/표시 모델, `12R-21~25` 다섯 실제 action의 modal adapter, `12R-26~27` local draft 변경과 완료·일정 확인창 submit adapter를 분리했다. 12R-28에서는 334줄 규모의 `SupplementStudentModal` 본체를 전용 파일로 옮겼고, 12R-29에서는 `SupplementCenter`의 모달 저장·일정·결석취소·완료 callback adapter 네 개를 전용 controller로 분리했다. 실제 task/lesson/출결 저장·Supabase 재조회·React 전역 갱신·notification/Solapi 작업은 App이 주입하는 callback 경계에 남고 새 controller에는 직접 API/notification/Solapi 호출이 없다. production 410/410, build를 근거로 AI gate를 통과했다. 다음은 `SupplementCenter` 목록 카드의 별도 완료 확인 adapter와 후보·탭 표시 모델을 한 의미 단위씩 inventory한다.
