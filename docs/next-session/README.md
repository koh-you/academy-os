...
6. `docs/slack-integration-guide.md`를 보며 Slack Incoming Webhook과 Render 환경변수를 설정합니다.
7. `/api/integrations/status`에서 `slackConfigured: true`가 나오는지 확인합니다.
8. `/api/notifications/slack-today-schedule` 수동 테스트로 선택한 Slack 채널에 오늘 운영 요약이 도착하는지 확인합니다.
9. Supabase SQL editor에서 `supabase/20260710_academy_reminders.sql` 적용 여부를 확인하고, 대시보드 `운영 알림 원본` 저장/새로고침 유지를 검수합니다.
10. 수업일지에서 저장 버튼과 Solapi 버튼이 분리되어 있는지 확인합니다.
11. 이미 예약 시간이 지난 수업에서 `솔라피 발송결과` 버튼을 눌러 OS 상태가 Solapi 원천과 맞게 갱신되는지 확인합니다.
12. 시험분석 탭 진입이 정상인지 확인합니다.
13. 시험분석 산출물 패널에서 `GPT 대화세션 체크리스트`, `GPT 기획 패킷 복사`, `산출물 ZIP`이 보이고, `블로그형 카드 디자인 Gate 3`/카드 미리보기 렌더러가 보이지 않는지 확인합니다.
14. 웹앱 체크리스트에 입력한 선생님 확정값이 `산출물 저장` 후 새로고침해도 유지되는지 확인합니다.
15. 자동저장 위험 후보 화면에서 `자동저장 위험` 배지와 `왜 위험한가` 버튼을 눌러 저장 원천/중단 조건이 보이는지 확인합니다.
