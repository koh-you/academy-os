...
4. App.jsx 리팩터링 연속 작업
   ...
   - `StudentTodayTab`, `ParentPortal`, `StudentPortalShell`도 저장/API 없이 분리했고 미사용 legacy `StudentPortal`을 제거했습니다. `StudentPortalV2`는 학생 선택·탭 상태·파생 계산·쓰기 권한/callback을 계속 소유합니다. 다음 단위는 10번 포털 완료 audit입니다.
...
- `StudentTodayTab`, `ParentPortal`, `StudentPortalShell`도 분리됐고 호출되지 않던 legacy `StudentPortal`은 제거됐습니다. shell은 화면 조합만 담당하고 `StudentPortalV2` controller가 파생 데이터·상태·쓰기 권한/callback을 계속 소유합니다.
- 다음 리팩터링 시작점은 10번 포털 완료 audit입니다. 남은 controller helper, 실제 로그인/강사 미리보기 권한, 저장 API/Storage/보안 후속 gate를 표로 다시 확인하고 안전한 추가 분리가 없으면 10번을 닫으세요. 교사 확인 API와 시험지 Storage 열기 권한은 현재 교사 bearer session이 없는 기존 인증 공백이므로 별도 `교사 세션 인증 + 파일 열람 권한` 고위험 gate로 분리합니다.
