...
> **UI 배포 후 누적 시각 검수 — 작업 중단 아님:** UI-2C는 수업일지 탐색/행동과 숨긴 달력 상단바, UI-2D는 학사일정·학생관리·시험관리·정산 페이지 머리말, UI-2E는 학부모 응대·학사 개요·학생 포털·시험분석·알림·특강·운영 알림·수업메모·수업연구·보충·오답·보고서 등 모든 섹션 머리말의 제목/설명/건수/actions 줄바꿈과 모바일 폭을 확인한다. UI-3A는 알림/특강·알림관리·시험분석·시험관리·설정·학생관리·정산의 탭 전환, 390px 탭 내부 가로 스크롤, 키보드 `Tab` focus를 확인한다. 저장·삭제·취소·알림 발송·AI 생성·업로드·다운로드·질문 변경 버튼은 누르지 않는다. 이 시각 검수는 뒤 UI 단위와 독립적이므로 다음 UI 작업을 계속 진행한다.
