...
App.jsx 보충관리 리팩터링은 최신 `origin/main`에 rebase한 뒤 `12R-10` 저장상태 patch 계산부터 `12R-20` 알림톡 제어창 source view model까지 분리했고, `12R-21~22`에서 보충 내용 저장·완료 처리 modal adapter를 각각 `supplementTaskContentSaveController.js`, `supplementTaskPassController.js`로 이동했다. 실제 task 저장·Supabase 재조회·React 전역 갱신·완료 시 학생 11시 job 취소는 App이 주입하는 callback 경계에 남고 controller에는 직접 API/notification/Solapi 호출이 없다. 앞선 고태영 저장·완료·새로고침 사람 확인과 새 deterministic fixture, production 403/403, build를 근거로 AI gate를 통과했다. 다음은 일정 생성·변경 modal adapter 한 단위다. 새 화면 검수가 필요하면 독립 `재시험` 탭을 가정하지 말고 `숙제보충` 또는 `결석보강`에 노출되는 격리 고태영 데이터를 먼저 준비한다.
