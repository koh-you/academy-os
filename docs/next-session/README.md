...
4. App.jsx 리팩터링 연속 작업
   ...
   - 9번 `test manager`는 완료됐습니다. 10번 `student-parent portals`의 저위험 표시 영역과 학생/학부모 숙제 탭 분리를 완료했으며, 다음은 읽기 전용 학생 마이페이지의 local state/helper 경계를 inventory합니다.
...
현재 리팩터링 이어받을 지점:
- 9번 `test manager`는 완료됐습니다. 10번 `student-parent portals`의 저위험 표시 영역과 학생/학부모 숙제 탭 분리도 완료했고, 다음 시작점은 학생 마이페이지 inventory입니다.
...
- 다음 후보: 10번 `student-parent portals`의 읽기 전용 학생 마이페이지. local sub-tab state와 helper 의존성을 먼저 inventory하고 포털 shell·인증·학생 쓰기 기능은 뒤로 둡니다.
...
- 다음 리팩터링 시작점은 10번 `student-parent portals`의 학생 마이페이지 inventory입니다. 포털 공개 링크·인증·저장 원천·모바일 표시 inventory와 저위험 표시 영역/학생·학부모 숙제 탭 분리는 완료됐습니다.
