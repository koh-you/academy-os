...
- 시험대비 상세에서 해당 일요일의 `오늘 진행한 내용`을 간단히 적고 명시적으로 저장할 수 있다. 학생별 내용이 다르면 `김OO: 경우의 수 보충`처럼 한 칸에 줄 단위로 적는다.
- 이 내용은 그 날짜 수업에만 저장되고 이후 일정의 시간 전파 대상이 아니다. 자동 시험대비 재계산과 추가 일정 수정도 작성 내용을 기본 `시험대비`로 덮지 않는다.
