...
- `+ 수업 등록 -> 휴강 보충`에서 시간대별 학생만 선택하고 `휴강 보충 알림 예약`을 켜면 일정 저장 후 학생·학부모 다음 정각 안내와 학생 당일 11시 알림을 함께 예약한다.
- 기존 휴강보충 수업도 수정 화면에서 같은 기능을 쓴다. 운영 박수빈·이하민·김재연 일정은 아직 실제 알림을 예약하지 않았으며, 배포 후 각 일정을 열어 대상·시간을 확인하고 명시적으로 예약해야 한다.
