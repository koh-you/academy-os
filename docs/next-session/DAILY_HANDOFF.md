...
- 다음 수업의 `확인할 숙제`에서 `확인 완료`를 누르면 원 `lesson_student_records` followup 필드를 versioned CAS·재조회로 비운 뒤에만 표시가 사라진다. 레거시 followup 메모 줄도 함께 정리하되 다른 메모·숙제 판정·코멘트는 유지한다.
- 충돌/실패에서는 표시와 재시도 버튼을 유지한다. 검증: lesson `16/16`, lint, production `823/823`, build `399 modules`·main `943.28 kB`·lazy `12/12`, 집중 browser `1/1`, 전체 safe browser `36/36`.
