...
## 2026-08-04 시험정보 전체 조회와 자동행 삭제

- `시험정보`에 `전체 반` 조회를 추가했다. 전체 조회는 모든 활성 학생의 학교·학년 행을 보여 주며, 시험 후 제출 탭은 기존 반별 흐름을 유지한다.
- 반·고사 선택은 더 이상 시험행을 자동생성하지 않는다. 운영 2학기 자동생성 23행 중 대화에서 확인된 11행을 보존하고 미확인 빈 행 12건만 감사 삭제했다. Supabase 재조회는 전체 64→52행, 삭제 대상 0건, 연결 시험대비 수업 0건이다.
- 전용 fixture·runtime lint·production `827/827`·build `410 modules`·로컬 안전 브라우저는 통과했다. 첫 원격 검사가 발견한 탭 선택 setter 정적 계약도 기존 setter를 보존하는 형태로 교정했다.
