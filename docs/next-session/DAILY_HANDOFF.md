...
- 저장 자체는 명단 0명도 허용한다(기존 계약 그대로 · 휴강처럼 명단 없는 수업이 있다). 대신 푸터에 「포함 학생 0명으로 저장됩니다」 를 띄운다. 휴강 보충·신입생 보강은 예전처럼 1명 이상을 강제한다.
- 스크린샷: `C:/Users/PC/github/academy-os-ui-audit/after/lesson-modal-rework-{1440x950,1280x800,1024x768,390x844}-{top,scrolled}.png`.

## 2026-09-26 수업 등록 모달 검증 반영 (PR #425)

- 사람 확인: ① 신규 등록이 「직접 입력 일정」 으로 열리고, 오른쪽 「반 불러오기」 에서 **목록 첫 반**을 골라도 수업명·시간·명단이 채워지는지. ② 기존 수업을 열어 시작만 옮겼을 때 직접 정해 둔 종료 시간이 유지되는지. ③ 휴대폰(390)에서 「수업 취소」 가 전폭이 아니고 저장 행 위에 있는지. ④ 반관리 › + 반 개설 모달이 예전과 같아 보이는지.
- 검색 중에는 「전체 N명 선택/해제」 토글을 숨긴다(대상이 화면에 보이는 목록이 아니라 재원생 전체라서). 검색 중 일괄 선택은 「검색 결과 N명만 선택」 하나만 쓴다.
- 스크린샷: `C:/Users/PC/github/academy-os-ui-audit/after/lesson-modal-fix-*.png`(register/edit × 1440·1280·1101·1024·390, register-top × 1024·1101·390, class-template × 1440).

## 2026-09-26 설명 문구 정리 완료

- 사람 확인: ① 자주 쓰는 화면(수업일지·학생관리·시험분석)에서 물음표에 커서를 올려 설명이 읽을 만한지, 지워진 문장 중 아쉬운 것이 있는지. ② 반관리·자료함처럼 헤더 설명이 사라진 화면이 허전하지 않은지. ③ 태블릿(1024)·휴대폰(390)에서 물음표를 눌러 말풍선이 잘리지 않는지.
- 되돌리고 싶은 문구가 있으면 `docs/ui-helptext-cleanup-2026-09-26.md` 의 표에서 PR 번호를 찾아 그 커밋만 되돌리면 된다. 전수 목록은 작업 폴더 `academy-os-ui-audit/helptext-inventory-2026-09-26.md`.
- 다음 후보: 오답관리 390px 가로 넘침(기존 문제), 시험분석 산출물 패널의 `작성 도움말 보기` 토글과 물음표 이원화 정리, 학사일정 수정 모드 물음표 브라우저 커버리지.

## 2026-09-19 UI 구조 정돈 14 PR 병합

- 사람 확인: ① 운영 화면에서 알림관리 › 개별 발송 → '즉시 발송' 이 확인 모달을 거치는지(수신 건수·미리보기·[N건 지금 발송]). ② 수업일지 편집 → [편집 취소] 와 하단바 '저장 전 변경 N건' 표시. ③ iPad 가로(또는 브라우저 1024px)에서 사이드바가 아이콘 레일이고 ‹ 로 펼쳐지는지. ④ 교재관리 › 교재 삭제가 ⋯ 메뉴 → 확인 대화상자인지.
- Vercel: 19일 오후 빌드 한도로 미리보기/배포가 막혔다. 20일에 `academy-os-blue.vercel.app` 이 main(`76f5dc84` 이후)으로 갱신되지 않았으면 빈 커밋 push 또는 Vercel 대시보드 재배포.
- 다음 단위 후보(보고서 §10): 총평 'AI 수정' 실행 중 비활성 · 설정 '기본값 복원' ConfirmDialog · 자동 생성 수업 즉시 삭제 2곳 확인 · 정산 저장 바 [되돌리기] · 전역 뷰 전환 dirty 가드 설계 · window.confirm 25곳 순차 교체.
## 2026-09-21 개념원리 고등 6권 등록

- 사람 확인: 바탕화면 `문제은행-패키지\개념원리-공통수학2-라텍스-패키지`(839) · `개념원리-대수-라텍스-패키지`(885) · `개념원리-기하-라텍스-패키지`(624) · `개념원리-미적분1-라텍스-패키지`(591) · `개념원리-미적분2-라텍스-패키지`(719) · `개념원리-확률과통계-라텍스-패키지`(496)를 교재관리 › 패키지 등록(문항·답 이미지 = 문항 수 · 해설 없음 — 빠른정답만 원천). 미리보기: 공통수학2 118-286(보기 그래프 ①~⑤ 라벨) · 21-26(수직선 아래 배치) / 대수 291-686(재크롭 그림) · 180-426(과정 상자 안 그래프) / 기하 158-314(소문항별 TikZ 격자) · 141-288(구 방정식 두 줄) / 미적분Ⅰ 199-406(별도 줄 수식) · 220-452(그림 아래 배치) / 미적분Ⅱ 260-529(두 줄 lim) · 36-64(선분 첨자) / 확통 51-92(파스칼의 삼각형 빈칸) · 164-365(두 행 수식).
- 🔴 반영 완료 2건(대수 291-686 · 기하 158-314). 검수 요약 `latex-bank/<bank>/검수/2026-09-21/00-검수-요약.md`. 확통 73-151 은 그림을 문항 맨 아래에 둘 배치값이 없어 현행(상자 앞) 유지 — 필요하면 build.mjs `figure_layout: "end"` 추가.
## 2026-09-21 개념원리 중3-1 등록

- 사람 확인: 바탕화면 `문제은행-패키지\개념원리-공통수학1-라텍스-패키지`(810 · 답 810)를 교재관리 › 패키지 등록. 미리보기: 139-e1(예제 그래프) · 91-e1(특강 예제) · 286-627(cases 별도 줄) · 34-55(조립제법) · 270-601(삽화 크롭).
- 사람 확인: 바탕화면 `문제은행-패키지\rpm-기하-라텍스-패키지`(806 · 해설 500) · `rpm-미적분2-라텍스-패키지`(1177 · 해설 774)를 교재관리 › 패키지 등록. 미리보기: 기하 0727(두 직선 별도 줄)·0435(그림 아래 배치)·0574(재크롭) · 미적분Ⅱ 1071(과정 상자 두 행)·0256(그림 「…」).
- 사람 확인: 바탕화면 `문제은행-패키지\rpm-대수-라텍스-패키지`(1126 · 해설 1110) · `rpm-미적분1-라텍스-패키지`(885 · 해설 833) · `개념원리-중3-2-라텍스-패키지`(472)를 교재관리 › 패키지 등록. 미리보기: 대수 1117(긴 증명 문항) · 0945(그림 「…」) · 미적분Ⅰ 0744(별도 줄 등식) · 0112(그림 아래 배치) · 3-2 38-03(상자 안 그림) · 127-03(자료 두 줄).
- 사람 확인: 바탕화면 `문제은행-패키지\rpm-확률과통계-라텍스-패키지`(문항·답 596 · 해설 539)를 교재관리 › 패키지 등록. 미리보기: 0487(보기 ⑤ 수정)·0433(보기 그래프)·0150(분수 높이 빈칸)·0510(표 인라인)·0592(표준정규분포표).
- 사람 확인: 바탕화면 `문제은행-패키지\개념원리-중3-1-라텍스-패키지`(문항·답 788 · 해설 없음 — 정답 PDF 의 빠른정답만 원천)를 교재관리 › 패키지 등록. 미리보기: 78-18(지수 빈칸)·177-01(표+좌표평면)·90-03(십자 곱셈 그림 소문항별)·205-03(소문항별 그래프)·145-c4(2단 보기).
- 🔴 없음. 검수 요약 `latex-bank/gn-m31/검수/2026-09-21/00-검수-요약.md`. 100-h5·100-h6·101-06 의 「UP」 배지는 tag 「UP」 으로 적었음(규약 외 · 확인).
- RPM 대수·기하·미적분Ⅰ·확통·미적분Ⅱ 는 전사·병합 후 검수 단계(사람 gate 없음).

## 2026-09-20 RPM 중3-2(22개정) 등록

- 사람 확인: 바탕화면 `문제은행-패키지\rpm-중3-2-22개정-라텍스-패키지`(문항·답·해설 673)를 교재관리 › 패키지 등록. 교재별 오답에서 0044(삼각비 표 그룹)·0509(보기 그래프 5개)·0579(표 below)·0627(상자그림 → 보기 순서)·0653(단위 표기) 미리보기.
- 🔴 없음. 검수 요약 `latex-bank/rpm-m32/검수/2026-09-20/00-검수-요약.md`.

## 2026-09-20 RPM 중3-1(22개정) 등록

- 사람 확인: 바탕화면 `문제은행-패키지\rpm-중3-1-22개정-라텍스-패키지`(문항 1133·답 1129+해설 1133)를 교재관리 › 패키지 등록. 교재별 오답에서 0160(수직선 지시문 그룹)·0526(인수분해 과정 상자)·0943(보기 그래프 5개)·1022(지시문 긴 수식)·0522(패치 문항) 미리보기.
- 🔴 없음. 검수 요약 `latex-bank/rpm-m31/검수/2026-09-20/00-검수-요약.md`.

## 2026-09-19 100발100중 3권 등록

- 사람 확인: 바탕화면 `문제은행-패키지@발100중-3-2-중간-라텍스-패키지`(660) · `…3-2-기말…`(490) · `…3-1-기말…`(723)을 교재관리 › 패키지 등록. 교재별 오답에서 예제(「N쪽 예제 1」 배지)·보기가 그래프인 문항(3-2 중간 0733 · 3-1 기말 130-71)·산점도 문항(3-2 기말 77-02) 미리보기.
- 해설 이미지는 세 권 모두 미포함(스캔 답지 해설 정렬은 별도 작업). 답 이미지는 전사본 answer 조판 100%.

## 2026-09-19 정의여고 프린트 변형 시범 확인

- 사람 확인: 바탕화면 `문제은행-패키지\정의여고-1-2-중간-프린트\` 의 `2-숫자변형-1세트.pdf`·`3-창의변형-1세트.pdf` 를 훑어 난이도·표현이 수업에 맞는지 본다(`변형-노트.md` 에 문항별 유지·변경·답, 정독 ★). 특히 창의 25(닮음 통찰 유지 재설계 · 답 71)·03·11 과 숫자 25(넓이비 9/4 배)·18(답이 항상 ①).
- 다음: 변형을 오답은행 등록 패키지로 내보내려면 `build.mjs --export --package output/problem-bank/jeongui-1-2-mid --bank-only` 를 v1·v2 은행에 돌린다(등록용 manifest 는 원본 패키지 것을 이어받음 · 미실행).

## 2026-09-19 쎈 중3-2 · 라이트쎈 공통수학1·2 등록

- 사람 확인: 바탕화면 `문제은행-패키지\쎈-중3-2-수학-라텍스-패키지`(문항 753·답 753·해설 696) · `라이트쎈-공통수학1-라텍스-패키지`(1414·1414·1381) · `라이트쎈-공통수학2-라텍스-패키지`(1230·1230·1160)를 교재관리 › 패키지 등록(「등록 완료 · 서버 재조회 일치」까지 화면 유지). 교재별 오답에서 쎈 0270(등거리 눈금)·0332(접점 라벨)·0733(산점도 보기 2-2-1) · 라이트쎈1 0154(3차식) · 라이트쎈2 0678(집합 지시문)·1381(④ 보기) 미리보기.
- 검토 필요 목록(📝)은 교재별 화면에서. 🔴 없음.

## 2026-09-18 교재관리 재등록

- 사람 확인: 바탕화면 `rpm-공통수학1-라텍스-패키지` · `베이직쎈-공통수학1-라텍스-패키지` · `베이직쎈-공통수학2-1~3단원-라텍스-패키지` 를 교재관리 › 패키지 등록으로 **다시 올리기**(끊긴 이미지만 채워진다 · 「등록 완료 · 서버 재조회 일치」까지 화면 유지). 나머지 두 권도 다시 올리면 검토 메모(manifest)가 반영된다(이미지는 건너뜀).
- 검토 필요 목록(🔴): rpm-cm1 0989(장식 삽화 생략) · 쎈 cm1 186-11(원문 오식 의심) · 📝 는 교재별 화면에서.

## 2026-09-18 RPM 공통수학1 · 베이직쎈 공통수학1 조판 완료

- 사람 확인: ① 바탕화면 `문제은행-패키지\rpm-공통수학1-라텍스-패키지`(문항 1184·해설 1184·답 1183) · `베이직쎈-공통수학1-라텍스-패키지`(문항 1570·해설 1570·답 1570) 등록 → 교재별 오답에서 0079(조립제법)·0714(연립부등식 배지)·1118(행렬)·베이직쎈 18-04(TikZ)·123-01(수직선 보기)·186-11 미리보기. ② 베이직쎈 186-11 은 원문 오식 의심(검수 요약 🔴) — 실물 확인 뒤 items.json 수정 여부 결정. ③ 베이직쎈 해설 누락 116문항은 `answer-overrides.json` 으로 채워 0(9-18 후반).
- 9-17 handoff 의 「공통수학1 은 한글이 살아 있는 PDF 를 다시 주어야」는 취소(우리 pdfjs 설정 문제였음 · STATUS 9-18 정정).

## 2026-09-17 RPM 공통수학2 조판 · 공통수학1 보류 (9-18 정정: 파일 문제 아님)

- 사람 확인: ① 바탕화면 `문제은행-패키지\rpm-공통수학2-라텍스-패키지` 등록(문항 1132·해설 1132·답 1125) → 교재별 오답에서 0394(보기 그래프)·0346(과정 상자)·0151(그림 옆 조건) 미리보기. ② **공통수학1 은 한글이 살아 있는 PDF 를 다시 주어야** 진행할 수 있다(현재 파일은 본문·답지 모두 한글 0). ③ 공통수학2 답지도 한글이 빠진 파일이라 해설 이미지가 수식만 남는다 — 완전한 답지 PDF 가 있으면 `ingest-answers.mjs --solutions <새 답지> --out output/problem-bank/rpm-cm2` 뒤 `build.mjs --export` 만 다시.
- `latex-bank/rpm-cm2/검수/2026-09-17/00-검수-요약.md` 「🟡」 중 어간 벌어짐·집합식 인라인 처리는 사람 판단(원문처럼 별도 줄로 바꾸려면 items.json 에서 `\par\smallskip\centerline{…}`).

## 2026-09-16 RPM 조판 · 답지 누락

- 사람 확인: ① 바탕화면 `문제은행-패키지\rpm-중3-2-수학-라텍스-패키지` 를 교재관리 › 패키지 등록(문항 644 · 해설 644 · 답 644 재조회 일치) → 교재별 오답에서 문항 이미지가 조판본인지, 0051~0053(삼각비 표 참조)·0438(보기 그림)·0615(긴 세로 보기) 를 미리보기로 확인. ② `베이직쎈-공통수학2-1~3단원-라텍스-패키지` 도 다시 등록(해설 420/420 · 답 334 로 갱신됨 · 등록을 다시 해도 학생 기록은 남음). ③ `latex-bank/rpm-m3-2/검수/2026-09-16/00-검수-요약.md` 「🟡 남김」 4건.
- 바탕화면 `문제은행-패키지` 는 `rpm-중3-2-수학`(원본 크롭) · `rpm-중3-2-수학-라텍스-패키지`(조판본 · 등록용) · `베이직쎈-…-라텍스-패키지`(등록용) · `…-오답은행.pdf` · `올림포스-공통수학1`(보류) 만 남았다.
- 다음: 올림포스 공통수학1(스캔 · 313문)을 조판할지 결정. 스캔이라 RPM 방식(벡터 크롭)은 못 쓰고 베이직쎈 방식(TikZ)이다.

## 2026-09-15 베이직쎈 2·3단원 전사 완료 · 시험지 제작

- 사람 확인: ① 바탕화면 `문제은행-패키지\베이직쎈-공통수학2-1~3단원-라텍스-패키지` 를 교재관리 › 패키지 등록(1단원 조판본 패키지를 덮어씀 · 문항 420) → 오답관리 › 시험지 제작에서 2·3단원 문항으로 시험지 미리보기 ② `latex-bank/ssen-basic-cm2/검수/2026-09-16/00-검수-요약.md` 의 🟡 사람 판단 항목(조사 분리 5문 · 59-11 긴 조건 · 36-10 그림 배치 · 28-16/17 보기 배치) ③ 정독 자산의 `분류 이슈 목록`(2단원 31 · 3단원 16행)은 기록만 — 나중에 변형 단계에서 판단.
- 다음 교재: `latex-bank/README.md` 「배치 전사 절차」대로 make-batches → latex-transcriber ×N → check-batch → merge → build --review → 검수·정독 → export. 거부하는 에이전트가 있으면 완료한 에이전트에게 SendMessage 로 이어 맡긴다.
- 남은 도구 일: 스캔 type_label 카운터 회귀(`--types` 목록 옵션) · 시험지 바구니 서버 저장 · 풀이(해설) 전사 · 숫자 변형(level 1).

## 2026-09-15 베이직쎈 1단원 검수 반영

- 검수 에이전트 4축 결과·반영 내역: `latex-bank/ssen-basic-cm2/검수/2026-09-15/00-검수-요약.md`(바탕화면 `…-라텍스\검수-2026-09-15` 에도). 사람이 볼 것: ① ★ 라벨을 올린 5문(11-10·20-02·19-18·21-09·19-15 → ★3)과 통찰 라벨 바꾼 9문이 본인 기준에 맞는지(정독 md 각 블록 `[검수 2026-09-15]` 줄) ② 21-10 크롭 그림 화질 ③ 난이도(★·통찰 라벨·대상층) 분류 문제는 **기록만** 하고 당장은 손대지 않는다(사용자 결정 2026-09-15 — 목표는 LaTeX 조판 + 오답은행). 오답지에 조판본을 쓰려면 바탕화면 `문제은행-패키지\베이직쎈-공통수학2-1단원-라텍스-패키지`(build.mjs --export 결과)를 교재관리 › 패키지 등록에 올린다.
...
- 수업 취소는 수업일지 하단에서 수업 수정 모달 왼쪽으로 이동했다. 일반 수업은 기존 중첩 확인 모달, 생성 수업은 브라우저 확인을 사용하며 성공 뒤 수업 수정 모달과 수업일지를 함께 닫는다.
- lesson 22/22, focused browser, duplication, `check:fast` 828/828, `test:production` 308/308, safe browser 97/97 및 실제 안전 미리보기 확인 완료. API·DB·운영 데이터·알림 변경 없음.
## 2026-09-13 문제은행 피드백 7건 뒤

- 운영 DB 에 교재가 없다(삭제 뒤 등록 실패). 사용자: 교재관리 › 패키지 등록 › 바탕화면 `문제은행-패키지\rpm-중3-2-수학` → 등록(문항 644 · 해설 644 · 답 644 재조회 일치). 이미지가 안 보이는 문항이 있으면 교재 상세 「이미지 누락 검사」 결과를 받는다.
- EBS 올림포스 6권(바탕화면/다운로드 `…Marked.pdf`, 스캔·회전·기울기)은 스캔 경로 신규 작업. 글자 레이어가 없어 번호 인식 방식(로컬 OCR vs 비전 1회)을 먼저 정한다.

## 2026-09-13 문제은행 정답·해설

- 사람 Gate 2개: ① `supabase/20260913_problem_bank_answers.sql` 을 SQL Editor 에 적용 ② 교재관리 › 패키지 등록에서 바탕화면 `문제은행-패키지\rpm-중3-2-수학` 폴더 하나 선택 → 등록(문항 644 · 해설 644 · 답 644 · 서버 재조회 일치 확인). 정답·해설이 같은 폴더에 있어 따로 올릴 필요가 없다. 그 뒤 교재별 오답 › 선택 인쇄에서 빠른정답·해설을 켜 인쇄본을 본다.
- 다른 교재의 해설 PDF 는 「답」 글리프와 배너 문구를 먼저 확인한다(`docs/problem-bank/README.md` 1-2).

## 2026-09-13 문제은행 피드백 반영

- 바탕화면 `문제은행-패키지
pm-중3-2-수학` 을 고친 CLI 로 다시 만들었다(0126번류 17건 하단 정정). 교재관리에서 같은 폴더를 다시 등록하면 이미지만 바뀌고 학생 기록은 남는다.
- 다음: 정답 별책 PDF 연결(빠른정답·해설 옵션 활성화), 스캔 PDF(쎈) 경로.
...
- 응시 기록 과목 드롭다운에 `중학 3-1`, `중학 3-2`를 추가했다. 쎈 단원 카탈로그 과목 목록은 분리 유지한다.
- 시험지 모델·쎈 과목 parity, `check:fast` 828/828, 선택·저장·가상 서버 재조회·회차 재열기 focused browser 1/1 통과. API·DB 계약·운영 데이터 변경 없음. 병합·배포 전.
...
- 운영 9월 6일 13~18시 시험대비 수업은 복구 완료다. 수업 달력의 `삭제한 수업 복구` 버튼은 서버에서 최근 7일 취소 수업을 읽어 새로고침 뒤에도 복구한다.
- UI 복구는 기존 `/api/lesson-journal/history-action`의 `undo_cancel` CAS·Supabase 재조회 계약을 사용한다. 실제 알림 예약·취소는 포함하지 않는다.
- 시험대비 수업의 stale 생성 숨김 키도 복구 창에서 감지해 `달력 표시 복구`로 해제한다. 이 경계까지 운영 화면에서 확인한 뒤 완료한다.

업데이트: 2026-08-31

## 2026-08-31 개인 요일별 시간표 수업 연속성 교정

- `lessonHomeworkContinuity`가 반·요일·개별 시간표가 아니라 학생별 실제 출석·지각·체크인 기록으로 직전 일반 수업을 고른다. `lessonJournalPreviousMemoSelector`는 가장 최근 등원 수업 1건만 교재·진도 원천으로 사용하며 빈 값을 오래된 수업으로 채우지 않는다.
- 출결이 저장된 직전 수업에 다음 숙제가 없으면 오래된 숙제로 넘어가지 않는 계약을 유지한다. 이진후의 8월 31일 오연결 `previous` 숙제는 배포 뒤 versioned rows CAS로 제목·연결 원천을 비웠고 Supabase 재조회까지 완료했다.

## 2026-08-29 휴강보충 분할 등록·그룹 알림 예약

- `+ 수업 등록 -> 휴강 보충`에서 시간대별 학생만 선택해 일정을 만들고, 수업 수정의 `휴강 보충 알림 관리` 버튼으로 상세 알림관리 모달을 연다. 학생·학부모·당일 11시 문구 3종과 예약 대상을 확인·수정한 뒤 `일정 저장 후 알림 예약`을 누르면 일괄 예약한다. `{{학생명}}`은 그룹의 각 학생 이름으로 치환된다.
- 기존 휴강보충 수업도 수정 화면에서 같은 기능을 쓴다. 운영 박수빈·이하민·김재연 일정은 아직 실제 알림을 예약하지 않았으며, 배포 후 각 일정을 열어 대상·시간을 확인하고 버튼으로 명시적으로 예약해야 한다.

## 2026-08-26 JSON 요청 한글 청크 손상 수정

- 공용 HTTP JSON 수신을 청크별 문자열 연결에서 Buffer 전체 누적 후 단일 UTF-8 디코딩으로 바꿨다. 수업일지 다건 저장의 간헐적 한글 손상과 성공 후 실패 오판을 막는다.
- 기존 운영 손상 문자열은 자동 보정하지 않았다. 배포 후 수업일지 한글 다건 저장·새로고침 smoke를 확인하고, 별도 사람 Gate에서 손상값 대조·복구 범위를 결정한다.

## 2026-08-25 월 정규수업 열기 요일별 명단 오류 수정

- `monthlyRegularLessonOpen.js`가 반 전체의 지난달 마지막 수업 1건(요일 무관) 명단을 새 달 모든 요일에 복사하던 버그를 고쳤다. 요일별 마지막 수업 명단을 쓰도록 변경. 개별시간표로 특정 요일에만 다니는 학생(박지현 사례)이 다른 요일 새 회차에 영향받지 않는다.
- 사용자가 이미 잘못 생성된 9월 정규수업 52건을 Supabase SQL Editor에서 직접 삭제할 예정이다. 삭제 후 "월 정규수업 열기"를 다시 실행하면 고친 코드로 정상 재생성된다.
- PR·main 병합은 사용자 확인 후 진행 예정이다. 다음 세션에서 이어받을 것: 사용자가 9월 삭제·재생성을 완료했는지, 결과가 기대대로인지 후속 확인 필요.
...
- 서빈·김보현의 운영 학생/수업 명단은 삭제되지 않았다. 기본 앞반 수업시간과 개별시간이 일부 겹치는 학생도 수업일지 명단에 유지하며, 이후 이런 시간표를 저장할 때는 확인 모달에서 `기본 반 명단에 표시하고 저장`을 선택한다.

## 2026-08-13 명단 재계산 적용일

- 학생 프로필 `수정 → 현재 시간표로 명단 재계산`에서 현재 저장된 반·개별 시간표를 수업일지 명단에 다시 적용한다.
- 오늘 수업 행이나 기록이 있으면 기본값은 `오늘 명단 유지 · 내일부터 적용`이다. 오늘 명단도 바꾸려는 경우에만 `오늘 명단 포함 · 오늘부터 적용`을 선택하고 저장한다.
- 보강·특강 등 수동 일정은 유지된다. 이진후·박지현처럼 이미 시간표만 저장된 학생은 이 작업을 한 번 실행해야 미래 명단이 보정된다.

## 2026-08-13 개별 시간표 명단 배치

- 반 이름은 학생의 기본 소속으로 유지해도 된다. 프로필 개별 시간표가 `월 16:00-19:00 / 수금 19:00-22:00`이면 월요일은 4-7 수업일지, 수·금은 7-10 수업일지 명단에 나타난다.
- 박지현처럼 `수 19:00-22:00 / 토 13:00-16:00`으로 저장하면 토요일 1-4반 명단에 나타난다. 수업일지 행에는 상세 시간을 반복하지 않고 `개별` 배지만 표시한다.
- 저장 시 미래 정규수업 명단을 CAS 후 재조회해 확인한다. 수동 보강·과거 기록은 변경하지 않는다.

## 2026-08-12 시험대비 진행 내용

- 시험대비 상세에서 그날 학생마다 별도의 `오늘 진행한 내용` 칸을 제공한다. 각 학생 값은 독립된 수업기록으로 저장하며 같은 내용은 필요할 때 일반 복사·붙여넣기로 입력한다.
- 각 내용은 그 날짜·그 학생에게만 저장되고 이후 일정의 시간 전파 대상이 아니다. 이전에 저장한 공통 기록은 참고용으로만 표시한다.
- 일정 수정의 반·학교·학생 선택 checkbox는 모바일에서도 20×20px이며 페이지 가로 overflow가 없다.
...
- 활성 학생이 없는 학교·학년 행도 시험관리에서 `연결된 활성 학생 없음`으로 보이며 상세관리에서 제외·복원할 수 있다.
- 같은 고사·학교·학년·과목의 중복 저장이 있으면 대표 행의 포함·제외 상태를 전체 중복에 함께 적용한다. 학사일정은 포함 상태 행만 카드·달력·학교 필터에 표시한다.
- 운영 상계고 원본은 AI가 변경하지 않았다. 배포 후 교사가 상계고 행을 열어 `이번 고사 내신 준비 제외 → 변경 저장` 한 번만 실행하면 된다.

## 2026-08-11 보충관리 수업일지 날짜 정렬

- 보충관리 정렬 선택에 `수업일지 날짜순`이 있으며 연결 수업일의 오래된 항목부터 표시한다. 날짜 미입력은 마지막이다.
- 좁은 화면에서도 `정렬` 라벨은 두 글자가 세로로 갈라지지 않고 선택창과 한 줄을 유지한다. 저장·알림 side effect는 없다.

## 2026-08-11 시험관리 학교·학년별 학생 확인

- 시험정보 표는 `학교명 → 학년 → 해당 학생` 순서다. 선택한 반의 활성 학생 중 학교·학년이 같은 학생을 이름 배지로 모두 보여주며 이름 검색도 지원한다.
- 학생·시험정보 저장 원천은 그대로다. 퇴원생은 제외되고, 좁은 화면에서는 문서가 아니라 시험정보 표만 가로 스크롤한다.

## 2026-08-11 시험대비 명단·시간 전파

- 현재 시험관리의 포함 학교·학년과 재학생이 자동 명단 원천이다. 과거 생성 수업의 stale 학교·학생은 표시에서 제거되고 유효 학생의 기존 개별 시간만 유지된다.
- 전용 일정 수정에서 반별·학교별·학생별로 선택한다. 선택한 날짜부터 이후의 해당 학생 시험대비 일정만 새 시간으로 바뀌며 앞선 날짜는 보존된다.
- 저장은 여러 lesson을 CAS·재조회·rollback으로 묶는다. 연결 기록·알림이 있는 학생 제거는 보호되며 알림 side effect는 없다.

## 2026-08-11 시험대비 캘린더·명단

- 달력 첫 화면에서 시험대비 대상 학교가 개별 배지로 모두 보인다. 날짜별 학교 수와 참여 학생 수도 함께 표시한다.
- 클릭하면 당일 학생이 시간순으로 열리고 `학교별`로 전환할 수 있다. 개인 시간이 저장된 학생은 개인 시간을, 나머지는 수업 공통 시간을 표시한다.
- 제외 SQL Gate는 사용자가 적용 완료했다. 이번 표시 변경에는 새 SQL이나 운영 쓰기가 없다.

## 2026-08-11 학사 달력 시험기간 표시

- 학교별 전체 시험기간은 상단 카드와 하단 달력의 옅은 학교색 띠에 함께 표시되며, 기간 안의 수학시험 날짜는 진한 학교색 탭으로 보인다.
- 달력 필터에 `시험기간`이 추가됐고 범례가 띠·탭·일반 학사일정을 구분한다. 모바일은 페이지가 아닌 달력 영역만 가로 스크롤한다.
- 기존 시험정보 원천과 명시 저장·CAS·재조회는 그대로이며 운영 데이터는 수정하지 않았다.

## 2026-08-11 시험정보 하단 작업 정리

- 시험정보 상세 하단은 저장 상태 단독 행, 왼쪽 `관리 범위 변경`, 오른쪽 `시험 후 총평 → 닫기 → 변경 저장`으로 분리했다.
- 제외·복원은 계속 local draft만 바꾸고 `변경 저장` 뒤 기존 CAS·서버 재조회로 반영된다. 미저장 초안을 닫으면 서버 원천은 바뀌지 않는다.
- runtime lint, build, 시험관리 fixture, focused safe browser와 safe 화면 배치를 확인했다.

## 2026-08-11 시험정보 제외 Gate 완료

- branch에 고사별 `is_excluded` 저장, 상세관리 제외·복원, 기본 목록·파생 일정·자동 시험대비·시험분석 제외가 준비돼 있다.
- 사용자가 Supabase SQL Editor에서 `supabase/20260811_exam_prep_exclusion.sql` 적용을 완료했다.

## 2026-08-10 시험정보 저장 UI

- 시험정보는 더 이상 필드 변경마다 자동저장하지 않는다. 상세 모달의 `변경 저장` 한 번으로 변경 행만 저장·재조회하며, 실패하면 입력을 유지한다.
- 시험기간 표시는 `2026`을 포함한 전체 범위를 보존하고 종료일까지 보이도록 열을 조정했다.
...
- 배포 뒤 용화여고 고1 `2026-10-13~2026-10-19`, 정의여고 고1 `2026-09-28~2026-10-02`를 다시 입력하면 된다. 운영 일정은 AI가 직접 쓰지 않았다.

## 2026-08-10 반 이동·퇴원 적용일

- 오늘 수업일지 행/record가 있는 학생의 반 이동·퇴원은 기본 `내일부터`로 오늘 행을 보존하고, 교사가 명시적으로 `오늘부터`를 선택할 수 있다.
- 이채빈 운영 원천은 AI가 직접 쓰지 않았다. 배포 후 학생관리에서 기존 반을 오늘 기준으로 복구한 뒤 새 반을 `내일부터 반 이동`으로 저장하면 된다.
...
- 월수금 7-10반의 8월 7일 숙제는 중간 휴강 보충 일정에 연결돼 있었고, 그 일정의 진행 기록·새 숙제가 없어 8월 10일 지난 숙제가 비었다.
- 배포 뒤에는 진행 흔적이 전혀 없는 중간 일정만 건너뛴다. 실제 출결·수업내용·과제상태 또는 새 숙제가 있는 수업은 경계로 유지하므로 과거 숙제를 잘못 재사용하지 않는다.
- 운영 데이터 보정은 필요 없다. 최신 화면을 열면 기존 Supabase의 8월 7일 숙제 원천이 자동 표시된다.

## 2026-08-10 수업일지 저장 충돌 복구

- 다른 화면에서 출결·숙제 상태가 갱신돼도 현재 수업일지의 비중첩 문구·수업내용 수정은 최신 원천에 안전 병합한다. 동일 필드의 서로 다른 수정은 덮어쓰지 않고 계속 충돌 처리한다.
- 상태가 이미 같은 숙제의 `checkedAt` 시각 차이는 충돌로 보지 않고 현재 명시 저장 시각을 적용한다. 새 화면은 상태가 바뀌지 않은 숙제를 저장 계획에서 제외한다.
- 배포 뒤 사용자는 새로고침하지 않은 현재 화면에서 `변경 저장`을 다시 눌러 12건 수정본을 저장한다.
...
- 시험분석은 `C:\Dev\academy-os-exam-analysis` 전용 branch에서만 계속한다. 새 정산 단가 입력이 없으면 임의 기능·리팩터링을 자동 시작하지 않는다.
- 수동 보충 safe browser는 고정된 당일 대신 서울 기준 14일 뒤를 사용해 실행 시각과 무관하게 학생·학부모·당일 학생 reminder 3종을 검증한다.

## 2026-08-08 퇴원 최신 원천 충돌 복구

- 퇴원 처리는 클릭 시점의 최신 Supabase 학생·수업 원천으로 재기준화한다. 다른 화면에서 학생 행이 갱신된 오래 열린 탭도 최신 정보는 보존하면서 퇴원 상태와 미래 명단을 저장·재조회한다.

## 2026-08-08 출결 태블릿 자동 확인 3초

- 출결 preview가 확정되면 3초 뒤 자동 저장하고 성공 응답 뒤 번호 키패드로 복귀한다. 같은 날 수업이 여러 개면 수업 선택 후부터 3초를 센다.
...
- `확인할 숙제`는 이제 과제 상태 셀이 아니라 학생별 수업메모에 표시된다. `확인 완료` 체크가 원본 follow-up을 저장·재조회하고 성공한 뒤 이후 수업일지에서 사라진다.
- 체크박스는 `16×16px`이며 기존 숙제 판정·알림 예약 본문은 유지한다. lesson `21/21`, production `827/827`, build, 집중 safe browser `1/1`을 통과했다.

## 2026-08-08 수동 보충 입력 검증 회귀

- 수동 보충 작성의 필수 시간 누락 시 입력 초안과 모달을 유지하며 상세 검토로 전환하지 않는다.
- 실패 동선에서 보충 원천·수업·알림 API 쓰기는 0건이다. 제품 runtime 변경 없이 supplement `10/10`, lint, build, 집중 safe browser `1/1`을 통과했다.

## 2026-08-07 수동 보충 수업일지·알림 연결

- 보충관리 상단 `수동 보충 등록`에서 재원생·제목·사유·날짜·시간을 직접 입력한다. 다음 검토 화면의 `수업일지 일정 만들기`가 보충 원천과 1인 수업일지를 함께 저장한다.
- 같은 동작에서 학생·학부모 다음 정각 일정 안내와 보충 당일 학생 11시 알림을 연결한다. safe fixture 외 운영 저장·실제 발송은 하지 않았다.

## 2026-08-07 중첩 모달 ESC 닫기 복구

- 공용 모달 ESC는 DOM상 최상위 modal backdrop 하나만 처리한다. 수업일지 위 알림톡에서 첫 ESC는 알림톡만 닫고 수업일지는 유지하며, 두 번째 ESC가 수업일지를 닫는다.
- close-disabled 최상위 모달은 ESC를 소비해 상위 팝업이 대신 닫히지 않는다. 알림 저장·발송 동작에는 변화가 없다.

## 2026-08-07 수업일지 표 레이아웃 복구

- 데스크톱 수업일지의 학생~학생 알림톡 10개 열을 1290px 기준으로 맞춰 오른쪽 잘림을 제거하고, 더 좁은 화면에서는 기존 `DataTableShell` 가로 스크롤을 유지한다.
- 긴 `확인할 숙제` 문항 목록은 과제 상태 셀 안에서 줄바꿈한다. 저장·알림 의미는 그대로이며 lesson `21/21`, lint, scenario `827/827`, build, 집중 safe browser `1/1`을 통과했다.
...
- 수업일정표 `PDF 인쇄`는 새 문서를 만든 직후 직접 인쇄를 요청하며, 새 창의 `인쇄하기` 버튼으로도 재호출할 수 있다.
- 출력 내용·선택 항목·저장 원천은 유지했고 student `15/15`, runtime lint, build, 집중 safe browser `1/1`을 통과했다.

## 2026-08-06 수업일지 숙제 동시 생성 충돌 복구

- 숙제가 다른 화면에서 먼저 생성된 409 응답은 최신 `currentHomework`를 포함하며, 수업일지는 로컬에 없는 경우에만 이를 병합하고 작성 중 입력을 유지한다.
- 자동 덮어쓰기 없이 안내 후 두 번째 `변경 저장`에서 최신 원천 기반 CAS 갱신한다. lesson `21/21`, production `827/827`, build, 집중 safe browser `1/1`을 통과했다.

## 2026-08-06 보강 수업일정표 시간 원천 수정

- 학생 프로필 개별 시간표는 기본 정규수업·휴강에만 적용하며, 보강은 수업일지에 저장된 자체 시간을 수업일정표·출결 계산에 사용한다.
- 강민준 운영 오류 형태를 13시 금요일 보강 fixture와 safe browser로 고정했고 전체 production `827/827`을 통과했다.

## 2026-08-06 수업메모 배포 교체 복구

- 오래 열린 탭의 이전 `LessonNestedPanels` 자산 요청 실패는 수업일지에서 명확히 안내하고 `최신 화면으로 새로고침`으로 복구한다.
- 저장·API 변경 없이 lesson `21/21`, runtime lint, build, 집중 safe browser `2/2`를 통과했다.
...
- 운영 쓰기·Storage·AI/provider 없이 domain `70/70`, production `827/827`, build, 격리 safe browser `55/55`를 통과했고 registry 23 + 직접 route 97 = 전역 120을 유지한다.
- 다음 4-3r은 유료 AI·Storage와 분리된 시험분석 문항 수 사람 확정 POST route다.
