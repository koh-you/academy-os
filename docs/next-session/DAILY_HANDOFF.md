...
- 최종 정규 횟수 상세 모달에 명시적 저장·Supabase 재조회 확인과 저장 확인값 PDF 출력을 추가했다. 안전 브라우저에서 입력·저장·PDF·재열기 유지까지 검증했다.
- 후속 UI 요청은 수업 등록 버튼을 월 탐색 박스 안으로 이동하고, 월별 출결·수업 모달을 넓혀 달력 스크롤을 줄이는 것이다. 긴급 정산 저장 배포 뒤 별도 기능·개선 단위로 진행한다.
