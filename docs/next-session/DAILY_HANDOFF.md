...
## 2026-09-15 베이직쎈 2·3단원 · 시험지 제작

- **사람 결정**: 2·3단원(316문) LaTeX 원문 전사를 할지. 전사 에이전트가 전부 저작권(시판 교재 전면 복제)을 이유로 거부했고, 본 세션은 사용자 확인 전에는 진행하지 않았다. 대안 ① 2·3단원은 원문 크롭(이번에 번호 빠짐 0 으로 다시 잘라 둠)으로 오답지·시험지에 바로 등록 — `output/problem-bank/ssen-basic-cm2` 를 교재관리 › 패키지 등록(1단원은 `-latex` 조판본 패키지로 덮어쓰기) ② 정독 자산은 발문 요약으로 만들 수 있음(원문 전사 불필요) ③ 시험지용은 동형 자작 문항(수치·상황 변경)을 같은 규약으로 생성.
- 시험지 제작 탭은 등록된 어떤 교재(크롭·조판본)에서든 쓸 수 있다. 저장 목록(시험지 이름으로 바구니 보관)은 다음 작업.
- 남은 도구 일: 스캔 type_label 카운터 회귀(2·3단원 개념/유형 번호 밀림 — export 는 전사본 section 으로 덮어쓰지만 크롭 패키지를 그대로 등록하면 라벨이 틀림 · `--concepts` 처럼 유형 제목 목록을 받는 `--types` 를 붙이면 해결).
