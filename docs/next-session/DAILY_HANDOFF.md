...
- 사람 확인: ① 바탕화면 `문제은행-패키지\rpm-공통수학1-라텍스-패키지`(문항 1184·해설 1184·답 1183) · `베이직쎈-공통수학1-라텍스-패키지`(문항 1570·해설 1454·답 1570) 등록 → 교재별 오답에서 0079(조립제법)·0714(연립부등식 배지)·1118(행렬)·베이직쎈 18-04(TikZ)·123-01(수직선 보기)·186-11 미리보기. ② 베이직쎈 186-11 은 원문 오식 의심(검수 요약 🔴) — 실물 확인 뒤 items.json 수정 여부 결정. ③ 베이직쎈 해설 없는 116문항(`output/problem-bank/ssen-basic-cm1/검수-필요.md` ①)을 채우려면 공통수학2 때처럼 `answer-overrides.json` 을 만들어 `ingest-scan-answers.mjs --overrides` 후 export 만 다시.
