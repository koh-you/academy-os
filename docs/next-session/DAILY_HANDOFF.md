...
- 사람 Gate 2개: ① `supabase/20260913_problem_bank_answers.sql` 을 SQL Editor 에 적용 ② 교재관리 › RPM 중3-2 수학 › 정답·해설 패키지 등록에서 바탕화면 `문제은행-패키지\rpm-중3-2-수학-정답해설` 선택 → 등록(해설 644 · 답 644 · 서버 재조회 일치 확인). 그 뒤 교재별 오답 › 선택 인쇄에서 빠른정답·해설을 켜 인쇄본을 본다.
