...
- `lessonHomeworkContinuity`가 개별 시간표 학생의 모든 요일별 정규수업과 보충을 시간순으로 연결한다. `lessonJournalPreviousMemoSelector`는 이 공용 목록을 사용하므로 반 ID가 다른 요일도 직전 교재·진도 선택에서 빠지지 않는다.
