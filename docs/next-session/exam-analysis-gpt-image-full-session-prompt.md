...
> **2026-07-21 구현 보류 안내:** 사용자는 Academy OS 안의 이미지 생성 API·버전 DB·Storage 제작실을 먼저 만드는 방향이 아니라, 교사 확정 분석에서 장별 완성형 프롬프트를 만들어 ChatGPT 프로젝트에 복사하는 방향으로 확정했다. 이 문서의 블로그 주소·카드 역할·프롬프트 연구는 참고하되 API/DB/UI 구현 지시는 별도 재승인 전 실행하지 않는다. 최신 조사 원천은 `docs/exam-analysis-blog-corpus-gate1-2026-07-21.md`다.
