...
- 사용자 기준: 월 정산 범위는 4.2주나 12회가 아니라 해당 월 1일부터 말일까지다. 계속 재원생은 실제 정규 회차·시수 증감과 무관하게 월 고정금액 전액을 적용한다. 신입생은 인정 시작일부터 말일까지, 퇴원생은 1일부터 인정 종료일까지의 정규 회차·시수 비율로만 부분월 계산한다. 대기·출석·지각은 정산에 포함하고 결석도 별도 차감 요청이 없으면 자동 차감하지 않는다.
- 화면: `운영 > 월별 수업 정산`을 추가했다. 정산월, 정규 기준 총액/예상 수령액, 특강 기준 총액/예상 수령액, 전체 예상 수령액을 분리 표시한다. 학생별로 계산 방식, 월 고정금액, 월별 스케줄, 신입·퇴원 인정 기간, 정규 예정·인정 회차/시수, 보충 회차/시수, 정규 적용금액, 특강 총액, 조정액, 메모를 수정할 수 있다.
...
- 부분월 계산: 학생별 저장 스케줄로 그 달의 실제 날짜를 1일~말일까지 생성하고, 신입·퇴원 인정 기간에 포함되는 정규 시수를 전체 월 정규 시수와 비교한다. 시수 정보가 없을 때만 회차를 사용하며 비율은 월 고정금액을 넘지 않게 제한한다. 퇴원생 시작일 기본값은 해당 월 1일이고 종료일 기본값은 학생 퇴원일이다. 신입 후보는 그 달 최초 정규 수업일을 표시하지만 안전을 위해 교사가 `신입생 · 회차/시수`를 선택하기 전에는 월정액을 유지한다.
