...
2. `발송 알림톡 템플릿 설정 관리`
   - 상태: 부분 진행 - 보충관리의 결석보강 학생 11시 리마인더와 보충 일정 확정/변경 안내 문구는 `app_state.aiSettings.notificationTemplates` 원천으로 `설정 > 알림톡`에 노출했다. 나머지 발송 경로 전체 inventory와 설정 이관은 앞으로 해야 한다.
...
- 구현 결과: 결석보강 학생 11시 알림톡 초안은 `일시: 7/17(금) 04:00`처럼 보강 일시를 먼저 보여주고, 다음 줄에 `7/16(목) 화목 4-7 / 토 10-1반 결석 수업 보강을 진행합니다.`처럼 원 결석 수업분을 안내한다.
...
- 사람 검토 gate: 결석보강 상세에서 배정일을 `2026-07-17`, 시간을 입력하고 초안을 확인하면 `2026/7(금)` 또는 `2026-07-16` 같은 연도 노출 없이 `일시: 7/17(금) ...`, `7/16(목) ... 결석 수업 보강을 진행합니다.`가 보여야 한다.
- 중단 조건: 날짜가 `2026/7(금)`처럼 다시 깨짐, 알림톡 문구에 연도가 남음, 보강 일시와 원 결석 수업 날짜가 한 문장에 섞여 의미가 흐려짐, 기존 사용자 편집 초안이 자동으로 덮어써짐.
