...
- Canva 판단: 현재 Canva 디자인은 반복 사용 가능성을 검토하는 별도 gate가 필요하다. 카드별 고정 역할, PNG 슬롯 크기, 텍스트 최대 길이, 대각선 리사이즈 가능 여부, 다음 시험에서 바꿀 데이터 영역과 고정 브랜드 영역을 구분해 본다. 실제 산출물 PNG를 2/3/4/5/7번 슬롯에 넣고 모바일/블로그 가독성을 확인한 뒤 웹앱 PNG 비율을 조정한다.
