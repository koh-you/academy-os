...
- 보호 범위: 공지 작성기에서 만든 미발송 공지 초안의 삭제 기능만 기존대로 유지한다. 수업일지·보충·출결 등 다른 유형의 실패/취소 이력에는 삭제 버튼을 노출하지 않는다. 새 예약/발송 경로와 문구 원천은 변경하지 않았고 DB 스키마 변경이나 SQL 적용도 없다.
...
- 중단 조건: Solapi 그룹이 예약 상태인데 OS만 `canceled`가 됨, 숨겨진 예약 유형이 남음, 취소 후 목록에서 이력이 삭제됨, 발송 완료 건에 취소 버튼이 보임, 공지 외 알림에 삭제 버튼이 보이면 다음 알림 작업을 중단한다.
