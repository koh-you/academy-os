...
- 숙제·질문·시험 제출 저장 상태의 긴 실패 문구와 선택·제출된 시험 파일명이 카드 안에서 줄바꿈되게 했다.
- 시험 후 제출 input·select·textarea·file control과 파일 목록을 한 열 폭 안에 제한하고 제출 버튼에 전용 class를 부여했다.
- 화면 shell·CSS 외 bearer 세션, local draft, Supabase 재조회, Storage 업로드·제출 저장·실패 cleanup 순서와 callback은 변경하거나 실행하지 않았다.
- AI 검수: 학생 쓰기 행동·상태·폼·파일명과 기존 persistence 단계 보존 정적 fixture가 통과했다. `npm run build`는 128 modules, `git diff --check`는 오류 없이 통과했고, 전체 `npm run test:production`은 523개 중 기존 기준선 `90a`만 실패해 신규 회귀가 없다.
- 사람 검수: desktop/390px 쓰기 카드의 배치만 UI-8J 통합 절차로 나중에 확인한다. 완료·질문·시험 제출·파일 선택/열기는 누르지 않고 UI-8J-3을 계속한다.

## 2026-07-30 UI-8J-1. 포털 계정 맥락·읽기 탐색

- 교사 미리보기 머리말의 중복 `로그아웃`을 제거해 `관리 화면으로`만 남기고, 실제 학생은 학생이 둘 이상 전달될 때만 선택기를 표시한다.
- 학생·학부모 계정 행동에 이름과 group 의미를 부여했으며 기존 로그아웃·학생 선택 callback은 변경하지 않았다.
- 수업 기록 미니 달력에 학생 이름과 grid·요일·날짜·선택 의미, focus 표시를 추가하고 날짜 선택 local state를 보존했다.
- 자료·리포트·수업 기록·숙제 읽기 카드의 긴 문자열을 내부 줄바꿈하고 모바일 자료 링크를 한 열 전체 폭으로 정리했다.
- 화면 shell·접근성·CSS 외 숙제 완료·질문·시험 제출·파일·Supabase·Storage 원천은 변경하거나 실행하지 않았다.
- AI 검수: 계정 행동·수업 기록 달력·긴 읽기 카드와 기존 callback 보존 정적 fixture가 통과했다. `npm run build`는 128 modules, `git diff --check`는 오류 없이 통과했고, 전체 `npm run test:production`은 522개 중 기존 기준선 `90a`만 실패해 신규 회귀가 없다.
- 사람 검수: 교사 미리보기·학부모 포털·실제 학생 읽기 화면은 UI-8J 통합 절차로 나중에 확인한다. 로그아웃·파일 열기·학생 쓰기는 누르지 않고 UI-8J-2를 계속한다.
...
- 최종 문구 편집, AI 수정, 명시적 저장, 예약/발송 버튼과 실제 발송 미리보기는 유지했다.
- 실제 번호/테스트 번호 라우팅, 알림 제외 시 발송 차단, 예약 시각 경과 시 수동 재발송, 미저장 문구 발송 차단 로직은 표시와 분리해 그대로 유지했다.
- 실제 알림톡 예약·발송·취소, notification job, 운영 데이터는 검증 중 실행하지 않았다.
...
- 수업일지 예약 상태도 `Academy OS 상태 -> 상태 문구 -> Solapi 그룹` 순서로 맞췄다.
- 공통 표시 helper가 `provider === "solapi"`인 작업만 Solapi group 참조로 반환하므로 내부 claim이나 다른 provider 참조를 Solapi로 잘못 부르지 않는다.
- 예약 취소, 알림 기록 취소·삭제 callback과 disabled 조건을 그대로 유지했다.
- notification_jobs/Solapi 실제 조회·예약·취소·발송과 Supabase/app_state·수업·AI·출결 원천을 실행하거나 변경하지 않았다.
- AI 검수: source label·Solapi provider 제한·callback 불변 정적 계약, `npm run build` 128 modules, `git diff --check`가 통과했다. 전체 `npm run test:production`은 459개 중 기존 기준선 `90a`만 실패했고 모든 개별 fixture와 신규 계약은 통과했다.
- 사람 검수: 알림관리 발송 기록과 수업일지 예약 확인의 상태 열을 desktop/390px에서 확인하는 절차를 누적했다. 실제 결과 대조·예약 취소·기록 삭제는 실행하지 않는다.
...
- `아는 것만 풂`의 학부모 문구를 `풀기 어려운 문제도 더 고민하는 습관이 필요합니다`, 학생 문구를 `모르는 문제를 더 고민해와 !`로 공통 화면 원천과 서버 알림 원천에 동일하게 반영했다.
- 기존 Solapi 예약·notification job을 조회·수정·재예약·발송하지 않았다. 이미 provider에 예약된 본문은 자동 변경하지 않으며, 새 미리보기·새 예약 또는 사용자가 명시적으로 예약 업데이트한 건부터 새 문구를 사용한다.
- 전용 deterministic fixture로 두 문구 원천의 일치와 편집·읽기 카드의 긴 문자열 줄바꿈 계약을 고정했다.
- AI 검수: 전용 fixture, 서버 문법 검사, `npm run build` 128 modules, `git diff --check`가 통과했다. 전체 운영 시나리오는 456개 중 이번 문구 계약을 포함한 455개가 통과했고 기존 기준선 `90a` 1건만 실패했다.
- 사람 검수: 배포 후 쉼표로 이어진 긴 문항 번호가 다음 칸을 침범하지 않는지, `아는 것만 풂` 선택 시 학부모·학생 미리보기 문구가 각각 요청값인지 확인한다. 실제 예약 업데이트·발송은 이 시각 검수에 필요하지 않다.
...
- 불변 계약: 보충 상태 선택, 학생 검색/선택, 시험 제출 대상, 연구 필터, HWPX local form, `onAddMathExamItem`/`onUpdateMathExamItem`과 저장 상태 계산은 그대로 유지한다. 실제 보충 저장, 수업 저장, 시험 제출 확인, 연구 삭제, HWPX 다운로드, 학사일정 저장, 정산 저장, 응시 저장은 실행하지 않았다.
- 진행 수치: 원시 JSX `sectionHeader`는 40개에서 28개로 줄었다. 남은 28개는 AI/시험 호출 작업 바와 운영 저장·삭제·발송 작업 바로 분리한다.
- AI 검수: 전체 기능 fixture가 통과했고 production 시나리오 406개 중 새 UI-2E-3A 계약을 포함한 405개가 통과했다. 기존 비관련 `90a` 1개만 기준선 실패다. `npm run build`, `git diff --check`도 통과했다.
- 사람 검수: 필수 중단 gate는 없다. 입력/선택 버튼을 실제 저장하지 않고 배치만 보는 시각 검수를 누적한 뒤 UI-2E-3B를 계속한다.
...
- 실행/저장: 기존 Render·GitHub 5분 dispatch와 Render API의 1분 내부 loop가 같은 자동 대조기를 호출한다. `scheduled`/`send_unconfirmed`, `provider=solapi`, provider 그룹 참조가 있는 최근 48시간 job만 조회하고, 미확정 결과는 5분 간격으로 재확인한다. 결과는 Supabase `notification_jobs`에 `sent`/`failed`/`canceled`/`send_unconfirmed`로 저장하며, 수업 코멘트형은 기존 계약대로 해당 `lesson_student_records`의 학생·학부모 발송상태도 함께 갱신한다.
- 화면 반영: 수업일지 또는 알림관리 화면이 열려 있으면 1분 간격과 창 focus 복귀 때 현재 범위의 알림 job만 조용히 다시 읽는다. 따라서 예약 확인 모달을 열지 않아도 상태가 바뀌며, 기존 수동 예약 확인/결과 확인은 상세 감사·예외 복구용으로 유지한다.
- 안전 경계: 자동 작업은 Solapi 결과 조회와 기존 상태 대조 저장만 수행한다. 새 알림 발송·재발송·예약·취소, 학생·수업·출결·숙제 변경은 실행하지 않는다. 자동 대조 실패는 발송 dispatch를 실패시키지 않고 다음 주기에 재시도한다.
- AI 검증: 순수 정책 fixture가 정확히 5분 경계, 너무 이른 job, provider 참조 없음, 완료 job 제외, 5분 재시도 throttle, 48시간 lookback, 오래된 순서·limit을 검사한다. 이어 production 정적 계약, build, `git diff --check`를 실행한다.
- 사람 검토 절차: 배포 후 실제 예약 알림 한 건을 별도 조작하지 않고 둔다. 예약 시각 5~10분 뒤 같은 수업일지 또는 알림관리 화면을 열어 `발송 완료`나 실제 실패 상태로 자동 전환되는지 확인한다. 수동 `예약 확인`/`Solapi 결과 확인`을 누르지 않았고 새 중복 발송·예약이 생기지 않으면 통과다.

## 2026-07-28 P0. 사람 gate 운영 정책 정정

- 사용자 결정: 학생 포털 실제 쓰기와 Solapi 특강 템플릿 검수는 현재 보류 목록에서도 제거한다. 필요해질 때 별도 작업으로 다시 시작하며, 새 세션이 이를 자동으로 질문·검수·발송 연결하지 않는다.
- 완료 기록: 교사 bearer·시험 후 제출 Storage 소유권 보안은 사용자 확인에 따라 실제 배포 검증까지 완료로 기록한다.

## 2026-07-28 P0. 전체 UI 규칙 통일 연쇄 작업 설계

- 전수 인벤토리 결과: `App.jsx`는 약 27,895줄, `App.css`는 약 17,324줄이며 페이지별 독자적인 header, filter, card, modal, action bar CSS가 누적돼 있다. 공통 기반은 `Modal`, `InlineSaveStatus`, `StickySaveBar`, `MetricCard`, `EmptyState`, `AutosaveRiskNotice`까지 이미 존재한다.
- 결정: 데이터 저장·알림톡·예약·출결의 동작을 바꾸지 않는 UI-only 단위부터 시작한다. 전체 계획과 단위별 AI 검수/독립 사람 gate 처리 방식은 `docs/ui-unification-strategy-2026-07-28.md`를 기준으로 한다.
- 운영 원칙: 사람이 실제 외부 상태를 확인해야 하는 단위는 정확한 절차를 다음 세션 README에 남긴다. 그 검수 결과를 소비하지 않는 UI 단위는 중단하지 않고 다음 순서로 진행한다.

## 2026-07-28 UI-1. 디자인 토큰·앱 프레임

- 범위: `src/app/App.css`의 공통 색상·간격·테두리·그림자·터치 대상 토큰과 앱 프레임, 사이드바, 본문, 페이지 상단, 기본 버튼의 CSS만 정리한다. JSX, callback, 저장 상태 전이, Supabase/app_state, `notification_jobs`, Solapi는 변경하지 않는다.
- 기대 결과: 데스크톱에서는 사이드바가 화면에 고정되고 내부에서만 길게 스크롤되며, 본문은 독립적으로 읽을 수 있다. 1100px 이하에서는 기존처럼 일반 문서 흐름으로 돌아가 모바일/태블릿 레이아웃을 방해하지 않는다. 키보드 포커스, 기본 버튼 hover/disabled, 공통 표면의 색상·간격이 같은 규칙을 사용한다.
- AI 검수: CSS 토큰/프레임 정적 대조, `npm run build`, `git diff --check`를 실행한다. UI-only이므로 사람 gate와 외부 side effect는 없다.

## 2026-07-29 UI-2A. 공통 페이지 머리말 primitive

- 범위: 공통 `PageHeader`를 만들고 구조가 단순한 반관리·AI 도구·보충관리·자료함 네 화면에 먼저 적용한다. 제목, 짧은 설명, eyebrow, 우측 행동의 위치만 통일하고 기존 버튼 callback과 표시 데이터는 그대로 전달한다.
- 불변 계약: 반 추가 버튼의 기존 미연결 상태, 보충 내역 모달 callback, 자료함 건수 계산을 바꾸지 않는다. Supabase/app_state, 저장, 출결, `notification_jobs`, Solapi는 읽거나 수정하지 않는다.
- AI 검수: 네 화면의 제목·설명·행동 텍스트와 callback 정적 계약, `npm run build`, `git diff --check`를 실행한다. 이 단위는 사람 gate 없이 다음 UI-2B로 이어갈 수 있다.

## 2026-07-29 UI-2B. 상태·행동이 있는 페이지 머리말
...
- 결과: 화면의 붉은 자동저장 경고 7개를 현재 코드와 대조했다. `app_state 전체 snapshot 저장`은 이미 변경 key만 저장하는 구조로 해결되어 기존 문구를 제거하고 `app_state key별 자동저장`으로 교체했다. 다만 다중 탭/요청 역전과 저장 후 재조회·version 대조는 남아 있어 설정·수업연구·오답관리의 경고는 유지한다. 시험정보 행 입력과 학사일정 연동 저장도 같은 이유로 유지한다.
- 전수 기준: 최신 판정은 `docs/save-persistence-audit-2026-07-28.md`다. 특강 회차·미래 수업일지, 수업 bulk/휴강, 학생/Tally, 보충 취소, 정산, 강사 메모, 시험분석은 A+ 계약으로 재확인했다. app_state 자동저장, 시험정보 행, 학사일정 연동은 B 위험으로 남겼다.
- 범위/부작용: 경고 문구와 감사 문서만 갱신하며 Supabase·Storage·학생/수업·`notification_jobs`·Solapi를 읽거나 변경하지 않는다.

## 2026-07-28 P1. 좌측 메뉴 전환 시 본문 상단 복귀

- UI: 긴 화면에서 좌측 메뉴를 아래로 스크롤해 누를 때, 선택한 새 작업 화면의 본문도 같은 스크롤 위치에서 열리던 불편을 없앴다. 모든 좌측 메뉴 전환은 새 view를 선택한 직후 브라우저의 공용 페이지 스크롤을 즉시 맨 위로 복귀시킨다.
- 범위/부작용: 화면 위치만 바꾸며 학생·수업·출결·Supabase·`notification_jobs`·Solapi 데이터를 읽거나 수정하지 않는다. 같은 메뉴를 다시 누르는 경우도 상단 복귀 동작은 의도적으로 유지한다.
- AI 검증: 좌측 메뉴 scroll-reset 정적 계약, `npm run build`, `git diff --check`로 확인한다. 기존 무관한 수업 모달 정적 검사 `90a`는 별도 기준선 실패다.
...
- UI: 640px 이하에서 메뉴 그룹마다 두 칸짜리 터치 버튼으로 정리하고, 전체 화면의 상단 헤더·작업 버튼·카드는 한 열 우선으로 보이게 했다. 밀도가 높은 기록표는 화면 전체가 깨지지 않도록 해당 표 안에서만 가로 스크롤한다. 알림 원본·학부모 응대의 개별 모바일 최적화도 함께 유지한다.
- 원천/부작용: CSS/메뉴 표시만 변경하며 학생·수업·출결·`notification_jobs`·Supabase·Solapi·카카오 채널 데이터를 읽거나 수정하지 않는다.
- AI 검증: `npm run build`, `git diff --check` 및 모바일 메뉴 정적 계약으로 확인한다. 기존 무관한 수업 모달 검사 `90a`는 별도 기준선 실패다.

## 2026-07-28 P1. 알림관리 학부모 응대 탭 복원·모바일 최적화

- UI: 알림관리의 기존 첫 탭 `개별 발송`(운영 알림 원본)은 유지하고, 그 옆에 `학부모 응대` 탭을 복원했다. 발송 완료된 학부모 데일리 리포트/강사코멘트의 저장 본문, 학생명, 학부모 번호, 발송 시각을 검색·확인한 뒤 `별관 채널 확인 정보 복사`로 카카오비즈니스 파트너센터 상담방을 찾는다. 채널 답장은 Academy OS에 저장하지 않는다.
- 모바일: 알림관리의 작업 탭은 가로 스크롤로, 수신 선택·학생 선택·작성·발송 버튼은 한 열과 큰 터치 영역으로, 예약/발송 기록표는 표 내부만 가로 스크롤로 정리했다. 학부모 응대 카드도 세로로 쌓인다.
- 원천/부작용: 읽기 원천은 기존 `notification_jobs`와 학생 연락처다. 구현·검증에서 Supabase, `notification_jobs`, Solapi, 카카오 채널 대화를 수정·발송·예약·취소하지 않았다.
- AI 검증: `npm run build`, `npm run test:parent-response-context`, `git diff --check` 통과. 전체 정적 시나리오의 새 응대/모바일 계약은 통과했고, 기존 무관한 수업 모달 검사 `90a` 한 건만 실패한다.

## 2026-07-28 P1. 모바일 교사 알림 전용 진입

- UI: 화면 폭 640px 이하의 선생님 접속은 로그인/자동로그인 뒤 `알림관리`를 기본으로 연다. 사이드바는 `운영` 그룹의 `알림관리` 한 항목만 한 열로 남기고, 수업일지·학생·시험·정산·설정 등 데스크톱 업무 메뉴는 모바일에서 숨긴다.
- 범위/부작용: 화면 진입과 메뉴 표시만 바꾼다. 알림관리의 기존 학부모 수신 선택, 예약 확인, Solapi 결과 대조 기능은 그대로이며 `notification_jobs`, Supabase, Solapi 저장·발송·취소를 실행하지 않는다.
- AI 검증: `npm run build`, `git diff --check` 통과. 정적 시나리오의 새 모바일 메뉴 계약은 통과했고, 기존 무관한 수업 모달 검사 `90a` 한 건만 실패한다.

## 2026-07-28 P1. 학생 프로파일 강사 운영 메모

- UI: `학생관리 > 학생 프로파일 > 기본정보`에 Tally 제출 원천과 별도인 `강사 운영 메모`를 추가했다. 진도·교재·분실/특이사항·다음 수업 계획을 자유 서술로 작성하며, 기본정보 수정 모드와 별도로 `메모 수정 → 강사 운영 메모 저장`을 사용한다.
- 저장 원천/SQL: 새 `students` 컬럼을 만들지 않고 기존 Supabase `app_state.teacherOperatingMemos`에 학생 ID별 텍스트를 저장한다. 따라서 운영 Supabase SQL edit는 필요 없다. 저장 전 현재 원천을 다시 읽어 다른 학생 메모를 보존하고, 저장 뒤 같은 학생 ID의 텍스트를 재조회·대조한 후에만 완료를 표시한다. 실패하면 입력 draft는 유지한다.
- 노출/부작용: 교사 학생 프로파일에서만 표시한다. 학생·학부모 포털, 수업일지, Tally 원문, 알림톡/`notification_jobs`/Solapi에는 읽거나 전달하지 않는다.
- AI 검증: `npm run build`, `git diff --check` 통과. 전체 `npm run test:production`은 이번 추가 계약을 통과했고 기존 무관한 수업 모달 정적 검사 `90a` 한 건만 실패한다.

## 2026-07-28 P0. 학부모 응대 화면 운영 복구

- 조치: `운영 > 알림관리 > 학부모 응대` 추가 뒤 모바일에서 전체 첫 화면이 빈 파란 화면으로 보인다는 운영 보고를 받아, 기능 추가 커밋 `bd095be2`와 후속 방어 커밋 `d376700d`를 모두 되돌렸다. 현재 알림관리에는 해당 탭이 없다.
- 보존: 되돌리기는 프런트 코드·테스트·문서만 대상으로 하며 학생, 수업, 출결, `notification_jobs`, Solapi 발송·예약·취소, 카카오 채널 대화 원천을 읽거나 변경하지 않았다.
- 배포 확인: Vercel이 복구 번들 `main-DJLidbaZ.js`를 제공하는 것을 확인했고, 새 Chrome 브라우저 프로필에서 운영 로그인 첫 화면이 정상 렌더링됐다.
- AI 검증: `npm run build`, `git diff --check` 통과. 전체 정적 시나리오는 기존 수업 모달 검사 `90a` 한 건이 실패했으며, 되돌린 코드가 문제 전 커밋 `c0a235f7`과 동일함을 대조했다. 이 실패는 이번 알림 화면 코드와 무관하므로 별도 조사한다.

## 2026-07-28 P0. 모바일 로그인 첫 화면 폭 복구

- 원인: 390px 모바일 렌더링에서 로그인 카드의 `width: min(560px, 100%)` 계산이 화면 폭보다 넓어져 오른쪽이 잘리고 파란 배경만 주로 보였다.
- 조치: 로그인 카드를 `width: 100%`와 `max-width: 560px`로 분리하고, 모바일에서는 카드·닫기 버튼·제목·탭·입력칸의 여백과 크기를 축소했다. 화면 높이는 모바일 브라우저 주소창 변화에 맞는 `100dvh`와 safe-area 여백을 사용한다.
- AI 검증: 390px × 844px Chrome 렌더링에서 기존 잘림을 재현해 원인을 확인했고, `npm run build`, `git diff --check`를 통과했다. 전체 정적 시나리오는 기존 `90a` 검사 1건만 실패했다. 배포 후 실제 휴대폰에서 로그인 카드 전체·학생/학부모/선생님 탭·입력칸을 확인한다.
- 후속 보강(2026-07-28): 휴대폰 브라우저가 작은 논리 폭을 보고해도 가로 스크롤이 생기지 않게 전역 `min-width: 320px`을 제거하고 로그인 카드 폭을 `100vw` 기준으로 제한했다. `fa9d87d4` 배포 뒤에는 홈화면 바로가기보다 직접 URL에서 먼저 실제 기기를 확인한다.
- 카카오 인앱 브라우저 복구(2026-07-28): 쿼리 문자열이 있는 직접 링크는 정상인데 기본 도메인 URL만 빈 화면인 iPhone 캡처를 기준으로, 문제를 레이아웃이 아니라 루트 HTML의 이전 번들 캐시로 판정했다. `vercel.json`에서 `/`와 `/index.html`에 `Cache-Control: no-store, max-age=0`를 설정해 카카오톡 기본 링크도 매번 최신 시작 HTML을 받도록 한다.

## 2026-07-28 P1. 보충 학생·학부모 알림톡 형식 통일
...
## 2026-07-28 P1. 학생 프로파일 Tally 제출 데이터 접힘

- UI: 학생 프로파일의 기본정보 영역에서 노란 `학생 메모` 박스를 제거하고, 기본 접힘인 `학생이 Tally로 제출한 데이터` 패널을 추가했다. 펼치면 제출 양식·제출 시각과 Tally 질문·답변을 읽기 전용으로 표시한다.
- 원천/안전: 일반 Tally 접수는 학생 ID를 직접 보관하지 않아, 이름과 학생·학부모 전화 또는 학교·학년이 함께 일치하는 제출만 보여 준다. 기존 `specialNote`/Tally 원천을 삭제하거나 수정하지 않으며, 학생·수업·출결·알림/Solapi 저장·발송을 실행하지 않는다.
- AI 검증: 학생 프로파일 정적 계약을 Tally 접힘 패널로 갱신하고, `npm run build`, `git diff --check`, `npm run test:production`을 실행한다. 기존 수업 모달 `90a` 정적 검사 1건은 이번 UI 변경과 무관한 기준선 실패로 유지된다.

## 2026-07-28 P1. 수업일지 달력 상단 제어줄 숨김
...
- 원천/부작용: PDF 인쇄용 문구만 변경하며 학생, 수업, 출결, 숙제, Tally, 알림/Solapi를 저장하거나 변경하지 않는다.
- AI 검증: `npm run test:student-handover-pdf`, `npm run build`, `git diff --check`를 실행한다. 인쇄 창에서 두 문구가 보이지 않고 본문이 유지되는지 확인한다.

## 2026-07-28 P1. 퇴원생 목록 이름·퇴원일 정렬

- UI: `학생관리 > 퇴원생 목록` 상단에 `이름순`(가나다순)과 `퇴원일순 (최근)` 선택기를 추가했다. 선택·저장·퇴원 취소·영구 삭제·인수인계서 PDF 동작은 정렬 뒤에도 같은 학생 ID를 사용한다.
- 원천/부작용: 화면의 로컬 표시 순서만 바꾸며 학생, 수업, 출결, 숙제, Tally, 알림/Solapi 원천을 읽거나 저장하지 않는다.
- AI 검증: `node scripts/test-student-list-sort.mjs`, `npm run build`, `git diff --check`를 통과했다. 운영 화면에서 두 정렬을 각각 고른 뒤 체크 선택과 인수인계서 대상이 유지되는지 확인한다.

## 2026-07-27 P0. 월 정규수업 열기 (7월 → 8월)

- 요청/계약: 교사가 수업일지 달력에서 대상 월의 `정규수업 열기`를 누르면, 직전 월 마지막 실제 정규수업을 원천으로 반별 일정과 재원생 명단을 다음 달에 추가한다. 8월은 7월 정규 반 원천을 쓴다. 기존 8월 정규수업은 건드리지 않고 빈 날짜만 추가한다.
- 보존: 새 lesson에는 반·날짜·시간·명단만 만든다. 출결, 수업기록, 진도, 숙제 row, 알림 예약/발송을 복사하거나 생성하지 않는다. 수업일지의 기존 `getLessonHomework(..., "previous")` 직전수업 탐색은 첫 8월 회차에서 같은 반 7월 마지막 수업의 `다음 숙제`를 계속 읽는다.
...
- UI 후속(2026-07-28): 대상 월에 네 정규 반이 모두 하나 이상 개설돼 있으면 달력 상단 `정규수업 열기` 버튼을 숨긴다. 중간 입반 등으로 월초 회차가 없는 반도 해당 월에 이미 수업이 있으면 미개설로 오인하지 않는다. 반 자체가 하나라도 없을 때만 버튼을 유지한다.

## 2026-07-27 P1. 인수인계서 기본정보 퇴원일·반 제외

- UI: 인수인계서 PDF의 기본정보에서 퇴원일과 반을 제거했다. 학생·학생 번호·학부모 번호·학교·학년만 출력한다.
- 저장·부작용: PDF 출력 파생 정보만 변경하며 학생 원천의 퇴원일·반 정보는 수정하지 않는다.
- AI 검증: `npm run test:student-handover-pdf`, `npm run build`, `git diff --check`를 실행한다.
...
- UI: 다중 삭제 모달에 학생별 연결 데이터 종류와 건수를 펼쳐 표시한다. 연결 기록 없는 학생은 `학생 원천만 삭제 가능`으로 구분한다. 선택 체크박스는 전역 input 높이·padding 규칙의 영향을 받지 않도록 18px로 고정해, 세로 문구·과도한 빈 영역이 생기던 CSS를 제거했다.
- 저장·부작용: 모달 표시와 읽기 전용 audit 결과만 변경했으며, 학생·수업·수업기록·숙제·알림 원천 및 Solapi를 수정·삭제·발송·예약·취소하지 않았다.
- AI 검증: `npm run test:production`, `npm run build`, `git diff --check`를 실행한다.

## 2026-07-27 P1. 퇴원생 선택 작업 하단 고정바

- UI: 퇴원생 표 상단에 있던 선택 작업 바를 실제 브라우저 화면 하단 fixed bar로 옮겼다. 사이드바 펼침/접힘과 좁은 화면의 콘텐츠 폭을 반영해, 스크롤 위치와 무관하게 하단에 유지한다. 선택 수·전체 선택·선택 해제·저장·퇴원 취소·영구 삭제·인수인계서 PDF 동작은 유지한다.
- 저장·부작용: 화면 배치만 변경하며 학생·수업·수업기록·알림 원천과 Solapi를 수정하지 않는다.
- AI 검증: `npm run build`, `git diff --check`를 실행한다.

## 2026-07-27 P0. 퇴원생 선택 영구 삭제 실행 흐름 보강

- 원인: 선택 toolbar의 `영구 삭제`가 한 명 선택일 때만 활성화되어, 전체 선택처럼 여러 명을 고르면 버튼이 동작하지 않았다.
- UI/안전: 한 명이면 기존 이름 입력 확인 흐름을 유지한다. 여러 명이면 선택한 전원의 Supabase 연결 기록을 먼저 읽어 대상별 건수를 보여 주고, 연결 기록이 하나라도 있으면 강제 정리 영향 확인을 요구한다. `영구 삭제` 문구 입력과 브라우저 최종 확인을 모두 거친 뒤에만 학생별 삭제·Supabase 재조회를 순서대로 실행한다.
- 저장·부작용: 실제 삭제는 사용자가 최종 확인한 선택 대상에만 실행된다. 구현·AI 검증에서는 운영 학생, lessons, 수업기록, 숙제, `notification_jobs`, Solapi를 수정·발송·예약·취소하지 않았다.
- AI 검증: production scenario에 다중 선택 audit·문구 확인·순차 삭제·강제 삭제 뒤 단일 새로고침 계약을 추가하고 `npm run test:production`, `npm run build`, `git diff --check`를 실행한다.

## 2026-07-27 P1. 퇴원생 표 체크박스 보조문구 제거

- UI: 퇴원생 표의 왼쪽 선택 칸에서 세로로 표시되던 `학생명 선택` 보조문구를 제거하고 체크박스만 남겼다. 상단의 선택 작업 바와 표 헤더 `선택`은 유지한다.
- 저장·부작용: 표시 전용 변경이며 학생·수업·수업기록·알림 원천과 Solapi를 수정하지 않는다.
- AI 검증: `npm run build`, `git diff --check`를 실행한다.

## 2026-07-27 P0. 퇴원생 과거 수업일지 표시 보존 보정

- 운영 원천 읽기 전용 대조: 조소현은 2026-07-27 퇴원 상태지만, `lessons.studentIds`에는 7월 9·11·14·16·18(2건)·21·23·25일의 과거 수업 9건이 남아 있다. 7월 25일 수업기록에도 출석·교재·진도가 남아 있어 원천 데이터 삭제가 아니라 화면 표시 차단 문제였다.
- 원인/수정: 수업일지가 모든 `paused` 학생을 재원생 필터로 일괄 제외했고, 앱 시작 자동 정리도 모든 퇴원생을 `today`부터 재제외했다. 수업일지 전용 명단은 `수업일 <= 한국 시간 퇴원일`인 경우 퇴원생도 표시하고, 자동 명단 정리는 학생별 퇴원일의 다음 날부터만 제거하도록 바꿨다. 신규 일정·알림의 재원생 필터는 유지한다.
- 보존/부작용: 과거·퇴원 당일 `lessons`, `lesson_student_records`, 출결, 숙제는 수정하거나 삭제하지 않는다. 구현·검증 중 운영 Supabase, `notification_jobs`, Solapi 예약·발송·취소도 실행하지 않았다.
- AI 검증: 경계 fixture가 7월 25일·27일 표시, 7월 28일 제외, 한국 시간 퇴원일 파싱을 검사한다. production scenario 391건과 build를 통과했다.
- 사람 gate: 배포 후 `Ctrl+F5` → 7월 25일 조소현 수업일지를 열어 조소현 행과 출석·교재·진도가 표시되는지 확인한다. 이어 7월 27일 수업이 있으면 같은 방식으로 남고, 7월 28일 이후 수업 명단에는 나타나지 않으며 새로고침 뒤에도 유지되면 통과다.

## 2026-07-27 P0. 퇴원생 목록 선택형 작업 바로 정리

- UI: 퇴원생 표의 행별 `저장/퇴원 취소/중복 데이터 삭제/인수인계서 PDF` 버튼을 제거했다. 맨 왼쪽 체크박스로 대상을 고르고 상단 작업 바에서 `선택 저장`, `퇴원 취소`, `영구 삭제`, `인수인계서 PDF`를 실행한다. 표의 최소 폭도 1,714px에서 1,510px로 줄였다.
- 안전: 저장은 선택한 변경 행만 처리한다. 퇴원 취소·영구 삭제·PDF는 한 명을 선택해야 활성화된다. `영구 삭제`는 명칭만 정리했으며, 기존 영향 원천 점검·이름 입력·강제 정리 확인·Supabase 재조회 검증을 그대로 사용한다.
- AI 검증: production scenario에 선택형 toolbar, 행별 action stack 제거, 영구 삭제 명칭을 추가하고 `npm run test:production`, `npm run build`, `git diff --check`를 실행한다.
...
- 수정: 예약 생성 시 정규화된 `reservationFingerprint`를 job payload에 함께 저장하고, 반영 상태는 그 저장 지문을 현재의 정규화 기대 지문과 비교한다. 같은 저장본으로 성공한 예약은 다시 업데이트를 요구하지 않는다. 기존 job에 지문이 없을 때도 미확인 숙제의 이전 숙제를 같은 빈값 규칙으로 정규화해 비교한다.
- 저장·부작용: 변경은 예약 상태 판정과 새 job payload에만 적용한다. 구현·AI 검증에서 `notification_jobs`, Solapi 예약·취소·발송을 실행하거나 운영 데이터를 변경하지 않는다.
- AI 검증: production scenario가 저장 지문, 미확인 숙제 정규화, 상태 비교 fallback을 검사하며 `npm run test:production`, `npm run build`, `git diff --check`를 실행한다.

## 남은 사람 gate - 상태 보고 최상단 표시

모든 상태/진행/완료 답변은 완료 목록보다 먼저 남은 사람 gate와 blocked next action을 표시한다. 세부 보고 규칙의 source of truth는 `AGENTS.md` 최상단 `Remaining Human Gates` 섹션이다.
...
- 상태: draft collection `useState/useEffect`, 원천 sync, get/update/mark/build를 `useSupplementTaskDraftController.js`로 분리했다.
- 동작 보존: App은 동일 normalized templates와 안정된 dependency adapter를 전달하고 기존 `updateTaskDraft`에서 저장 상태 표시만 계속 갱신한다.
- side effect: hook은 local React state만 소유하며 API, Supabase, `notification_jobs`, Solapi 및 저장/예약 callback을 참조하지 않는다.
- 검증/gate: production 382/382와 build가 통과했다. 운영 호출은 없어 12R 사람 gate는 아직 실행 전이다.
- 다음 단위: 최신 `origin/main` rebase 후 보충 내용 저장 action을 분리하고 고태영 저장·재조회 검증을 수행한다.
...
- 동작 보존: App은 동일 local state updater에서 순수 함수를 호출하고 기존 문구 builder·시간 normalize를 주입한다. 자동 초안만 일정/방법 변경에 따라 갱신하며 선생님 최종본과 명시적 빈 값은 유지한다.
- 저장 원천/side effect: API, Supabase, `notification_jobs`, Solapi와 React state 소유권은 이동하지 않았다.
- 검증/gate: deterministic fixture와 production 381/381, build가 통과했다. 실제 운영 호출이 없어 12R 사람 gate는 아직 실행 전이다.
- 다음 단위: 최신 `origin/main` rebase 후 draft collection sync/state 경계를 분리한다.
...
## 2026-07-27 P0. 퇴원일 수업 보존 경계 수정

- 요청/계약: 퇴원 처리는 학생 원천을 `paused`로 저장하되, 등원일부터 퇴원일까지의 학생·수업·수업기록·출결·숙제는 모두 보존한다. 수업 명단에서 제외하는 범위는 퇴원일 다음 날부터다. 예를 들어 2026-07-27에 조소현을 퇴원 처리하면 7월 27일 수업은 보존하고 7월 28일 이후 수업 명단에서만 제외한다.
- 원인/수정: 퇴원 확인 모달은 이미 `오늘까지 보존 · 내일 이후 제외`를 안내했지만 `App.jsx`가 `today`부터 명단을 제거해 당일 수업도 제외했다. `withdrawalLessonBoundary`의 한국 시간 다음 날 계산을 사용하도록 바꿨다.
- 저장·부작용 경계: 기존 학생 `students` 저장과 이후 lesson roster bulk 저장 경로를 유지한다. 이번 수정은 삭제 시작일만 바로잡으며, 과거/당일 lesson·lesson_student_records·출결·숙제는 요청하지 않는다. 미래 명단 제거에 연결된 기존 알림 예약 정리 동작은 변경하지 않았고 Solapi 발송을 실행하지 않는다.
- AI 검증: `npm run test:student-withdrawal-boundary`, `npm run test:production`, `npm run build`, `git diff --check`를 실행한다. 경계 fixture는 7월 27일 퇴원 → 7월 28일부터 제외와 연말 경계를 검사한다.

## 2026-07-27 P0. 같은 달 신입·퇴원 정산을 첫 수업일~퇴원일로 제한

- 사용자 증상/원인: 조소현은 7월 9일 첫 정규 수업 뒤 7월 27일 퇴원했지만, 기존 정산은 계산 방식이 `신입생`이면 퇴원 여부와 무관하게 첫 수업일부터 7월 말일까지의 예정 수업을 세어 10회 × 35,000원 = 350,000원으로 표시했다.
- 운영 원천 읽기 전용 확인: 학생 원천은 `paused`, 퇴원 시각은 2026-07-27, 스케줄은 `화목 16:00-19:00 / 토 10:00-13:00`이다. 7월 정규 수업일지는 7월 9·11·14·16·18·21·23·25일 8건이며 첫 수업일 7월 9일, 마지막 수업일 7월 25일이다. 운영 학생·수업일지·정산 snapshot은 수정하지 않았다.
- 계산 계약: 선택 월이 학생의 첫 정규 수업 월이면서 같은 달에 퇴원했다면 자동 신입 회당 계산을 유지하되 기간 종료를 말일이 아니라 학생 퇴원일로 제한한다. `첫 정규 수업일 <= 월별 스케줄 날짜 <= 퇴원일`인 횟수만 인정하고 학년별 회당 단가를 곱한다. 조소현은 8회 × 35,000원 = 280,000원이다. 일반 신입생은 첫 수업~말일, 기존 퇴원생은 1일~마지막 정규 수업일 비율, 교사가 명시적으로 고른 계산 방식은 기존대로 유지한다.
- 화면/저장: 계산 방식 문구를 `신입생 · 첫 수업~말일/퇴원일`로 넓히고, 중첩 사례의 자동 정산 기간과 횟수에는 `첫 수업 ... ~ 퇴원일 ...`, `신입·퇴원 정산 N회`를 표시한다. 월 스냅샷을 실제 저장할 경우에도 이 행의 `startDate/endDate`가 첫 수업일/퇴원일로 들어가 기존 Supabase 재조회 검증을 거친다.
- 부작용: 이번 구현·검증은 화면 파생 계산과 기존 월 스냅샷 저장 payload만 보정했다. 운영 학생, 퇴원 원천, lessons, 수업일지·출결, 특강, `notification_jobs`, Solapi를 수정하거나 발송·예약·취소하지 않았다. 새 SQL은 없다.
- AI 검증: 조소현과 같은 화·목·토 스케줄, 7월 9일 첫 수업, 7월 27일 퇴원 fixture에서 자동 `new` 방식, 기간 7월 9~27일, 인정 8회, 수업일지 8회, 금액 280,000원을 직접 검사했다. `npm run test:monthly-settlement`, production 시나리오 386건, 전체 `npm run test:production`, `npm run build`, `git diff --check`를 통과했으며 build에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① 배포 후 `Ctrl+F5`하고 `운영 > 정산 > 월별 정산 > 2026-07`을 연다. ② 조소현 행이 `신입생 · 첫 수업~말일/퇴원일`, `첫 수업 2026-07-09 ~ 퇴원일 2026-07-27`, `신입·퇴원 정산 8회`인지 확인한다. ③ 계산식이 `8회 × 35,000원`, 정규 적용금액이 280,000원인지 확인한다. ④ 횟수 버튼을 눌러 7월 9·11·14·16·18·21·23·25일 수업일지 8건이 보이고 7월 28·30일 예정분이 정산에서 제외되는지 확인한다. ⑤ 다른 일반 신입생은 계속 첫 수업~말일, 기존 퇴원생은 계속 1일~마지막 수업으로 계산되고 학생·수업일지·출결·알림 원천에 변화가 없으면 통과다.
...
- 사용자 증상/원인: 7월 28일 고1 공통수학2 특강을 상황에 따라 휴강 처리하려 했지만, 기존 수업 수정 모달이 `initialLesson.date < today`인 과거 수업에만 휴강 버튼을 열어 미래·당일 수업은 일괄 차단했다. 이는 미래 수업의 알림 예약 취소 계약이 없던 시점에 넣은 임시 날짜 gate였다.
- 새 판단 기준: 기존 수업은 과거·당일·미래와 관계없이 휴강을 선택할 수 있다. 저장 직전 읽기 전용 `/api/lessons/closure-preflight`가 Supabase 최신 lesson, `lesson_student_records`, `notification_jobs`를 다시 읽는다. `scheduled/queued/pending_send/send_unconfirmed` 알림이 1건이라도 있으면 저장하지 않고 기존 수업일지의 `예약 확인`에서 정리하도록 안내하며, 같은 검사를 서버 bulk/upsert 직전에도 반복한다.
- 보존 계약: 휴강 전환은 같은 lesson ID, 기존 학생 전체 명단, `lessonTrackId/lessonTrackType`, 특강 안내문·회차 ID·회차 번호, 학생별 특강시간, 기존 수업기록을 유지한다. 모달에서는 휴강 선택 중 명단 편집을 잠그고 보존 명단·기록 건수와 화면 기준 알림 위험을 표시한다. 저장 후 전체 lessons 재조회 snapshot에는 특강 메타데이터까지 포함해 일치해야 완료한다.
- 소비 계약: 보존된 결석/출석 row는 삭제하지 않는다. 대신 휴강 lesson의 출결은 학생 마이페이지 통계에서 제외하고 학생 수업기록에는 `휴강`으로 표시한다. 결석보강·재시험 후보와 보충관리 배지, 이전/다음 수업·숙제·준비메모 연속성에서도 휴강을 실제 수업으로 취급하지 않는다. 월별/특강 정산은 기존 `lessonType=closure` 제외 계약을 유지한다.
- 알림 안전: 휴강은 알림 계획을 강제로 `알림톡 없음`으로 해석하고 새 수업 알림 예약·수동 시각 예약을 만들지 않는다. 드문 저장/dispatch 경쟁 상황에도 서버 발송 직전 최신 lesson이 휴강이면 해당 job을 `lesson_closure` 사유로 취소한다. 휴강 저장 버튼 자체가 Solapi 취소를 자동 실행하지는 않는다.
- 대상 운영 원천 읽기 전용 확인: 7월 28일 `2026 고1 공통수학2 유형 문제풀이`는 `specialLecture`/특강 4회차, 명단 2명, 결석 row 2건, 차단 알림 0건으로 확인되어 새 기준상 `canConvert=true`다. 이번 구현·검증에서는 해당 수업, 출결, 명단, 특강 회차, `notification_jobs`, Solapi를 수정하지 않았다.
- AI 검증: 미래 특강 휴강 선택, 저장된 휴강 역변환 잠금, 퇴원생을 포함한 원 명단 보존, 특강 회차 메타데이터 보존, 예약 위험 상태 차단, 휴강 출결 통계 제외 fixture를 `test:lesson-closure`로 추가했다. production 시나리오 385건, 전체 production test, build, `git diff --check`를 통과했다.
- 사람 gate: ① 배포 후 `Ctrl+F5`하고 7월 28일 고1 공통수학2 특강의 `수업 수정`을 연다. ② `휴강`이 활성화되고 선택 시 `기존 명단 2명·수업기록 2건·특강 회차 연결 보존`, `발송 가능한 예약 없음` 안내가 보이며 명단 편집만 잠기는지 확인한다. ③ 실제 휴강이 맞을 때만 `보충 없음` 또는 `보충 수업일지 함께 생성`을 선택하고 저장한다. ④ 저장 완료 후 같은 특강 회차가 달력/수업일지에서 휴강으로 보이고, 새로고침 후에도 유지되며 기존 두 학생과 특강 회차 연결이 남는지 확인한다. ⑤ 학생 화면 출결이 `결석`이 아니라 `휴강`이고 출결 통계·결석보강 후보에서 빠지며 새 알림 예약·발송이 없으면 통과다. 예약 경고가 보이면 저장하지 말고 먼저 기존 수업일지 `예약 확인`에서 정리한다.

## 2026-07-27 P0. 특강 학생별 시간 동기화가 다른 학생 출결로 차단되는 오류 수정

- 근본원인 후속: 상세 도입 이력, 서버 부작용, 전용 저장 계약, `/api/lessons/bulk` 6개 호출 경로 전수 분류는 `docs/special-lecture-student-sync-root-cause-audit-2026-07-27.md`가 기준이다.
- 사용자 증상: 신초봄의 회차 계획은 Supabase에 저장됐지만 4·5회차 수업일지 반영이 `보호된 수업 2개`로 실패했고, 같은 카드에는 초록색 `수업일지 반영됨`이 표시됐다.
- 운영 원천 읽기 전용 대조: 해당 enrollment는 중3 신초봄 한 건으로 확인됐다. 저장된 계획은 4회차 `13:00~16:00`, 5회차 `12:00~14:00`이지만 두 lessons의 신초봄 학생별 시간은 모두 기존 `13:00~15:00`이라 실제로 불일치했다. 4·5회차의 결석 row는 신초봄 것이 아니라 김연우 것이며, 신초봄 record는 두 회차 모두 0건이다. 대기 `notification_jobs`도 0건이다.
- 원인: 학생 한 명의 회차 계획을 동기화하면서 `records.some(record.lessonId === lessonId)`로 같은 수업의 모든 학생 기록을 회차 전체 잠금으로 처리했다. 그 결과 김연우의 미래 결석 예정이 신초봄 시간 변경까지 막았다. 카드 상태도 학생 ID 포함 여부만 확인하고 저장된 학생별 시간은 비교하지 않아 실패와 동시에 초록 완료를 표시했다.
- 수정: 미래 회차의 학생 추가·시간 변경은 기존 lesson, 다른 학생 명단·시간, 모든 `lesson_student_records`를 그대로 보존하고 대상 학생의 `studentIds`·`specialLectureStudentSchedules`만 교체한다. 다른 학생 기록·알림은 더 이상 차단 사유가 아니다. 오늘/과거 수업, 완료 수업, 대상 학생 대기 알림은 계속 보호하고, 학생을 회차에서 제외할 때는 바로 그 학생의 기록이 있을 경우에만 제거를 차단한다.
- 전용 서버 mutation: 기존 lesson의 학생별 변경은 더 이상 범용 `/api/lessons/bulk`를 사용하지 않는다. `/api/lessons/special-lecture-student-schedule`가 최신 lesson과 대상 학생 record·pending job을 다시 읽고, 대상 학생만 병합한 뒤 `updated_at` compare-and-swap으로 `student_ids`, `special_lecture_student_schedules`만 PATCH한다. 범용 bulk의 수업 전체 알림 취소·record 정리 후처리도 호출하지 않는다. 다른 탭에서 lesson이 먼저 바뀌면 덮어쓰지 않고 충돌로 중단한다. 기존 lesson이 없는 신규 회차만 bulk 생성한다.
- UI 신뢰성: 회차 카드는 명단 ID, 학생별 시작·종료, 공식/조정 유형, 조정 사유가 모두 현재 화면 계획과 같을 때만 초록색 `수업일지 시간 일치`로 표시한다. 시간이 다르면 주황색 `수업일지 시간 불일치 · 현재 HH:MM-HH:MM`으로 표시한다. 차단 시에는 단순 건수가 아니라 `N회차(오늘·과거 수업/완료 수업/해당 학생 기록/대기 알림)`처럼 실제 보호 사유를 표시한다.
- 저장/부작용: 구현·검증 중 운영 enrollment, lessons, 김연우 결석 기록, 신초봄 시간, `notification_jobs`, Solapi를 수정하지 않았다. 사용자가 모달 하단 2단계를 실행할 때만 신초봄 4·5회차 학생별 시간이 저장되며, 저장 뒤 lessons 전체 재조회 snapshot이 일치해야 완료된다. 새 SQL은 없다.
- AI 검증: 김연우 결석·김연우 대기 알림과 신초봄 시간 변경을 같은 lesson에 둔 실행형 fixture를 추가했다. 다른 학생 기록·알림 비차단, 대상 학생 record가 있는 시간 변경 허용, 대상 학생 제거 시 record 차단, 대상 학생 pending job 차단, 다른 학생 일정·명단·lesson 필드 보존을 직접 검사한다. production 시나리오에는 전용 API, 대상 학생 범위, CAS, 전체 재조회 계약을 추가했다. `npm run test:special-lecture-plan-sync`, `npm run test:production` 385건, `npm run build`, `git diff --check`를 통과했다.
- 사람 gate: ① 배포 후 `Ctrl+F5`하고 `Lesson Hub > 특강관리 > 신초봄 > 회차·진행 관리`를 연다. ② 저장 전에는 4회차가 `현재 13:00-15:00`, 5회차도 `현재 13:00-15:00`인 주황색 시간 불일치로 보이는지 확인한다. ③ 하단 `2단계 · Supabase 저장 + 미래 수업일지 반영`을 한 번 눌러 보호 실패 없이 저장 완료되는지 본다. ④ 4회차가 `13:00-16:00`, 5회차가 `12:00-14:00` 시간 일치로 바뀌고 새로고침 후 유지되는지 확인한다. ⑤ 7/29·7/31 수업일지에서 김연우 결석(7/29 사유 `여행`)은 그대로이고 신초봄 학생별 시간만 바뀌며, 새 알림 예약·발송·취소가 없으면 통과다.

## 2026-07-27 P0. 미저장 결석보강 후보에도 기존 `보강만 취소` 적용

- 사용자 증상: 결석 수업에서 자동 생성된 보강 후보를 열면 하단에 `결석 기록 취소`만 보이고, 앞서 구현한 `보강만 취소`가 보이지 않았다. 이 화면은 아직 `makeup_tasks`에 저장되지 않은 local draft라 기존 표시 조건이 저장된 결석보강에만 버튼을 허용한 것이 원인이었다.
- 기능 중복 없음: 새 `보강 안 함` 기능은 만들지 않았다. 기존 `보강만 취소 · 원 결석기록 유지` 저장·재조회 흐름을 미저장 후보까지 확장했다. 미저장 후보에도 이미 생성된 임시 makeupTaskId가 있으므로, local 전용 필드만 제거한 뒤 같은 취소 row를 `makeup_tasks`에 저장해 재노출을 막는다.
- UI 의미 분리: 미저장 결석보강 상세 하단에 `보강만 취소 · 결석기록 유지`와 `결석 원본 자체를 취소` 두 영역을 나란히 표시한다. 전자는 보강 후보만 취소 이력으로 저장하고 결석 상태·사유를 유지한다. 후자의 `결석 기록 취소`는 결석 입력 자체가 잘못됐을 때만 원 수업일지 출결을 대기로 되돌린다. 확인창도 미저장 후보에는 `취소 이력을 저장하고 후보 닫음`, `생성된 보강 수업일지 없음`, `새 예약·발송 없음`을 명시한다.
- 저장/재조회: `보강만 취소`를 확정하면 `makeup_tasks.status=canceled`, `cancellationMode=keep_source_absence`, `sourceAttendancePreserved=true`를 저장하고 task와 원 `lesson_student_records`를 다시 읽는다. 원 결석의 상태·사유·등하원 snapshot이 같을 때만 완료한다. 미저장 후보에는 연결 lesson·알림 예약이 없으므로 새 수업일지나 `notification_jobs`를 만들지 않고 Solapi 예약·발송·취소도 없다.
- 구현 영향: 코드·자동검증 중 운영 `makeup_tasks`, lessons, 수업일지·출결, `notification_jobs`, Solapi를 수정하지 않았다. 사용자가 확인창에서 `보강만 취소`를 확정할 때만 해당 후보의 취소 이력 한 건이 Supabase에 저장된다. 새 SQL은 없다.
- AI 검증: 전용 fixture에 아직 일정·연결 lesson이 없는 draft 결석보강의 canceled snapshot을 추가했다. production 시나리오는 local draft에서 기존 handler를 사용하고, 같은 화면에 `보강만 취소`와 의미가 다른 `결석 기록 취소`가 분리되는 계약을 검사한다. `npm run test:supplement-cancellation`, `npm run test:production` 385건, `npm run build`, `git diff --check`를 통과했으며 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① 배포 후 `Ctrl+F5`로 새로고침하고 `보충관리 > 결석보강 > 김보현`의 같은 7월 23일 후보를 연다. ② 하단에 `보강만 취소 · 결석기록 유지`와 `결석 원본 자체를 취소`가 각각 보이는지 확인한다. ③ `보강만 취소`를 누른 확인창에서 원 결석은 유지, 보강 후보는 취소 이력으로 닫힘, 보강 수업일지와 알림 예약은 생성되지 않음이 보이는지 확인한다. ④ 실제 확정 후 결석보강 기본 목록에서 사라지고 `최근 한 달 보충 내역`에 `보강 취소 · 결석 유지`로 남는지 확인한다. ⑤ 원 7월 23일 수업일지의 출결·사유가 전과 같고 알림관리·Solapi에 새 예약·발송·취소가 없으면 통과다.

## 2026-07-27 P0. 특강 회차 저장 재조회 UTC 표기 오판 수정

- 사용자 증상: 특강 학생 회차의 실제 저장 버튼을 누르면 `특강 회차 계획 저장 실패: Supabase 재조회 값이 수정한 회차 계획과 다릅니다`가 표시됐다. 실패 화면은 새 1단계/2단계 UI가 반영되기 전 열린 탭이었다.
- 읽기 전용 운영 확인: 운영 프론트 최신 번들에는 `1단계 · 화면 초안에 적용`, `2단계 · 실제 저장` UI가 이미 포함돼 있다. 운영 `special_lecture_enrollments`의 `planReviewedAt` 13건은 모두 Supabase가 반환한 `+00:00` 형식이었다. 브라우저 저장값은 같은 UTC 시각을 `Z` 형식으로 만들기 때문에 회차·시간·상태가 같아도 문자열 snapshot이 달라 실패로 오판했다.
- 수정: enrollment 저장 snapshot이 `planReviewedAt`을 `Date.parse -> UTC ISO`로 정규화한 뒤 비교한다. 같은 시각의 `Z`/`+00:00`은 일치하지만 실제 회차 선택, 시작·종료 시간, 조정 사유, 메모, 학생·안내문 연결은 기존처럼 엄격하게 비교한다.
- 부분 성공 표시: enrollment POST가 성공한 뒤 Supabase 재조회나 대조에서 실패하면 전체 `저장 실패`라고 표시하지 않는다. `1/2 Supabase 저장 요청은 성공했지만 재조회 확인을 끝내지 못해 2/2 수업일지는 실행하지 않음`과 재확인 방법을 표시한다. POST 자체가 실패한 경우에만 기존 전체 저장 실패 문구를 사용한다.
- 현재 원천 확인: 스크린샷의 1~5회차·4회차 12:00~14:00 패턴과 일치하는 enrollment는 운영 원천에 한 건이며, 현재 1~5회차 수업일지 모두 같은 학생 연결을 포함한다. 이번 조사·구현에서는 운영 enrollment, lessons, 수업일지·출결, `notification_jobs`, Solapi를 수정하지 않았다. 새 SQL은 없다.
- AI 검증: 전용 fixture에서 `2026-07-27T04:01:02.345Z`와 `2026-07-27T04:01:02.345+00:00`이 같은 snapshot이 되는지, 실제 회차 시간 또는 신청 상태가 달라지면 불일치가 유지되는지 검사한다. `npm run test:special-lecture-enrollment-save`, `npm run test:production` 385건, `npm run build`, `git diff --check`를 통과했으며 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① 운영 화면을 `Ctrl+F5`로 새로고침하고 특강 학생의 `회차·진행 관리`를 다시 열어 새 1단계/2단계 UI인지 확인한다. ② 1~5회차가 모두 `수업일지 반영됨`이면 추가 저장 없이 닫는다. ③ 실제 수정이 필요한 경우에만 화면 초안을 변경하고 하단 2단계를 한 번 누른다. ④ `1/2 Supabase 저장 -> 2/2 미래 수업일지 반영 -> 저장 완료`가 순서대로 보이고 새로고침 후 회차·시간이 유지되는지 확인한다. ⑤ 실제 값이 다를 때만 불일치 오류가 나며, 알림관리·Solapi에 새 예약·발송·취소가 없으면 통과다.

## 2026-07-27 P0. 특강 회차 화면 초안·실제 저장 단계 분리 UI

- 사용자 증상: 특강 4회차는 enrollment 회차 계획만 저장되고 실제 수업일지 명단 반영이 빠져 있었으며, 공통 시간의 `선택 회차에 적용` 버튼이 실제 저장처럼 보여 1단계 화면 적용과 하단 저장·반영을 혼동하기 쉬웠다. 이번 작업에서는 4회차 운영 원천을 수정하지 않고 UI와 상태 표시만 보강했다.
- 1단계 화면 초안: 회차 선택, 전체 선택·해제, 공통 시간, 개별 시간, 메모 변경 영역을 `1단계 · 화면 초안 편집`으로 명시했다. 공통 시간 버튼은 `1단계 · 화면 초안에 적용`, 공식 시간 버튼은 `화면 초안: 공식 시간으로`로 바꾸고 진한 최종 버튼 스타일을 제거했다. 초안 변경 메시지는 아직 Supabase와 수업일지에 저장되지 않았음을 현재 모달 안에서 알린다.
- 2단계 실제 저장: 하단 고정 저장 바의 이름을 `2단계 · 실제 저장`, 최종 버튼을 `2단계 · Supabase 저장 + 미래 수업일지 반영`으로 바꿨다. 이 버튼만 기존대로 enrollment 저장·재조회 후 안전한 미래 lessons 반영·재조회를 실행한다. 성공 뒤에도 버튼을 `저장·수업일지 반영 다시 확인`으로 남겨, 과거에 계획만 저장된 누락 회차를 저장본 기준으로 다시 reconcile할 수 있다.
- 회차별 상태: 각 학생 회차 카드가 현재 lessons의 `specialLectureSessionId`, `studentIds`, `specialLectureStudentSchedules`를 읽어 `수업일지 반영됨`, `수업일지 미반영 · 하단 2단계 필요`, `제외 초안 · 하단 2단계 후 미래 명단 제외`, `수강 제외 · 과거 수업일지 보존`을 표시한다. 미반영·미래 제외 대기 카드는 주황색 행으로 강조한다.
- 저장/부작용: UI는 기존 저장 orchestration을 재사용하며 API·DB 스키마·알림 경로를 바꾸지 않았다. 구현·검증 중 운영 `special_lecture_enrollments`, lessons, 수업일지·출결, `notification_jobs`, Solapi를 수정하거나 예약·발송·취소하지 않았다. 새 SQL은 없다.
- AI 검증: production 시나리오에 화면 초안/실제 저장 단계 문구, 초안 버튼 비강조, 최종 저장·반영 버튼, 회차별 수업일지 연결 상태, 성공 뒤 재확인 버튼과 기존 enrollment/lessons Supabase 재조회 계약을 추가했다. `npm run test:production` 385건, `npm run build`, `git diff --check`를 통과했으며 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① `Lesson Hub > 특강관리`에서 해당 학생의 `회차·진행 관리`를 연다. ② 상단에 1단계 `화면 초안 편집/서버 저장 아님`, 2단계 `Supabase 저장 + 미래 수업일지 반영/실제 저장` 카드가 분리되어 보이는지 확인한다. ③ 4회차 카드가 주황색 `수업일지 미반영 · 하단 2단계 필요`인지 확인한다. ④ 공통 시간 입력 뒤 `1단계 · 화면 초안에 적용`을 누르고 아직 저장되지 않았다는 안내와 하단 실제 저장 버튼을 확인하되 운영 데이터를 바꾸지 않으려면 여기서 저장하지 않고 닫는다. ⑤ 실제 4회차 반영을 진행할 때만 하단 2단계를 누르고 `1/2 회차 계획 저장 -> 2/2 미래 수업일지 반영 -> Supabase 재조회 완료` 뒤 4회차가 `수업일지 반영됨`으로 바뀌는지 확인한다. ⑥ 기존 학생·시간·과거 기록·출결·알림 예약이 그대로면 통과다.

## 2026-07-27 P0. 신입생 월 전체 횟수 비율 제거·회당 단가 계산

- 사용자 정정: 신입생 금액은 `월 고정금액 × 첫 수업~말일 횟수 / 월 전체 횟수`로 나누지 않는다. 수업일지에서 확인된 첫 정규 수업일부터 말일까지의 예정 수업 횟수에 3타임 회당 단가를 바로 곱한다. 기본 단가는 중등 35,000원, 고등 37,500원이다.
- 계산식: `신입 적용금액 = 첫 수업일~말일 월별 스케줄 횟수 × 학년별 회당 단가 + 수기 조정`이다. 선택 월 중간에 조회해도 첫 수업일부터 말일까지 정상 수업한다고 보고 예정 횟수를 포함한다. 결석은 기존 원칙대로 자동 차감하지 않고 보충은 참고만 한다. 월 전체 13회 같은 분모와 월 고정금액은 신입 계산에 사용하지 않는다.
- 화면/수정: 신입생 행에는 기존 `10/13회 · 76.9%` 대신 `10회 × 35,000원`처럼 표시한다. `기준 금액` 입력은 신입생일 때 회당 단가를 보여 주고 학생별로 수정할 수 있다. 재원생은 기존 월 고정금액 입력, 퇴원생은 기존 `1일~마지막 수업` 월별 스케줄 횟수 비율을 유지한다.
- 7월 기대값: 앞선 읽기 전용 원천 기준으로 중등 김보현·서빈·이윤성·조소현·최지민·이윤채는 각 `10회 × 35,000원 = 350,000원`, 중등 김지민은 `5회 × 35,000원 = 175,000원`이다. 이전 작업기록의 323,077원·161,538원 비율 계산 미리보기는 이 기준으로 폐기한다.
- 저장/부작용: 학생별 회당 수정값은 기존 `app_state.monthlyInstructorSettlements` 월 스냅샷의 `newStudentSessionAmount`로 저장하고 Supabase 재조회 snapshot에 포함한다. 계산·자동판정은 화면 draft에서만 수행하며 이번 구현 중 운영 정산 snapshot, 학생, lessons, 수업일지, 출결, 특강, 알림톡/Solapi를 수정하지 않았다. 새 SQL은 없다.
- AI 검증: 서로 다른 길이의 월별 수업 스케줄에서도 고등 신입은 횟수 × 37,500원, 중등 신입은 횟수 × 35,000원으로 계산하고, 회당 수정값이 저장 재조회 snapshot에 포함되는 fixture를 추가했다. 퇴원생의 기존 월 고정금액 횟수 비율과 수기 월정액 예외도 회귀 검사한다. `npm run test:monthly-settlement`, `npm run test:production` 385건, `npm run build`, `git diff --check`를 통과했으며 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① `운영 > 정산 > 월별 정산 > 2026-07`에서 자동 신입생 행의 계산 방식 아래가 `N회 × 35,000원` 또는 `N회 × 37,500원`으로 보이고 `/13회`와 횟수 비율이 사라졌는지 확인한다. ② 중등 10회 학생의 정규 적용금액이 350,000원, 중등 5회 학생이 175,000원인지 확인한다. ③ 고등 신입생은 같은 방식으로 `횟수 × 37,500원`인지 확인한다. ④ 기준 금액의 회당 단가를 한 학생만 수정한 뒤 다른 학생에게 번지지 않는지 보고 원래 값으로 되돌린다. ⑤ 실제 저장할 경우 Supabase 재조회 일치와 새로고침 유지를 확인하고 학생·수업일지·출결·특강·알림 원천에 변화가 없으면 통과다.

## 2026-07-27 P2. 정산 단일 메뉴·상단 월별/특강 탭

- 사용자 요청: 사이드바 운영 영역에 `월별 수업 정산`, `특강 정산`이 각각 표시되던 구조를 하나의 `정산` 메뉴로 합치고, 정산 화면 상단 버튼으로 두 정산을 전환하게 했다.
- 화면 구조: `운영 > 정산`을 열면 `월별 정산`이 기본으로 보이며 상단의 `월별 정산`·`특강 정산` 버튼으로 전환한다. 활성 버튼은 짙은 색으로 구분하고 작은 화면에서는 두 버튼이 같은 너비로 표시된다.
- 원천/저장 보존: 월별 패널은 기존 `app_state.monthlyInstructorSettlements`, 특강 패널은 기존 `app_state.specialLectureInstructorSettlements`를 그대로 읽고 저장한다. 탭은 화면 진입 경로만 합쳤으며 계산식·명단·저장 API·Supabase 재조회 검증은 바꾸지 않았다. 두 패널 모두 수정 즉시 기존 localStorage 임시 초안을 기록하므로 저장 전 탭을 전환해도 작성값을 복구한다.
- 부작용 경계: 운영 학생·수업일지·출결·특강 enrollment·정산 snapshot을 수정하지 않았고 새 SQL, `notification_jobs`, Solapi 예약·발송·취소도 없다.
- AI 검증: production 시나리오에 단일 사이드바 메뉴, 상단 탭, 두 기존 정산 패널과 저장 callback 연결, 기존 별도 메뉴 제거 계약을 반영했다. `npm run test:production` 385건과 `npm run build`를 통과했으며 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① 사이드바 `운영`에 `정산` 메뉴가 하나만 보이고 기존 두 정산 메뉴는 사라졌는지 확인한다. ② `정산`을 누르면 상단에 `월별 정산`·`특강 정산` 버튼이 보이고 월별 정산이 기본으로 열리는지 확인한다. ③ 특강 정산 버튼을 눌러 기존 전체과정 정산표와 수기 조정값이 그대로 보이는지 확인한다. ④ 각 탭에서 값을 하나 수정한 뒤 다른 탭으로 갔다 돌아와 입력값이 유지되는지 확인한다. ⑤ 저장 후 각 패널의 Supabase 재조회 일치 문구와 새로고침 유지 여부를 확인하고, 학생·수업일지·출결·특강 명단·알림 원천에 변화가 없으면 통과다.

## 2026-07-27 P0. 이번 달 최초 수업 학생의 수업일지 자동 부분월 정산

- 사용자 증상: `운영 > 월별 수업 정산`에서 여러 학생에게 `이번 달 최초 수업 · 신입 여부 확인`이 표시되지만 계산 방식과 금액은 `재원생 · 월정액`으로 남아 있었다. 기존 구현은 전체 정규 수업일지에서 최초 등장 월을 찾고도 안전 확인용 경고만 표시하고 교사가 직접 계산 방식을 바꾸기 전에는 전액을 유지했다.
- 운영 원천 읽기 확인: 운영 `lessons.studentIds`, 학생, 2026-07 정산 `app_state`를 읽기 전용으로 대조했다. 현재 재원 ID 기준 첫 정규 수업일은 이윤채 7월 8일, 김보현·서빈·이윤성·조소현·최지민 7월 9일, 김지민 7월 21일이며, 7월 정산에는 모두 `mode=fixed`이고 자동판정 원천 표시는 없었다.
- 변경된 자동판정: 취소·휴강·보충·특강을 제외한 전체 정규 수업일지에서 학생 ID의 가장 이른 날짜를 찾는다. 그 날짜가 선택 월 안이고 1일보다 늦으면 기존 `fixed` 스냅샷도 로컬 정산 draft에서 `mode=new`, `modeSource=lesson_journal`로 바꾸고 `첫 수업일~말일` 월별 스케줄 횟수 비율을 즉시 계산한다. 화면에는 초록색 `수업일지 자동 · YYYY-MM-DD 첫 수업`을 표시한다.
- 수기 예외: 교사가 계산 방식 드롭다운을 한 번 바꾸면 `modeSource=teacher`를 저장한다. 이후 새로고침에서도 수업일지 자동판정이 월정액·신입·퇴원 수기 선택을 덮어쓰지 않는다. 과거 달 정규 수업일지가 있거나 해당 월 1일이 최초 수업인 학생은 자동 신입 후보가 아니다.
- 현재 원천 미리보기: 새 로직을 운영 데이터에 읽기 전용 적용했을 때 김보현·서빈·이윤성·조소현·최지민은 10/13회·323,077원, 이윤채는 10/13회·323,077원, 김지민은 5/13회·161,538원으로 계산됐다. 이는 현재 저장된 중등 고정금액 420,000원과 학생별 월 스케줄을 사용한 배포 전 미리보기이며 아직 운영 정산 스냅샷에 쓰지 않았다.
- 저장/부작용: 자동판정은 화면의 저장 전 draft에만 반영되고 `월별 정산 저장`을 눌러야 `app_state.monthlyInstructorSettlements`에 저장된다. `modeSource`도 저장 후 Supabase 재조회 snapshot에 포함한다. 학생, lessons, 수업일지, 출결, 보충, 특강, 알림톡/Solapi는 읽기만 하며 변경하지 않는다. 새 SQL은 없다.
- AI 검증: 수업일지가 이번 달부터 시작한 legacy `fixed` 행의 자동 신입 전환, `modeSource=lesson_journal`, 이전 달 수업이 있는 학생 제외, 교사의 `modeSource=teacher` 월정액 선택 보존, 저장 재조회 snapshot 원천 보존을 전용 fixture로 검사했다. production 시나리오에는 자동판정·수기 예외·저장 필요 안내·초록 상태 표시 계약을 추가했다. `npm run test:monthly-settlement`, `npm run test:production` 385건, `npm run build`, `git diff --check`를 통과했으며 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① 배포 후 `운영 > 월별 수업 정산 > 2026-07`을 새로 열어 위 학생들의 계산 방식이 `신입생 · 첫 수업~말일`이고 이름 아래 초록색 첫 수업일이 보이는지 확인한다. ② `자동 정산 기간`, `정산 기준 N/전체 N회`, 적용금액을 위 읽기 전용 미리보기와 대조한다. ③ 예외 학생 한 명을 `재원생 · 월정액`으로 바꾸고 저장해 `수업일지 최초 수업 · 월정액 수기 적용`이 보이는지 확인한다. ④ `월별 정산 저장` 후 Supabase 재조회 일치 문구와 새로고침 유지 여부를 확인한다. ⑤ 학생 명단·수업일지·출결·특강·알림관리 원천에 변화가 없으면 통과다.

## 2026-07-27 P0. 결석보강 취소·원 결석기록 유지

- 상태: 기능·UI 구현과 전용/시나리오 검증 완료. 운영 데이터에는 쓰지 않았으며 배포 후 실제 결석보강 한 건으로 사람 gate를 진행한다.
- 사용자 기준: 기존 `결석 기록 취소`는 원 수업일지의 출결을 `결석 -> 대기`로 되돌리는 기능이다. 새 `보강만 취소`는 이 기능과 분리해 원 수업일지의 결석 상태·사유·등하원 필드를 그대로 유지하면서 저장된 결석보강 항목만 취소한다.
- 보강 취소 저장 흐름: `makeup_tasks` 항목을 삭제하지 않고 `status=canceled`, `cancellationMode=keep_source_absence`, 취소시각·사유·결석보존 플래그를 저장한다. 연결된 보강 수업일지가 있으면 같은 lesson을 `status=canceled`로 보존하고, 해당 보강 task/lesson에 연결된 아직 발송되지 않은 학생·학부모 일정 안내와 당일 학생 예약을 실제 취소 API로 처리한다. 이미 발송된 내역은 되돌리지 않고 이력으로 남긴다.
- 완료 재조회: 보강 task, 취소한 연결 수업일지, 원 `lesson_student_records`를 Supabase에서 다시 읽는다. 보강 task snapshot과 연결 lesson status가 일치하고, 원 결석의 상태·사유·등원·하원 필드 snapshot이 취소 전과 완전히 같을 때만 완료한다. 취소 lesson 재조회에만 `GET /api/lessons?includeCanceled=true`를 사용하고 일반 수업 목록은 계속 취소 lesson을 숨긴다. 기존 `결석 기록 취소`에도 API 저장 후 Supabase 재조회·대기 상태 대조를 추가했다.
- 목록/이력: 취소된 보강은 활성 결석보강 후보와 학생 포털 일정에서 제외한다. `최근 한 달 보충 내역`에는 `보강 취소 · 결석 유지`로 남아 원 결석기록을 보존한 취소임을 확인할 수 있다. 원 결석 자체가 남아도 같은 취소 task가 해당 결석 후보를 닫아 다시 미처리 건으로 나타나지 않는다.
- UI 정리: 상세 모달의 내용 저장·일정 반영·완료 버튼 묶음과 취소 영역을 분리했다. 저장된 결석보강에는 주황색 `보강 일정만 취소` 영역과 `보강만 취소` 버튼을, 미저장 결석 후보에는 붉은 `결석 원본 자체를 취소` 영역과 `결석 기록 취소` 버튼을 표시한다. 확인창은 원 결석기록, 보강 항목, 연결 일정, 알림톡에 각각 어떤 변화가 생기는지 표로 보여 주고 `결석기록 유지`와 `결석기록 변경`을 다른 색으로 구분한다.
- 저장/외부 영향: 새 SQL은 없다. 구현·자동검증 중 운영 `makeup_tasks`, lessons, `lesson_student_records`, `notification_jobs`, Solapi를 수정하거나 취소하지 않았다. 배포 후 사용자가 확인창에서 `보강만 취소`를 확정할 때만 선택한 보강 원천과 연결된 미발송 예약에 위 side effect가 발생한다.
- AI 검증: 순수 fixture로 결석보강 task 취소, 연결 lesson 취소, task/source/lesson ID 보존, 원 결석 snapshot 불변, 다른 보충 유형 차단을 검사했다. production 시나리오에는 두 취소 의미 분리, 확인창, 활성 목록 제외, 취소 이력, 관련 예약만 취소, task/lesson/source 재조회 계약을 추가했다. `npm run test:supplement-cancellation`, `npm run test:production` 384건, `npm run build`, `git diff --check`를 통과했으며 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① `보충관리 > 결석보강`에서 원 결석 상태·사유와 현재 보강 일정, 알림 예약 수를 먼저 기록한다. ② 저장된 결석보강 상세 하단에 별도 주황색 영역과 `보강만 취소`가 보이고, 미저장 후보에는 대신 붉은 `결석 기록 취소`가 보이는지 확인한다. ③ `보강만 취소`를 누른 확인창에서 `원 결석기록: 결석 상태·사유 그대로 유지`, 연결 수업일지·미발송 예약 취소가 명시되는지 확인한다. ④ 실제 확정 후 활성 결석보강 목록과 학생 포털 일정에서 사라지고 달력의 연결 보강 수업도 사라지는지 확인한다. ⑤ 원 정규 수업일지를 열어 출결이 여전히 결석이고 사유·시간 필드가 전과 같은지 확인한다. ⑥ `최근 한 달 보충 내역`에 `보강 취소 · 결석 유지`가 보이며, 알림관리에서는 해당 보강의 미발송 예약만 취소되고 이미 발송된 이력·다른 학생 예약은 그대로면 통과다.

## 2026-07-27 P1-5. 특강 전체과정 별도 정산·김연우 7회 원천 복구

- 상태: 별도 특강 정산 화면 구현, 운영 Supabase 원천 보정, 독립 재조회 검증 완료. 전체 자동검증·배포 후 사람 화면 gate를 진행한다.
- 계산 경계: `운영 > 월별 수업 정산`은 선택 월의 정규 수업료만 계산한다. 특강은 월별 표와 합계에서 완전히 분리해 새 `운영 > 특강 정산`에서 안내문별 전체 과정으로 계산한다. 7월에 시작해 8월에 끝나는 특강도 선택·확정된 전체 회차와 유효 시간을 한 행에서 계산하고, 정규와 같은 고정 상수 `0.5 × 0.967 × 0.985`를 적용해 예상 수령액을 표시한다.
- 실제 원천/수기값 분리: 실제 수강 계획은 `special_lecture_enrollments`와 `app_state.specialLectureGuides`에서 읽어 계속 표시한다. 정규 보강 대체처럼 일부 회차만 유료 적용할 때는 실제 회차를 삭제·필터링하지 않고 `유료 회차 조정`, 별도 합의금액은 `금액 직접 입력`과 필수 사유로 저장한다. 직접 저장 원천은 새 `Supabase app_state.specialLectureInstructorSettlements`이며 local draft, 명시 저장, API 성공, Supabase 재조회 snapshot 일치 뒤에만 저장 완료로 표시한다.
- 운영 수기 정산 반영: 김보현은 실제 클리닉 5회를 보존한 채 유료 3회와 `특강 참여 중 정규 보강 대체분 2회를 제외하고 유료 3회 적용` 사유를 저장했다. 신초봄은 실제 1~5회차를 보존한 채 직접 확정금액 116,000원과 사유를 저장했다. 두 값은 특강 명단·회차·수업일지·출결을 바꾸지 않는다.
- 김연우 원천 복구: 활성 클리닉 신청에 누락됐던 2회차를 되살려 2026-07-24, 07-27, 07-29, 07-31, 08-03, 08-05, 08-07 총 7회로 복구했다. 7월 24일 수업일지에는 김연우의 기존 `present`, `13:00` 기록과 정상 check-in 이벤트가 이미 있었으므로 출결을 새로 쓰지 않고 enrollment의 회차 계획만 연결했다. 복구 후 7개 공식 수업일지 모두 같은 학생 ID를 포함하고 있음을 재조회했다.
- 7월 23일 원천 정리: 공통수학2 특강 수업일지의 실제 명단·수업기록 2명(김룡기, 김형준)은 보존했다. `studentIds`에는 없지만 학생별 시간 배열에만 남은 오래된 3개 참조를 제거해 명단 2명·시간 참조 2명·수업기록 2건이 일치하게 했다. 수업일지나 출결 기록을 삭제하지 않았다.
- 저장/외부 영향: 운영 쓰기는 위 김연우 enrollment, 7월 23일 수업일지의 오래된 학생별 시간 참조, 새 특강 정산 app_state 한 키로 제한했다. 변경 전후 7월 23일·24일 `notification_jobs` snapshot이 같음을 확인했으며 Solapi 예약·발송·취소는 실행하지 않았다. 새 SQL은 없다.
- AI 검증: 전용 계산 fixture에서 김연우 7회·262,500원, 김보현 실제 5회/유료 3회·112,500원, 신초봄 실제 5회/직접 116,000원, 다른 달 회차 포함, 실제 수강 원천 보존, 수기 사유, 저장 직전 같은 안내문의 다른 학생 설정 보존을 검사한다. 운영 독립 재조회에서 김연우 active plan 7개와 7개 수업일지 연결, 7월 24일 `present 13:00`, 7월 23일 명단·시간·기록 각 2건, 김보현·신초봄 수기 원천을 다시 확인했다. `npm run test:monthly-settlement`, `npm run test:production` 383건, `npm run build`, `git diff --check`를 통과했고 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① 배포 후 `운영 > 특강 정산`을 열어 클리닉이 월별로 잘리지 않고 7월·8월 전체 과정으로 한 번만 보이는지 확인한다. ② 김연우가 `7회`, 날짜가 `7/24·7/27·7/29·7/31·8/3·8/5·8/7`, 자동금액이 262,500원인지 확인하고 7월 24일 수업일지에서 `출석 13:00`인지 연다. ③ 김보현은 실제 5회와 유료 3회가 함께 보이고 112,500원 및 조정 사유가 보이는지 확인한다. ④ 신초봄은 실제 5회와 직접금액 116,000원 및 사유가 보이는지 확인한다. ⑤ 임의 학생을 수기 모드로 바꾸되 사유를 비우면 행 경고와 저장 차단이 생기고, 사유를 입력해 저장하면 `저장 중 -> 서버 반영 확인 중 -> 저장 완료` 뒤 새로고침에도 유지되는지 확인한 후 원래 값으로 복원한다. ⑥ `운영 > 월별 수업 정산`에는 특강 표·특강 합계가 없고 정규 금액만 보이는지 확인한다. ⑦ 알림관리에서 새 예약·발송·취소가 없으면 통과다.

## 2026-07-27 P2. 월별 정산 스케줄 경고 행 강조

- 상태: 구현·자동검증 완료. 운영 배포 후 화면 확인 대기.
- 사용자 요청: 월별 스케줄을 해석하지 못해 0원으로 중단된 경고를 문구만 표시하지 말고, 해당 학생 행의 색상을 다르게 해 직접 찾고 수정하기 쉽게 한다.
- 화면: `운영 > 월별 수업 정산`에서 신입·퇴원 계산 방식이고 정규 수업일지는 있으나 월별 스케줄을 읽지 못한 행 전체를 연한 주황색으로 표시한다. 고정된 학생 칸에는 왼쪽 강조선과 `스케줄 수정 필요` 배지를, 월별 스케줄 입력칸에는 진한 경고 테두리를 표시한다. 스케줄을 올바르게 수정해 월별 횟수가 계산되면 행 경고가 즉시 사라진다.
- 계산/저장 영향: 기존 `신입·퇴원 + 정규 수업일지 있음 + 월 스케줄 0회` 경고 판정을 그대로 사용하며 금액, 단가, 수업일지 경계, 월별 정산 Supabase 저장 구조는 바꾸지 않았다. 학생·수업일지·출결·특강·알림 원천과 Solapi side effect도 없다. 새 SQL은 없다.
- AI 검증: 경고 조건과 전체 행·고정 학생 칸·스케줄 입력칸 강조 CSS를 production 시나리오에 추가했다. `npm run test:production` 382건과 `npm run build`, `git diff --check`를 통과했다. 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① `운영 > 월별 수업 정산`에서 신입 또는 퇴원 방식인데 스케줄 형식을 읽지 못하는 학생을 찾는다. ② 그 학생 행 전체가 연한 주황색이고 학생 칸에 `스케줄 수정 필요`, 스케줄 입력칸에 진한 주황색 테두리가 보이는지 확인한다. ③ 스케줄을 `월수금 19:00-22:00`처럼 지원 형식으로 고치면 즉시 일반 행 색상으로 돌아오고 정산 기준 횟수·금액이 계산되는지 확인한다. ④ 다시 잘못된 형식으로 바꾸면 경고색과 0원이 재현되는지 확인한 뒤 실제 스케줄로 복원한다. ⑤ 필요한 경우 `월별 정산 저장` 후 Supabase 재조회 완료와 새로고침 유지를 확인하고, 학생·수업일지·출결·특강·알림 데이터가 변하지 않으면 통과다.
...
- 사용자 기준: `기존 데이터 삭제`는 Tally 제출 전에 선생님이 임의로 만든 학생 기본정보를 Tally 제출값으로 교체한다는 뜻이다. 임시 학생에게 이미 연결된 특강 회차·학생별 시간·메모, 수업일지 명단, 출결·진행 기록은 삭제 대상이 아니다. 같은 학생 ID와 기존 운영 기록을 유지한 채 Tally 신청 원본과 기본정보를 추가 연결해 그 학생을 계속 사용한다.
- 구현: 특강 신청 연결 전과 이미 연결된 학생 카드의 버튼을 `Tally 기본정보로 교체 후 연결`, `학생 기본정보를 Tally로 다시 반영`으로 바꿨다. Tally가 소유하는 이름·학교·학년·학생/학부모 연락처·특이사항은 제출값으로 교체하고 Tally 빈칸은 해당 기본정보를 비운다. 학생 ID·로그인·PIN·정규반·교재·개별 시간표와 기존 `sessionPlans/sessionIds/memo/planSource/planReviewedAt`은 그대로 저장한다.
- 기록 보존: 기존 enrollment가 있으면 Tally 신청에 회차 정보가 없거나 달라도 현재 확정 회차·시간·메모를 우선 보존한다. `applicationId` 연결과 학생 기본정보만 갱신하며 lessons나 `lesson_student_records` 저장 API는 호출하지 않는다. 기존 enrollment가 없는 신규 학생만 Tally 신청 회차를 초기 초안으로 사용한다.
- 신초봄 적용 의미: 중3 신초봄의 기존 특강 ID, 1~5회차 13:00~15:00, 수업일지·출결·숙제는 계속 원본으로 남는다. Tally 제출의 연락처 등 기본정보와 신청 연결만 같은 학생에 붙는다. 이미 완료된 운영 병합 데이터에 추가 쓰기는 하지 않았다.
- 저장/외부 영향: 학생 기본정보 교체는 기존 `students` 저장·재조회, 특강 신청 연결은 `special_lecture_enrollments` 저장·재조회를 사용한다. 회차·수업일지 보존값도 enrollment 재조회 snapshot에 포함된다. 새 SQL, 운영 DB 수동 수정, `notification_jobs`, Solapi 예약·발송·취소는 없다.
- AI 검증: 빈 Tally 특강 신청으로 기존 enrollment를 다시 연결해도 active 회차, 13:00~15:00 조정 시간, 수기 메모, 계획 원천, 검토 완료시각이 모두 유지되는 순수 함수 테스트를 추가했다. 운영 Supabase의 신초봄 enrollment를 읽기 전용 fixture로 넣은 결과도 교체 전후 active 5회와 5개 13:00~15:00 시간, 메모·원천·검토시각이 모두 같았다. production 시나리오도 `기본정보만 교체 + 회차·수업일지 보존` 문구와 계약을 검사하도록 교체했다. `npm run test:tally-student-merge`, `npm run test:production` 381건, `npm run build`, `git diff --check`를 통과했고 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① `Lesson Hub > 특강관리`의 Tally 연결 학생 카드에서 버튼이 `학생 기본정보를 Tally로 다시 반영`인지 확인한다. ② 누른 뒤 확인창에 삭제될 수 있는 Tally 소유 기본정보와 함께 `기존 특강 회차·시간·메모와 과거·미래 수업일지·출결은 모두 유지`가 표시되는지 확인하고 먼저 취소한다. ③ 신초봄 카드의 1~5회차 13:00~15:00와 기존 메모를 기록한 뒤 실제 기본정보 반영을 실행한다. ④ 저장·재조회 완료 후 새로고침해 연락처 등 Tally 기본정보는 반영되고 같은 1~5회차·시간·메모가 유지되는지 본다. ⑤ 7월 22일 이후 수업일지·출결·숙제가 그대로이고 새 알림 작업이 없으면 통과다.

## 2026-07-27 P1-4. 신입·퇴원생 첫·마지막 수업 기준 정산

- 상태: 구현·자동검증 완료. 운영 정산 스냅샷이나 학생·수업일지 원천은 수정하지 않았으며 배포 후 사람 화면·저장 gate 대기.
- 사용자 기준: 신입생은 선택 월 수업일지에서 그 학생의 첫 정규 수업일을 찾고 `첫 수업일~말일`에 해당하는 수업 횟수만 적용한다. 중도 퇴원생은 마지막 정규 수업일을 찾아 `1일~마지막 수업일` 횟수만 적용한다. 수기 인정일, 학생 프로필의 퇴원일, 시수 비율은 금액 계산 경계로 사용하지 않는다.
- 계산식: 학생별 저장 월 스케줄을 선택 월 1일부터 말일까지 실제 달력 요일로 펼쳐 전체 횟수를 만든다. `신입 적용금액 = 월 고정금액 × 첫 수업일~말일 스케줄 횟수 / 월 전체 스케줄 횟수`, `퇴원 적용금액 = 월 고정금액 × 1일~마지막 수업일 스케줄 횟수 / 월 전체 스케줄 횟수`다. 시작·종료일 수업을 모두 포함하며 서로 다른 수업시간이 섞여도 시수 합계가 아니라 회차 수를 사용한다. 스케줄 형식을 읽을 수 없으면 수업일지 일부만으로 100%를 추정하지 않고 `스케줄 확인 필요 · 0원`으로 중단한다.
- 화면: 계산 방식 이름을 `신입생 · 첫 수업~말일`, `퇴원생 · 1일~마지막 수업`으로 바꾸고, 기존 날짜 입력칸을 제거했다. `자동 정산 기간`에 수업일지에서 찾은 첫·마지막 날짜를 읽기 전용으로 표시하며, 계산 방식 아래와 횟수 참고 칸에 `정산 기준 N/전체 N회`와 횟수 비율을 함께 표시한다. 학생 달력도 금액 기준 횟수와 기간 내 실제 수업일지 횟수를 구분한다.
- 모드 판정: 선택 월에 처음 수업일지가 나타난 학생은 기존처럼 `이번 달 최초 수업 · 신입 여부 확인`으로 알리고, 교사가 신입 계산 방식을 선택하기 전에는 재원생 월정액을 유지한다. 선택 월에 퇴원일이 있는 학생의 기존 퇴원 모드 기본값은 유지한다.
- 저장: 직접 저장 원천은 기존 `Supabase app_state.monthlyInstructorSettlements`다. 월별 정산 저장 시 표시된 자동 첫·마지막 경계일을 snapshot의 호환 필드에도 기록하고, 기존처럼 POST 뒤 같은 월을 재조회해 전체 snapshot 일치 후에만 완료 표시한다. 계산 중 `students`, `lessons`, `lesson_student_records`, 특강 enrollment·안내문, `notification_jobs`, Solapi를 수정하지 않는다. 새 SQL은 없다.
- AI 검증: 신입생의 잘못 저장된 수기 시작일 무시, 첫 수업일~말일 포함 횟수, 퇴원일·수기 종료일 무시, 1일~마지막 수업일 포함 횟수, 시간 길이가 서로 다른 스케줄에서도 시수 비율이 아닌 횟수 비율 사용을 전용 테스트에 추가했다. production 시나리오에는 자동 경계 UI와 날짜 입력 제거 계약을 추가했다. `npm run test:monthly-settlement`, `npm run test:production` 381건, `npm run build`, `git diff --check`를 통과했다. 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① `운영 > 월별 수업 정산`에서 해당 월의 실제 신입생을 `신입생 · 첫 수업~말일`로 선택한다. ② 자동 정산 기간의 첫 날짜가 그 학생이 처음 포함된 정규 수업일지 날짜와 같은지 확인한다. ③ `정산 기준 N/전체 N회`가 학생의 월별 스케줄을 달력에 펼친 횟수와 맞고 적용금액이 `고정금액 × N/전체 N`인지 계산한다. ④ 중도 퇴원생은 `1일~마지막 수업`을 선택해 마지막 날짜가 퇴원일이 아니라 실제 마지막 정규 수업일지인지 확인한다. ⑤ 저장 후 Supabase 재조회 일치 문구와 새로고침 유지 여부를 확인한다. ⑥ 학생·수업일지·출결·특강·알림관리 원천이 변하지 않으면 통과다.

## 2026-07-27 P0. 고3 이채빈 일반·특강 학생 원천 병합

- 상태: 운영 Supabase 병합과 전 원천 재조회 검증 완료. 코드 변경은 없다.
- 병합 판단: 일반 Tally 학생 `student_intake_tally_kb0MYvM`에는 로그인 `04이채빈`, 학생·학부모 연락처, 정의여고 고3, 월수금 4–7반, 정규 수업일지·숙제·예약 알림이 있었다. 특강 전용 학생 `student_special_lecture_special_lecture_tally_RWZ64rJ`에는 여름 개별 진도 클리닉 enrollment·8회차·특강 수업기록·출결이 있었다. 예약된 정규 알림을 건드리지 않고 학생 포털·정규반 원천을 유지하기 위해 일반 Tally ID를 최종 재원 원천으로 확정했다.
- 이전 범위: 활성 특강 enrollment의 학생 연결과 8개 공식 특강 lesson의 `studentIds/specialLectureStudentSchedules`를 일반 Tally ID로 옮겼다. 기존 특강 수업기록 2건은 record ID와 내용은 유지하고 학생 ID만 바꿨으며, 7월 24일 `present` 출결 이벤트 1건도 같은 record 연결·등원시각을 유지한 채 일반 ID로 연결했다. 회차 선택 8건, 공식 13:00~16:00 시간, 신청 연결, 메모·상태는 바꾸지 않았다.
- 기존 원천 보존: 일반 Tally 제출 원본과 등록완료 메모, 학생 로그인·PIN·연락처·정규반, 7월 정규 수업일지 5회·15시간, 정규 숙제 2건을 그대로 유지했다. 기존 정규 학생·학부모 예약 알림 2건은 payload·상태·예약시각을 전후 대조해 같았으며 취소·재예약·발송을 실행하지 않았다.
- 중복행 처리: 특강 전용 중복 학생은 삭제하지 않고 로그인 `merged-special-RWZ64rJ`, 상태 `paused`, 사유 `중복 학생 병합`으로 보관했다. 병합 뒤 이 ID의 수업기록, 출결 이벤트, 숙제, 보충, 오답, 성적, 테스트, 운영 알림, 발송 이력·예약 작업, 특강 enrollment, lessons/app_state 참조를 재조회해 모두 0건임을 확인했다.
- 정산 재검증: 2026년 7월 이채빈은 최종 일반 ID 한 명으로 정규 5회·15시간, 확정 특강 5회·15시간·187,500원으로 계산된다. 전체 7월 특강 대상 8명과 기준 총액 1,275,000원은 병합 전후가 같다.
- 외부 영향: 학생 연결 병합 외에 Tally 원문, 특강 안내문, 회차 내용, 숙제, 알림 예약, `notification_jobs`, Solapi 예약·발송·취소를 수정하지 않았다. 레코드를 영구 삭제하지 않았고 새 SQL은 없다.
- 사람 gate: ① `학생관리`를 새로고침해 재원 목록에 이채빈이 `고3 / 정의여고 / 04이채빈 / 월수금 4–7반` 한 명으로 보이는지 확인한다. ② 퇴원생 목록에는 `merged-special-RWZ64rJ` 중복행이 보이는지 확인한다. ③ `Lesson Hub > 특강관리`에서 이채빈의 8회차 신청과 13:00~16:00 시간이 한 학생으로 이어지는지 확인한다. ④ 7월 24일 특강 출석과 기존 정규 수업일지 5회·숙제 2건이 유지되는지 확인한다. ⑤ `운영 > 월별 수업 정산 > 2026년 7월`에서 이채빈이 정규 한 행, 특강 `5회·15시간·187,500원`으로 보이고 전체 특강 총액이 1,275,000원인지 확인한다. ⑥ 알림관리의 기존 정규 예약 2건이 그대로이고 새 예약·발송이 없으면 통과다.

## 2026-07-27 P1-3. 월별 정산 제외 행 숨김·특강관리 원천 정산

- 상태: 구현·자동검증·운영 읽기 전용 대조 완료. 운영 배포 후 사람 화면·월별 저장 gate 대기. 구현과 대조 중 운영 정산 스냅샷, 특강, 학생, 수업일지, 출결, 알림 데이터를 수정하지 않았다.
- 정산 제외 화면: 정규 행의 `정산 제외`를 누르면 기존처럼 해당 월 `studentSettings[studentId].excluded=true`를 local draft에 기록하되, 흐린 0원 행을 표에 남기지 않고 정규 정산표에서 즉시 숨긴다. 숨긴 학생은 표 아래 `삭제한 정산 행 N명 · 복원` 접힘 목록에서 `행 복원`할 수 있다. 학생·수업일지·출결 원천을 삭제하는 기능이 아니며 `월별 정산 저장 -> Supabase 재조회 대조`를 통과해야 새로고침 뒤에도 제외 상태가 유지된다.
- 정규/특강 명단 분리: 정규 정산표는 선택 월의 실제 정규 수업일지 `lessons.studentIds`만 사용한다. 특강 수업일지만 있는 학생은 더 이상 0원 정규 행으로 표시하지 않고 아래 특강 정산표에만 나타난다. 휴강·보충·취소 수업은 정규 명단 원천이 아니다.
- 특강 금액 원천: 학생별 수기 `특강 총액` 입력 열을 제거했다. 별도 `특강 정산` 표가 `app_state.specialLectureGuides`의 `pricingMode/pricePerSession/pricePerHour/sessions`와 Supabase `special_lecture_enrollments`의 활성 학생·선택 회차·학생별 유효 시간을 읽어 선택 월 회차만 계산한다. `planReviewedAt`이 있는 확정 회차만 합계에 반영하고 미확정 회차는 0원·확정 필요로 표시한다. 같은 학생·같은 특강의 중복 enrollment가 남아 있어도 같은 session ID는 한 번만 계산하고 중복 원천 건수를 화면에 알린다.
- 제외/특강 경계: 정규 정산에서 제외한 학생은 같은 달 특강 정산표와 특강 합계에서도 숨긴다. 특강만 있는 학생의 취소·제외는 월별 수기 금액으로 덮지 않고 특강관리 enrollment 원천에서 관리한다. 과거 월 스냅샷의 `specialGrossAmount` 필드는 호환·감사 목적으로 보존하지만 화면과 합계에서는 읽지 않는다.
- 운영 읽기 전용 대조: 2026년 7월 운영 Supabase의 특강 안내문 2건·enrollment 13건을 새 계산기에 대입했다. 확정 대상은 8명이며 특강 기준 총액은 1,275,000원이다. 김연우는 3회·9시간·112,500원, 신초봄은 학생별 조정 시간 기준 5회·10시간·187,500원으로 계산됐다. 대조는 조회만 수행했고 어떤 row도 저장하지 않았다.
- 저장/외부 영향: 월별 제외·복원만 기존 `Supabase app_state.monthlyInstructorSettlements`에 명시 저장한다. 특강 안내문과 enrollment는 읽기 전용이며 정산 화면에서 수정하지 않는다. 새 SQL, `notification_jobs`, Solapi 예약·발송·취소, 학생·수업일지·출결 변경은 없다.
- AI 검증: 정규/특강 명단 분리, 활성·확정 enrollment만 금액 반영, 선택 월 회차 필터, 회차/시간 단가, 학생별 시간 변경, 중복 session 1회 계산, 취소 enrollment 제외, 미확정 0원, 월별 정산 제외 연동을 전용 테스트에 추가했다. `npm run test:monthly-settlement`, `npm run test:production` 379건, `npm run build`, `git diff --check`를 통과했다. 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① 배포 후 `운영 > 월별 수업 정산`에서 2026년 7월을 연다. ② 정규 표에 특강만 있는 학생의 0원 행과 수기 `특강 총액` 열이 없고, 아래 `특강 정산` 표가 따로 보이는지 확인한다. ③ 특강 기준 총액이 1,275,000원인지, 김연우 `3회·9시간·112,500원`, 신초봄 `5회·10시간·187,500원`인지 특강관리 회차와 대조한다. ④ 정규 학생 한 명의 `정산 제외`를 누르면 행이 즉시 사라지고 정규·특강 합계에서 빠지는지 확인한다. ⑤ `삭제한 정산 행`을 열어 `행 복원`하면 행과 합계가 돌아오는지 확인한다. ⑥ 다시 필요한 상태로 정한 뒤 `월별 정산 저장`을 눌러 Supabase 재조회 일치 문구를 확인하고 새로고침 뒤 숨김/복원이 유지되는지 본다. ⑦ 학생관리·특강관리·수업일지·출결·알림관리 원천이 변하지 않으면 통과다.

## 2026-07-27 P0. 퇴원생 중복 데이터 점검·강제 영구 삭제

- 상태: 구현·자동검증·운영 읽기 전용 audit 완료. 운영 배포 후 사람 화면 gate 대기. 구현과 검증 중 학생·수업·출결·숙제·특강·알림·`app_state`를 실제 삭제하거나 수정하지 않았다.
- 사용자 요청: 퇴원생 목록에서 중복 학생 데이터를 삭제할 수 있게 하고, 연결 이력이 있어도 영향 범위를 보여 준 뒤 `그래도 삭제하시겠습니까?`를 물어 강제 삭제할 수 있게 한다.
- 화면 흐름: `학생관리 > 퇴원생 목록` 각 행에 `중복 데이터 삭제`를 추가했다. 누르면 먼저 Supabase 연결 기록을 조회한다. 0건이면 안전 안내, 1건 이상이면 원천별 건수와 `app_state` key를 표시한다. 연결 기록이 있으면 영향 확인 체크, 모든 경우 학생 이름 정확히 입력, 마지막 브라우저 확인창의 `그래도 삭제하시겠습니까?`까지 통과해야 삭제 요청을 보낸다.
- 점검 원천: `students` 퇴원 상태를 다시 확인하고 수업 명단/특강 학생별 시간, 수업일지 학생 기록, 출결 이벤트, 숙제, 보충·재시험, 오답, 성적, 테스트 응시, 운영 알림, 발송 이력·예약 작업, 시험 후 제출/파일, 특강 수강 등록, 학생 지정 자료, `app_state`의 상담·질문·정산 등 중첩 참조를 검사한다. 운영에 아직 없는 선택 테이블은 0건으로 취급하되, 단순 미적용이 아닌 조회 오류는 삭제를 중단한다.
- 삭제 계약: 연결 0건은 `students` 원천 행만 삭제한다. 강제 삭제는 화면 확인 당시 연결 건수 fingerprint와 삭제 직전 재점검 결과가 같아야 진행하며, 달라지면 새 영향 범위를 다시 보도록 중단한다. lessons·resource_materials의 학생 ID와 특강 학생별 시간, `app_state`의 해당 학생 객체/ID, `special_lecture_enrollments`를 먼저 정리한 뒤 학생 행을 삭제한다. FK 정책에 따라 수업일지·숙제·오답·성적·테스트·시험 제출은 함께 삭제되고 출결·알림 이력은 학생 연결이 해제될 수 있음을 화면에 명시한다. 삭제 뒤 학생 행과 모든 점검 원천을 다시 조회해 대상 ID가 0건이어야 완료 처리하고, 프런트도 `/api/students`를 다시 읽어 목록에서 사라진 것을 확인한다. 강제 정리 뒤에는 로컬의 오래된 연결 상태가 되살아나지 않도록 화면을 자동 새로고침한다.
- 운영 읽기 전용 audit: 중복 김연우 `student_intake_tally_xV9pMZ5`는 연결 0건으로 일반 영구 삭제 가능, 중복 신초봄 `student_intake_tally_7XJGj9R`은 특강 수강 등록 1건이 있어 영향 확인 후 강제 삭제 경로가 필요함을 확인했다. 이 audit에서는 삭제 요청을 보내지 않았다.
- 외부 영향: 삭제 기능은 `notification_jobs`/Solapi 예약·발송·취소를 새로 실행하지 않는다. 다만 강제 삭제 대상에게 기존 알림 row가 연결돼 있으면 DB 정책에 따라 학생 연결이 해제되므로, 화면에서 사전 건수를 확인해야 한다. 새 SQL은 없다.
- AI 검증: 중첩 배열·객체·학생 ID key를 정리하면서 다른 학생과 단순 메모 문자열을 보존하는 전용 순수 함수 테스트를 추가했다. `npm run test:withdrawn-student-deletion`, `npm run test:production` 378건, `npm run build`, `node --check`, `git diff --check`를 통과했다. 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① 배포 후 `학생관리 > 퇴원생 목록`에서 삭제할 중복행의 이름·학교·학년·학생 ID가 맞는지 확인한다. ② `중복 데이터 삭제`를 눌러 원천별 연결 건수를 확인한다. ③ 먼저 김연우 중복행처럼 0건인 대상을 이름 입력 후 삭제하되 마지막 `그래도 삭제하시겠습니까?`에서 취소하면 아무 변화가 없는지 확인한다. ④ 다시 실행해 최종 확인하면 `삭제 중 -> Supabase 재조회 확인 -> 삭제 완료`가 보이고 새로고침 후 퇴원 목록에서만 사라지는지 확인한다. ⑤ 신초봄 중복행은 `특강 수강 등록 1건`이 표시되는지 확인하고, 기존 중3 신초봄이 아닌 중복 학생 ID가 맞을 때만 영향 체크·이름 입력·마지막 확인으로 강제 삭제한다. ⑥ 새로고침 후 기존 중3 신초봄의 학생정보·1~5회차·수업일지·숙제가 유지되고 중복 ID 참조만 0건이며 새 Solapi 작업이 없으면 통과다.

## 2026-07-27 P0. 휴강 수업일지·선택형 휴강 보충·과거 수업 전환

- 상태: 구현·자동검증 완료. 운영 배포 후 사람 화면·Supabase 새로고침 gate 대기.
- 사용자 요청: 수업 등록 유형에 `휴강`을 추가하고, 휴강 보충 유무를 물은 뒤 필요하면 보충 수업일지를 함께 생성한다. 휴강과 보충 모두 수업일지에서 확인할 수 있어야 한다. 후속으로 이미 등록된 과거 수업도 휴강으로 바꿀 수 있어야 한다.
- 원천 계약: 휴강은 달력에서 숨는 `status=canceled`가 아니라 `lessons.lesson_type=closure`, `lesson_topic=휴강`, `status=scheduled`로 저장한다. `보충 수업일지 함께 생성`을 선택하면 같은 반·학생 명단으로 `lesson_type=makeup`, `lesson_topic=휴강 보충` 수업을 같은 bulk 요청에서 생성한다. 두 수업은 `sourceLabel`에 서로의 lesson ID를 남겨 수업일지에서 원 휴강/연결 보충 날짜와 시간을 확인한다.
- 화면: 수업 등록 모달의 6번째 유형으로 `휴강`을 추가하고 `보충 없음 / 보충 수업일지 함께 생성`을 명시 선택하게 했다. 보충 선택 시 날짜·시작·종료를 편집하며 기본 날짜는 휴강 7일 뒤다. 과거 일반 수업의 `수업 수정`에서는 유형 탭과 색상 선택 양쪽에서 `휴강`을 선택할 수 있고, 연결 보충도 과거 또는 미래 날짜로 함께 만들 수 있다. 아직 지나지 않은 기존 일반 수업의 휴강 전환은 알림 예약 취소 계약과 섞지 않기 위해 이번 범위에서는 열지 않았다. 달력에는 회색 `휴강 · 수업명` pill과 별도 `휴강` 필터를 제공한다. 휴강/휴강 보충 수업일지 상단에는 실제 수업·정산 포함 여부와 연결 일정을 안내한다. 휴강은 태블릿 등하원 수업 후보에서 제외하고 휴강 수업일지의 학생별 출결 버튼도 `휴강`으로 잠가 잘못된 출석 기록이 붙지 않게 했다.
- 저장 신뢰성: 수업 등록·수정은 더 이상 로컬 화면을 먼저 닫고 백그라운드 POST하지 않는다. 모달 안에서 `저장 중 -> Supabase 반영 확인 중 -> 저장 완료/저장 실패`를 표시하고, `/api/lessons/bulk` 저장 뒤 `/api/lessons`를 다시 조회해 ID·유형·주제·연결값·날짜·시간·명단·상태가 모두 일치해야 완료 처리한다. 과거 수업 전환은 기존 lesson ID를 유지하고 유형·주제만 휴강으로 바꾸며 기존 `lesson_student_records`를 삭제하거나 다시 만들지 않는다. 현재 퇴원·삭제 상태라 편집 명단에 보이지 않는 과거 학생 ID도 저장 payload에 보존해, 서버의 `명단에서 빠진 학생 기록 정리`가 과거 기록을 지우지 않게 했다. 등록 모달이 열린 동안 휴강과 보충 lesson ID를 고정해 POST 성공 뒤 재조회만 실패한 경우 다시 저장해도 새 중복 ID를 만들지 않는다. 실패 시 모달과 입력값을 유지하고, 완료 후에는 중복 등록을 막기 위해 입력을 잠근 뒤 `달력에서 확인`으로 닫는다.
- 정산 경계: `closure`는 학생별 정규 수업 횟수·시수와 월 정산 대상 명단에서 제외한다. 함께 만든 `makeup`은 기존처럼 보충 횟수·시수 참고값에는 표시되지만 정규 월 고정금액 계산은 바꾸지 않는다.
- 외부 영향: 구현·테스트 중 운영 lessons를 생성하거나 과거 수업을 수정하지 않았다. 수업 등록·과거 휴강 전환만으로 `notification_jobs`, Solapi 알림톡/문자 예약·발송·취소, 출결, 숙제, 보충 task를 생성하거나 변경하지 않는다. 새 SQL은 없다.
- AI 검증: 휴강·연결 보충 UI/저장/재조회/달력·수업일지 연결 계약에 더해 과거 수업만 전환 가능, 유형 탭·색상 팔레트 제한 일치, 기존 lesson ID와 비활성 과거 학생 명단 보존을 production 시나리오 380건으로 검증했다. 월 정산 테스트에는 휴강이 정규 횟수에서 제외되고 휴강 수업일지만 있는 학생이 정산 명단에 들어오지 않는 fixture를 유지했다. `npm run test:monthly-settlement`, `node scripts/scenario-tests-production.cjs`, `npm run test:production`, `npm run build`, `git diff --check`를 통과했다. 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① 운영 `수업일지 > + 수업 등록`에서 `휴강`을 선택하고 `휴강 보충이 있나요?`가 보이는지 확인한다. ② 삭제 가능한 테스트 반/과거 날짜로 먼저 일반 수업을 만든 뒤 그 수업의 `수업 수정`을 열어 `휴강`을 선택한다. ③ `보충 없음`으로 저장해 `저장 중 -> Supabase 반영 확인 중 -> 저장 완료` 후 같은 lesson ID의 회색 `휴강 · 수업명`과 휴강 수업일지 안내가 보이고 기존 학생 명단·수업기록이 남는지 확인한다. ④ 새로고침 후 같은 휴강이 남고 월 정산의 정규 횟수·시수에는 포함되지 않는지 확인한다. ⑤ 다른 삭제 가능한 과거 수업에서 `보충 수업일지 함께 생성`을 선택해 과거 또는 미래의 보충 날짜·시간을 정하고 저장한다. ⑥ 휴강 pill과 별도 `수업명 · 휴강 보충` pill이 각각 열리며 서로의 원 휴강/연결 보충이 표시되는지 확인한다. ⑦ 아직 지나지 않은 기존 일반 수업에서는 휴강 유형과 휴강 색상 선택이 잠겨 있는지 확인한다. ⑧ 이 과정만으로 알림관리 예약 건수나 Solapi 내역, 출결·숙제가 변하지 않으면 통과다.
- 남은 저장 gate: 이번 단위에서 일반 수업 생성·수정의 재조회 gate까지 보강했다. 수업 복사·취소/되돌리기는 기존 동작을 유지했으며 같은 저장 신뢰성 기준의 별도 후속 단위다.

## 2026-07-27 P0. 신초봄 기존 특강 데이터·일반 Tally 정보 병합

- 상태: 운영 Supabase 병합 및 저장 후 재조회 검증 완료. 코드 변경은 없다.
- 병합 기준: 특강 Tally가 명시한 `창일중·중3`과 실제 특강 수업·수업일지·숙제가 연결된 `student_special_lecture_special_lecture_tally_DqXP5Zq`를 계속 사용할 학생 ID로 확정했다. 일반 Tally 원본의 학년 입력은 문맥 없는 `3`이었고 파생 행이 `고3`으로 잘못 만들어졌으므로, 학년은 특강 원본의 명시값 `중3`을 따른다.
- 기본정보 병합: 기존 중3 학생에 일반 Tally 행의 로그인 `04신초봄`, PIN, 학생전화 `01056495839`, 학부모전화 `01085795839`, 특이사항 `없습니다`를 옮겼다. 학교 `창일중`, 학년 `중3`, 미배정 상태와 기존 학생 ID는 유지했다.
- 중복 행 처리: `student_intake_tally_7XJGj9R`은 삭제하지 않고 `paused`로 전환했으며 로그인은 `merged-tally-7XJGj9R`로 바꿔 활성 학생의 `04신초봄`과 충돌하지 않게 했다. 퇴원 코멘트에는 기존 특강 학생 ID로 병합했고 이전 로그인이 `04신초봄`이었다는 사실을 남겼다.
- Tally 연결: 일반 Tally 후보 `tally_7XJGj9R`의 파생 학년을 `중3`으로 바로잡고 등록완료 메모가 기존 학생 ID와 `04신초봄`을 가리키게 했다. Tally `rawPayload`의 사용자가 제출한 원본 학년 `3`과 특강 신청 원본은 수정하지 않았다.
- 기록 보존: 기존 학생의 7월 22일~31일 특강 1~5회차 13:00~15:00 명단·시간·사유, 수업일지 1건, 숙제 3건을 그대로 보존했다. 기존 활성 enrollment의 신청 연결·회차·시간·메모와 중복 행에 남아 있던 취소 enrollment도 감사 이력으로 변경하지 않았다. 중복 행에는 수업·수업일지·숙제가 0건이어서 기록 이관이나 삭제가 필요하지 않았다.
- 외부 영향: `notification_jobs`는 전후 0건이며 Solapi 예약·발송·취소, 출결/숙제 삭제, enrollment/lessons 재저장은 실행하지 않았다. 새 SQL은 없다.
- 사람 gate: ① `학생관리`를 새로고침해 재원 목록에는 신초봄이 한 명만 보이는지 확인한다. ② 해당 행이 `중3 / 창일중 / 04신초봄`이고 두 연락처가 보이는지 확인한다. ③ 퇴원생 목록에는 `merged-tally-7XJGj9R` 중복행이 남아 있는지 확인한다. ④ `Lesson Hub > 특강관리`에서 기존 1~5회차 13:00~15:00와 수기 정정 사유가 그대로인지 확인한다. ⑤ 7월 22일 수업일지 `RPM 미적분1 / 01. 함수의 극한`과 숙제 3건이 유지되고 새 알림 예약이 없으면 통과다.

## 2026-07-27 P0. 김연우 복구 요청 UTF-8 문자 손상 복원

- 상태: 운영 Supabase 원문 복원 및 재조회 검증 완료. 코드 변경은 없다.
- 원인: 아래 김연우 운영 복구를 실행한 PowerShell 수동 요청이 JSON 문자열을 UTF-8 바이트로 명시하지 않아, 요청에 포함된 한글과 이모지가 `?` 또는 대체문자로 저장됐다. Academy OS 브라우저 저장 경로의 문제가 아니라 이번 수동 운영 요청의 인코딩 오류다.
- 손상 범위: 김연우 기존/중복 학생 2행의 이름·학교·학년·특이사항·중복행 로그인/퇴원 코멘트, 일반 Tally 제출본 1건의 표시 필드와 `rawPayload`, 활성 특강 enrollment 메모, 7월 27일~8월 7일 클리닉 6개 수업의 수업명·주제·학생별 시간조정 사유였다.
- 원문 복구: Tally 제출 미리보기와 같은 양식의 정상 제출본에서 `김연우`, `선덕중학교`, `중3`, `신입생`, `공통수학 2 도형의 이동까지 했습니다`, `친절하게 설명해주셨으면 좋겠습니당` 및 양식명·질문 라벨을 복원했다. 손상되지 않은 7월 22일·24일 클리닉에서 `2026 여름 개별 진도 클리닉`, `개별 클리닉`, `개인일정`, `영어학원 일정`, `선생님 수기 입력 정정`을 복원했고 enrollment 메모는 `개념원리 공수2 집합~`으로 되돌렸다.
- 저장 방식: 모든 수정 요청은 JSON을 명시적인 UTF-8 바이트로 변환하고 `application/json; charset=utf-8`로 전송했다. 저장 뒤 관련 학생·Tally·enrollment·6개 lessons의 문자열을 재귀 대조해 `??` 연속, 문자열 앞 `?`, Unicode 대체문자가 0건임을 확인했다.
- 보존 검증: 6개 수업의 학생 명단과 학생별 시작·종료 시간·일정 유형은 복구 전후가 같고, 7월 27일 기존 수업일지/출결 레코드 ID 3건도 모두 유지됐다. `notification_jobs`와 Solapi 예약·발송·취소는 0건이며 레코드 삭제도 실행하지 않았다.
- 사람 gate: ① 문제가 보였던 수업일지 화면을 새로고침한다. ② 왼쪽 학생 정보가 `김연우 / 중3 · 선덕중학교 / 13:00-16:00`으로 보이는지 확인한다. ③ 7월 27일~8월 7일 클리닉의 수업명이 `2026 여름 개별 진도 클리닉`, 주제가 `개별 클리닉`으로 보이는지 확인한다. ④ 특강관리에서 메모 `개념원리 공수2 집합~`과 학생별 조정 사유가 한글로 보이는지 확인한다. ⑤ 7월 27일 기존 다른 학생 3명의 출결이 그대로이고 새 알림 예약이 생기지 않았으면 통과다.

## 2026-07-27 P0. 김연우 기존 원천·과거 특강 데이터 복귀

- 상태: 운영 Supabase 복구 및 저장 후 재조회 대조 완료. 이 수동 요청에서 발생한 한글 인코딩 손상은 위 최신 작업으로 모두 복원했다. 코드 변경은 없다.
- 최종 학생 원천: 사용자의 정정에 따라 기존 특강 학생 `student_special_lecture_special_lecture_tally_rD01VW5`를 김연우의 계속 사용할 원천으로 복구했다. 이 행은 `active`로 되돌리고 퇴원일·사유·코멘트를 비웠으며, 일반 Tally `tally_xV9pMZ5`의 이름·출생연도·학교·학년·학생/학부모 전화·특이사항·정규반 값을 기존 ID에 덮어썼다. 학생 ID·기존 로그인·PIN과 과거 기록은 유지했다.
- 중복 행 처리: 앞선 잘못된 원천 판정으로 만들어진 `student_intake_tally_xV9pMZ5`는 삭제하지 않고 `paused`로 전환해 두 재원 김연우가 동시에 나타나지 않게 했다. 일반 Tally 후보의 등록완료 메모도 기존 학생 ID를 가리키도록 수정했다.
- 특강 복귀: `special_lecture_tally_rD01VW5` 활성 enrollment를 기존 학생 ID로 되돌렸다. 7월 27일·29일·31일, 8월 3일·5일·7일 총 6개 클리닉 수업의 `studentIds`와 `specialLectureStudentSchedules`도 기존 ID로 일치시켰고 신규 중복 ID는 모두 제거했다. 7월 22일 제외 회차와 기존 enrollment 회차 선택·시간·메모는 임의 변경하지 않았다.
- 보존 검증: 7월 24일 김연우 `present` 13:00 출석 1건이 기존 ID에 그대로 남아 있음을 확인했다. 7월 27일 회차는 기존 학생 3명의 수업일지/출결 3건을 모두 보존한 채 김연우 기존 ID만 명단에 추가했다. 신규 중복 ID에 연결된 수업일지·출결은 0건이었고, 영향 수업의 알림 작업도 0건이었다.
- 외부 영향: Solapi 예약·발송·취소와 `notification_jobs` 생성·수정은 실행하지 않았다. 학생/일반 Tally 등록 메모/특강 enrollment/해당 6개 lessons 외에는 수정하지 않았고, 레코드 삭제도 실행하지 않았다. 새 SQL은 없다.
- 사람 gate: ① `학생관리`에서 김연우가 재원 1명으로 보이고 학교 `선덕중학교`, 중3, 학생·학부모 연락처가 있는지 확인한다. ② 퇴원생 목록에는 중복 Tally 생성행만 남고 기존 특강 ID 김연우는 없는지 확인한다. ③ `Lesson Hub > 특강관리`에서 김연우의 기존 클리닉 신청과 7월 27일~8월 7일 회차가 한 학생으로 이어지는지 확인한다. ④ 7월 24일 출석이 `출석 · 13:00`으로 보이고, 7월 27일 기존 다른 학생 3명의 수업일지가 그대로면 통과다. ⑤ 알림관리에서 이 복구로 새 예약·발송이 생기지 않았는지 확인한다.

## 2026-07-27 P0. Tally 완전 덮어쓰기·신초봄 기존 특강 초안 초기화 경로 (이후 계약 정정)

- 상태: 당시 구현 기록이며, 특강 회차·시간·메모까지 초기화하는 계약은 위 최신 `특강 Tally 기본정보 교체·수업기록 보존 계약 정정`에서 철회됐다. 일반 Tally 학생 기본정보 교체와 특강 운영기록 보존을 구분해 읽는다.
- 최신 사용자 정정: `덮어쓰기`는 빈 Tally 값을 기존 값으로 보강하는 의미는 아니지만, 삭제 대상은 Tally 전에 임시로 입력한 학생 기본정보다. 기존 특강 회차·시간·메모와 수업일지·출결은 Tally 제출 뒤에도 계속 보존한다.
- 일반 Tally: 기존 학생 선택 후 `빈칸 유지하고 보강`과 `기존 기본정보 삭제 후 Tally 덮어쓰기`를 분리했다. 완전 덮어쓰기는 이름·출생연도·학교·학년·학생/학부모 전화·특이사항·정규반을 Tally 값으로 통째로 교체하며, Tally 빈칸은 기존 값을 빈 값으로 지운다. 정규반이 비면 안전한 미래 정규 수업 명단에서도 제외하고 lessons를 재조회 대조한다.
- 특강 Tally: 당시에는 연결 전과 연결 후 모두 기존 `sessionPlans/sessionIds/memo`를 지우도록 구현했으나 잘못된 해석이었다. 최신 구현은 학생 기본정보만 Tally 값으로 교체하고 기존 enrollment 회차·시간·메모·원천·검토상태를 그대로 보존한다.
- 신초봄 원천 확인: 당시 운영에는 `창일중·중3` 특강 Tally 학생과 `창일중·고3` 일반 Tally 학생 두 행이 남아 있었고, 중3 특강 enrollment에는 수기로 확정한 1~5회차 13:00~15:00가 있었다. 사용자의 후속 지시로 위 최신 병합 작업을 실행해 중3 기존 ID에 일반 Tally 기본정보를 합치고 고3 중복행을 퇴원 상태로 보관했다. 따라서 신초봄 카드의 완전 덮어쓰기와 기존 1~5회차 초기화는 더 이상 실행 대기 gate가 아니며, 사용자 재지시 없이 실행하지 않는다.
- 보존 경계: 학생 ID·로그인·PIN과 모든 과거 기록을 유지한다. 특강 Tally에서는 정규반·출생연도처럼 해당 폼이 소유하지 않는 학생 필드뿐 아니라 기존 enrollment와 과거·미래 lessons도 변경하지 않는다.
- 저장/외부 영향: 학생과 enrollment는 각각 Supabase 저장·재조회 대조 후에만 완료로 표시한다. 새 SQL은 없다. 자동검증과 구현 과정에서 운영 Supabase 쓰기, 학생/특강 실제 덮어쓰기, lessons/출결 삭제, `notification_jobs`, Solapi 예약·발송·취소를 실행하지 않았다.
- AI 검증: 당시의 `빈 특강 회차가 전 회차 제외·0회` 기대값은 더 이상 유효하지 않다. 최신 검증은 위 정정 작업의 기존 회차·시간·메모 보존 테스트를 따른다.
- 사람 gate: 이 절의 특강 회차 초기화 절차를 실행하지 않는다. 위 최신 `특강 Tally 기본정보 교체·수업기록 보존 계약 정정`의 사람 gate를 따른다.

## 2026-07-27 P0. 기존 학생 Tally 보강·정규반 미배정 특강 편성

- 상태: 구현·자동검증 완료. 운영 학생·특강·수업일지 데이터는 실제 저장하지 않았으며 배포 후 사람 gate 대기.
- 일반 Tally 등록: `학생 추가 > Tally 접수`에서 `새 학생으로 등록` 또는 기존 재원 학생을 `Tally 반영 대상`으로 명시 선택한다. 이름이 같고 학교·학년이 충돌하지 않는 재원생이 한 명뿐이면 후보를 먼저 제안하되, 동명이인이나 학교·학년 충돌은 자동 선택하지 않는다. 기존 학생은 위 최신 작업에 따라 빈칸 보존 보강과 기존 기본정보 삭제 후 완전 덮어쓰기를 다시 선택한다.
- 덮어쓰기 계약: 기존 학생을 선택하면 Tally의 비어 있지 않은 이름·출생연도·학교·학년·학생/학부모 전화·특이사항·정규반만 보강한다. 기존 `studentId`, 로그인 ID, PIN, 교재, 개별 시간표, 상태와 과거 수업·출결은 유지하고 Tally 빈 값으로 기존 값을 지우지 않는다. 정규반이 Tally에서 비어 있으면 기존 반을 유지하며, 교사가 반을 명시 변경했을 때만 안전한 미래 정규 수업 명단에서 이전 반 제거·새 반 추가 후 lessons 재조회 대조까지 완료한다.
- 저장 신뢰성: 저장 직전 `/api/students`를 Supabase 원천으로 재조회하고, 학생 저장 뒤 ID·로그인·PIN·프로필·정규반·개별 시간표·상태를 다시 대조한다. Tally 후보도 기존 학생 ID가 포함된 등록완료 메모로 저장·재조회한 뒤에만 완료를 표시한다. 학생 원천만 성공하고 미래 수업 명단이나 후보 완료 저장이 실패하면 부분 실패 문구를 남겨 중복 등록을 막는다.
- 특강 편성: `Lesson Hub > 특강관리`의 수동 명단과 Tally 신청 연결 목록은 정규반 배정 여부와 무관한 모든 재원생을 보여 주며 `정규반 미배정`을 명시한다. Tally 신청을 기존 학생과 연결할 때 `학생정보 유지하고 연결`과 `Tally 기본정보로 교체 후 연결`을 분리한다. 후자는 학생 기본정보만 교체하며 기존 특강 회차·시간·메모와 수업일지는 유지한다.
- 원천/외부 영향: 직접 저장 원천은 기존 `students`, 일반 Tally의 `student_intake_applicants`, 특강의 `special_lecture_enrollments`, 정규반을 명시 변경한 경우의 안전한 미래 `lessons`다. 새 SQL은 없다. Tally 원본, 과거 수업·출결, `lesson_student_records`, `notification_jobs`, Solapi 예약·발송·취소는 변경하지 않는다.
- AI 검증: 비어 있지 않은 Tally 필드만 덮어쓰기, ID/로그인/PIN/교재/개별 시간표/기존 반 보존, 퇴원생 후보 제외, 충돌하는 동명이인 자동 선택 금지를 전용 테스트로 확인했다. `npm run test:tally-student-merge`, `npm run test:production` 374건, `npm run build`, `git diff --check`를 통과했다. 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① `학생 추가 > 한 명씩`에서 정규반 `미배정`으로 테스트 학생을 저장한다. ② 같은 학생의 Tally 접수를 열어 `Tally 반영 대상`에 기존 학생이 제안되고 변경 예정 필드가 보이는지 확인한다. ③ `Tally 정보로 기존 학생 보강`을 누른 뒤 확인창에서 ID·로그인·PIN·기록 보존 안내를 확인하고 저장한다. ④ 새로고침 뒤 학생이 한 명만 존재하며 전화·학교 등 Tally 정보는 반영되고 학생 ID·로그인·PIN·정규반 미배정·기존 기록은 유지되는지 확인한다. ⑤ 특강관리에서 해당 학생을 수동 추가할 때 `정규반 미배정`으로 선택 가능한지 확인한다. ⑥ 해당 학생의 특강 Tally 신청에서는 `학생정보 유지하고 연결`과 `Tally 기본정보로 교체 후 연결`이 모두 보이고, 기본정보 교체 뒤에도 기존 회차·시간·메모와 수업일지가 유지되는지 확인한다. ⑦ 새 알림/Solapi 작업이 생기지 않으면 통과다.

## 2026-07-27 P0. 김연우 퇴원 취소 기능·클리닉 학생 원천 보정

- 상태: 퇴원 취소 기능 구현·자동검증 완료. 아래 운영 김연우 원천 판정과 연결 보정은 사용자의 후속 정정에 따라 위 `김연우 기존 원천·과거 특강 데이터 복귀` 작업으로 되돌렸으며 더 이상 현재 운영 기준이 아니다.
- 당시 중복 원천 판정: 일반 학생 Tally로 등록된 `student_intake_tally_xV9pMZ5`를 재원 원천으로 보았으나 사용자 확인 결과 잘못된 판정이었다. 현재 기준은 기존 `student_special_lecture_special_lecture_tally_rD01VW5`가 재원 원천이고 신규 Tally 생성행이 퇴원 상태다.
- 퇴원 취소 기능: `학생관리 > 퇴원생 목록`에 `퇴원 취소`를 추가했다. 확인창에서 학생 상태만 복원하고 퇴원 당시 제외된 반·미래 수업 명단은 자동 복원하지 않는다고 안내한다. `/api/students` 저장 뒤 `/api/students`를 다시 조회해 `status=active`, 퇴원일·사유·코멘트가 모두 비었는지 일치해야만 저장 완료를 표시한다. 실패 시 행을 재원으로 낙관 전환하지 않고 오류와 재시도를 남긴다.
- 클리닉 연결 보정: `special_lecture_tally_rD01VW5` 신청의 활성 enrollment 학생 ID를 일반 Tally 재원 김연우로 변경했다. 이어 출결/수업일지와 알림 작업이 없음을 확인한 7월 29일, 7월 31일, 8월 3일, 8월 5일, 8월 7일 총 5개 미래 클리닉 수업의 `studentIds`와 `specialLectureStudentSchedules`를 같은 재원 ID로 연결했다. 각 원천은 저장 후 Supabase 재조회로 일치함을 확인했다.
- 보존·잠금: 7월 24일 옛 클리닉 행에 저장된 출석(`present`, 13:00)은 과거 증빙으로 그대로 보존했다. 7월 27일 13:00~16:00 진행 회차는 수정 직전 재조회에서 수업일지/출결 2건이 생겨 쓰기 전에 자동 중단했으며 해당 회차 데이터는 변경하지 않았다. 수업 종료 후 별도 대조 없이 현재 회차 ID나 기록을 이관·삭제하지 않는다.
- 외부 영향: 학생/특강 enrollment/안전한 미래 수업 명단 외에는 수정하지 않았다. Tally 원본, 과거 출결, 현재 진행 회차, `notification_jobs`, Solapi 예약·발송·취소는 변경하지 않았고 새 알림 작업도 0건이다. 새 SQL은 없다.
- 저장 검증 보강: 특강 enrollment 재조회 snapshot에 기존 회차·메모·상태뿐 아니라 `applicationId`, `specialLectureGuideId`, `studentId`도 포함해 학생 연결이 잘못 저장되면 완료 처리하지 않게 했다.
- AI 검증: 퇴원 취소의 명시 확인, Supabase POST·재조회, 활성 상태와 퇴원 필드 초기화 대조, 실패/재시도 UI를 production scenario에 추가했다. `node scripts/scenario-tests-production.cjs` 373건과 `npm run build`, `git diff --check`를 통과했다. 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: 퇴원 취소 기능 자체의 검수 절차는 유지한다. 김연우 운영 데이터 검수는 위 최신 복귀 작업의 사람 gate를 따르며, 이 과거 절의 ⑤~⑦은 사용하지 않는다.

## 2026-07-27 P1-2. 월별 정산 수업일지 원천·정산 제외

- 상태: 사용자 확정 기준 구현·자동검증 완료. 운영 정산 스냅샷이나 학생·수업일지 데이터는 실제 저장하지 않았으며 배포 후 사람 gate 대기.
- 명단 원천: 선택 월의 취소되지 않은 `lessons.studentIds`에 들어 있는 학생을 그대로 정산 명단으로 사용한다. 재원생 전원, 해당 월 퇴원생, 과거 월 스냅샷 ID를 별도 조건으로 추가하지 않는다. 따라서 7월 수업일지가 없는 김해란과 테스트 중복 이윤채 행은 7월 정산 명단에 나타나지 않고, 7월 1일 시험직전 수업일지가 있는 김민영은 남아 교사가 정산 제외 여부를 결정한다.
- 회차/시수 원천: 정규·보충·특강 횟수와 시수는 선택 월에 실제 저장된 lessons 수업일지만 집계한다. 월별 학생 스케줄은 주간 시수별 단가 판정과 참고 표시용으로 유지하지만 말일까지의 가상 정규 회차를 생성하지 않는다. 출결 레코드는 해당 수업일지의 출석·지각·대기·결석 상태 증빙으로만 읽는다.
- 정규/특강 경계: 특강 수업일지만 있는 학생도 명단에는 보이지만 정규 수업일지가 없으면 정규 월정액은 0원이다. 특강 금액은 기존 학생별 특강 총액 입력으로 별도 집계한다.
- 정산 제외: 각 행의 `정산 제외`를 누르면 해당 월 `studentSettings[studentId].excluded=true`가 draft에 들어가고, 정규 금액·특강 금액·조정액을 합계에서 모두 0원 처리한다. 학생·수업일지·출결 원천은 삭제하지 않는다. 제외 행은 흐리게 남고 입력이 잠기며 `정산 복원`으로 되돌릴 수 있다. `월별 정산 저장` 후에만 Supabase `app_state.monthlyInstructorSettlements`에 저장되고 재조회 대조를 통과해야 완료된다.
- 운영 재현: 새 규칙을 운영 Supabase 읽기값에 대입했을 때 대상 세 학생 중 김민영과 정상 재원 이윤채만 남았다. 김민영은 정규 수업일지 1건으로 기본 450,000원이므로 교사가 `정산 제외` 후 저장해야 하고, 정상 이윤채는 정규 수업일지 11건·특강 5건과 중등 고정급 420,000원으로 계산됐다. 이번 작업에서는 그 제외 저장을 대신 실행하지 않았다.
- 저장/외부 영향: 직접 저장 원천은 기존 `app_state.monthlyInstructorSettlements` 한 키이고 새 SQL은 없다. `students`, `lessons`, `lesson_student_records`, 보충, 특강 원천, `notification_jobs`, Solapi에는 쓰기나 발송을 수행하지 않는다.
- AI 검증: 수업일지 없는 재원·퇴원·과거 저장 ID 제외, 상태와 무관한 수업일지 학생 포함, 가상 스케줄 회차 제거, 특강-only 정규 0원, 정산 제외의 정규·특강·조정 0원, 제외 저장 snapshot 대조, 복원 가능한 UI 계약을 전용 테스트에 추가했다. `npm run test:monthly-settlement`, `npm run test:production` 372건, `npm run build`, `git diff --check`를 통과했다. 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① 배포 후 `운영 > 월별 수업 정산`에서 2026년 7월을 연다. ② 김해란과 `테스트` 중복 이윤채가 없고 정상 중3 이윤채만 한 행인지 확인한다. ③ 이윤채의 정규 11회·특강 5회가 수업일지와 일치하는지 달력으로 확인한다. ④ 김민영 행에서 `정산 제외`를 누르면 행이 흐려지고 정규 적용금액이 `정산 제외`, 전체 정규·특강 예상액에서 해당 금액이 빠지는지 확인한다. ⑤ `정산 복원` 시 즉시 합계가 돌아오는지 확인한 뒤 다시 제외한다. ⑥ `월별 정산 저장`을 누르고 Supabase 재조회 일치 문구를 확인한 뒤 새로고침해 김민영 제외가 유지되는지 확인한다. ⑦ 학생관리·7월 수업일지·출결·알림관리 데이터가 변하지 않으면 통과다.

## 2026-07-27 P1-1. 월별 수업 정산 중·고등 단가 연결

- 상태: 사용자 제공 1차 단가표 구현·자동검증 완료. 운영 데이터 실제 저장은 실행하지 않았으며 배포 후 사람 gate 대기.
- 적용 단가: 중등 기본 고정급 420,000원, 중등 주 6시간 308,000원, 고등 주 6시간 341,000원, 그 외 고등 기본 고정급 450,000원이다.
- 시수 판정: 학생의 월별 정산 스케줄을 요일 수 × 시작·종료 시간으로 합산해 주간 시수를 표시하고, 정확히 주 6시간이면 해당 단가를 제안한다. 예를 들어 `월수 19:00-22:00`은 주 6시간, `월수금 19:00-22:00`은 주 9시간이다.
- 수정 우선순위: 아직 숫자 고정금액이 저장되지 않은 새 월 학생행과 과거 `단가 미설정` 빈 값에 새 기본 단가를 적용한다. 기존 월 스냅샷에 숫자로 저장된 금액은 자동 변경하지 않는다. 화면에서 스케줄을 직접 바꿀 때 현재 금액이 이전 기본 단가와 같은 경우에만 새 시수 단가로 전환하고, 학생별 수동 수정금액은 그대로 보존한다.
- 화면: 상단 계산 원칙에 현재 4개 단가를 표시하고, 학생별 고정금액 아래에는 적용 기준, 스케줄 아래에는 계산된 주간 시수를 표시한다. 고정금액은 계속 직접 수정 가능하다.
- 저장/외부 영향: 기존 `Supabase app_state.monthlyInstructorSettlements` 월별 스냅샷 저장 계약만 사용한다. 이번 구현과 자동검증에서 Supabase 운영 데이터, 수업일지, 출결, 보충, 특강, `notification_jobs`, Solapi를 수정하거나 호출하지 않았다.
- AI 검증: 중등/고등 주 6시간과 기본 단가, 과거 단가 미정 빈 값의 기본급 적용, 기존 숫자 저장금액 보존, 기본 단가 행의 스케줄 변경 추종, 수동 수정금액 보존을 `test:monthly-settlement`에 추가했다. `npm run test:monthly-settlement`, `npm run test:production` 372건, `npm run build`, `git diff --check`를 통과했다. 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① `운영 > 월별 수업 정산`에서 아직 저장하지 않은 새 정산월을 연다. ② 중학생 일반 스케줄은 420,000원인지 확인한다. ③ 중학생 스케줄을 `월수 19:00-22:00`으로 바꾸면 `주 6시간`과 308,000원이 보이는지 확인한다. ④ 고등학생을 같은 주 6시간 스케줄로 바꾸면 341,000원인지 확인한다. ⑤ 고등 주 6시간이 아닌 학생은 450,000원인지 확인한다. ⑥ 한 학생 금액을 임의로 수정한 뒤 스케줄을 바꿔도 수정금액이 유지되는지 확인한다. ⑦ 과거 저장 월을 열어 기존 저장금액이 바뀌지 않았는지 확인한다. ⑧ 저장·새로고침 뒤 같은 값이 유지되고 수업일지·출결·알림 데이터가 변하지 않으면 통과다.
- 후속: 추가 시수별 단가를 받으면 같은 표에 확장하되 과거 저장 스냅샷은 자동 마이그레이션하지 않는다.

## 2026-07-27 P1. 운영 월별 수업 정산

- 상태: 1차 기능 구현·자동검증·데스크톱/모바일/달력 모달 렌더 검수 완료. 이후 명단·회차 원천은 위 `P1-2`의 수업일지 기준으로 대체됐다. 운영 데이터 실제 저장은 실행하지 않았으며 배포 후 사람 gate 대기.
- 사용자 기준: 월 정산 범위는 4.2주나 12회가 아니라 해당 월 1일부터 말일까지다. 계속 재원생은 실제 정규 회차·시수 증감과 무관하게 월 고정금액 전액을 적용한다. 신입·퇴원생의 최신 부분월 기준은 위 `P1-4`의 수업일지 첫·마지막 수업 경계와 월별 스케줄 횟수 비율이다. 대기·출석·지각은 정산에 포함하고 결석도 별도 차감 요청이 없으면 자동 차감하지 않는다.
- 화면: `운영 > 월별 수업 정산`을 추가했다. 정산월, 정규 기준 총액/예상 수령액, 특강 기준 총액/예상 수령액, 전체 예상 수령액을 분리 표시한다. 학생별로 계산 방식, 월 고정금액, 월별 스케줄, 자동 정산 기간, 실제·정산 기준 회차, 보충 회차/시수, 정규 적용금액, 조정액, 메모를 수정·확인할 수 있다.
- 출결 달력: 학생의 `정규 N회 · N시간`을 누르면 해당 월 달력에서 정규 예정, 실제 출석/지각/대기/결석, 보충, 특강을 구분해 확인한다. 보충은 증빙 횟수·시수로만 표시하고 정규 금액에 추가하지 않는다. 시험 달 일요대비 같은 추가 정규수업은 참고 횟수·시수에 나타나지만 계속 재원생 월정액은 바꾸지 않는다.
- 부분월 계산: 초기 구현의 수기 인정일·시수 비율은 위 `P1-4`에서 대체됐다. 현재는 수업일지의 첫·마지막 수업일을 자동 경계로 삼고, 학생별 월 스케줄의 포함 횟수/전체 횟수로 계산한다. 신입 후보는 그 달 최초 정규 수업일을 표시하지만 안전을 위해 교사가 `신입생 · 첫 수업~말일`을 선택하기 전에는 월정액을 유지한다.
- 단가/정산식: 이 1차 구현 당시 고등학생 월 고정금액 기본값은 450,000원이었고 중학생 단가는 보류했다. 이후 중·고등 단가는 위 `P1-1`에서 연결했다. 정규와 특강 각각 `기준 총액 × 0.5 × 0.967 × 0.985`를 적용한 뒤 두 예상 수령액을 합산한다.
- 저장 신뢰성: 직접 저장 원천은 `Supabase app_state.monthlyInstructorSettlements` 한 키다. 월별 학생 스케줄·고정금액·정산 방식·기간·특강 총액·조정액·메모를 스냅샷으로 저장한다. 저장 직전에 최신 app_state를 재조회해 다른 월을 병합하고, POST 뒤 다시 조회한 같은 월 snapshot이 요청값과 일치해야 `저장 완료`로 바뀐다. 실패 시 화면 draft와 같은 원천 버전의 localStorage 복구 초안을 유지한다.
- 원천/외부 영향: `students`, `classTemplates`, `lessons`, `lesson_student_records`를 읽기 원천으로 사용한다. 정산 저장으로 학생 프로필 스케줄, 수업일지, 출결, 보충 task, 특강 enrollment, `notification_jobs`, Solapi 예약·발송·취소를 수정하지 않는다. 새 SQL은 없다.
- AI 검증: 월 말일/윤년, 실제 달력 스케줄, 계속 재원생 고정액, 신입·퇴원 부분월, 대기 원천 보존·포함, 보충 별도 집계, 정규/특강 정산 상수를 검증하는 `npm run test:monthly-settlement`을 `test:production`에 연결했다. `npm run test:production` 372건, `npm run build`, `git diff --check`를 통과했다. Chrome fixture로 데스크톱 1440px, 좁은 화면 390px, 학생 달력 모달 1280px 렌더를 확인했다. 기존 Vite chunk size 경고와 기존 파일의 인코딩 검사 대체문자 2건만 남고 이번 diff에는 새 대체문자가 없다.
- 사람 gate: 기본 정산 흐름은 유지하며, 단가 확인 절차는 위 `P1-1`의 최신 gate를 따른다. 계속 재원생의 실제 횟수와 무관한 월정액, 신입·퇴원 부분월 비율, 출결 달력, 저장 후 재조회·새로고침 유지도 함께 확인한다.
- 후속: 추가 시수별 단가가 제공되면 `P1-1`의 저장 우선순위 계약을 유지해 확장한다.

## 2026-07-25 P0. 전일 수업일지 다음 숙제 Solapi 원천 반영

- 상태: 운영 데이터 저장·Supabase 재조회 대조 완료.
- 사용자 요청: 2026-07-24 금요일 `월수금 7-10반` 수업일지의 `다음 숙제`를, 7월 25일 학생에게 실제 발송한 Solapi 알림톡 본문 기준으로 표시한다.
- 저장 원천: Solapi 실제 학생 메시지 8건을 읽기 원천으로 사용했고, 대상 수업 원천은 `lessons.lesson_2026-07-24_월수금-7-10반_1783229921009`, 저장 대상은 `lesson_student_records.next_homework`이다. 기존 수업기록은 보존했고 행이 없던 학생은 표준 `lsr_...` ID로 수업기록을 만들었다.
- 반영값: 김룡기·김서윤·김형준·박지현·서정원·이진후는 `수학비서로 제출된 모의고사 30문제 / 숙제 - 다음 주 월요일 등원 전까지 제출`, 박수빈은 `라이트쎈 294~343 이어서 풀기 및 앞부분 미완료분 채우기`, 김예나는 `평면좌표 개념원리·라이트쎈 미완료분을 월요일까지 채우기`로 Solapi 본문 그대로 기록했다.
- 검증: 8명 각 `POST /api/lesson-records` 성공 뒤 `GET /api/lesson-records?date=2026-07-24`로 Supabase 재조회하여 8명 모두 일치함을 확인했다. 수업 내용·출결·알림 예약/발송은 변경하지 않았다.
- 사람 검토: 운영 Lesson Hub에서 7/24 `월수금 7-10반` 수업일지를 열어 8명 모두 `다음 숙제`가 오늘 학생별 알림톡과 같은지 확인한다. 특히 박수빈·김예나의 개별 숙제가 공통 모의고사 숙제로 덮이지 않았는지 확인하고, 새로고침 뒤 값이 유지되면 통과다.

## 2026-07-23 P3. 시험분석 프롬프트 3단계 사용 UI

- 상태: 구현·검증·운영 배포 완료. `58ddee0d`를 `origin/main`에 푸시했고 운영 Vercel 번들 `/assets/main-Cr9Mkky5.js`, `/assets/main-Bz4qkPw7.css`에서 세 단계 버튼·임시 입력 안내·14px 단계 CSS 포함을 확인했다. 사람 화면 gate 대기.
- 사용자 흐름: 각 슬라이드의 가로 3열 `상세 생성/수정/QA` 출력을 `① 처음 만들기 -> ② 결과 검사 -> ③ 필요한 부분만 수정` 세로 단계로 바꿨다. 상단에도 같은 순서와 용도를 보여 주며, 각 단계에 한 개의 대표 복사 버튼만 둔다. 1단계는 바로 보이고 2·3단계는 필요할 때 펼친다.
- 프롬프트 역할 보존: 1단계는 기존 마스터+상세 생성 프롬프트, 2단계는 기존 QA 프롬프트, 3단계는 기존 revision 프롬프트를 그대로 사용한다. 생성 규칙, QA 검사 항목, 수정 시 미요청 요소 보존 계약, 전체 시리즈 수정 프롬프트, TXT/JSON 다운로드는 바꾸지 않았다.
- 수정 요청 입력: 3단계 안에 `수정할 내용` 임시 textarea를 추가했다. 입력한 문장은 기존 `[여기에 수정할 내용만 입력]` 위치에만 대입되어 복사되고, 빈 상태에서는 수정 복사 버튼이 잠긴다. 분석 run을 바꾸면 임시 입력은 초기화되며 Supabase·localStorage·프롬프트 작업본에는 저장하지 않는다.
- 가독성: 11px 3열 프롬프트 본문을 14px/1.6 줄간격의 한 열 출력으로 바꾸고, 필수·항상 권장·선택 표시와 이미지 첨부 시점을 추가했다. 900px 이하에서는 안내 단계와 내부 단계가 한 열/줄바꿈으로 표시된다.
- 저장/부작용 경계: 기존 `/api/exam-analysis-runs/save-prompt-studio` 저장과 Supabase 재조회 대조는 변경하지 않았다. 이번 임시 수정 요청은 저장 버튼 대상이 아니다. AI 호출, GPT Image 생성, QA 실행, 이미지 업로드·다운로드를 실행하지 않았다.
- AI 검증: UI 계약 테스트에 새 단계명·대표 버튼·임시 입력 비저장 안내·빈 수정 요청 복사 잠금·14px 본문을 추가했다. 대상 테스트, `npm run test:production`, `npm run build`, `git diff --check`를 통과했고 Chrome fixture로 1536px 데스크톱과 390px 모바일 배치를 확인했다. 기존 Vite chunk size 경고만 남았다.
- 사람 gate: 운영 시험분석 프롬프트 제작실에서 ① 한 슬라이드를 펼쳐 1단계만 바로 보이는지 확인한다. ② `처음 만들기 프롬프트 복사`가 기존 마스터+상세 내용을 복사하는지 확인한다. ③ 결과 검사 단계를 펼쳐 이미지 첨부 안내와 QA 프롬프트가 보이는지 확인한다. ④ 수정 단계에서 빈 상태의 복사 버튼이 잠기고, 수정 문장을 입력하면 버튼이 열리며 복사문 안의 placeholder가 입력문으로 바뀌는지 확인한다. ⑤ 다른 분석 run으로 이동했다 돌아왔을 때 임시 수정 요청이 남지 않는지 확인한다. 실제 이미지 생성·QA 실행은 이 UI gate에서 하지 않는다.
- 중단 조건: 3단계 순서가 다름, 생성 프롬프트에서 마스터 누락, QA가 수정용으로 안내됨, 수정 요청 없이 복사 가능, 수정 요청이 작업본 저장에 섞임, 원래 revision 보존 문구 소실, 모바일에서 가로 3열로 남음.

## 2026-07-23 P2. 시험분석 쎈 순서·프롬프트 입력 UI 정리

- 상태: 1~3번 구현과 대상 자동검증·데스크톱/모바일 CSS 렌더 완료. `49ca4f3f`를 `origin/main`에 푸시했고 운영 Vercel 번들 `/assets/main-Cr3R8uVT.js`, `/assets/main-aaaQkfsr.css`에서 새 입력 안내·접힘 문구·17px 체크박스·쎈 순서 fallback 포함을 확인했다. 사람 화면 gate 대기. 사용자가 분석만 요청한 4번 프롬프트 출력 구조는 변경하지 않았다.
- 쎈 중단원 순서: 최종 미리보기의 쎈 중단원 출제 비중·주요유형 상세·중단원별 난이도가 더 이상 문항 수 내림차순으로 정렬되지 않는다. 선생님 검수 저장본의 `unitNo`를 우선하고, 과거 저장본에 `unitNo`가 없으면 `mainTypeCode`의 쎈 단원 번호를 복구해 교재 순서로 정렬한다. 번호가 없는 수동 중단원만 뒤에서 한글순으로 표시한다. 집계값·색상·문항 원천은 다시 저장하거나 바꾸지 않는다.
- 역할 체크박스 CSS: 전역 input 크기의 영향을 받던 슬라이드 역할 체크박스를 이 화면에서만 17×17px로 고정하고 native checkbox·disabled 상태·선택 handler를 유지했다. 역할의 필수/선택 계약과 슬라이드 번호 재계산은 변경하지 않았다.
- 입력 영역 정리: 항상 펼쳐져 길던 입력 카드를 `1. 공통 정보`, `2. 시험 분석`, `3. 주요문항`, `4. 다음 대비`, `5. 마무리 안내` 순서의 접힘 카드로 바꿨다. 공통 정보와 시험 분석만 처음 열리고 주요문항·다음 대비·마무리는 필요할 때 펼친다. 각 요약에는 작성 목적과 현재 문항/행동 수를 표시하고, 학교 스타일·배점 구조·시험 범위·난이도 근거·총평·다음 대비·CTA에 실제 작성 형태를 보여주는 예시와 짧은 설명을 추가했다. 접힘은 local draft를 제거하지 않으며 저장 API와 Supabase 재조회 검증 계약은 그대로다.
- 벤치마크 문구: 드롭다운에서 짧은 분류명만 보여 주던 방식을 바꿔, 선택 후 실제 입력될 저장된 벤치마크 초안 문장 전체를 옵션에 표시한다. 선택만으로 값을 쓰지 않고 기존 `선택 문구 적용/이어 붙이기`를 눌러야 local draft에 반영되며 이후 자유 수정·명시 저장한다. 블로그 원문을 새로 복제한 것이 아니라 기존 검수된 paraphrase 문구 라이브러리의 `draft`를 그대로 노출한다.
- 4번 분석 결론(코드 미변경): 현재 각 장의 3종은 동등한 선택지가 아니다. `상세 생성 프롬프트`는 처음 이미지를 만들 때 `마스터+상세 복사`로 사용하고, `QA 프롬프트`는 생성된 이미지를 첨부해 사실·오탈자·잘림을 검사할 때 사용하며 이미지를 수정하지 않는다. `수정 프롬프트`는 QA 실패나 사람 수정 요청이 생겼을 때 기존 이미지를 첨부하고 `[여기에 수정할 내용만 입력]`을 채운 뒤 사용한다. 현재 문제는 3열 11px 출력이 이 순서를 설명하지 않고, 수정 요청 미입력 상태도 복사 가능하며, QA가 검사 전용이라는 구분이 약한 것이다. 다음 기능 단위 권장안은 `① 처음 만들기 → ② 결과 검사 → ③ 필요한 부분만 수정` 단계 UI, 기본 14px 이상, 각 단계의 한 개 대표 버튼, QA 결과 FAIL일 때만 수정 입력을 여는 구조다.
- 저장/부작용 경계: 이번 변경은 `exam_analysis_questions`와 `audit_summary.promptStudio`를 읽어 정렬·표시할 뿐 자동 저장하지 않는다. 작업본 저장은 기존 명시 버튼과 `/api/exam-analysis-runs/save-prompt-studio -> Supabase 재조회 대조`만 사용한다. AI 호출, GPT Image 생성, 자산 업로드, 다운로드, 수업/알림 데이터 변경은 실행하지 않았다.
- AI 검증: 쎈 `unitNo`와 `mainTypeCode` 복구 순서 fixture, 전체 벤치마크 문장 옵션, 접힘 카드, 17px checkbox 계약을 대상 테스트에 추가했다. 대상 테스트, `npm run test:production`, `npm run build`, `git diff --check`를 통과했다. 실제 CSS fixture를 Chrome으로 렌더해 데스크톱 1536×900에서 체크박스가 작고 정렬되며 두 필수 카드만 열리고 나머지 세 카드가 한 줄로 접히는 것을 확인했다. 모바일 390×844에서도 한 열 접힘 구조와 전체 문장 드롭다운을 확인하고 긴 select/summary의 최소 폭을 제한했다. 기존 Vite chunk size 경고만 남았다.
- 사람 gate: 운영 시험분석 최종 미리보기에서 ① 중단원 카드·상세·난이도가 문항 수가 아니라 쎈 교재 순서인지 확인한다. 공통수학1 예시는 `복소수(03) -> 이차방정식(04) -> 이차방정식과 이차함수(05) -> 여러 가지 방정식(06) -> 일차부등식(07) -> 이차부등식(08) -> 행렬과 그 연산(10)` 순서다. ② 역할 체크박스가 글자보다 과도하게 크지 않고 선택 역할만 켜고 끌 수 있는지 확인한다. ③ 공통 정보·시험 분석은 열리고 나머지 세 카드는 접혀 있으며 펼침 후 기존 입력이 유지되는지 확인한다. ④ 벤치마크 드롭다운에 적용될 문장 전체가 보이고 선택만으로는 값이 바뀌지 않으며 적용 후 저장·새로고침 유지되는지 확인한다. AI 생성이나 실제 자산 작업은 이 gate에서 실행하지 않는다.
- 중단 조건: 문항 수 순서로 계속 표시, unitNo 없는 기존 행이 앞뒤로 섞임, 체크박스 클릭 영역/disabled 계약 손상, 카드 접을 때 local draft 소실, 드롭다운 선택만으로 값 자동 덮어쓰기, 저장 없이 Supabase 변경, 4번 출력/복사 로직이 달라짐.

## 2026-07-23 P1. 수업일지 발송결과 상태·과제 미기재 후속

- 상태: 원인 수정, UI 정리, 저장·발송 직전 계약 보강과 자동검증 완료. 운영 배포 후 사람 화면 gate 대기.
- 발송결과 원인/수정: 과거 예약의 Solapi job을 `sent`로 reconcile한 뒤에도 지난 예약시각 자체를 재확인 대상으로 간주해 `발송결과 확인 필요`가 반복됐다. 이제 provider 참조가 있는 `scheduled(시각 경과)`/`send_unconfirmed` job만 확인 대상으로 삼는다. 확인 후 `sent`가 있으면 `발송 결과 반영 완료`, 실패가 있으면 `발송 실패 N건`으로 표시하며 결과 버튼은 사라진다. 지난 시각인데 결과 job 자체가 없으면 기존 `예약 시간 지남` 안전 상태를 유지한다.
- 버튼 정리: 사용자 요청에 따라 `발송 전 점검` 버튼, 토글 상태, 누락 행 배경 강조와 전용 CSS를 제거했다. 수업일지 명시 저장, 예약 설정/확인, 발송결과 reconcile, Solapi 예약 반영 기능은 유지한다.
- 과제 `미기재` 원천: 과제 상태 옵션에 빈 `선택`과 구분되는 `not_entered(미기재)`를 추가했다. 기존 `lesson_student_records.assignment_status`에 저장되고 수업일지 일괄 저장의 Supabase 재조회 대조 계약을 그대로 사용하므로 새 SQL은 없다. `미기재`는 과제를 검사한 결과가 아니므로 연결된 지난 과제의 `teacherStatus`, `checkedAt` 원천을 자동 변경하지 않는다.
- 알림톡 제외 계약: `미기재`는 신입생처럼 이번에 검사할 지난 과제가 없는 경우다. 학부모·학생 미리보기, 예약 payload/fingerprint, 서버의 실제 예약/발송 직전 최신 원천 재조립에서 `과제 상태`, `지난 과제`, `미완료 과제`, 지난 과제의 숙제 후속 안내만 제외한다. 이번 수업에서 새로 나간 `다음 과제`는 학부모·학생 알림톡에 계속 포함한다. 강의 교재·내용, 테스트, 재시험, 별도 보충 일정, 교사 코멘트도 유지한다. 이미 교사가 직접 작성해 저장한 자유 코멘트의 문장을 자동 삭제하거나 재작성하지 않는다.
- 외부 영향: 검증 중 실제 Solapi 조회·예약·발송·취소, 운영 Supabase 쓰기, 출결 변경을 실행하지 않았다. `발송 결과`는 기존과 같이 사용자가 눌렀을 때만 Solapi 결과를 조회해 OS job/수업일지 발송 상태를 reconcile한다.
- AI 검증: `git diff --check`, 서버·알림 경로와 `미기재` 정적 계약, `npm run test:production` 372건, `npm run build`를 통과했다. `미기재`가 명시 값으로 normalize되고 숙제 원천 동기화를 건너뛰며 프론트 미리보기·예약 payload·서버 발송 직전·최종 템플릿 조립에서 지난 과제 결과는 제외하고 다음 과제는 보존하는 경로를 모두 대조했다. 운영 서버 배포를 읽기 전용으로 판별할 수 있도록 `/health.features.lessonJournalNotificationFollowup=result_reconciled_unrecorded_preserve_next` 표식을 사용한다. 기존 Vite chunk size 경고만 남았다.
- 배포 확인: 최초 기능 커밋 `4f087b69`, health 표식 커밋 `dd955f9b`, 신입생 다음 과제 보존 보정 커밋 `4a7a5eeb`를 `origin/main`에 푸시했다. 운영 프론트가 JS `/assets/main-CYGi3_s5.js`, CSS `/assets/main-CR09Fsf2.css`로 전환됐고 번들에서 `not_entered`, `미기재`, `다음 과제`, `발송 결과 반영 완료`를 확인했으며 제거 대상 `발송 전 점검`과 `.preSendCheckButton`은 없다. Render `/health`도 `lessonJournalNotificationFollowup=result_reconciled_unrecorded_preserve_next`, `ok=true`를 반환했다.
- 사람 gate: 운영 수업일지에서 ① 이미 결과 확인을 마친 지난 수업은 `발송 결과 반영 완료`로 보이고 `발송 결과` 버튼이 사라지는지 확인한다. 실패 건이 있으면 완료 대신 `발송 실패 N건`이어야 한다. ② `발송 전 점검` 버튼이 없는지 확인한다. ③ 다음 과제가 입력된 테스트 학생 행에서 `수정 시작 -> 과제 상태 미기재 -> 변경 저장` 후 저장 완료와 새로고침 유지를 확인한다. ④ 학부모·학생 알림톡 작성창을 각각 열어 과제 상태/지난 과제/지난 과제 후속 안내는 없지만 `다음 과제`는 양쪽에 보이는지 확인한다. 강의·테스트·별도 보충·코멘트도 남아야 한다. ⑤ 일반 과제 상태로 되돌려 저장하면 지난 과제 관련 블록이 다시 나타나는지 확인한다. 이 gate에서는 실제 Solapi 예약 업데이트나 발송을 누르지 않는다.
- 중단 조건: 결과 확인 후에도 확인 필요 반복, sent/failed가 뒤바뀜, `미기재` 저장 후 새로고침 복귀, 지난 과제 검사 원천 변경, 학부모·학생 중 한쪽에 과제 블록 잔존, 별도 보충/테스트/강의/코멘트까지 소실, 저장만으로 Solapi side effect 발생.

## 2026-07-23 P1. 수업일지 발송 상태·작업 버튼 2단 정리

- 상태: 기능/부작용 경계를 보존한 UI 재구성, 자동검증과 데스크톱·모바일 Chrome 렌더 확인 완료. 운영 배포 후 사람 화면 gate 대기.
- 사용자 요청: 수업일지 상단에 상태 배지와 버튼이 한 줄로 과도하게 나열돼 복잡하므로 기능은 유지하면서 상태와 실행을 분리한다.
- 1단 상태 줄: `발송 상태`, 현재 예약 계획과 시각, `하원 미체크 N명`, Solapi 동기화 상태만 표시한다. 기본 예약 문구와 선택 버튼의 중복을 없애고, 모바일에서는 배지를 여러 줄로 늘리지 않고 상태 줄만 가로 이동한다.
- 2단 작업 줄: `발송 전 점검`, 편집 전의 `수정 시작`, `예약 설정` 드롭다운, `예약 확인`, 필요한 경우의 `발송 결과`, 실제 반영이 필요할 때만 `Solapi 예약 업데이트/취소 반영`을 표시한다. 편집을 시작하면 `수정 중` 중복 버튼은 숨기고 하단 고정 저장 바가 편집 상태와 `변경 저장`을 계속 담당한다.
- 예약 설정: 기존 `기본 예약`, `30분 지연`, `알림톡 없음` 세 버튼을 네이티브 드롭다운 하나로 통합했다. 현재 선택값과 각 예약 시각을 표시하고 만료된 예약 선택지는 기존과 동일하게 disabled 처리한다. 선택은 기존 `onUpdateLessonNotificationPlan`으로 OS 계획만 저장하며 Solapi 예약·취소는 실행하지 않는다.
- 기능/side effect 보존: 발송 전 점검의 누락 행 강조, 수업일지 local draft와 명시 저장, 예약 원천 확인 모달, Solapi 발송결과 reconcile, 명시적 Solapi 반영, 수업 수정·취소 handler는 변경하지 않았다. 화면 진입이나 드롭다운 열기만으로 `notification_jobs`, Solapi, 수업일지, 출결을 쓰지 않으며 실제 반영 버튼은 기존 `canApplySolapiReservation` 조건을 그대로 사용한다.
- AI 검증: 새 2단 구조와 예약 드롭다운, 세 계획 mode의 기존 handler 연결, 조건부 Solapi 버튼을 production 계약에 추가했다. 과거 상단 `수정 중` 문구를 고정하던 검사는 하단 고정 바가 편집 상태를 담당하고 상단에는 편집 전 `수정 시작`만 존재하는 새 계약으로 갱신했다. `git diff --check`, `node --check scripts/scenario-tests-production.cjs`, `npm run test:production` 371건, `npm run build`를 통과했다. 실제 `App.css`로 Chrome 렌더한 결과 데스크톱 1536×760은 두 줄 안에 모든 상태·작업이 보이고 가로 스크롤이 없으며, 모바일 390×844는 각 줄 높이를 유지한 채 작업 줄만 가로 이동하고 페이지 전체 overflow가 없었다. 기존 Vite chunk size 경고만 남았다.
- 배포 확인: 기능 커밋 `ec15d815`을 `origin/main`에 푸시했고 운영 프론트가 JS `/assets/main-BaNUBM6V.js`, CSS `/assets/main-Bnu6lQ1o.css`로 전환된 뒤 `lessonNotificationStatusRow`, `lessonNotificationActionRow`, 예약 설정 select와 조건부 Solapi 작업 구조가 포함된 것을 확인했다.
- 사람 gate: 운영 수업일지에서 ① 상태가 `예약 계획/시각 · 하원 미체크 · Solapi 상태` 한 줄로 보이는지, ② 작업이 `발송 전 점검 · 수정 시작 · 예약 설정 · 예약 확인` 순으로 정리됐는지 확인한다. ③ `예약 설정`에서 기본/30분 지연/알림 없음이 선택되고 상태 줄만 바뀌며 Solapi는 즉시 반영되지 않는지 확인한다. ④ `수정 시작` 후 상단 중복 `수정 중`은 사라지고 하단 고정 바가 나타나는지 확인한다. ⑤ Solapi 업데이트가 필요한 수업에만 강조 버튼이 보이고, 모바일에서 두 줄을 좌우로 움직여 모든 기능에 접근 가능한지 확인한다. 실제 Solapi 반영 버튼은 사람 gate 중 누르지 않는다.
- 중단 조건: 예약 선택만으로 실제 Solapi 예약/취소 실행, 편집 시작 또는 하단 저장 기능 소실, 발송 전 점검 행 강조 소실, 예약 확인/발송결과 접근 불가, 정상 상태인데 반영 버튼 상시 노출, 데스크톱 세로 버튼 누적, 모바일 페이지 전체 가로 넘침.

## 2026-07-23 P1. 공통 하단 고정 저장 바 4단계 - 시험분석 검수·산출물

- 상태: 문항 검수본과 산출물 작업본의 원천을 분리해 공통 UI 적용, 자동검증과 Chrome 렌더 확인 완료. 운영 배포 후 사람 화면/저장 gate 대기.
- 전 단계 gate: 사용자가 다음 작업 진행을 요청해 특강 회차·진행의 3단계 사람 gate를 통과한 것으로 기록했다. 수업일지 상단 다중 버튼 CSS·위계 정리는 기존 후속 큐에 보존한다.
- 원천 inventory: `선생님 검수`는 문항별 `exam_analysis_questions.teacher_fields/final_fields`와 `teacher_review` 진행 상태를 `/api/exam-analysis-runs/save-question-reviews`로 저장한다. `산출물`은 `exam_analysis_runs.audit_summary.outputDrafts`의 입력칸·블로그/인스타 선생님 수정본을 `/api/exam-analysis-runs/save-output-drafts`로 저장한다. `분석 저장`, `최종 미리보기 > GPT Image 프롬프트 제작실`의 `audit_summary.promptStudio`, PDF·AI 실행은 각각 별도 원천/동작이다.
- 검수 탭 적용: 상단의 중복 `검수 저장`을 제거하고 긴 문항 표 하단에 `문항 검수본 저장` 고정 바를 배치했다. 문항 변경/저장 중/저장 완료/저장 실패와 확정 수를 같은 위치에 표시한다. `모두 확정`은 local draft 변경, `AI 2차 수정`은 과금 가능한 별도 AI 동작이므로 상단에 분리해 유지하며 고정 바가 자동 실행하지 않는다.
- 산출물 탭 적용: 상단과 블로그·인스타 편집기 안에 중복되던 저장 버튼 3개를 제거하고 `산출물 작업본 저장` 고정 바 하나로 통일했다. 입력칸과 선생님 수정본을 함께 저장하는 기존 범위를 그대로 사용한다. 블로그/인스타 AI 초안 생성, ZIP 생성, 복사, TXT 내보내기는 저장과 다른 비용·로컬 파일 동작이므로 기존 위치에 분리해 유지한다.
- 저장 완료/부작용 경계: 두 바는 서로의 API를 호출하지 않는다. 각 API가 Supabase 저장 뒤 재조회한 run/questions의 원천이 `supabase`인지 확인하고, 프론트가 문항별 검수 요청 필드 또는 산출물 입력칸·선생님 수정본을 요청값과 대조한 뒤에만 완료로 전환한다. 누락·불일치면 실패로 남기고 실패 직후 상세 재조회로 local draft를 덮어쓰지 않는다. 검수 저장은 산출물·promptStudio를 저장 완료로 합산하지 않고, 산출물 저장은 문항 검수·일반 분석을 저장 완료로 합산하지 않는다. OpenAI/Anthropic, Storage, ZIP/TXT, `notification_jobs`, Solapi, 수업일지, 출결을 자동 실행하거나 변경하지 않는다.
- AI 검증: 각 저장 handler가 한 번만 렌더되고 두 독립 `StickySaveBar`가 올바른 handler에 연결되며 Supabase 원천·요청값 대조가 존재하는 production 계약을 추가했다. `git diff --check`, `node --check scripts/scenario-tests-production.cjs`, `npm run test:production` 370건, `npm run build`를 통과했다. 실제 긴 문항 표와 긴 산출물 편집 화면을 `App.css`로 렌더한 Chrome fixture에서 데스크톱 1280×720, 모바일 390×844의 첫 화면·중간·맨 아래 모두 바와 버튼이 viewport 안에 유지됐다. 모바일 상태 설명은 말줄임되고 저장 대상·버튼은 잘리지 않았다. 기존 Vite chunk size 경고만 남았다.
- 배포 확인: UI 기능 커밋 `816986df`와 Supabase 재조회 대조 보강 커밋 `988765f5`를 `origin/main`에 푸시했다. 운영 프론트가 JS `/assets/main-BUEFpFw2.js`, CSS `/assets/main-DcymRzfU.css`로 전환된 뒤 두 고정 바 클래스와 저장 문구, 문항 검수·산출물 요청값 불일치 오류 계약, 공통 sticky 스타일이 모두 포함된 것을 확인했다.
- 사람 gate: 운영 시험분석의 테스트 run에서 ① `선생님 검수` 탭을 열고 문항 표를 중간/하단까지 스크롤해 `문항 검수본 저장` 바가 따라오는지 확인한다. ② 문항 메모나 난이도를 바꿔 `변경됨`을 확인하고 저장한 뒤 완료·새로고침 유지를 확인한다. ③ `산출물` 탭에서 입력칸 또는 선생님 수정본을 바꿔 `산출물 작업본 저장`으로 저장하고 완료·새로고침 유지를 확인한다. ④ 검수 저장이 산출물 저장 완료로 표시되지 않고, 산출물 저장이 문항 검수 완료로 표시되지 않아야 한다. ⑤ 저장만으로 AI 초안 생성, ZIP/TXT 다운로드, 알림/수업 데이터 변화가 없어야 한다.
- 중단 조건: 두 저장 원천의 상태 합산, Supabase 재조회 결과 불일치인데 완료 표시, 실패 후 local draft 소실, 스크롤 시 바 이탈, 모바일 버튼 잘림, 저장만으로 AI 호출/다운로드/알림/수업 데이터 변화. 이 gate 통과 전 보충·알림톡 같은 고위험 화면에 공통 바를 확대하지 않는다.

## 2026-07-23 P1. 공통 하단 고정 저장 바 3단계 - 특강 회차·진행

- 상태: 2단계 저장 계약을 보존한 공통 UI 적용, 자동검증·운영 배포·사용자 다음 작업 진행으로 사람 gate 통과.
- 전 단계 gate: 사용자가 운영 학생 프로파일의 하단 고정 작업바를 확인해 2단계 사람 gate를 통과했다. 수업일지 상단 다중 버튼 CSS·위계 정리는 기능 유지 후속 작업으로 계속 보류하고, 사용자 지시대로 고정 작업바 연속 적용을 먼저 진행한다.
- 적용 범위: `Lesson Hub > 특강관리 > 특강 수업 > 특강 명단 > 회차·진행 관리` 모달의 기존 하단 액션을 공통 `StickySaveBar`로 교체했다. 바에는 `회차·수업일지` 저장 상태와 1/2·2/2 진행 문구, `닫기`, `회차 계획 및 미래 수업일지 반영`을 계속 표시한다. 저장 완료와 실패 문구는 공통 상태 색상으로 구분한다.
...
- 사람 gate: 운영의 스크롤 고정, 변경·저장 1/2→2/2, 새로고침 유지, 잠긴 회차와 알림/취소/출결 분리 확인 뒤 사용자가 다음 작업 진행을 요청해 통과로 기록했다.
...
## 2026-07-23 P1. 공통 하단 고정 저장 바 2단계 - 학생 프로파일 기본정보

- 상태: 저장 원천 분리, UI 적용, 자동검증·운영 배포·사용자 사람 화면 gate 통과.
- 전 단계 gate: 사용자가 운영 수업일지의 하단 고정 작업바를 확인해 1단계 사람 gate를 통과했다. 별도 요청인 수업일지 상단 다중 버튼 CSS·위계 정리는 기능을 유지하는 후속 작업으로 `AGENTS.md` 모달 통일 큐에 기록했고, 사용자 지시대로 고정 작업바 연속 적용을 먼저 진행한다.
- 적용 범위: 학생 프로파일에서 `수정`을 누르면 공통 `StickySaveBar`가 나타난다. 바에는 기본정보의 `변경됨/저장 중/저장 완료/저장 실패`, 실제 변경 필드 수, `기본정보만 저장` 버튼을 표시한다. 상단의 중복 기본정보 저장 버튼은 제거했지만 수정 취소와 원천별 상태 표시는 유지했다.
- 저장 원천 분리: 고정 바는 `students`의 학교/학년/교재/연락처/로그인/PIN/학생 메모/개별 스케줄만 기존 `POST /api/students -> GET /api/students -> 필드 대조` 계약으로 저장한다. 상담·성적·테스트는 각각의 `app_state` 행 단위 저장, 운영 알림은 `/api/academy-reminders` 저장 버튼을 그대로 사용한다. 고정 바는 이 원천들을 일괄 저장하거나 `모두 저장 완료`로 합산하지 않는다.
- draft 보존 보강: 기본정보를 저장할 때 운영 알림 작성 draft가 남아 있으면 상담·성적·테스트 draft와 동일하게 편집 모드를 유지한다. 기본정보 저장 때문에 별도 운영 알림 작성창이 접혀 저장 전 입력을 놓치는 기존 빈틈을 막았다. 외부 알림톡, Slack 발송, `notification_jobs`, Solapi, 출결, 수업일지는 실행하거나 변경하지 않는다.
- AI 검증: 과거 상단 버튼 문구를 고정한 production 검사 1건을 새 고정 바와 `기본정보만 저장` 범위로 갱신했으며 저장·편집 gate는 낮추지 않았다. `git diff --check`, `node --check scripts/scenario-tests-production.cjs`, `npm run test:production` 369건, `npm run build`를 통과했다. 실제 모달 스크롤 컨테이너와 `App.css`를 사용한 Chrome fixture에서 데스크톱 1280×720, 모바일 390×844의 첫 화면·중간·맨 아래 모두 바 전체가 viewport 안에 유지되고 모바일 버튼이 잘리지 않는 것을 좌표와 스크린샷으로 확인했다. 기존 Vite chunk size 경고만 남았다.
- 배포 확인: 기능 커밋 `c5927687`을 `origin/main`에 푸시했고 운영 프론트가 JS `/assets/main-BPs8k5zX.js`, CSS `/assets/main-p4PNTmHv.css`로 전환된 뒤 `기본정보만 저장`, `각 영역에서 별도 저장`, `.studentProfileStickySaveBar`가 포함된 것을 확인했다.
- 사람 gate: 운영 학생관리에서 테스트 학생 프로파일을 열어 `수정`, 스크롤 고정, 변경 필드 수, 기본정보 저장과 새로고침 유지, 별도 draft 보존을 확인했고 사용자가 통과를 확정했다.
- 중단 조건: `기본정보만 저장`이 상담·성적·테스트·운영 알림까지 저장함, 다른 draft가 사라짐, Supabase 재조회 불일치인데 완료 표시, 스크롤 시 바 이탈, 모바일 버튼 잘림, 저장만으로 알림톡/Slack/Solapi/수업일지/출결 변화. 이 gate 통과 전 특강 회차·진행 모달 적용으로 넘어가지 않는다.

## 2026-07-23 P1. 공통 하단 고정 저장 바 1단계 - 수업일지

- 상태: 목표 생성, 공통 구조와 수업일지 1단계 적용, 자동검증·운영 배포·사용자 사람 화면 gate 통과.
- 설정한 목표: 긴 편집 화면에 공통 하단 고정 저장 바를 도입하되, 화면별 저장 원천과 외부 부작용을 분리하고 한 화면씩 자동검증·사람 gate·배포까지 완료한다. 적용 순서는 `수업일지 -> 학생 프로파일 -> 특강 회차·진행 -> 시험분석 검수/산출물`이며 보충·알림톡은 별도 고위험 검토 뒤 결정한다.
- 공통 구조: `StickySaveBar`를 shared component로 추가했다. 현재 작업 영역 안에서만 `position: sticky`로 하단을 따라가며 `InlineSaveStatus`, 짧은 상태 설명, 대표 저장 버튼 영역을 같은 구조로 제공한다. 모바일에서도 한 줄의 상태+대표 버튼을 유지하고 긴 상태 문구는 말줄임 처리하며 safe-area 하단 여백을 반영한다.
...
- AI 검증: `git diff --check`, `node --check scripts/scenario-tests-production.cjs`, `npm run test:production` 369건, `npm run build`를 통과했다. 실제 `App.css`를 사용하는 긴 수업일지 fixture를 Chrome으로 렌더해 데스크톱 1280×720과 모바일 390×844의 첫 화면·중간 스크롤·맨 아래에서 바 전체가 viewport 안에 유지되는 것을 좌표와 스크린샷으로 확인했다. 모바일 상태 문구는 말줄임되고 저장 버튼은 화면 안에 유지됐다. 기존 Vite chunk size 경고만 남았다.
...
- 다음 단위 사전 inventory: 학생 프로파일은 하나의 모달 안에 네 저장 계약이 섞여 있다. `기본정보·학생 메모·개별 스케줄`은 `POST /api/students -> GET /api/students -> 필드 대조`가 끝나야 완료되는 `students` 원천이고, `상담기록`, `성적`, `테스트`는 각각 `app_state.studentConsultations`, `app_state.scoreRecords`, `app_state.academyTests` 키의 별도 행/초안 저장이며, `학생별 운영 알림`은 `/api/academy-reminders` 별도 원천이다. 다음 적용 시 고정 바는 `기본정보만 저장`한다고 명시하고 다른 세 원천과 운영 알림을 일괄 저장하지 않는다. 상담·성적·테스트·운영 알림은 각 펼침 영역의 행 단위 저장 버튼을 유지하며, app_state 재조회/version 보강 전에는 공통 바에서 `모두 저장 완료`로 합산하지 않는다.
...
## 2026-07-23 P2. 시험분석 쎈 중단원 기준 집계

- 상태: 집계·표시 수정과 자동검증 완료. 운영 배포 후 사람 화면 검토 대기.
- 사용자 요청/원인: 최종 미리보기의 `단원별 출제 비중`이 쎈 기준 중단원이 아니라 `partName` 대단원으로 묶여, `이차방정식`, `복소수`, `여러 가지 방정식` 등이 모두 `방정식` 한 항목으로 합쳐졌다.
- 구현: 선생님 검수 저장본의 쎈 계층 중 `unitName`을 출제 비중 도넛·상단 중단원 수·중단원별 난이도·내보내기 차트의 공통 집계 기준으로 사용한다. 각 중단원 아래 상세 행은 같은 중단원에 속한 `mainType` 주요유형과 문항 수를 표시한다. `partName` 대단원은 더 이상 이 시각화의 묶음 기준으로 사용하지 않으며, 중단원 미입력 문항은 임의 추측이나 대단원 대체 없이 `중단원 미입력`으로 분리한다.
- 원천/저장 계약: `finalFields > teacherFields > ssenMeta > 기존 문항값`의 현재 검수 저장본 우선순위를 유지한다. 이번 변경은 저장된 분류를 다시 쓰거나 AI로 재분류하지 않는 읽기 전용 파생 시각화이며 Supabase 스키마·API·local draft는 변경하지 않았다.
- 외부 영향: 유료 AI, 이미지 생성 API, Storage, `notification_jobs`, Solapi, 수업일지, 출결을 호출하거나 변경하지 않았고 새 SQL은 없다.
- AI 검증: 쎈 대단원명이 같은 문항도 서로 다른 `unitName`이면 별도 항목으로 집계되고, 중단원별 주요유형과 난이도가 같은 기준을 쓰는 deterministic fixture를 `npm run test:production`에 추가했다. `git diff --check`, 새 fixture와 시나리오의 `node --check`, `npm run test:production` 369건, `npm run build`를 통과했다. 기존 Vite chunk size 경고만 남았다.
- 배포 확인: 기능 커밋 `bd3b0502`를 `origin/main`에 푸시했고 운영 프론트가 `/assets/main-D2y6KAIC.js`로 전환된 뒤 `쎈 중단원별 출제 비중`과 `주요유형 분포` 문구가 포함된 것을 확인했다.
- 사람 gate: 운영 시험분석에서 스크린샷과 같은 시험의 `최종 미리보기`를 연다. ① 도넛이 `방정식/행렬/부등식` 3개가 아니라 저장된 쎈 중단원(예: `이차방정식`, `이차방정식과 이차함수`, `여러 가지 방정식`, `복소수`, `행렬과 그 연산`, `이차부등식`, `일차부등식`)별로 나뉘는지 확인한다. ② 각 중단원 문항 수의 합이 전체 문항 수와 같은지 확인한다. ③ 상세 행이 중단원명 아래 해당 주요유형을 표시하고, `쎈 중단원별 난이도`도 같은 중단원 목록을 쓰는지 확인한다. ④ 검수 표에서 테스트 문항의 중단원을 저장한 뒤 새로고침해 최종 미리보기 집계가 저장본대로 유지되는지 확인한다.
- 중단 조건: 대단원 3개로 계속 합쳐짐, 중단원 미입력을 대단원명으로 대신 표시함, 전체 문항 수와 중단원별 합계 불일치, 주요유형이 다른 중단원에 섞임, 단순 미리보기 진입으로 저장된 분류가 변경됨.

## 2026-07-23 P1. 학생 프로파일 메모·접힘 구조와 시험분석 선택 카드 CSS 보강

- 상태: UI·저장 검증 수정과 자동검증 완료. 운영 배포 후 사람 화면/저장 gate 대기.
- 사용자 증상/요청: 시험분석의 학교·학년·고사 선택 카드에서 굵은 한글과 기본 스크롤바가 겹치며 글자가 뭉개져 보였다. 학생 프로파일에는 학생별로 계속 확인할 메모 입력 위치가 필요했고, 긴 모달을 펼침/접힘으로 정리해야 했다.
- 시험분석 CSS: 선택 카드의 제목·보조문구에 충분한 줄높이와 세로 여백을 주고 카드 내부 넘침을 제한했다. 각 열의 스크롤바는 얇게 만들고 별도 gutter와 우측 여백을 확보했으며 WebKit 기본 화살표 버튼을 숨겨 카드와 겹치지 않게 했다. 데이터 선택·삭제·시험분석 저장 동작은 변경하지 않았다.
- 학생 메모 원천: 새 임시 원천이나 SQL 컬럼을 추가하지 않고 기존 `students.special_note`/프론트 `specialNote`를 `학생 메모`로 프로파일 최상단에 승격했다. 수정 모드의 local draft는 기존 `profileDraft.specialNote`에 유지되고 상단 `기본정보 저장`으로 명시 저장한다. 새 SQL은 없다.
- 저장 완료 계약: 학생 기본정보 POST 응답만으로 완료 처리하던 흐름을 `POST /api/students -> GET /api/students -> studentId 재탐색 -> 학교/학년/교재/연락처/로그인/PIN/학생 메모/개별 스케줄 전 필드 대조`로 보강했다. 재조회 누락이나 불일치 시 `저장 실패`로 남기고 프로파일 draft를 유지하며, 일치한 전체 학생 원천으로 화면을 갱신한 뒤에만 `저장 완료`를 표시한다.
- 모달 구조: 상단 학생명·수정·저장 상태는 고정하고 `기본정보와 학생 메모`, `학생별 운영 알림`, `상담 기록`, `성적 기록`, `테스트 성적`을 `<details>` 접힘 영역으로 나눴다. 기본정보와 메모만 처음 열리며 나머지는 필요한 항목만 펼친다. 접힘은 DOM을 제거하지 않으므로 입력 중 local draft와 기존 저장 버튼/disabled 조건을 유지한다.
- 외부 영향: 이번 변경은 `notification_jobs`, Solapi 예약·발송·취소, 수업일지, 출결, 미래 반 명단, 상담/성적/테스트 저장 원천을 자동 실행하거나 변경하지 않는다. 학생 메모 저장은 기존 학생 기본정보 저장 버튼을 눌렀을 때만 실행된다.
...
- 사람 gate: ① 시험분석에서 학교·학년·고사 열을 스크롤해 굵은 한글이 잘리지 않고 스크롤바가 카드와 겹치지 않는지 확인한다. ② 학생관리에서 테스트 학생 프로파일을 열어 기본정보/메모만 열리고 나머지 네 영역이 접혀 있는지, 각 제목으로 펼침·접힘이 되는지 확인한다. ③ `수정`을 누르고 학생 메모에 식별 가능한 테스트 문구를 입력해 `기본정보 저장` 후 `저장 완료`를 확인한다. ④ 모달을 닫고 새로고침·재로그인 후 같은 학생 프로파일에서 메모가 유지되는지 확인한다. 실제 운영 학생 메모는 테스트 문구 확인 후 필요한 내용으로 다시 저장한다.
- 중단 조건: 한글 글자 잘림/스크롤바 겹침 지속, 접을 때 draft 소실, 메모 저장 실패인데 완료 표시, 새로고침 후 메모 복귀, 학생 메모 저장만으로 반 명단·출결·수업일지·알림 예약 변화, 기존 상담/성적/테스트 버튼 기능 소실.

## 2026-07-23 P2. 보충 수업일지 반영 상태와 하단 작업 버튼 compact 정리

- 상태: UI 수정과 자동검증 완료. 운영 배포 후 사람 화면 검토 대기.
- 사용자 요청: 수업일지에 이미 반영된 보충 카드 상단에는 여러 상태 배지와 일시를 쌓지 않고 `수업일지에 반영되었습니다.` 한 문장만 표시하며, 하단 기능 버튼은 세로로 길어지지 않게 한 줄로 배치한다.
- 구현: `linkedLessonId`가 있는 카드의 상태 영역은 요청 문장 하나만 렌더링한다. 아직 수업일지에 연결되지 않은 카드에는 기존 `수정 중`, 알림톡 문구 저장 여부, 결석사유, 최근 숙제 정보를 그대로 유지해 생성 전 맥락을 잃지 않는다. 카드 하단 작업 영역은 보이는 버튼 수만큼 같은 너비의 가로 열로 배치하고 좁은 화면에서는 카드 높이를 늘리는 대신 해당 작업 줄만 가로 스크롤할 수 있게 했다.
- 기능/저장 계약: 표시와 CSS만 변경했다. 보충 내용·알림톡 저장, 수업일지 일정 만들기/변경, 결석 처리 취소, 보충 완료 처리 함수와 disabled 조건은 그대로다. Supabase, `lessons`, `makeupTasks`, `notification_jobs`, Solapi 예약·발송·취소를 실행하거나 변경하지 않았고 새 SQL은 없다.
- AI 검증: `git diff --check`, `node --check scripts/scenario-tests-production.cjs`, `npm run test:production` 367건, `npm run build`를 통과했다. 기존 Vite chunk size 경고만 남았다.
- 사람 gate: 운영 보충관리에서 수업일지에 연결된 학생 카드를 열어 상태가 정확히 `수업일지에 반영되었습니다.` 한 줄만 보이는지 확인한다. 하단의 현재 사용 가능한 기능 버튼이 데스크톱에서 한 줄에 같은 너비로 보이고, 좁은 화면에서는 세로로 쌓이지 않으며 작업 줄만 좌우로 움직이는지 확인한다. 각 버튼의 최종 확인 동작은 누르지 않는다.
- 중단 조건: 연결 전 카드의 결석사유/저장 상태가 사라짐, 버튼이 숨겨지거나 대상 학생이 바뀜, 버튼 클릭 없이 저장·수업일지·알림 예약 상태가 변함, 카드 전체가 가로로 넘침.

## 2026-07-23 P1. 특강 회차 시간 저장과 미래 수업일지 자동 반영 통합

- 상태: 원인 확인, 코드 수정과 자동검증 완료. 운영 배포 후 김룡기 학생 사람 반영 gate 대기.
- 사용자 증상: 김룡기 학생의 토요일 특강 시간을 13:00~16:00으로 바꿨지만 수업일지에는 기존 16:00~19:00이 보였다.
- 운영 원천 대조: `special_lecture_enrollments`의 3·6·9회차에는 13:00~16:00이 저장돼 있었지만, 2026-07-25·2026-08-01·2026-08-08 `lessons.specialLectureStudentSchedules`에는 김룡기 학생이 16:00~19:00 `official`로 남아 있었다. 회차 계획 저장 후 별도 `특강 수업일지 반영`을 실행하지 않아 두 원천이 갈라진 것이 원인이다.
- 구현: 학생 회차 모달의 저장 버튼을 `회차 계획 및 미래 수업일지 반영`으로 변경했다. 1/2에서 enrollment 저장과 Supabase 재조회를 완료하고, 2/2에서 이번 학생의 저장 회차와 lesson 학생별 시간 차이를 계산해 안전한 미래 lesson만 bulk 저장한 뒤 Supabase 재조회한다. 두 원천이 모두 일치한 뒤에만 `저장 완료`를 표시한다.
- 안전 범위: 오늘/과거 수업, 완료 수업, 기존 `lesson_student_records`가 있는 수업, 대기 중인 `notification_jobs`가 있는 수업은 자동 반영하지 않고 부분 완료/확인 필요로 표시한다. 이번 저장 흐름은 출결·수업기록·알림 예약·Solapi·Tally 원본을 생성·변경·취소하지 않는다.
- 재시도 계약: enrollment 저장 후 lesson 반영이 실패하면 `1/2 저장 완료 · 2/2 실패`를 표시한다. 같은 버튼을 다시 누르면 저장된 enrollment와 해당 학생의 lesson schedule을 다시 비교해 누락된 안전 미래 수업만 재시도한다. 미저장 draft가 아니라 이미 저장된 원천을 재시도 기준으로 사용한다.
- 운영 데이터 조치: 이번 코드 작업 중에는 운영 lesson을 쓰지 않았다. 배포 후 김룡기 `회차·진행 관리`에서 시간을 다시 확인하고 통합 저장 버튼을 사람이 눌러 세 미래 토요일 lesson을 반영한다.
- AI 검증: `git diff --check`, `node --check scripts/scenario-tests-production.cjs`, `npm run test:production` 366건, `npm run build` 통과. 최초 production 검사에서 과거 안내문 문자열을 찾던 1건은 오늘/지난 회차 원천 편집 허용 안내를 유지해 계약을 보존한 뒤 다시 통과했다. 기존 Vite chunk size 경고만 남았다.
- 사람 gate: 김룡기 `회차·진행 관리`에서 3·6·9회차가 13:00~16:00인지 확인하고 `회차 계획 및 미래 수업일지 반영`을 누른다. `1/2 -> 2/2 -> 저장 완료`를 확인한 뒤 새로고침한다. 2026-07-25·2026-08-01·2026-08-08 수업 버튼과 실제 수업일지 학생 시간이 모두 13:00~16:00이어야 한다. 다른 학생, 평일 회차, 출결, 기록, 알림 예약은 바뀌지 않아야 한다.
- 중단 조건: 보호 수업이 자동 반영됨, enrollment만 저장됐는데 완료 표시, lessons 재조회 불일치인데 완료 표시, 다른 학생 시간 변경, 출결/기록/notification job/Solapi 변화, 새로고침 후 16:00~19:00으로 복귀.

## 2026-07-23 P2. 특강 회차 설정·진행 보기 단일 화면 통합

- 상태: UI 수정과 자동검증 완료. 운영 배포 후 사람 화면 검토 대기.
- 사용자 요청: 학생 카드의 `회차 설정`과 `진행 보기`를 하나로 합치고, 회차 확정 후 각 수업 버튼을 눌러 해당 회차 진행상황을 확인한다.
- 구현: 활성 학생 카드의 별도 `진행 보기` 버튼을 제거하고, 미확정 학생은 `회차 설정`, 확정 학생은 `회차·진행 관리` 단일 버튼을 사용한다. 통합 모달 상단 `확정 수업 진행`에는 마지막 저장 완료된 active 회차만 수업 버튼으로 표시한다. 버튼을 누르면 같은 모달에서 날짜·시간·출결·진행 기록·다음 과제와 `수업일지 열기`를 확인한다. 기존 회차 체크·공통/개별 시간·운영 메모·저장 영역은 하단 `회차 신청 수정`으로 유지했다.
- 저장/조회 계약: 회차 확정 기준은 Supabase 재조회가 끝난 `special_lecture_enrollments.planReviewedAt/sessionPlans` 저장본이다. 진행 표시는 해당 회차의 `lessons`와 학생별 `lesson_student_records`를 읽기만 한다. 편집 중 local draft는 하단 회차 신청 수정에만 적용되며 `회차 계획 저장` 전에는 확정 수업 버튼을 바꾸지 않는다.
- 기능 보존: 취소된 enrollment의 과거 감사기록용 `기존 기록 보기` 모달은 유지한다. 회차 저장 read-after-write, 미래 수업일지 명단 별도 반영, 취소 확인창과 저장 함수는 변경하지 않았다.
- 외부 영향: 수업 버튼 선택과 진행 조회는 운영 데이터, 출결, `notification_jobs`, Solapi, Tally 원본을 변경하지 않는다. Solapi 특강 템플릿은 임시 구조를 유지한다. 새 SQL은 없다.
- AI 검증: `git diff --check`, `node --check scripts/scenario-tests-production.cjs`, `npm run test:production` 366건, `npm run build` 통과. 기존 Vite chunk size 경고만 남았다. 로컬 Chrome 렌더 fixture에서 단일 카드 버튼, 확정 회차별 수업 버튼, 선택 회차 진행 상세와 좁은 화면 배치를 확인했다.
- 사람 gate: 운영 `Lesson Hub > 특강관리 > 특강 수업 > 특강 명단 펼치기`에서 미확정 학생은 `회차 설정`, 확정 학생은 `회차·진행 관리` 하나만 보이는지 확인한다. 확정 학생을 열어 저장된 수강 회차만 버튼으로 나오는지, 각 버튼이 서로 다른 해당 회차 기록을 보여주는지 확인한다. 회차 체크를 바꾸되 저장하지 않았을 때 상단 확정 버튼은 바뀌지 않아야 하며, 저장 완료 후 모달에서 Supabase 저장본 기준으로 갱신돼야 한다. 실제 알림 발송·예약·취소는 실행하지 않는다.
- 중단 조건: 미확정 draft가 확정 수업 버튼에 즉시 반영됨, 다른 학생/다른 특강 기록 노출, 회차 버튼과 날짜·수업일지 불일치, 단순 진행 조회로 저장·출결·알림 예약 변화, 저장 실패인데 확정 버튼 변경, 좁은 화면 가로 넘침.
...
- 동작 보존: App이 기존 task/local draft/diff/save status와 방법 label 함수를 전달하고 반환 props에 기존 callback만 결합한다. fallback, 상태 우선순위와 안내 문구는 그대로다.
- 저장 원천/side effect: 새 모델은 순수 계산만 수행한다. local React state, `notification_jobs` 선택, API, Supabase, Solapi 예약·취소는 App에 남아 있다.
- 검증/gate: deterministic fixture가 원천 fallback, 메타, 일정 유무/변경, 저장 상태 override, 네 gate 문구 분기를 고정한다. production 379/379과 build가 통과했으며 추가 사람 gate나 유료 호출은 없다.
- 다음 단위: 최신 `origin/main` rebase 후 알림 control 표시 계산 또는 modal shell 중 state/API를 이동하지 않는 경계를 검토한다.

## 2026-07-21 P1. 12N 보충 task local draft 순수 모델 분리

- 상태: 세 알림 문구 field/config, 선생님 최종본 field 판정, task→local draft 생성, dirty 비교/diff, source version/fingerprint, 저장 객체 정리를 `supplementTaskDraft.js`로 분리했다.
- 동작 보존: 알림 문구 생성과 방법 normalize 함수는 App에서 의존성으로 주입한다. 기존 저장값·선생님이 명시적으로 비운 최종본·자동 생성 초안의 우선순위와 기존 App 호출부 signature를 유지한다.
- 저장 원천/side effect: 새 모델은 전달받은 객체만 계산하며 React state, API, Supabase, `notification_jobs`, Solapi를 소유하지 않는다. local draft state와 실제 저장·예약·취소 handler는 App에 남아 있다.
- 검증/gate: deterministic fixture가 config, edited-field 정리, 빈 최종본 보호, draft 생성, dirty/diff, fingerprint/source version, persistable 변환을 고정한다. production 378/378과 build가 통과했으며 추가 사람 gate나 유료 호출은 없다.
- 다음 단위: 최신 `origin/main` rebase 후 task별 표시 계산을 순수 view-model helper로 한 단위씩 분리한다.
...
- 동작 보존: App이 기존 diff와 세 상태, 최초 확정/기존 변경 안내 문구를 계산해 전달한다. 화면 순서도 `diff/status -> 알림 draft workspace -> 일정 gate 안내`를 그대로 유지한다.
- 저장 원천/side effect: 새 표시 파일은 상태 label helper만 읽고 hook/input/action/API/알림 side effect를 소유하지 않는다. 실제 저장·예약은 App에 남아 있다.
- 검증/gate: production 88b-10이 표시 순서, 세 상태, gate 문구와 callback 비소유를 확인한다. 추가 사람 gate는 없다.
- 다음 단위: 최신 `origin/main` rebase 후 task 카드 상단 메타 또는 하단 action bar를 callback-only 표시로 분리한다.
...
- 동작 보존: App이 현재 task/job/config, 마스킹 번호, 표시 시각, preview 우선순위, 예약·취소 가능 여부를 계산하고 기존 `handleNotificationControlAction`을 callback으로 전달한다. 취소·실패한 과거 문구 대신 현재 저장 문구를 보여주는 계약도 유지한다.
- 저장 원천/side effect: 모달은 props 렌더와 close/cancel/reserve callback 호출만 수행한다. busy/feedback state, `notification_jobs`, API와 Solapi action은 App에 남아 있다.
- 검증/gate: production 88b-7은 hook/API/job state 비소유와 callback 연결을 검사하고 88d-1/88f는 실제 control 연결 및 과거 preview 차단 계약을 분리 파일까지 확인한다. 추가 사람 gate와 실제 유료 호출은 없다.
- 다음 단위: 최신 `origin/main` rebase 후 task 카드의 원천 맥락·일정 입력·상태 표시 중 callback-only 조각을 하나씩 분리한다.
...
- 동작 보존: 취소 가능 예약의 과거시각 경고, sent/canceled/failed/dry_run/send_unconfirmed fallback, 학생 11시 deterministic job 상태, skipped/즉시 안내 mapping을 유지한다. App은 공통 시각 판정·job status formatter만 adapter로 주입한다.
- 검증/gate: 모든 label/tone/status 분기와 unknown fallback을 deterministic fixture로 `npm run test:production`에 연결했다. production check는 React/API/state/Solapi가 없는 순수 표시 경계를 확인한다. 추가 사람 gate는 없다.
- 다음 단위: 최신 `origin/main` rebase 후 `SupplementStudentModal`의 알림 draft tab/control 표시 조각을 callback props 기반 컴포넌트로 분리한다.

## 2026-07-21 P1. 12D 최근 보충내역 모달·순수 필터 분리

- 상태: `SupplementHistoryModal`을 별도 JSX로, 날짜/status/view item과 query 필터를 `supplementHistory.js`로 분리했다.
- 동작 보존: 전달된 task 순서, 학생명·학교·학년·유형·방법·원천·상태 검색, 날짜 fallback 우선순위, 완료 항목만 복귀 callback 노출을 유지한다. 유형/방법 라벨은 App에서 주입한다.
- 검증/gate: 세 상태 표시, 날짜 fallback, 미등록 학생, 검색 trim/case/학교/방법, 순서 보존 fixture를 `npm run test:production`에 연결했다. production check는 API/notification/Solapi와 local hook이 없는 읽기 경계를 확인한다. 추가 사람 gate는 없다.
- 다음 단위: 최신 `origin/main` rebase 후 `SupplementStudentModal` 내부의 읽기 전용 알림 상태/탭 표시 조각을 하나씩 분리한다.

## 2026-07-21 P1. 12C 보충 일정 변경 확인 모달 분리

- 상태: `SupplementScheduleChangeConfirmModal`을 별도 JSX 파일로, 항목 fallback·초기 local draft·confirm patch를 `supplementScheduleChangeDraft.js`로 분리했다.
- 동작 보존: 원 수업/숙제 seed 우선, 없으면 현재 항목 fallback, 변경 사유 빈 시작, 두 저장 버튼이 같은 현재 local patch를 전달하는 계약과 busy 문구를 유지한다. seed/type label과 실제 confirm callback은 App에서 주입한다.
- 검증/gate: 숙제/결석 항목 fallback, seed 우선, 선생님 수정 patch 복사와 불필요 필드 제외를 deterministic fixture로 `npm run test:production`에 연결했다. production check는 모달의 두 callback 연결과 API/notification/Solapi 비소유를 확인한다. 실제 저장·예약 호출은 없어 추가 사람 gate는 없다.
- 다음 단위: 최신 `origin/main` rebase 후 읽기 중심 `SupplementHistoryModal`의 필터/표시를 분리한다.

## 2026-07-21 P1. 12B 보충 완료 확인 모달 분리

- 상태: `SupplementPassConfirmModal`을 `src/domains/supplements/SupplementPassConfirmModal.jsx`로 분리했다.
- 동작 보존: 숙제보충 항목 fallback, 유형·일정 표시, 오류 문구, busy disabled/버튼 문구, cancel/confirm callback을 유지한다. 유형 라벨 함수는 App에서 주입한다.
- 검증/gate: 새 production check가 컴포넌트 내용과 두 App 호출부 연결을 확인하고, hook/API/notification/Solapi가 없는 callback-only 경계를 고정한다. 화면 문구·CSS·저장 원천 변화가 없어 추가 사람 gate는 없다.
- 다음 단위: 최신 `origin/main` rebase 후 `SupplementScheduleChangeConfirmModal`의 local reason/detail 초기값과 두 confirm payload를 fixture로 고정하고 분리한다.
...
- 동작 보존: lesson과 makeup task 저장이 끝난 뒤 알림을 실행하는 순서, 11시→일정 pair의 직렬 순서, 중복 방지 시 안내 문구, status fallback을 유지한다. 실제 예약 함수는 기존 wrapper를 주입한다.
- 검증/gate: 전체 예약, 모두 미적용, 11시는 갱신하지만 현재 일정 pair를 재사용하는 세 분기를 fixture로 고정했다. production 정적 검사는 App 고정 문자열 대신 새 orchestration의 주입 호출까지 추적하게 갱신했다. 기존 11B-9 운영 gate와 같은 실제 예약 함수만 사용하므로 추가 과금 검증은 하지 않는다.
- 다음 단위: 최신 `origin/main` rebase 후 보충 일정 저장 전 검증·중복 판정·lesson/task plan을 별도 의미 단위로 좁혀 분리한다.

## 2026-07-21 P1. 11B-11 활성 보충 일정 안내 일괄 취소 orchestration 분리

- 상태: 같은 보충 task의 취소 가능한 학생 일정·학부모 일정·legacy 학부모 안내를 선택해 기존 배치 취소 함수로 넘기는 경계를 `cancelActiveSupplementScheduleNoticesRequest`로 분리했다.
- 동작 보존: 이전 일정까지 정리하는 selector 계약, 취소 사유, 실제 React 상태·API·Solapi 취소 함수는 그대로 주입한다. 이미 발송된 job과 다른 task는 제외한다.
- 검증/gate: 세 활성 유형의 선택 순서, 다른 task·발송 완료 제외, 취소 사유/함수 전달을 fixture로 고정했다. 새 외부 호출은 없고 기존 11B 운영 gate 범위 안이므로 추가 과금 검증은 하지 않는다.
- 다음 단위: 최신 `origin/main` rebase 후 `handleScheduleSupplementTask`의 저장 전 결정 또는 저장 후 결과 조립을 별도 의미 단위로 좁혀 분리한다.
...
- 동작 보존: 학생 일정·학부모 일정·학생 11시의 대상 선택, 기존 문구/제목/시각 builder, 취소 사유, 실제 API·React 상태 wrapper를 그대로 주입한다. 문구·운영 기능은 바꾸지 않았다.
- 자동검증: 새 예약, 기존 예약 재사용, 학생 11시 위임, 잘못된 task·학생 없음·빈 선생님 최종 문구 차단을 deterministic fixture로 고정했다.
- 운영 gate: 승인된 미배정 고태영 데이터로 학생 일정·학부모 일정·학생 11시를 각각 실제 예약했다. 세 건 모두 OS `notification_jobs`와 Solapi 그룹의 대상·예약시각·최종 문구가 일치했고, 개별 취소 후 OS `canceled`와 Solapi `예약취소/1070`을 확인했다. 임시 미래 결석보강 task는 삭제 후 재조회에서 없음을 확인했다. 취소 감사 row는 이력으로 유지한다.
- 다음 단위: 최신 `origin/main` rebase 후 `cancelSupplementStudentReminder`처럼 App에 남은 보충 알림 orchestration을 한 단위씩 계속 분리한다.

## 2026-07-21 P1. 11B-8 개별 알림 취소 control orchestration 분리

- 상태: 취소 가능성 검증, 기존 취소 wrapper 호출, control 결과 조립을 `cancelSupplementNotificationControlRequest`로 분리했다.
- 동작 보존: 기존 취소 사유, 오류·완료 문구, 반환 status fallback을 유지한다. 실제 취소 함수와 판정은 App에서 주입한다.
- 검증/gate: 성공과 취소 불가 fixture를 추가했다. 실제 API/Solapi 호출이 없어 추가 사람 gate는 없다.

## 2026-07-21 P1. 11B-7 학생·학부모 일정 pair 예약 orchestration 분리

- 상태: 대상·학생 검증, 기존 활성 안내 취소, 공통 예약시각, 학생/학부모 job 생성과 병렬 예약을 `reserveSupplementScheduleNoticesRequest`로 분리했다.
- 동작 보존: 두 job은 같은 시각을 사용하고 대상별 payload 필드와 실패 prefix를 유지한다. 문구·제목·취소·단일 예약 함수는 기존 경계를 주입한다.
- 검증/gate: pair 순서, 동일 시각, 학생 `reminderBody`, 학부모 `commentBodyOverride/message`, 비대상·학생 없음 분기를 fixture로 고정했다. 실제 호출이 없어 추가 사람 gate는 없다.

## 2026-07-21 P1. 11B-6 학생 11시 예약 orchestration 분리

- 상태: 대상 task, 선생님 빈 최종 문구, 학생·배정일·과거시각 검증과 job 생성·예약 결과 조립을 `reserveSupplementStudentReminderJobRequest`로 분리했다.
- 동작 보존: 순수 builder는 직접 import하고 시간 판정, 제목, normalize, 실제 reserve, 시각 표시는 기존 App 함수를 주입한다. 기존 차단·성공·실패 문구와 예약 사유를 유지한다.
- 검증/gate: 정상 예약과 다섯 차단 분기를 deterministic fixture로 검증한다. 실제 API/Solapi 호출이 없어 추가 사람 gate는 없다.

## 2026-07-21 P1. 11B-5 보충 일정 단일 예약 orchestration 분리

- 상태: 학생·학부모 단일 일정 안내의 연락처 검사, 기존 reserve 호출, 확정/변경 결과 상태·문구 조립을 `supplementNotificationOrchestration.js`로 분리했다.
- 동작 보존: 번호 정규화, 실패 감사 row, 실제 예약, 한국 시각 표시는 기존 App 함수를 주입한다. 예약 사유와 학생/학부모·확정/변경 문구를 그대로 유지한다.
- 검증/gate: scheduled, dry_run, provider 실패, 연락처 누락 fixture를 `npm run test:production`에 연결했다. 실제 API/Solapi 호출은 없어 추가 사람 gate가 없다.

## 2026-07-21 P1. 11B-4 notification job 배치 취소 orchestration 분리

- 상태: 여러 활성 job의 병렬 취소와 반환 job 수집을 `cancelNotificationJobsRequest`로 분리했다.
- 동작 보존: selector가 고른 같은 배열, 기존 취소 함수, 기존 사유를 주입한다. 빈 배열은 API 호출 없이 빈 결과를 반환하고 개별 응답에 job이 없는 항목은 기존처럼 제외한다.
- side effect/gate: 실제 API/Solapi는 fixture에서 호출하지 않았다. 기존 취소 호출의 얇은 이동이므로 추가 사람 gate는 없고 11B-1 실제 예약·취소 gate 결과를 유지한다.
...
- 상태: 코드 분리, 자동검증, 리팩터링 후 운영 사람 gate 완료. 전용 브랜치 commit/push 진행.
- 착수 gate: 사용자가 결석보충 작업의 기존 예약·취소 흐름이 정상이라고 확인하고 11B 진행을 승인했다. 코드 이동 전에는 이 리팩터링 세션에서 실제 운영 예약·취소를 실행하지 않았다.
- 범위: `/api/notification-jobs/reserve`, 예약 실패 row 저장, `/api/notification-jobs/cancel` 호출과 반환 job의 React 상태 반영을 `src/domains/notifications/notificationJobApi.js`로 분리했다. `App.jsx`는 기존 `postJson`과 `upsertNotificationJobState`를 명시적으로 주입한다.
- 동작 보존: 학생 11시·학생 일정·학부모 일정의 대상, 예약시각, 선생님 최종 문구 우선순위, job ID/payload, 실패 문구, Solapi 서버 fingerprint/pending reuse/canceled-during-reserve 계약은 변경하지 않았다. 수업일지 예약도 같은 기존 wrapper를 통해 새 API 어댑터를 사용하지만 payload와 서버 경계는 동일하다.
- 회귀 고정: `scripts/test-notification-job-api.mjs`가 예약 성공 payload/상태 반영, 예약 실패 row와 오류 문구, 취소 payload/상태 반영, ID 없는 취소 차단을 네트워크 없이 검증한다. `npm run test:production`에 연결했다.
- 테스트 하네스 보강: Windows CRLF에서만 실패하던 04d-1의 다중 행 문자열 검사를 줄바꿈 정규화 후 같은 기대 문자열로 비교하게 했다. 기대 동작이나 검사 항목은 낮추지 않았다. 최신 main의 54b 계약은 그대로 보존했다.
- 검증: 최신 `origin/main` `41a31943` 기준 `node --check`, `npm run test:production` 364/364, `npm run build`, `git diff --check` 통과. 기존 Vite 500KB chunk 경고만 남는다.
- 운영 사람 gate: 반 미지정 `고태영` 테스트 학생의 학생·학부모 번호가 같은 사용자 통제 번호임을 재확인했다. `11B-1 검증용` 미래 결석보충 task를 만들고 학생 일정(`schedule_reminder`), 학부모 일정(`notice_parent`), 당일 학생 11시(`student_reminder`)를 하나씩 실제 Solapi 예약했다. 최종 검증 세 건 모두 OS row의 type/target/recipient/scheduledAt/previewBody와 Solapi 수신번호·본문 marker가 일치했고 예약 전 그룹 상태는 `SCHEDULED`였다. 취소 후 OS는 세 건 모두 `canceled`, Solapi는 `reason=예약취소`, `statusCode=1070`, 발송 성공 0건이었다. job ID 중복은 없었고 테스트 task는 삭제 후 재조회에서 0건을 확인했다. 첫 취소 표현 진단 1건과 최종 검증 3건을 합친 취소 `notification_jobs` 총 4건은 감사 이력으로 유지한다.
- 다음: `codex/refactor-supplement-11b`에 commit/push한다. 다음 11B 의미 단위는 다시 최신 `origin/main` rebase와 별도 범위·외부 side effect gate 확정부터 시작한다.
...
1. `운영 OS 저장 신뢰성 보강`
   ...
   - 진행 상태: 특강 회차 계획과 안전한 미래 수업일지 명단 반영 통합을 완료했다. 2026-07-27 수업 생성·수정은 휴강/선택형 휴강 보충 기능과 함께 bulk 저장·Supabase 재조회·모달 draft 유지 gate를 보강했고, 과거 일반 수업의 동일 ID·과거 학생 명단·수업기록 보존 휴강 전환도 추가했다. 운영 사람 gate 뒤 남은 수업 복사·취소/되돌리기 저장 gate를 각각 별도 단위로 진행한다.
   - 후속 순서: 학생 신규/Tally 기존 학생 보강과 정규반 미배정 특강 편성은 2026-07-27 완료. 남은 학생/반/Tally 저장 gate -> app_state key별/version -> examPrep/Tally 입력 경쟁 -> 숙제/포털/자료함/보고서 -> 보충/알림 reconcile -> 비연결 버튼.
...
4. `App.jsx 리팩터링 연속 작업`
   ...
   - 다음 세션 시작 규칙: 최신 커밋과 git diff를 확인하고 학생 포털 사람 gate와 보안 gate가 보류 중임을 알린다. 11B-1의 OS row/Solapi 그룹 gate는 통과했으므로 다음 11B 의미 단위의 원천·외부 side effect·추가 사람 gate 필요 여부를 먼저 정하고, 한 단위만 진행한다.
...
- 리팩터링 중단 지점: 10번 포털 표시 구조 audit, 11A 보충 알림 순수 builder/selector 분리, 11B-1 예약·취소 API orchestration 어댑터 분리와 실제 OS row/Solapi 예약·취소 gate까지 완료했다. 다음 11B 의미 단위는 최신 main 위에서 범위와 추가 gate를 새로 확정한다.
- 통과한 리팩터링 사람 gate 1: 삭제 가능한 미래 보충 task와 통제된 학생/학부모 번호로 `학생 알림톡`, `학부모 알림톡`, `당일 학생 11시 알림톡`의 대상·예약시각·선생님 최종 문구, OS/Supabase row, Solapi 그룹, 취소, 새로고침 유지, 중복 없음, 대상 교차 없음을 확인했다.
