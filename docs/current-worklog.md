...
## 2026-07-27 P0. Tally 완전 덮어쓰기·신초봄 기존 특강 초안 초기화 경로

- 상태: 구현·자동검증 완료. 신초봄을 포함한 운영 학생·특강·수업일지 데이터는 실제 저장하지 않았으며 배포 후 명시 실행 gate 대기.
- 사용자 정정: `덮어쓰기`는 빈 Tally 값을 기존 값으로 보강하는 의미가 아니다. 교사가 명시적으로 완전 덮어쓰기를 선택하면 Tally가 소유하는 기존 기본정보와 기존 특강 회차·시간·메모 초안을 삭제하고 Tally 원본으로 교체한다.
...
- 특강 Tally: 연결 전뿐 아니라 이미 Tally 신청에 연결된 활성 특강 명단 카드에도 `기존 데이터 삭제 후 Tally로 다시 덮어쓰기`를 추가했다. 특강 Tally가 소유하는 이름·학교·학년·연락처·특이사항은 빈칸까지 교체하고, 기존 `special_lecture_enrollments.sessionPlans/sessionIds/memo`는 보존하지 않는다. Tally 회차 신청이 비어 있으면 모든 회차를 미선택, 시간·사유·메모를 빈 값, `planReviewedAt`을 미검토로 저장한다.
...
- 보존 경계: 완전 덮어쓰기도 학생 ID·로그인·PIN과 과거 수업·출결을 삭제하지 않는다. 일반 Tally에서 교재·개별 시간표, 특강 Tally에서 정규반·출생연도처럼 해당 Tally 폼이 소유하지 않는 운영 필드도 유지한다. 특강 enrollment를 교체하는 순간 기존 미래 lessons는 자동 수정하지 않고, 열린 회차 모달에서 `회차 계획 및 미래 수업일지 반영`을 다시 명시 저장해야 한다. 잠긴 과거/당일 수업일지와 출결은 기존 보호 규칙을 따른다.
...
- AI 검증: 일반 Tally 완전 교체 시 빈 출생연도·전화·특이사항·정규반 삭제와 ID/로그인/PIN/교재/개별 시간표 보존, 특강 Tally 소유 필드만 교체, 빈 특강 회차가 전 회차 제외·0회·빈 메모·미검토가 되는 계약을 전용 순수 함수 테스트로 확인했다. `npm run test:tally-student-merge`, `npm run test:production` 376건, `npm run build`, `git diff --check`를 통과했다. 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① 일반 Tally에서 기존 테스트 학생을 선택해 `빈칸 유지하고 보강`과 붉은 `기존 기본정보 삭제 후 Tally 덮어쓰기`가 분리돼 있는지 확인한다. ② 붉은 버튼의 확인창에 Tally 빈칸으로 삭제될 항목과 정규반 해제 시 미래 정규 명단 제외가 표시되는지 확인하고, 취소 시 아무 값도 바뀌지 않는지 먼저 본다. ③ 특강관리의 중3 신초봄 활성 카드에서 `기존 데이터 삭제 후 Tally로 다시 덮어쓰기`를 누르면 확인창에 현재 회차가 모두 제거된다는 문구가 보이는지 확인한다. ④ 실제 실행 여부는 현재 신초봄 1~5회차를 정말 초기화할 의도가 맞을 때만 결정한다. 실행하면 회차 모달이 0회 미검토로 열리고 기존 enrollment 메모·시간이 사라지는지 새로고침 후 확인한다. ⑤ 이 단계만으로 과거 수업일지·출결, 미래 lessons, 알림/Solapi, 다른 `창일중·고3` 중복 학생 행이 자동 삭제·변경되지 않으면 통과다.
...
- 특강 편성: `Lesson Hub > 특강관리`의 수동 명단과 Tally 신청 연결 목록은 정규반 배정 여부와 무관한 모든 재원생을 보여 주며 `정규반 미배정`을 명시한다. Tally 신청을 기존 학생과 연결할 때 `학생정보 유지하고 연결`과 `기존 데이터 삭제 후 Tally 덮어쓰기`를 분리했다. 완전 덮어쓰기 범위는 위 최신 작업이 기준이며, Tally 회차/시간은 미검토 초안으로 열어 교사가 회차 모달에서 확인·저장하기 전 미래 수업일지에 자동 반영하지 않는다.
