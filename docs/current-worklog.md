...
## 2026-08-04 시험정보 전체 조회와 미확인 자동행 정리

- 시험관리 `시험정보`에 `전체 반` 필터를 추가해 활성 학생의 학교·학년과 연결된 시험행을 반 구분 없이 중복 제거해 조회한다. `시험 후 제출`로 전환하면 반별 상태로 돌아가 기존 제출 흐름을 유지한다.
- 반·고사 필터 변경 시 `onEnsureExamCycleRows`를 호출하던 자동생성 경로를 제거해 조회만으로 미확인 일정이 다시 만들어지지 않게 했다.
- 운영 Supabase의 2026년 2학기 자동생성 23행을 대조해 확인 일정 11행은 보존하고, 범위·부교재·총평·메모·수학 시험일이 없는 미확인 12행만 단일행 감사 삭제했다. 재조회 결과 전체 64→52행, 삭제 대상 잔존 0건이며 시험대비 수업은 0건이었다.
- 검증: 시험정보 전용 fixture, runtime lint, production `827/827`, build `410 modules`·lazy `12/12`, 로컬 안전 브라우저에서 전체 반 선택·탭 전환을 통과했다. 첫 원격 검사가 탭 선택 setter 정적 계약을 발견해 기존 setter를 유지하도록 교정했다.
