...
- 상태: 코드 수정/자동검증 완료, 배포 후 운영 사람 검토 및 신초봄 동명이인 원천 선택 대기.
- 사용자 요청: 클리닉 특강 신초봄 학생이 시간이 지난 1회차도 신청해야 하나 잠겨 있고, 13:00~15:00처럼 여러 회차의 시간이 같은 경우 회차마다 시간을 반복 입력하는 불편을 줄인다.
- 운영 원천 확인: `신초봄` 학생이 두 row로 존재한다. Tally 특강 신청과 연결된 `창일중 · 중3` 특강 전용 학생은 수강 회차 0개이고, 별도 `창일중 · 고3` 학생은 클리닉 2~5회차를 13:00~15:00으로 저장했다. 1회차 수업에는 기존 학생 3명, 출석 record 2건, 알림 job 0건이 있으며 신초봄은 없다. 동명이인 원천이 확정되지 않아 이 세션에서 운영 enrollment/lesson을 자동 변경하지 않았다.
...
- 사람 gate: 배포 후 클리닉 특강의 두 신초봄 행에서 학교·학년 경고를 확인하고 올바른 원천을 먼저 결정한다. 올바른 학생의 1~5회차를 체크하고 공통 13:00~15:00을 한 번 적용해 저장한다. 새로고침 후 같은 계획이 유지되는지 확인한 뒤, 미리보기에서 기존 학생/시간이 변하지 않고 신초봄만 추가되는 회차에 한해 `오늘/과거 회차 누락 학생 추가`를 실행한다.
