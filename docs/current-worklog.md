...
- 같은 저장 화면의 체크박스 대신 `휴강 보충 알림 관리` 버튼을 두고 전용 모달에서 수신 대상을 선택하게 했다. 모달의 `일정 저장 후 알림 예약`을 누르면 서버 재확인 후 학생·학부모 다음 정각 안내와 학생 당일 11시 알림을 일괄 예약한다. 실제 운영 알림 없이 safe fixture로 수정 화면·2명 수업·알림 3종·고정 ID를 검증했다.
