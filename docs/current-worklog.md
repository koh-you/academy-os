...
### 2026-07-08 P1. 자동저장 위험 화면 경고 UI와 수업일지 저장 구조 재설계

- 상태: 대기 - 후속 구현 필요
- 사용자 요청: 현재 자동저장이 위험한 화면들을 지침에 남기고, 나중에 해당 화면을 붉은 경고 UI로 표시한다. 경고 버튼을 누르면 무엇이 위험한지 설명해야 한다.
- 위험 화면 1순위: 수업일지와 수업알림톡. 현재 수업일지 입력 저장이 `lesson_student_records` 저장뿐 아니라 `notification_jobs` 갱신, Solapi 예약/취소, 알림 상태 patch와 연결될 수 있어 운영 사고 위험이 가장 크다.
- 위험 화면 2순위: 전역 `app_state` 자동저장. 작은 변경도 여러 key snapshot을 함께 저장할 수 있어 오래된 요청이 최신 값을 덮는 위험이 있다.
- 위험 화면 3순위: 시험정보/시험 후 기록지, 학생 프로필, 보충 task, 학교 일정처럼 입력마다 API를 호출하거나 큰 row를 통째로 저장하는 화면.
- 후속 UI 방향: 고위험 화면 상단 또는 편집 영역 가까이에 붉은 `자동저장 위험` 배지를 붙인다. `왜 위험한가` 버튼을 누르면 저장 원천, 자동 부작용, 중단 조건, 권장 수정 방향을 보여준다. 안전한 저장 구조로 바뀐 화면은 경고를 제거하고 제거 근거를 작업 기록에 남긴다.
- 수업일지/수업알림톡 권장 구조: `수정` 버튼을 눌러 local draft 편집 모드로 들어가고, 변경 행을 모아 `저장` 버튼으로 Supabase에 저장한다. 저장 완료 확인 뒤 별도 `예약 확인/예약 생성/발송` gate로 넘어간다. 수업일지 저장 버튼이 알림톡 예약/발송까지 자동 실행하면 안 된다.
- 일괄 저장의 남은 위험: 새로고침/탭 종료 시 미저장 draft 손실, 다중 탭 충돌, 일부 행 저장 실패, 오래된 데이터 덮어쓰기, 저장되지 않은 draft를 발송 미리보기로 착각하는 위험이 있다. 따라서 변경 행만 저장, 행별 성공/실패 표시, 저장 전후 diff, 서버 `updatedAt`/version 충돌 확인, 발송 전 재검수 gate가 필요하다.
- 이번 기록 범위: 지침/작업 큐 등록만 완료했다. 실제 붉은 경고 화면 구현과 수업일지 자동저장 제거는 별도 작업으로 진행한다.
