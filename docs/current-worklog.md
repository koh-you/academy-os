...
## 2026-07-27 P0. 휴강 수업일지·선택형 휴강 보충 생성

- 상태: 구현·자동검증 완료. 운영 배포 후 사람 화면·Supabase 새로고침 gate 대기.
- 사용자 요청: 수업 등록 유형에 `휴강`을 추가하고, 휴강 보충 유무를 물은 뒤 필요하면 보충 수업일지를 함께 생성한다. 휴강과 보충 모두 수업일지에서 확인할 수 있어야 한다.
...
- 화면: 수업 등록 모달의 6번째 유형으로 `휴강`을 추가하고 `보충 없음 / 보충 수업일지 함께 생성`을 명시 선택하게 했다. 보충 선택 시 날짜·시작·종료를 편집하며 기본 날짜는 휴강 7일 뒤다. 달력에는 회색 `휴강 · 수업명` pill과 별도 `휴강` 필터를 제공한다. 휴강/휴강 보충 수업일지 상단에는 실제 수업·정산 포함 여부와 연결 일정을 안내한다. 휴강은 태블릿 등하원 수업 후보에서 제외하고 휴강 수업일지의 학생별 출결 버튼도 `휴강`으로 잠가 잘못된 출석 기록이 붙지 않게 했다.
- 저장 신뢰성: 수업 등록·수정은 더 이상 로컬 화면을 먼저 닫고 백그라운드 POST하지 않는다. 모달 안에서 `저장 중 -> Supabase 반영 확인 중 -> 저장 완료/저장 실패`를 표시하고, `/api/lessons/bulk` 저장 뒤 `/api/lessons`를 다시 조회해 ID·유형·주제·연결값·날짜·시간·명단·상태가 모두 일치해야 완료 처리한다. 등록 모달이 열린 동안 휴강과 보충 lesson ID를 고정해 POST 성공 뒤 재조회만 실패한 경우 다시 저장해도 새 중복 ID를 만들지 않는다. 실패 시 모달과 입력값을 유지하고, 완료 후에는 중복 등록을 막기 위해 입력을 잠근 뒤 `달력에서 확인`으로 닫는다.
- 정산 경계: `closure`는 학생별 정규 수업 횟수·시수와 월 정산 대상 명단에서 제외한다. 함께 만든 `makeup`은 기존처럼 보충 횟수·시수 참고값에는 표시되지만 정규 월 고정금액 계산은 바꾸지 않는다.
- 외부 영향: 구현·테스트 중 운영 lessons를 생성하지 않았다. 수업 등록만으로 `notification_jobs`, Solapi 알림톡/문자 예약·발송·취소, 출결, 숙제, 보충 task를 생성하거나 변경하지 않는다. 새 SQL은 없다.
- AI 검증: 휴강·연결 보충 UI/저장/재조회/달력·수업일지 연결 계약을 production 시나리오 377건에 추가했다. 월 정산 테스트에는 휴강이 정규 횟수에서 제외되고 휴강 수업일지만 있는 학생이 정산 명단에 들어오지 않는 fixture를 추가했다. `npm run test:monthly-settlement`, `node scripts/scenario-tests-production.cjs`, `npm run test:production`, `npm run build`, `git diff --check`를 통과했다. 빌드에는 기존 Vite chunk size 경고만 남았다.
- 사람 gate: ① 운영 `수업일지 > + 수업 등록`에서 `휴강`을 선택하고 `휴강 보충이 있나요?`가 보이는지 확인한다. ② 삭제 가능한 테스트 반/날짜로 먼저 `보충 없음` 휴강을 저장해 `저장 중 -> Supabase 반영 확인 중 -> 저장 완료` 후 달력의 회색 `휴강 · 수업명`과 휴강 수업일지 안내가 보이는지 확인한다. ③ 새로고침 후 같은 휴강이 남고 월 정산의 정규 횟수·시수에는 포함되지 않는지 확인한다. ④ 다른 삭제 가능한 테스트 일정에서 `보충 수업일지 함께 생성`을 선택해 보충 날짜·시간을 정하고 저장한다. ⑤ 휴강 pill과 별도 `수업명 · 휴강 보충` pill이 각각 열리며 서로의 원 휴강/연결 보충이 표시되는지 확인한다. ⑥ 이 과정만으로 알림관리 예약 건수나 Solapi 내역, 출결·숙제가 변하지 않으면 통과다.
...
1. `운영 OS 저장 신뢰성 보강`
   ...
   - 진행 상태: 특강 회차 계획과 안전한 미래 수업일지 명단 반영 통합을 완료했다. 2026-07-27 수업 생성·수정은 휴강/선택형 휴강 보충 기능과 함께 bulk 저장·Supabase 재조회·모달 draft 유지 gate를 보강했다. 운영 사람 gate 뒤 남은 수업 복사·취소/되돌리기 저장 gate를 각각 별도 단위로 진행한다.
