...
- 구현 결과: 산출물 패널의 카드 미리보기를 `블로그형 카드 디자인 Gate 3`로 전환했다. 4:5 카드 프레임 안에 블루/화이트 전문 톤, 브랜드 헤더, 시험 메타, 큰 제목, 요약/구조/총평/문제 슬롯/손풀이 슬롯/CTA 영역을 카드 유형별로 다르게 렌더링한다.
...
- 운영 경계: 이번 gate는 실제 PNG export, crop 이미지 업로드, Canva 대체 확정이 아니다. 사람이 카드 수, 반복 구조, 제목/본문 밀도, 문제/손풀이 슬롯 크기, 모바일 블로그 삽입 가독성을 눈으로 확인하는 단계다.
...
- 구현 결과: `카드뉴스 미리보기 Gate 2`를 추가했다. 현재 `outputDrafts.inputs`, 주요문항 반복 블록, 최종 검수 모델을 읽어 1080x1350 비율의 카드뉴스 순서/문구/슬롯을 화면에서 먼저 확인한다.
...
- 사람/AI/코드 역할:
  ...
  - JS/React 렌더러: 확정 데이터와 선생님 저장본을 읽어 1080x1350 카드 10장을 미리보기/PNG/ZIP으로 렌더링한다.
