...
1. `운영 OS 저장 신뢰성 보강`
   ...
   - 첫 착수 단위: 수업 생성/수정/복사/취소 저장 gate. 화면을 먼저 바꾸는 fire-and-forget을 제거하고 API 완료/실패/draft 유지부터 고친다.
...
2. `모달 통일 작업`
   - 상태: 부분 진행 - 보충관리 등원보충 모달 1차 UI 정리, 결석보강 생성/상세 모달 1차 맥락 정리, 결석보강 알림톡 제목/본문 seed 정리, 실제 보강 수업 상세 화면과 생성 모달 문구 통일을 완료했다. 특강관리는 페이지 상위에 `특강 수업`과 `특강 안내문` 탭을 두고, 전체 반 학생 직접 연결/특강 전용 학생 등록, Tally/수동 접수 공통 학생별 회차 모달과 학생별 진행 모달을 추가했다. 회차 모달은 저장 중/완료/실패를 내부에 표시하고 Supabase 재조회 일치 후에만 완료 처리한다. 전체 모달 inventory/나머지 공통 구조 통일은 앞으로 해야 할 작업.
...
### 2026-07-20 P0-1. 공통수학2 특강 1회차 일정 변경 운영 반영

- 사용자 요청: `2026 고1 공통수학2 유형 문제풀이`의 기존 1회차 `2026-07-21 13:00-16:00`을 `2026-08-13 16:00-19:00`으로 변경한다. 연결된 활성 특강생 6명은 실제 수강생으로 유지한다.
- 운영 처리: Supabase `app_state.specialLectureGuides`의 기존 1회차를 `2026-08-13`, 목요일, `16:00-19:00`으로 갱신했다. 실제 회차 날짜 범위에 맞춰 안내문 기간도 `2026-07-23`부터 `2026-08-13`까지로 변경했다.
- 재조회 검증: `/api/special-lecture-guides` 재조회 원천이 `supabase`인지 확인하고, 이동된 회차의 날짜/시간과 기간 시작·종료가 수정본과 일치하는지 대조했다. 연결된 `lessons`의 고정 1회차 ID도 `2026-08-13 16:00-19:00`으로 일치했다.
