...
- 운영 재시도에서 실제 과제 상태가 같아도 `checkedAt`만 클릭 시각으로 계속 달라져 12건이 동일 필드 충돌로 남는 것을 확인했다. 이미 같은 상태가 적용된 숙제는 최신 `checkedAt`을 보존해 열린 구버전 탭도 저장 가능하게 하고, 이후 클라이언트는 의미 변화가 없는 숙제 쓰기를 만들지 않는다.
