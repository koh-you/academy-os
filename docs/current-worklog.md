...
- 레이아웃 판단: 벤치마킹 카드뉴스는 PNG 차트만 붙여넣은 구조라기보다, AI가 배경/아이콘/문항 이미지/해설 박스/강조 박스를 한 장 단위로 합성한 `완성형 카드`에 가깝다. 따라서 현재처럼 빈 슬롯에 PNG를 끼우는 방식만으로는 벤치마킹 느낌이 약할 수 있다.
- 다음 설계 방향: Canva는 `공통 배경/브랜드/카드 제목/강조 박스/이미지 자리`를 가진 10장 템플릿으로 유지하되, 각 카드 안의 시험 구조·총평·주요문항·손풀이 영역은 AI가 만든 완성형 카드 이미지 또는 Canva 요소로 한 번에 들어갈 수 있는지 검토한다.
- 비교할 제작 방식:
  1. `웹앱 PNG 생성 -> Canva에 삽입`: 저장 원천과 반복성이 좋지만 카드가 조립식으로 보일 수 있다.
  2. `AI 완성형 카드 이미지 생성 -> Canva에 배치`: 벤치마킹 느낌이 강하지만 문항/수치 오류와 수정 어려움이 커서 선생님 검수 gate가 필수다.
  3. `Canva 템플릿 + AI 텍스트/이미지 초안`: 가장 운영 안정적이다. 배경/레이아웃은 Canva가 고정하고, 텍스트와 일부 카드 이미지만 AI/웹앱이 만든다.
- 다음 검수 항목: 실제 테스트 run으로 블로그 18개 강사 섹션 입력, 블로그/인스타 초안 생성, 선생님 저장본 우선순위 확인, 산출물 ZIP 확인, Canva 10장으로 페이지 추가, 벤치마킹형 카드 1~3장을 실제로 만들어 가독성/수정 가능성/반복성을 비교한다.
- 중단 조건: Canva가 8장이라 앱 기준을 8장으로 낮추는 판단, AI가 문항 수/풀이/정답/유사문항을 새로 추측하는 결과, 선생님 저장본이 AI 재생성으로 덮이는 결과, Canva에서 수정 불가능한 통이미지만 남아 반복 제작성이 떨어지는 결과가 보이면 다음 단계로 가지 않는다.
...
- 다음 gate: 실제 테스트 run에서 18개 섹션 중 일부를 채워 블로그/인스타 초안을 생성하고, Canva에 10장 구조를 맞추기 위해 현재 8장 템플릿에 2장을 추가한 뒤 PNG/문항 이미지 삽입 가독성을 검토한다.
...
- 이번 작업 결과: 산출물 입력칸을 기존 4개에서 6개로 확장했다. 추가 입력은 `10장 이미지/슬롯 메모`, `학교별 변주/홍보 메모`이며, 새 SQL edit 없이 기존 `exam_analysis_runs.audit_summary.outputDrafts.inputs` JSON에 저장한다.
...
- 다음 gate: 실제 테스트 run에서 6개 입력칸을 채우고 블로그/인스타 초안을 생성한 뒤, 10장 Canva 템플릿에 산출물 PNG/문항 이미지 10개를 넣어 가독성과 반복 사용 가능성을 확인한다.
...
- Canva 판단: 현재 Canva 디자인은 반복 사용 가능성을 검토하는 별도 gate가 필요하다. 카드별 고정 역할, PNG 슬롯 크기, 텍스트 최대 길이, 대각선 리사이즈 가능 여부, 다음 시험에서 바꿀 데이터 영역과 고정 브랜드 영역을 구분해 본다. 시험지 정보 이미지가 10개이므로 Canva도 10장 구조로 고정하고, 실제 산출물 PNG/문항 이미지를 1~10번 카드 슬롯에 넣어 모바일/블로그 가독성을 확인한 뒤 웹앱 PNG 비율을 조정한다.
