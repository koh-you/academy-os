...
- 상태: 1~3번 구현과 대상 자동검증·데스크톱/모바일 CSS 렌더 완료. 전체 production 검증·운영 배포 후 사람 화면 gate 대기. 사용자가 분석만 요청한 4번 프롬프트 출력 구조는 변경하지 않았다.
...
- AI 검증: 쎈 `unitNo`와 `mainTypeCode` 복구 순서 fixture, 전체 벤치마크 문장 옵션, 접힘 카드, 17px checkbox 계약을 대상 테스트에 추가했다. `node scripts/test-exam-analysis-final-preview.mjs`, `node scripts/test-exam-analysis-prompt-studio-ui.mjs`, `npm run build`를 통과했다. 실제 CSS fixture를 Chrome으로 렌더해 데스크톱 1536×900에서 체크박스가 작고 정렬되며 두 필수 카드만 열리고 나머지 세 카드가 한 줄로 접히는 것을 확인했다. 모바일 390×844에서도 한 열 접힘 구조와 전체 문장 드롭다운을 확인하고 긴 select/summary의 최소 폭을 제한했다. 기존 Vite chunk size 경고만 남았다.
