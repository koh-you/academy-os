...
- 일반 Tally 등록: `학생 추가 > Tally 접수`에서 `새 학생으로 등록` 또는 기존 재원 학생을 `Tally 반영 대상`으로 명시 선택한다. 이름이 같고 학교·학년이 충돌하지 않는 재원생이 한 명뿐이면 후보를 먼저 제안하되, 동명이인이나 학교·학년 충돌은 자동 선택하지 않는다.
...
- 특강 편성: `Lesson Hub > 특강관리`의 수동 명단과 Tally 신청 연결 목록은 정규반 배정 여부와 무관한 모든 재원생을 보여 주며 `정규반 미배정`을 명시한다. Tally 신청을 기존 학생과 연결할 때 `학생정보 유지하고 연결`과 `Tally 정보 덮어쓰기 후 연결`을 분리했다. 덮어쓰기를 선택하면 기본정보 저장·Supabase 재조회 후 enrollment를 저장하고, Tally 회차/시간은 미검토 초안으로 열어 교사가 회차 모달에서 확인·저장하기 전 미래 수업일지에 자동 반영하지 않는다.
