...
1. `모달 통일 작업`
   - 상태: 부분 진행 - 보충관리 등원보충 모달 1차 UI 정리, 결석보강 생성/상세 모달 1차 맥락 정리, 결석보강 알림톡 제목/본문 seed 정리, 실제 보강 수업 상세 화면과 생성 모달 문구 통일을 완료했다. 특강관리는 `특강 안내문`과 `명단·회차 관리` 탭을 분리하고 Tally/수동 접수 공통 학생별 회차 모달과 학생별 진행 모달을 추가했다. 전체 모달 inventory/나머지 공통 구조 통일은 앞으로 해야 할 작업.
