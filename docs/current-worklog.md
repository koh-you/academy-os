...
4. `App.jsx 리팩터링 연속 작업`
   ...
   - 현재 이어받을 지점: 9번 `test manager`는 완료했다. 10번 `student-parent portals` inventory, 공용 읽기 전용 `PortalMaterialsTab`, 정적 빈 상태/평가 탭 분리를 완료했으며, 다음은 저장·인증과 무관한 공용 탭 내비게이션부터 이어간다.
