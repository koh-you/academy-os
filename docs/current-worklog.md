...
- 신초봄 원천 확인: 운영에는 `창일중·중3` 특강 Tally 학생과 `창일중·고3` 일반 Tally 학생 두 행이 남아 있다. 중3 특강 enrollment에는 수기로 확정한 1~5회차 13:00~15:00가 있고, 연결된 특강 Tally 원본의 회차 신청은 비어 있다. 배포 후 중3 신초봄 카드에서 완전 덮어쓰기를 실행하면 enrollment의 기존 1~5회차·시간·메모 초안은 제거되고 0회 미검토 상태가 된다. 서로 다른 학생 ID의 과거 중복 행 통합/삭제는 출결·수업일지 이관 작업이므로 이번 버튼이 자동 수행하지 않는다.
