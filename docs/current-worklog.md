...
- AI 검증: `git diff --check`, 서버·알림 경로와 `미기재` 정적 계약, `npm run test:production` 372건, `npm run build`를 통과했다. `미기재`가 명시 값으로 normalize되고 숙제 원천 동기화를 건너뛰며 프론트 미리보기·예약 payload·서버 발송 직전·최종 템플릿 조립에서 과제 블록을 제외하는 경로를 모두 대조했다. 기존 Vite chunk size 경고만 남았다.
