...
- 수정: 미래 회차의 학생 추가·시간 변경은 기존 lesson, 다른 학생 명단·시간, 모든 `lesson_student_records`를 그대로 보존하고 대상 학생의 `studentIds`·`specialLectureStudentSchedules`만 교체한다. 다른 학생 기록은 더 이상 차단 사유가 아니다. 오늘/과거 수업, 완료 수업, 대기 알림은 계속 보호하고, 학생을 회차에서 제외할 때는 바로 그 학생의 기록이 있을 경우에만 제거를 차단한다.
...
- AI 검증: production 시나리오에 대상 학생 기록만 제거 보호에 사용하는 계약, 다른 학생 출결 보존, 학생별 시간·유형·사유까지 비교하는 카드 상태, 실제 차단 사유 표시를 추가했다. `npm run test:production` 385건, `npm run build`, `git diff --check`를 통과했으며 빌드에는 기존 Vite chunk size 경고만 남았다.
