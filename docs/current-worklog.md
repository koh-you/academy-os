...
- 배포 확인: 기능 커밋 `4f087b69`와 health 표식 커밋 `dd955f9b`를 `origin/main`에 푸시했다. 운영 프론트가 JS `/assets/main-KxnhevJ0.js`, CSS `/assets/main-CR09Fsf2.css`로 전환됐고 번들에서 `not_entered`, `미기재`, `발송 결과 반영 완료`를 확인했으며 제거 대상 `발송 전 점검`과 `.preSendCheckButton`은 없었다. Render `/health`도 `lessonJournalNotificationFollowup=result_reconciled_unrecorded_omit`, `ok=true`를 반환했다.
