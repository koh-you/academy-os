...
- 경계: App은 `examPrepRowSaveRequestRef`, React save-state setter와 14B-1 API request callback을 주입한다. row 값 계산, Supabase transport, 삭제·lesson reconcile은 controller가 알지 못한다.
- 자동검증: empty 입력, 복수/중복 row ID, 성공, 실패, 동일 row의 겹친 deferred 요청을 검증했다. 이전 요청이 먼저 끝나도 최신 `saving`을 덮지 않고 최신 요청 완료만 `saved`가 되는 fixture를 production test에 연결했다. 실제 API 호출은 0회다. production scenario 476/476, build, `git diff --check`가 통과했다.
- 사람 gate: 순수 callback/request-order 경계라 새 사람 화면 조작은 필요하지 않다. 다음은 삭제 시 연결 시험대비 수업의 저장·삭제 계획을 순수 계산으로 먼저 분리하고, 실제 row/lesson mutation은 격리 사람 gate 전 이동하지 않는다.
...
- 경계: App은 resolved AI provider/model/prompt, autosave 위험 정보, 현재 날짜, exam row/save state와 실제 `onUpdateRow` callback을 주입한다. 시험정보 저장 원천, 삭제, 수업 reconcile orchestration은 이동하지 않았다.
- 동작 보존: 닫기 전 pending draft flush, section/원문 입력, AI 수정·복사 상태, `InlineSaveStatus`, autosave 위험 안내의 기존 표시와 callback 순서를 유지했다.
- 자동검증: production 계약이 App 내부 legacy modal 제거, 전용 파일의 hook/상태/DOM 조립, App 소유 설정·저장 callback 주입을 검사한다. 실제 AI·clipboard·Supabase·운영 데이터 호출은 0회다. production scenario 474/474, build, `git diff --check`가 통과했다.
- 사람 gate: 정적 callback 계약과 기존 fake fixture로 검증 가능한 구조 이동이라 새 사람 화면 조작은 필요하지 않다. 다음은 시험정보 저장·삭제·수업 reconcile 원천과 side effect를 다시 inventory하고, 격리 데이터가 필요한 사람 gate를 먼저 설계한 뒤 한 단위만 이동한다.
...
- 경계: App은 `AI 수정 중 -> 성공 revisedReview/provider -> 실패 message` 상태 전이와 `onUpdateRow` 저장을 계속 소유한다. adapter는 React state, exam row 저장, Supabase를 소유하지 않는다.
- 동작 보존: provider/model/설정 prompt, 교사 audience, 학년·시험일·학교/과목명, `spellingOnly`, raw draft와 기존 성공/오류 문구를 그대로 전달한다.
- 자동검증: fake fetch fixture가 URL, POST header/body, 전체 payload, 성공 result, 서버 오류와 기본 오류를 검증한다. 실제 유료 AI 호출은 0회다. production scenario 471/471, build, `git diff --check`가 통과했다.
- 사람 gate: 외부 AI를 호출하지 않고 transport 계약을 fixture로 검증했으므로 사람 화면 조작은 필요하지 않다. 다음은 총평 modal의 local React state/action을 전용 hook 또는 component 경계로 분리한다.
...
- 경계: App이 대상 builder, 학교/학년 matcher, 일정/문구 formatter, 파일 URL builder, 대상 app_state setter와 교사 확인 callback을 모두 주입한다. 새 component에는 state/effect/fetch/postJson/API 경로가 없다.
- 자동검증: 기존 인증·Storage·read-after-write scenario를 유지하면서 새 파일/App 연결, 기존 문구·버튼, 외부 boundary 주입과 side effect 부재를 `22f-2`로 추가했다. production 469/469, build, `git diff --check`가 통과했다.
- 사람 gate: 실제 학생 제출·파일 업로드/열기·교사 확인을 실행하지 않았다. 학생 포털 실제 쓰기와 bearer/Storage 보안 gate는 계속 보류이며 이번 표시 분리 통과로 대체되지 않는다.
- 다음: 시험 후 총평 modal의 순수 draft 생성·normalize·section helper를 먼저 분리하고, timer/저장/AI action은 다음 gate로 남긴다.
...
- 동작 보존: 빈 본문/대상/예약시각/busy guard, 과거시각 중단, 수신자별 scheduled job 생성·Solapi 예약, 예약 성공 local upsert, 예약 실패 job 생성·OS 저장·저장 실패 보고, 결과 안내·filter·busy 복구를 그대로 유지했다.
- 저장 원천/side effect: callback을 실제로 누르면 기존 Solapi 예약과 `notification_jobs` 저장이 실행된다. 이번 리팩터링은 호출 순서·payload·실패 fallback을 바꾸지 않았고, 검증은 injected reserve/persist/upsert/refresh mock만 사용해 운영 DB·번호·Solapi를 호출하지 않았다.
- 자동검증: guard·과거시각·다중 수신자 성공·부분/전체 예약 실패·실패 job 저장·저장 재실패·local upsert·feedback/filter/busy 상태 전이를 포함한 action fixture와 App→hook→action 경계를 production scenario 461개로 고정했다. production scenario 461/461와 build, `git diff --check`가 통과했다.
- gate 판정: 예약 성공·부분 실패·실패 저장 재실패까지 mock으로 재현했고 화면 DOM/문구/payload 원천은 바뀌지 않았다. 재시험/고태영 운영 데이터, 실제 번호·예약·발송, 사람 화면 검수가 필요하지 않은 AI gate다.
- 다음 경계: App의 Solapi 결과 대조 wrapper를 history hook에서 기존 `reconcileNoticeResultsAction`과 injected reconcile/refresh callback으로 결합할 수 있는지 확인한다.

## 2026-07-23 P1. 13F-6 알림 즉시발송 action binding 분리 — AI gate 통과

- 코드: App의 `sendNoticeNow` wrapper를 `useNotificationComposerState`로 이동했다. App은 timeout 판별, 수신자, persist callback, 45초 즉시발송 request, history/filter setter를 주입하고 hook은 기존 builder·draft·busy·refresh callback과 `sendNoticeNowAction`을 결합한다.
- 동작 보존: 빈 본문/대상 없음/busy guard, 수신자별 immediate job 생성, 발송 성공·dry-run, 45초 timeout의 `send_unconfirmed`, 일반 실패, 결과 job 저장, 기록 화면/filter 전환, 최종 busy 해제를 그대로 유지했다.
- 저장 원천/side effect: callback을 실제로 누르면 기존 알림톡 API와 `notification_jobs` 저장이 실행된다. 이번 리팩터링은 호출 순서·payload를 바꾸지 않았고, 검증은 injected send/persist/refresh mock만 사용해 운영 DB·번호·Solapi를 호출하지 않았다.
- 자동검증: guard, 다중 수신자 성공·dry-run, timeout, 발송 실패, 기록 저장 실패, refresh, feedback/filter/busy 상태 전이를 포함한 action fixture와 App→hook→action 주입 경계를 production scenario 460개로 고정했다. production scenario 460/460와 build, `git diff --check`가 통과했다.
- gate 판정: 외부 경계와 모든 주요 결과 분기를 mock으로 재현했고 화면 DOM/문구/payload 원천은 바뀌지 않았다. 재시험/고태영 운영 데이터, 실제 번호·발송, 사람 화면 검수가 필요하지 않은 AI gate다.
- 다음 경계: App의 예약발송 wrapper를 composer hook에서 기존 `scheduleNoticeAction`과 injected persist/reserve callbacks로 결합할 수 있는지 확인한다.
...
- 동작 보존: 각 state의 기존 초기값, local+서버 job 병합/필터 계산, filter 선택 시 history workspace 전환·펼침, 같은 ID를 제거하고 최신 80건을 유지하는 local upsert를 그대로 유지했다. App은 hook의 결과와 setter를 기존 발송·예약·대조·삭제·취소 action에 주입한다.
- 저장 원천/side effect: hook은 React local display state와 순수 model/action만 소유한다. Supabase `notification_jobs` 저장·삭제·조회, Solapi 예약·취소·결과 조회를 직접 실행하지 않는다.
- 자동검증: 기존 history model/filter/upsert fixture를 유지하고, hook의 여섯 state·model/action 조합·App 연결과 fetch/postJson/API/notification_jobs 부재를 production scenario 453개로 고정했다. production scenario 453/453와 build, `git diff --check`가 통과했다.
- gate 판정: local 전이와 파생 목록은 deterministic fixture·정적 hook 경계로 재현되며 외부 요청은 App에 남아 있다. 재시험/고태영 운영 데이터나 실제 Solapi 상태, 사람 화면 검수가 필요하지 않은 AI gate다.
- 다음 경계: 공지 제목·본문·template/meta·예약일시 draft와 busy/message state를 외부 요청 없이 전용 composer state hook으로 묶을 수 있는지 확인한다. 특강 안내문 적용 문구와 handler는 이동하지 않는다.
...
- 동작 보존: 특강 탭을 노출할 때 initial tab을 사용하고 숨길 때 항상 `notice`를 선택하며, 관련 prop이 바뀔 때 active tab을 다시 동기화한다. 특강 내부 초기값 `roster`, 공지 내부 초기값 `compose`를 그대로 유지했다.
- 저장 원천/side effect: hook은 컴포넌트 local navigation state만 소유한다. API/Supabase/notification job/Solapi 조회·저장·발송·예약 side effect가 없다.
- 자동검증: 특강 노출/숨김의 활성 탭 결과를 equality fixture로 고정하고, hook의 세 state·prop effect·App 연결과 외부 호출 부재를 production scenario 450개로 고정했다. production scenario 450/450와 build, `git diff --check`가 통과했다.
- gate 판정: state 초기값과 prop 동기화 입력 전체를 순수 fixture·정적 경계 검사로 재현했다. 재시험/고태영 운영 데이터나 실제 발송, 사람 화면 검수가 필요하지 않은 AI gate다.
- 다음 경계: 수신자 filter/search/mode/선택 local state와 기존 순수 recipient model·선택 정리 effect를 전용 hook으로 묶을 수 있는지 확인한다.
...
- 동작 보존: 기존 선택 순서 보존, 현재 반 밖 ID 제거, 미선택 학생은 배열 끝에 추가, 선택 학생은 같은 ID 전체 제거, 전체 선택은 현재 `searchableStudents` 순서대로 ID를 쓰는 계약을 그대로 유지했다. `선택 해제`는 기존 빈 배열 setter를 유지한다.
- 저장 원천/side effect: 선택값은 NotificationCenter local React state뿐이며 API/Supabase/notification job/Solapi side effect가 없다. fixture는 학생 ID 배열만 사용한다.
- 자동검증: 선택 정리 순서, 신규 toggle 추가, 중복 포함 기존 선택 제거, 전체 선택 순서를 `test:notification-center-model`로 고정했다. production scenario 441/441과 build, `git diff --check`가 통과했다.
- gate 판정: 순수 배열 변환을 deterministic equality로 검증했다. 화면이나 재시험/고태영 운영 데이터의 사람 조작이 필요하지 않은 AI gate다.
- 다음 경계: 일반 공지 template 선택 시 template ID/kind/special meta/title/body를 바꾸는 local action을 주입형 action으로 분리한다. 특강 템플릿 원문·외부 연결은 바꾸지 않는다.
...
- 동작 보존: `알림관리에서 예약 취소` 사유, `notificationJob.status === "canceled"` 성공 조건, Solapi 실제 취소/그룹 없음 완료 문구, callback 미연결·OS 취소 미확인 오류, 성공 뒤 local upsert와 기록 펼침, 성공·실패 모두 busy 해제를 그대로 유지한다.
- 저장 원천/side effect: action은 endpoint/fetch/React/Supabase/Solapi client를 직접 소유하지 않는다. 실제 Supabase/Solapi 취소 orchestration은 주입된 기존 callback 경계에 남는다. fixture에서는 callback과 모든 state setter를 mock해 실제 예약 취소·운영 row 변경이 0건이다.
- 자동검증: Solapi 그룹 취소 성공, 그룹 없는 OS-only 취소, callback 사유·결과 검증, 잘못된 취소 결과와 후속 state 차단, callback 미연결, 삭제/취소 중 guard, confirm 거절, 성공·실패 `finally`를 `test:notification-notice-actions`로 고정했다. production scenario 438/438과 build, `git diff --check`가 통과했다.
- gate 판정: 외부 취소 callback을 mock해 UI orchestration의 모든 분기를 실제 호출 없이 재현했다. 재시험/고태영 운영 데이터나 사람 조작이 필요하지 않은 AI gate다.
- 다음 경계: 완료된 발송·예약·대조·AI·삭제·취소 action 뒤 App에 남은 background refresh 오류 처리와 공지 template/config·local selection helper를 낮은 위험도부터 한 의미 단위씩 분리한다.
...
- 동작 보존: 모든 job을 먼저 만든 뒤 순차 예약하고, 성공한 서버 반환 job을 local state에 반영하며, 실패 시 원래 job에 `failed`·오류·현재시각을 붙여 local 반영 후 기록 저장을 시도하는 순서를 그대로 유지한다. 실패 row 기록 자체가 실패해도 로그만 남기고 다음 대상을 처리한다.
- 저장 원천/side effect: action은 endpoint, React, Supabase 또는 Solapi client를 직접 소유하지 않는다. App의 기존 `/api/notification-jobs/reserve`와 `/api/notification-jobs` wrapper가 주입되며 이번 검증에서는 실제 예약·발송·운영 row 변경이 0건이다.
- 자동검증: 과거시각 무동작, 3명 중 예약 성공 2건/실패 1건, 실패 row 기록 실패, 진행·완료 문구, local job 전체 구조, 고정 `updatedAt`, `failed`/`scheduled` filter, history/refresh/busy 순서를 `test:notification-notice-actions`로 고정했다. 실제 화면에 재시험 데이터가 없어도 예약 action의 모든 분기를 fixture로 검증하므로 운영 테스트 데이터를 만들지 않는다. production scenario 431/431과 build, `git diff --check`가 통과했다.
- gate 판정: 외부 함수와 UI setter를 모두 mock했고 실제 API·Solapi 호출 없이 기존 분기와 실행 순서를 재현했다. 사람 검토가 추가로 필요하지 않은 AI gate다.
- 다음 경계: `reconcileSolapiResultsForNoticeJobs`의 guard/loading/성공·부분실패·실패/refresh 순서를 별도 injected action으로 분리할 수 있는지 inventory한다. 취소·삭제는 각각 별도 의미 단위로 유지한다.
...
- 동작 보존: 전체 반에서는 퇴원 학생을 제외하고, 퇴원학생반에서만 휴원/퇴원/퇴원일 학생을 보이며, 검색은 표시 목록만 좁히고 선택 대상은 현재 반 전체를 기준으로 유지한다. 번호가 없는 대상은 발송 수신자에서 제외하는 기존 순서도 유지한다.
- 저장 원천/side effect: model은 `students`, `classTemplates`, local filter/search/selection 값과 normalize 함수만 읽는다. selection 정리 effect, React state, API, Supabase, notification job mutation, Solapi 발송·예약은 App에 남는다.
- 자동검증: 반 ID·이름 매칭, 퇴원 분리, 반 밖 선택 제외, 검색, 학부모/학생 모드, 번호 누락 제외를 `test:notification-center-model`에 추가했다. scenario `20a-5`가 App `useMemo` wiring과 순수 경계를 고정하며 production scenario 427/427, build, `git diff --check`가 통과했다.
- main 통합: 최신 `origin/main`의 수업일지·학생 프로파일 고정 저장 바 변경과 모든 유지보수 검사를 보존한 상태에서 rebase 및 통합 검증했다.
- gate 판정: 읽기 전용 수신자 derivation이며 실제 운영 학생 row를 수정하거나 발송하지 않는다. 테스트 데이터와 사람 조작 없이 AI gate로 통과했다.
- 다음 경계: `buildNoticePayload`와 `buildNoticeJob`의 순수 payload/job 생성 계약을 먼저 fixture로 분리하고, 즉시발송·예약 orchestration은 별도 고위험 단위와 사람 gate 필요성을 다시 판정한다.
...
- 동작 보존: 상태 fallback, 예약시각/생성시각 우선순위, 오류/preview/payload fallback, 예약 취소·삭제 busy 문구, action이 없는 row의 `보관` 표시를 그대로 유지한다.
- 저장 원천/side effect: 취소/삭제 가능 판정, 학생명·상태·종류 helper와 실제 `cancelNotificationJob`/`deleteNotificationJob` handler는 App이 소유하고 주입한다. 새 행에는 hook, API, Supabase 또는 실제 mutation 함수가 없다.
- 자동검증: App wiring과 행의 모든 표시 fallback·callback-only 경계를 `20b-3`으로 추가하고 기존 실제 Solapi 취소 scenario `20e-2`가 새 callback 연결까지 검사하도록 갱신했다. production scenario 424/424와 build, `git diff --check`가 통과했다.
- gate 판정: 실제 취소/삭제 orchestration은 이동하지 않은 presentational row 분리다. 운영 알림 row 조작 없이 AI gate로 통과했다.
- 다음 경계: 발송 기록 header/status/table/collapsed shell을 controlled 컴포넌트로 이동하고 Solapi 결과 대조·filter/open state는 App에 남긴다.
...
- 동작 보존: 각 action의 row 상태를 `saving`으로 바꾼 뒤 기존 callback을 기다리고, 저장된 task/record 기준 pending draft 정리와 `saved` 표시를 적용한다. 실패 시 기존 오류 문구를 row에 남기고 같은 오류를 다시 throw하는 순서를 유지한다.
- 저장 원천/side effect: 실제 task/lesson/출결 저장·Supabase 재조회·React 전역 갱신·notification job/Solapi 작업은 App이 전달하는 네 callback에 남는다. controller에는 직접 API, Supabase, notification/Solapi 호출이 없다.
- 자동검증: 네 action 성공 반환값·호출 순서·pending 정리·성공 문구와 네 실패의 row 오류·rethrow fixture를 `npm run test:production`에 연결했다. 기존 scenario `88b`의 탐색 범위를 새 controller까지 확장하고 `88b-44` 경계 검사를 추가했다. production 410/410, build, `git diff --check`가 통과했고 기존 대형 chunk 경고만 남았다.
- gate 판정: 12R-4~8과 12R-21~25에서 이미 사람/AI 검수한 실제 callback을 바꾸지 않은 얇은 adapter 이동이다. 재시험 데이터, 새 운영 데이터, Solapi 호출 없이 AI gate로 통과했다.
- 다음 경계: `SupplementCenter` 목록 카드의 별도 완료 확인 adapter와 후보·탭 표시 모델을 한 의미 단위씩 inventory한다.
...
- 저장 원천/side effect: 새 모달 파일에는 직접 `fetch`, API path, Supabase 갱신, `setNotificationJobs`가 없다. 실제 task/lesson/출결 저장·재조회·React 전역 갱신·notification job/Solapi 예약·취소는 `SupplementCenter`와 App이 주입하는 callback 경계에 남는다.
...
- 다음 경계: `SupplementCenter`에 남은 보충 목록·선택·실제 callback adapter를 inventory하고, side effect를 이동하지 않는 다음 의미 단위가 있는지 판정한다.
...
- 동작 보존: task ID guard, local draft 값 변경, 현재 task 저장상태 조회, 기존 `createSupplementDraftSaveStatusPatch` 계산, task별 상태 patch 적용 순서를 그대로 유지한다.
- 저장 원천/side effect: React 메모리의 local draft와 저장상태 표시만 갱신한다. API, Supabase, `notification_jobs`, Solapi 및 저장·완료·일정·예약취소 callback을 호출하지 않는다.
- 자동검증: 일정 필드, 알림 문구 필드, 일반 필드의 patch와 update→status 순서, ID guard fixture를 `npm run test:production`에 연결했다. production scenario `88b-41`, production 407/407, build, `git diff --check`가 통과했고 기존 대형 chunk 경고만 남았다.
- gate 판정: 운영 데이터·저장·발송과 무관한 local adapter이고 기존 순수 patch helper를 그대로 사용한다. 사용자 지시에 따라 별도 테스트 데이터나 사람 조작 없이 AI gate로 통과했다.
- 다음 경계: 완료·일정 확인창 submit adapter를 분리한 뒤 `SupplementStudentModal` 본체 파일 분리를 검토한다.
...
- 동작 보존: task 존재/전체 busy guard, `cancelAbsence` busy 시작, 기존 `cancelSupplementAbsenceSourceAction` 호출, 성공 반환, 오류 log, `finally` busy 해제 순서를 그대로 유지한다.
- 저장 원천/side effect: 실제 `lesson_student_records` 저장·Supabase 재조회·React record 갱신·후보 제거는 App이 전달하는 `onCancelAbsenceSource` callback에 남는다. controller에는 직접 API, Supabase, `notification_jobs`, Solapi 호출과 저장·완료·일정·예약취소 callback이 없다.
- 자동검증: 성공 시 record callback→feedback→모달 close→busy 해제, busy/task guard, 실패 시 모달 유지·log/finally fixture를 `npm run test:production`에 연결했다. production scenario `88b-39`, production 405/405, build, `git diff --check`가 통과했고 기존 대형 chunk 경고만 남았다.
- gate 판정: 12R-6에서 동일 callback 경계의 결석 처리 취소·후보 제거·새로고침을 사람/AI가 대조했다. 이번에는 guard와 화면 adapter만 이동했으므로 추가 운영 데이터 없이 AI gate로 통과했다. notification/Solapi 호출과 과금은 0건이다.
- 다음 경계: 개별 알림 예약·취소 modal adapter 한 단위만 분리한다. 11B-9/12R-8의 실제 예약·취소 gate와 동일 callback 경계인지 먼저 대조한다.
...
- 동작 보존: 현재 field의 draft를 우선 표시하고, 저장된 teacher-edited field 또는 이번 local edit field면 확정 수정본으로 판정한다. 현재 control job selector/display formatter와 12R-18 탭 모델을 그대로 조합한다.
- 저장 원천/side effect: task·local draft·job을 읽어 workspace 렌더 props만 계산한다. 문구 저장, job 생성·취소·수정, API, Supabase, Solapi 호출은 없다.
- 자동검증: 선택 config/draft, local teacher-final, 현재 job display, 탭 3종 fixture, production scenario `88b-34`, production 400/400, build, `git diff --check`를 통과했다. 기존 대형 chunk 경고만 남았다.
- gate 판정: 사람 수정본 우선순위와 표시 계산만 순수 분리했고 실제 저장·예약 경계를 건드리지 않았다. 사용자 지시에 따라 AI 검토로 통과하며 별도 사람 조작이나 검수 데이터는 요구하지 않는다.
