...
- 알림톡 제외 계약: `미기재`이면 학부모·학생 미리보기, 예약 payload/fingerprint, 서버의 실제 예약/발송 직전 최신 원천 재조립에서 `과제 상태`, `지난 과제`, `다음 과제`, `미완료 과제`, 숙제 후속 안내를 제외한다. 강의 교재·내용, 테스트, 재시험, 별도 보충 일정, 교사 코멘트는 유지한다. 이미 교사가 직접 작성해 저장한 자유 코멘트의 문장을 자동 삭제하거나 재작성하지 않는다.
...
- AI 검증: `git diff --check`, 서버·알림 경로와 `미기재` 정적 계약, `npm run test:production` 372건, `npm run build`를 통과했다. `미기재`가 명시 값으로 normalize되고 숙제 원천 동기화를 건너뛰며 프론트 미리보기·예약 payload·서버 발송 직전·최종 템플릿 조립에서 과제 블록을 제외하는 경로를 모두 대조했다. 운영 서버 배포를 읽기 전용으로 판별할 수 있도록 `/health.features.lessonJournalNotificationFollowup=result_reconciled_unrecorded_omit` 표식을 추가했다. 기존 Vite chunk size 경고만 남았다.
...
- 사람 gate: 운영 수업일지에서 ① 이미 결과 확인을 마친 지난 수업은 `발송 결과 반영 완료`로 보이고 `발송 결과` 버튼이 사라지는지 확인한다. 실패 건이 있으면 완료 대신 `발송 실패 N건`이어야 한다. ② `발송 전 점검` 버튼이 없는지 확인한다. ③ 테스트 가능한 학생 행에서 `수정 시작 -> 과제 상태 미기재 -> 변경 저장` 후 저장 완료와 새로고침 유지를 확인한다. ④ 학부모·학생 알림톡 작성창을 각각 열어 과제 상태/지난 과제/다음 과제/숙제 후속 안내는 없고 강의·테스트·별도 보충·코멘트는 남는지 확인한다. ⑤ 일반 과제 상태로 되돌려 저장하면 과제 블록이 다시 나타나는지 확인한다. 이 gate에서는 실제 Solapi 예약 업데이트나 발송을 누르지 않는다.
