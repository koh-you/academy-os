...
1. `모달 통일 작업`
   - 상태: 부분 진행 - 보충관리 등원보충 모달 1차 UI 정리, 결석보강 생성/상세 모달 1차 맥락 정리, 결석보강 알림톡 제목/본문 seed 정리, 실제 보강 수업 상세 화면과 생성 모달 문구 통일은 완료. 전체 모달 inventory/공통 구조 통일은 앞으로 해야 할 작업.
...
2. `발송 알림톡 템플릿 설정 관리`
   - 상태: 부분 진행 - 보충관리의 결석보강 학생 11시 리마인더, 숙제보충 학생 11시 리마인더, 보충 일정 확정/변경 안내 문구는 `app_state.aiSettings.notificationTemplates` 원천으로 `설정 > 알림톡`에 노출했다. 나머지 발송 경로 전체 inventory와 설정 이관은 앞으로 해야 한다.
...
### 2026-07-16 P1. 숙제보충 학생 11시 알림톡 문구 정리

- 상태: 완료 - 구현/자동검증 완료
- 사용자 요청: 숙제보충 알림톡도 결석보충과 같은 맥락으로 수정한다. 보충 일시는 `일시:`로, 보충 대상은 `보강 대상:`으로 분리해 보여준다.
- 구현 결과: 숙제보충 학생 11시 리마인더 기본 템플릿 `homeworkMakeupStudentReminder`를 추가하고, `설정 > 알림톡`의 `숙제보충 학생 11시 알림톡` 행에서 변수와 문구를 확인/수정할 수 있게 했다.
- 구현 결과: 새로 생성/재생성되는 숙제보충 초안은 `일시: 7/17(금) 오전 04:00`, `보강 대상: <등원해서 확인할 숙제>`처럼 결석보강과 같은 라벨 구조로 표시한다. 기존 `등원 후 ... 보충을 진행합니다`, `수업 후 ... 보충을 마무리합니다` 문장은 학생 11시 리마인더에서 제거했다.
- 저장 원천: 템플릿 원천은 Supabase `app_state.aiSettings.notificationTemplates`다. 숙제보충 대상 문구는 기존 Supabase `makeup_tasks.note` JSON의 `supplementHomeworkNote/sourceLabel/reason` 순서로 읽는다. 새 SQL은 필요 없다.
- 외부 side effect: 이번 변경은 즉시 발송/예약을 만들지 않는다. 이후 새로 생성하거나 변경 예약하는 숙제보충 학생 11시 리마인더의 payload 문구만 바뀐다. 이미 저장된 사용자 편집 초안이나 기존 `notification_jobs`는 자동 변경하지 않는다.
- 사람 검토 gate: `설정 > 알림톡`에서 `숙제보충 학생 11시 알림톡` 템플릿이 보이고 수정/기본값 복구가 가능해야 한다. 보충관리 숙제보충 상세에서 배정일/시간과 `등원해서 확인할 숙제`를 저장한 뒤 초안을 확인하면 라벨형 `일시`, `보강 대상`만 보여야 한다.
- 중단 조건: 학생 11시 숙제보충 알림톡에 `등원 후 ... 보충을 진행합니다` 또는 `수업 후 ... 보충을 마무리합니다`가 남음, 설정 화면에서 템플릿을 수정했는데 실제 예약 payload가 다른 문구를 사용함, 기존 사용자 편집 초안이 자동으로 덮어써짐.
