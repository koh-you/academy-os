...
1. `모달 통일 작업`
   - 상태: 부분 진행 - 보충관리 등원보충 모달 1차 UI 정리와 결석보강 생성/상세 모달 1차 맥락 정리는 완료. 전체 모달 inventory/공통 구조 통일은 앞으로 해야 할 작업.
   - 목표: 수업 생성/수업 상세/수업일지 관련 모달을 공통 구조로 정리하되, 정규수업, 숙제보충, 결석보충, 시험대비, 특강의 원천과 안내 정보를 잃지 않게 한다.
   - 후속 gate: 결석보강 알림톡 제목/본문 seed 정리, 실제 보강 수업 상세 화면과 생성 모달 문구 통일은 각각 별도 작업으로 진행한다.
