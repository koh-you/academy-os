...
- 상태: 저장 원천 분리, UI 적용, 자동검증과 Chrome 렌더 확인 완료. 운영 배포 후 사람 화면/저장 gate 대기.
...
- 사람 gate: 운영 학생관리에서 테스트 학생 프로파일을 열어 ① `수정` 후 하단에 `기본정보만 저장` 바가 나타나는지, ② 모달 중간/하단으로 스크롤해도 따라오는지, ③ 학생 메모나 기본정보 두 필드를 바꾸면 `기본정보 변경 2개`처럼 표시되는지, ④ 저장 후 `저장 완료`와 새로고침 유지가 확인되는지 본다. ⑤ 상담 또는 운영 알림 draft를 입력한 상태에서 기본정보만 저장해도 편집 모드와 별도 draft가 남고, 해당 영역의 저장 버튼을 누르기 전에는 그 원천이 저장 완료로 표시되지 않아야 한다.
