...
- 학생의 `scheduleOverride`에 포함된 모든 요일별 정규수업과 사이 보충을 하나의 개인 연속성으로 계산하도록 공용 이전·다음 수업 탐색과 메모 선택을 통합했다. 월·수·금 사례와 별도로 화·목·토가 각각 다른 시간·반인 fixture 및 실제 수업일지 safe browser를 추가했다.
- 운영 오연결 숙제는 고친 frontend 배포 뒤 versioned history action으로 현재값 대조 후 삭제하고 재조회한다.
