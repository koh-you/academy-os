...
- 상태: 퇴원 취소 기능 구현·자동검증 완료. 중3 김연우 운영 클리닉 원천은 안전한 범위까지 Supabase 저장·재조회 대조 완료.
- 중복 원천 판정: 일반 학생 Tally로 등록된 `student_intake_tally_xV9pMZ5`가 재원(`active`) 원천이고, 과거 클리닉 신청으로 따로 만들어진 `student_special_lecture_special_lecture_tally_rD01VW5`는 중복 사유로 퇴원(`paused`) 처리된 행이다. 중복 재원을 만들지 않기 위해 옛 클리닉 전용 행은 퇴원 상태로 유지했다.
...
- 사람 gate: ① 배포 후 `학생관리 > 퇴원생 목록`에서 실제 복원이 필요한 테스트 학생의 `퇴원 취소`를 누른다. ② 확인창에 반·미래 수업 명단은 자동 복원하지 않는다는 문구가 보이는지 확인하고 취소를 눌러 데이터가 변하지 않는지 먼저 확인한다. ③ 다시 실행해 확인하면 `Supabase 재조회로 확인` 완료 문구가 보이고 학생이 전체 재원 목록으로 이동하는지 확인한다. ④ 새로고침 뒤에도 재원 상태인지, 반이 자동 배정되지 않았는지 확인한다. ⑤ 김연우는 중복 방지를 위해 옛 클리닉 전용 퇴원 행에 이 버튼을 누르지 않는다. ⑥ `Lesson Hub > 특강관리`에서 김연우의 7월 29일 이후 회차가 일반 Tally 재원 학생으로 보이는지 확인한다. ⑦ 7월 24일 출결과 7월 27일 진행 회차가 그대로면 통과다.
