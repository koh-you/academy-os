...
## 2026-07-28 P1. 알림관리 학부모 응대 맥락

- UI: `운영 > 알림관리 > 학부모 응대`에서 발송 완료된 학부모 데일리 리포트·강사코멘트를 학생별로 읽는다. 당시 Academy OS에 저장된 발신 본문, 수업일, 발송 시각, 학부모 번호를 확인하고 별관 채널에서 찾을 정보를 복사할 수 있다.
- 원천/부작용: 직접 원천은 Supabase `notification_jobs`와 학생 정보의 읽기 전용 조회다. 채널 답장 원문을 수집·저장하지 않으며, 새 알림톡 발송·예약·취소, Solapi 조회/변경, 수업일지·출결·상담 기록 저장을 실행하지 않는다.
- AI 검증: parent response context fixture, `npm run test:production`, `npm run build`, `git diff --check`를 실행한다. 운영 화면에서 실제 발송 완료 기록만 보이고 예약·학생 알림은 제외되며 모바일에서 본문과 복사 버튼을 확인한다.
- 모바일 복구(2026-07-28): 기존 발송 기록의 비표준 시각값 때문에 날짜 표시가 실패해도 전체 화면이 중단되지 않도록 `발송 시각 미입력`으로 안전하게 표시한다.
