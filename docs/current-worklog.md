...
- 저장 완료/부작용 경계: 두 바는 서로의 API를 호출하지 않는다. 각 기존 API가 Supabase 저장 뒤 반환한 run/questions를 화면에 적용하고 목록·상세를 다시 조회하는 계약을 유지한다. 검수 저장은 산출물·promptStudio를 저장 완료로 합산하지 않고, 산출물 저장은 문항 검수·일반 분석을 저장 완료로 합산하지 않는다. OpenAI/Anthropic, Storage, ZIP/TXT, `notification_jobs`, Solapi, 수업일지, 출결을 자동 실행하거나 변경하지 않는다.
- AI 검증: 각 저장 handler가 한 번만 렌더되고 두 독립 `StickySaveBar`가 올바른 handler에 연결되는 production 계약을 추가했다. `git diff --check`, `node --check scripts/scenario-tests-production.cjs`, `npm run test:production` 370건, `npm run build`를 통과했다. 실제 긴 문항 표와 긴 산출물 편집 화면을 `App.css`로 렌더한 Chrome fixture에서 데스크톱 1280×720, 모바일 390×844의 첫 화면·중간·맨 아래 모두 바와 버튼이 viewport 안에 유지됐다. 모바일 상태 설명은 말줄임되고 저장 대상·버튼은 잘리지 않았다. 기존 Vite chunk size 경고만 남았다.
