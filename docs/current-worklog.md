...
4. `App.jsx 리팩터링 연속 작업`
   ...
   - 현재 이어받을 지점: 9번 `test manager`는 완료했다. 10번 `student-parent portals` inventory와 자료함/정적 탭/공용 탭 내비게이션/리포트 카드/학생 지표 행 분리를 완료했으며, 다음은 저장·인증과 무관한 학부모 알림 카드 목록부터 이어간다.
