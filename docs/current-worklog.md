...
- 단가/정산식: 고등학생 월 고정금액 기본값은 450,000원이며 학생별 수정 가능하다. 중학생은 단가표를 받기 전 `단가 미설정`으로 표시하고 총액에 0원 반영한다. 정규와 특강 각각 `기준 총액 × 0.5 × 0.967 × 0.985`를 적용한 뒤 두 예상 수령액을 합산한다.
...
- 사람 gate: ① 운영 사이드바 `운영 > 월별 수업 정산`을 연다. ② 정산월을 이번 달로 두고 고등학생은 450,000원, 중학생은 단가 미설정인지 확인한다. ③ 계속 재원생 한 명의 횟수·시수를 눌러 달력의 실제 출결과 말일까지 예정, 보충이 구분되는지 확인한다. ④ 그 학생의 회차가 많거나 적어도 정규 적용금액이 고정금액 전액인지 확인한다. ⑤ 신입 테스트 학생을 `신입생 · 회차/시수`로 바꾸고 시작일을 지정해 인정 회차·시수와 적용금액이 줄어드는지 확인한다. ⑥ 퇴원생은 1일~퇴원일로 계산되는지 확인한다. ⑦ 스케줄·특강 총액·조정·메모를 수정하고 `월별 정산 저장`을 눌러 `Supabase 재조회 값이 ... 일치` 문구를 확인한 뒤 새로고침해 유지되는지 확인한다. ⑧ 이 과정에서 수업일지·출결·보충·알림관리 데이터가 변하지 않으면 통과다.
- 후속: 사용자가 시수별 단가표를 제공하면 학년·주간 시수별 새 달 기본값을 연결한다. 이미 저장한 월별 학생 금액과 스케줄 스냅샷은 자동으로 덮어쓰지 않는다.
