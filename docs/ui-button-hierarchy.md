...
| `dangerSoftButton` | 확인 모달 **안**에서 실행되는 파괴적 액션의 확정 버튼(삭제 확정, 취소 확정 등)과 그 확인 모달로 **들어가는** 진입 버튼(수업 취소 처리·일정 삭제·영구 삭제 등, 2026-09-19 통일) | 확인 모달당 1개(진입 버튼은 화면당 1개 권장) |
...
1. **파괴적 액션은 기본적으로 확인 모달을 거친다.** 확인 모달의 확정 버튼은 `dangerSoftButton`을 쓴다(취소 버튼은 `softButton`, 왼쪽). 그 모달을 여는 진입 버튼도 `dangerSoftButton`이다(2026-09-19 · 기존 `dangerButton` 진입 4곳을 통일). `dangerButton`은 아래 2번의 확인 없는 인라인 케이스에만 남긴다.
