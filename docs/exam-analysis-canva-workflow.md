...
3. `Canva 10장 재료 보드`
   - 현재 `블로그형 카드 디자인 Gate 3` 명칭은 다음 구현에서 `Canva 재료 미리보기/구조 검수`로 바꾼다.
...
1. 산출물 패널 문구 변경
   - `블로그형 카드 디자인 Gate 3` -> `Canva 재료 미리보기/구조 검수`
   - 설명을 `최종 제작`이 아니라 `Canva 삽입 전 재료 확인`으로 변경

2. 카드별 Canva 재료 보드 추가
   - 각 카드에 고정 영역/교체 영역/source key/export 파일명을 표시
   - 카드 디자인 미리보기보다 체크리스트와 붙여넣기 텍스트를 우선
