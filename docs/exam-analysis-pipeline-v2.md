...
- 완료: 학교/분석 컬럼의 동작하지 않는 `추가` 버튼을 제거하고, PDF 업로드/드롭 시 현재 선택한 학교/학년/고사 기준으로 분석 run이 생성되는 흐름으로 정리했다.
- 완료: PDF 업로드 패널에 드래그 앤 드롭을 추가했고, 시험분석 컬럼 카드 크기를 줄여 테스트 화면 밀도를 높였다.
- 완료: AI 실행 버튼의 괄호 과금 표현을 제거했다.
- 완료: `AI 행 채움` 직후 `AI 결과 검수` 표의 local draft를 새 AI 결과로 다시 seed해, 새로고침 없이도 검수 표에 채움 결과가 보이게 했다.
...
- 미구현: AI 주요문항 추천, 최종 산출물 생성, 이미지 슬롯 연결

## 다음 구현 순서

1. 블로그/인스타/PPT/Canva 이미지 슬롯 설계
2. 필요 시 앱 안 최종 미리보기 표현 보강
3. 필요할 때 AI 주요문항 추천을 선생님 선택 전 초안 기능으로 추가
