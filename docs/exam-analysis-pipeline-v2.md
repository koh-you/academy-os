...
- 완료: 최종 미리보기 팔레트를 전문가 톤으로 조정하고, 최종 미리보기 상세표에서도 난이도를 바로 수정/저장할 수 있게 했다. 저장은 기존 선생님 검수 저장 경로를 사용한다.
...
- 난이도: AI 초안이 틀릴 수 있으므로 `AI 결과 검수` 표와 `최종 미리보기` 상세표에서 선생님이 수정한다. 최종 미리보기의 난이도 select는 기존 `save-question-reviews` API로 저장된다.
