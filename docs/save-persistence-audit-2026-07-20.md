...
현재 이 기준을 원천 저장 범위에서 만족하는 대표 경로는 `시험분석 저장본`과 `특강 학생별 회차 계획`이다. 다만 특강 회차 계획은 실제 수업일지 명단 반영이 별도 버튼으로 분리되어 있어 업무 전체 기준으로는 완료가 아니다.

## 앞으로 통일할 저장 로직과 UI

저장 원천만 보강하고 화면 표현을 그대로 두거나, 버튼 문구만 통일하고 실제 저장 순서를 그대로 두는 방식으로는 완료하지 않는다. 각 기능 단위에서 저장 로직과 UI를 함께 아래 상태 모델로 바꾼다.

| 상태 | 로직 기준 | 공통 UI 기준 |
|---|---|---|
| 편집 중 · 미저장 | 입력은 안정적인 local draft만 변경 | 저장 버튼 활성화, 현재 영역에 `변경사항 있음` 표시 |
| 저장 중 | API 요청 진행, 중복 요청 차단 | 버튼을 `저장 중...`으로 바꾸고 중복 클릭 방지 |
| 원천 저장 확인 중 | 직접 저장 원천을 GET 재조회해 draft와 대조 | `회차 계획 저장 확인 중`처럼 대상을 표시하고 아직 완료로 표현하지 않음 |
| 연결 반영 중 | 수업일지·달력·명단·포털 등 버튼이 약속한 소비 원천을 갱신 | `수업일지 명단 반영 중`과 반영 대상을 표시 |
| 반영 검증 중 | 연결 원천을 재조회하고 화면 기대 결과와 대조 | `최종 반영 확인 중` 표시 |
| 저장·반영 완료 | 직접 원천과 필수 연결 원천의 저장/재조회가 모두 통과 | `회차 계획·수업일지 반영 완료`처럼 실제 완료 범위를 표시 |
| 저장 실패 | 오류 기록, 기존 서버 원본 유지 | 모달/draft 유지, 가까운 위치에 오류와 재시도 버튼 표시 |
| 부분 실패 | 직접 원천 또는 연결 원천 중 일부만 성공 | 성공/실패 원천을 분리해 표시하고 실패 단계만 재시도 제공 |

공통 구현 원칙은 다음과 같다.

1. 저장 버튼과 상태 표시는 현재 편집 중인 모달·행·패널 안에 둔다.
2. 저장 완료 전에 모달을 자동으로 닫거나 화면 목록을 확정값처럼 바꾸지 않는다.
3. localStorage 반영이나 낙관적 state 변경을 저장 성공 근거로 사용하지 않는다.
4. 저장 후에는 같은 페이지 새로고침, 재로그인, 다른 브라우저에서도 같은 값이 보여야 한다.
5. 여러 원천을 함께 저장하면 원천별 결과를 확인하고 부분 성공을 `완료`로 숨기지 않는다.
6. 자동저장이 필요한 화면도 명시적 저장 화면과 같은 상태 이름, 실패 표시, 최신 요청 보장 기준을 사용한다.
7. 단순 닫기·탭 전환·필터 버튼과 저장·반영·발송 버튼은 문구와 시각적 위계를 구분한다.

### 저장·반영 계약

각 버튼은 구현 전에 아래 네 가지를 표로 정리한다.

1. 직접 저장 원천: 사용자가 편집한 값이 최종 저장되는 테이블/key.
2. 연결 반영 원천: 수업일지, 달력, 명단, 포털, 통계처럼 저장 결과를 소비하는 테이블/스냅샷/화면.
3. 외부 side effect: 알림 예약·발송, 파일 업로드, AI 호출, 삭제처럼 같은 버튼에서 자동 실행하면 안 되거나 별도 확인이 필요한 동작.
4. 완료 검증: 직접 원천과 필수 연결 원천 중 무엇을 재조회하고 어떤 값이 같아야 완료인지.

버튼 문구는 이 계약의 실제 범위를 그대로 말해야 한다. `저장`이라고 쓰고 필수 반영이 남아 있거나, `저장 및 반영`이라고 쓰고 직접 원천만 저장하는 상태는 모두 미완성이다. 후속 반영이 업무상 필수라면 화면 하단이나 다른 탭의 별도 버튼으로 숨기지 않고 같은 작업 영역에서 단계와 부분 실패를 보여준다.
...
| 특강 학생별 회차 계획 → 수업일지 명단 | `special_lecture_enrollments` → `lessons` | 회차 원천은 POST 후 `no-store` GET 대조하지만 lesson 명단 반영은 하단 별도 버튼 | B | 원천 저장 완료가 수업일지 반영 완료처럼 보이며, lesson 재조회 전 명단 0명 상태가 남을 수 있음 |
...
1. `특강 회차 계획 저장과 수업일지 명단 반영 통합`
   - enrollment 저장/재조회 뒤 안전한 미래 lesson 명단 반영/재조회를 같은 모달 흐름에서 수행한다.
   - 부분 실패와 잠긴 회차를 분리 표시하고 알림 예약/발송이나 기존 수업기록 삭제는 자동 실행하지 않는다.
2. `수업 생성/수정/복사/취소 저장 gate`
   - 모달/화면을 닫기 전에 API 완료를 기다리고, 실패 시 draft를 유지한다.
   - lesson과 연관 homework/record 변경의 부분 성공을 표시한다.
3. `학생 신규등록/Tally 정식등록/반 명단/퇴원 저장 gate`
   - student와 미래 lesson 명단을 하나의 검토 가능한 결과로 묶는다.
   - 행 `저장됨`은 두 원천이 모두 확인된 뒤에만 표시한다.
4. `app_state key별 dirty 저장 + version`
   - 전체 snapshot 자동저장을 중단하고 변경 key만 저장한다.
   - 서버 updatedAt/version 조건과 충돌 메시지를 추가한다.
5. `exam_prep_rows/Tally 입력 debounce + 최신 요청 보장`
   - 입력마다 POST하지 않고 draft/행 저장 또는 debounce+version을 적용한다.
6. `숙제 체크/포털/자료함/보고서 명시적 상태`
   - 작업 영역 안에 저장 중/완료/실패를 표시하고 실패 시 화면을 복구한다.
7. `보충/알림 다중 원천 reconcile`
