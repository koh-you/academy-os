...
- 진행: 수업일지 상세에 전달되던 저장·삭제·보충 mutation 11개와 알림·AI·예약 조회 provider 6개를 `lessonJournalEffectAdapter`의 `persistence`/`provider` 표면으로 고정했다. adapter는 함수 참조만 조립하고 API·Storage·React state를 소유하지 않으며, 실제 effect owner는 App에 유지한다.
