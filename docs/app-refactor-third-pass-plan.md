...
- 완료: 공지 수신자·작성 draft, 이력·삭제, Solapi 취소/reconcile, request binding과 세 화면 prop 조립을 `useNotificationNoticeController`로 이동했다. `NotificationCenter`는 469줄에서 144줄 render owner가 됐고, App effect callback·API timeout·확인/오류 문구·저장/예약/발송 상태 전이는 유지했다. 이력 탭 전환과 local draft 미리보기를 safe browser로 확인해 3-5를 닫는다.
...
- 완료: 출결·수업 달력과 최종 정규 횟수 편집을 `MonthlySettlementCalendar`, 정규 정산 표·제외 행 복원을 `MonthlySettlementRegularTable`로 물리 분리했다. 기준 main의 JSX 6,304자·9,847자를 각각 문자 단위로 보존했고 두 view는 React state/effect·API·Storage를 소유하지 않는다. `MonthlySettlementPanel`은 184줄 조립 owner, controller는 draft/selector owner, App은 Supabase 저장·재조회 owner로 경계를 닫아 3-6을 완료한다.
...
- 진행: 수업 화면 내부에서만 열리는 보충 전용 상세와 준비 메모 화면을 22.40 kB shared nested lazy chunk로 옮겼다. 13개 pure helper는 frozen runtime으로 주입하고 실제 일정 저장·알림 예약·보충 완료·수업메모 저장 callback owner는 App에 유지했다. main JS는 934.69 kB, gzip은 231.48 kB, App source는 11,000줄·476,602 bytes다.
- 완료: 3-0 대비 초기 main은 1,642.41→934.69 kB로 43.1%, gzip은 422.81→231.48 kB로 45.3% 줄었다. 12개 물리 lazy chunk와 21개 lazy component 경로, App 전용 Babel 500 KB 경고 제거를 수치로 고정했다. 700~900 kB 절대 목표 상단은 34.69 kB 초과하지만, 학생 로그인·핵심 modal을 추가 지연화하는 위험보다 이미 확보한 체감 효과가 크므로 종료 기준의 `동일한 체감 효과 수치 입증`으로 3-7을 닫는다.
