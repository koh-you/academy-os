...
- MV-3b: 조사 결과 범위를 재조정했다(선행 작업 완료, 본 클러스터 추출은 보류). `dedupeExamPrepRowsForDisplay`(App.jsx:1782)·`chooseRepresentativeExamPrepRow`(1769)는 순수하지만, 직접 의존하는 `getExamPrepLogicalKey`(1741)가 모듈 스코프 상수 `currentExamCycle`(App.jsx:1588, `getDefaultExamCycleForDate(today)`로 계산)을 참조하고, `today`(1042)는 `getKoreaDateString()`으로 계산된다. **선행 작업(2026-08-21, PR #190)**: `getKoreaDateString`이 App.jsx·server.js(2벌, 다른 이름 `getKoreaDateStringForAttendance` 포함)·`api/routes/coreData.js`·`src/domains/lessons/attendance.js`·`src/domains/specialLectures/specialLectureGuideUtils.js`·`src/shared/persistence/learningCalendarRowMappers.js`에 총 7벌 중복돼 있음을 확인, 3가지 코드 형태가 실제 호출부 기준 완전히 동치임을 검증한 뒤 `src/shared/utils/koreaDate.js`(AGENTS.md 정답 위치)로 통합했다. `learningCalendarRowMappers.js`만 `test-fourth-pass-row-mapper-baseline.mjs`의 "import 0개" 순수성 계약 때문에 로컬 사본을 예외로 유지한다. `buildExamCalendarEvents`(1992)·`ExamPrepLessonDetail`(8213 부근)까지 포함하면 시험관리·학사달력 순수 함수 ~25개·280줄이 서로 얽혀 있고 전부 같은 `currentExamCycle`/`today` 체인에 걸린다. **다음 단계(미착수)**: `currentExamCycle` 자체의 중복·export 여부를 조사한 뒤, 이 클러스터 전체를 한 번에 옮기는 안전 단위로 재설계한다.
