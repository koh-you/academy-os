...
- MV-3b: 조사 결과 범위를 재조정했다(보류, 사전 작업 필요). `dedupeExamPrepRowsForDisplay`(App.jsx:1782)·`chooseRepresentativeExamPrepRow`(1769)는 순수하지만, 직접 의존하는 `getExamPrepLogicalKey`(1741)가 모듈 스코프 상수 `currentExamCycle`(App.jsx:1588, `getDefaultExamCycleForDate(today)`로 계산)을 참조하고, `today`(1042)는 `getKoreaDateString()`으로 계산되는데 이 함수 자체가 App.jsx 안에 **export 없이** 정의돼 있다(App.jsx:9610, AGENTS.md가 이미 "여러 벌 존재"로 별도 flag한 그 함수). `buildExamCalendarEvents`(1992)·`ExamPrepLessonDetail`(8213 부근)까지 포함하면 시험관리·학사달력 순수 함수 ~25개·280줄이 서로 얽혀 있고 전부 같은 `currentExamCycle`/`today` 체인에 걸린다. 이 클러스터를 안전하게 옮기려면 `getKoreaDateString`을 먼저 `src/shared/utils/`(AGENTS.md 정답 위치)로 옮겨 공유 export로 만드는 선행 작업이 필요하다 — 억지로 지금 옮기면 중복 함수를 하나 더 만들거나(반복 계산이라 값은 같지만 AGENTS.md가 경고하는 바로 그 패턴) 여러 호출부의 함수 시그니처를 위험하게 바꿔야 한다. 의존성 없는 `getExamPrepRowCompleteness`(1750)·`isPlaceholderExamPrepRow`(1765)만 먼저 옮기는 것도 검토했으나 `getExamPrepRowCompleteness`가 App.jsx 다른 8곳에서도 쓰는 `normalizeMathExamEntries`를 호출해 절반만 옮기면 조립이 어색해진다. 다음 세션에서 `getKoreaDateString` 통합을 먼저 하거나, 이 클러스터 전체를 한 번에 옮기는 안전 단위로 재설계한다.
