...
- 쎈 답지(2026-09-18): 개념 쪽 짧은 답이 한 컬럼 안에서 반 컬럼 격자(「14 | 15」「01 ○ 02 ○ 03 ×」)로 실린다 → 격자 열 검출 + 줄 단위 순서. OCR 이 놓친 색 배지는 배지 x 띠의 색 잉크 덩어리(밀도 0.18~0.55)로 보완한다. LaTeX 전사본이 있으면 `scripts/latex-bank/sync-manifest-to-bank.mjs` 로 manifest 를 전사본 문항 목록에 맞춘 뒤 다시 돌린다(번호 밀림 방지).
...
- **사람 지정 상자(`--overrides`, 2026-09-16)**: OCR 이 놓친 풀이는 규칙을 더 깎지 않고 `latex-bank/<책>/answer-overrides.json` 에 쪽·정규화 좌표로 적어 덧씌운다(`"13-01": [{ "page": 4, "box": [x0, y0, x1, y1], "answer_line": true }]` · 여러 조각은 순서대로 · `verified_mismatches` 는 번호 오독이지만 내용이 맞다고 확인한 것). 위·아래는 잉크에 맞춰 조이고, 지정한 문항은 정렬 결과의 해설·답 줄을 버리고 새 조각으로 바꾼다 — 그래서 「컬럼 맨 위 풀이가 앞 문항에 이어 붙은」 오류도 앞 문항을 자기 상자로 다시 지정해 자른다. 결과는 `qa/override-pNNN.jpg`(파랑 상자)로 본다. 베이직쎈 1~3단원 답지(2~37쪽): 해설 못 찾은 48 → 0, 답 줄 270 → 334, 번호 불일치 17 → 0(13건은 상자 지정으로 고침 · 4건은 확인), 지정 122개.
