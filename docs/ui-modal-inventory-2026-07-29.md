...
`UI-5C-1`에서는 읽기 전용 모달 footer와 닫기 행동을 inventory한다. 인쇄/수업일지 열기처럼 실제 행동이 있는 모달은 공통 footer로 옮기기 전에 기존 버튼 순서와 print 예외를 정적 계약으로 고정한다.

## 사람 검수

필수 중단 gate는 없다. 대표 모달의 dialog 이름과 네 개 긴 읽기 모달에서 header 고정·본문 내부 스크롤·desktop/390px 높이를 확인한다. PDF 인쇄, 보충 완료 복귀, 수업일지 열기는 누르지 않는다.
