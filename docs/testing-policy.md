...
## 시험분석·SNS 스튜디오 기기 범위 · 2026-09-07

사용자는 현재 두 스튜디오를 데스크톱에서만 사용한다. 해당 UI의 검증 기준은 데스크톱 실제 가용 폭, 자료·편집·미리보기 배치, 키보드 조작, 저장·재조회와 입력 보존이다. 모바일 전용 최적화·검사를 별도 필수 Gate로 추가하지 않는다. 이미 있는 반응형 코드·회귀 검사는 유지할 수 있으나 데스크톱 사용성을 제약하는 기준으로 삼지 않는다. 학생 태블릿·학부모 모바일 우선 원칙은 해당 화면에만 유지한다.
