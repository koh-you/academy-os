...
| UI-2E | 진행 | 2E-1 읽기 전용, 2E-2 선택·보기 완료. 2E-3A 로컬 draft 행동부터 계속 |
...
| UI-2E | 모든 `sectionHeader`, `Toolbar`, `*Actions` | 섹션 제목/설명/건수/행동 순서와 compact/slim 규칙 통일 | 진행 — 2E-1/2 완료. 2E-3A 로컬 draft 행동, 2E-3B side-effect 행동 순 |
...
다음은 `UI-2E-3A 로컬 draft 행동 작업 바`다. 남은 원시 머리말 중 입력 초안 변경, 접힘/펼치기, 모달 열기처럼 서버 side effect 전 단계만 공통 actions 구조로 이관한다. 실제 저장·삭제·취소·발송 callback은 UI-2E-3B까지 옮기지 않는다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
