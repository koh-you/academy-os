...
다음은 `UI-4A 지표·요약 카드`다. 숫자·label·보조 설명·상태 badge를 표시하는 카드의 계산식과 원천을 inventory로 고정한 뒤 공통 `MetricCard` 표현을 주요 화면부터 적용한다. 집계·정산·저장·발송 계산은 바꾸거나 실행하지 않는다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
