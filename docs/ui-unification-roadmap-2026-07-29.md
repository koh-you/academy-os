...
> 최신 상태: `UI-4C-1` 표 인벤토리와 알림·학생 표 shell은 완료했다. 현재 다음 작은 목표는 `UI-4C-2 수업 상세·수업일지·시험정보 표 shell`이며, row key·editable record·filter·저장/삭제/발송 callback을 유지해 가로스크롤 region만 공통화한다.
