...
| UI-4D | 빈 화면·로딩·오류 | `EmptyState` 기반 문구, 재시도, 다음 행동 위치 통일 | 진행 — inventory와 포털 읽기 상태 완료, 재시도 callback 불변 |
...
> 최신 상태: `UI-4C` 데이터 표 공통화와 `UI-4D-1A` 포털 읽기 빈 상태를 완료했다. 현재 다음 작은 목표는 `UI-4D-1B callback 없는 일반 목록·표 0건 상태`이며, 기존 목록 판정과 행동은 바꾸지 않는다.

`docs/ui-state-inventory-2026-07-29.md`를 기준으로 운영 알림, 학생 목록, 수업·시험·오답 등 callback 없는 0건 표현만 공통 구조로 이관한다. 검색어에 따른 다음 행동, loading/error, retry는 후속 단위로 남긴다. 저장·발송·삭제·출결 변경은 실행하지 않는다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
