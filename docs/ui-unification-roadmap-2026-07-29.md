...
> 최신 상태: `UI-0`~`UI-7`과 `UI-8A-0·1`을 완료했다. 현재 다음 작은 목표는 `UI-8A-2 월간 달력 semantics`이며, 실제 저장·삭제·발송·출결 실행은 하지 않는다.

월간 달력의 grid·요일·날짜 cell에 현재 선택 날짜와 구조 semantics를 제공하되 숨긴 header와 날짜/수업 선택 callback은 변경하지 않는다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
