...
다음은 `UI-3A 상위 탭`이다. 알림·시험·설정·학생·정산의 상위 탭 inventory를 먼저 만들고 선택 state/setter와 렌더 조건을 고정한 뒤, 공통 높이·active/focus·좁은 화면 가로 스크롤 규칙을 적용한다. 탭 전환 외 실제 저장·발송·삭제 행동은 실행하지 않는다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
