...
| UI-2E | 진행 | 2E-1/2/3A와 2E-3B-1 완료. 2E-3B-2 운영 side-effect 작업 바로 계속 |
...
| UI-2E | 모든 `sectionHeader`, `Toolbar`, `*Actions` | 섹션 제목/설명/건수/행동 순서와 compact/slim 규칙 통일 | 진행 — 2E-1/2/3A/3B-1 완료. 2E-3B-2 운영 행동 16개 남음 |
...
다음은 `UI-2E-3B-2 운영 side-effect 작업 바`다. 남은 16개의 callback·인자·disabled·상태 표현을 그대로 `SectionHeader` slot에 전달한다. 실제 버튼 클릭이나 운영 원천 변경은 하지 않는다. 이 단위가 끝나면 원시 `sectionHeader` 0개를 확인하고 UI-3로 진행한다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
