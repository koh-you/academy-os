...
| UI-5C | 모달 footer | 취소·보조·저장·발송·위험 행동 순서와 disabled/진행 상태 통일 | 진행 — 읽기/출력 footer 완료, 단일 원천 후속 |
...
> 최신 상태: `UI-4` 전체와 `UI-5A`, `UI-5B-1~2`, `UI-5C-1`을 완료했다. 현재 다음 작은 목표는 `UI-5C-2 단일 원천·진입형 모달 footer`이며, 저장·발송·삭제·출결 모달은 이 단위에서 제외한다.

`docs/ui-modal-inventory-2026-07-29.md`에서 학사일정 날짜 조회/등록 진입, 시험대비 상세, 최근 보충 이력처럼 저장을 직접 실행하지 않는 footer 행동을 비교한다. 저장·삭제 callback이 함께 있는 모달은 UI-5D/UI-6로 남기고 기존 callback·문구·disabled를 먼저 정적 계약으로 고정한다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
