...
| UI-2E | 진행 | 2E-1 읽기 전용 섹션 머리말 완료, 2E-2 검색·필터·선택 작업 바부터 계속 |
...
| UI-2E | 모든 `sectionHeader`, `Toolbar`, `*Actions` | 섹션 제목/설명/건수/행동 순서와 compact/slim 규칙 통일 | 진행 — 2E-1 읽기 전용 요약 완료. 2E-2 선택 도구, 2E-3 side-effect 행동 순 |
...
다음은 `UI-2E-2 검색·필터·선택 작업 바`다. 인벤토리에서 검색·필터·대상 선택만 수행하는 영역을 골라 공통 meta/actions 구조로 이관하고, state setter·검색 normalization·결과 집합을 정적 대조한다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
