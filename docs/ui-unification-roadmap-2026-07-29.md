...
다음은 `UI-2E 섹션 머리말·작업 바`다. 모든 `sectionHeader`·toolbar·actions를 먼저 inventory하고 제목·설명·건수·행동 순서, `compact`/`slim` 규칙을 작은 화면군부터 통일한다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
