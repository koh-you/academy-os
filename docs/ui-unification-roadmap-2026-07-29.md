...
> 최신 상태: `UI-4C-3B` 오답·학생 성적 표 shell은 완료했다. 현재 다음 작은 목표는 `UI-4C-4 월별·특강 정산과 포털 읽기 성적 표 shell`이며, 정산 row key·계산 snapshot·입력/제외 callback과 포털 표시 값을 유지해 가로스크롤 region만 공통화한다.
