...
| 단위 | 화면/대상 | 목표 | AI 검수 및 gate |
| --- | --- | --- | --- |
| UI-4A | 지표·요약 카드 | 숫자, label, 보조 설명, 상태 badge 위계 통일 | 진행 — 4A-1 단순 카드 완료, 클릭형·포털 micro 지표 남음 |
...
다음은 `UI-4A-2 클릭형·포털 micro 지표`다. 알림 예약 확인, 학생 마이페이지 숙제·출결, 시험분석 최종 미리보기의 계산식·선택 callback을 고정하고 compact/micro `MetricCard` 표현으로 이관한다. Solapi 조회·학생 기록 저장·시험분석 생성은 실행하지 않는다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
