...
> 최신 상태: `UI-4` 전체와 `UI-5A`, `UI-5B-1~2`, `UI-5C-1~3`, `UI-5D-1`을 완료했다. 현재 다음 작은 목표는 `UI-5D-2 결석보강 취소 mode 위험 footer`이며, 실제 저장·발송·삭제·출결 실행은 하지 않는다.

결석보강의 `보강만 취소·원 결석 유지`와 `원 결석 기록 취소` mode별 영향 범위 문구, `isBusy` 닫기/실행 차단, `onCancel`/`onConfirm`을 정적 계약으로 고정한 뒤 공통 위험 footer만 적용한다. Supabase·수업·notification_jobs/Solapi 취소는 실행하지 않는다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
