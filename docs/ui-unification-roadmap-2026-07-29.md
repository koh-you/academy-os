...
| UI-3C | 학교·학년·반·상태·날짜 필터 | label, select/button 배치, 초기화 행동, 결과 건수 위치 통일 | 진행 — 3C-1/2 완료, 날짜/month 필터 남음 |
...
다음은 `UI-3C-3 날짜/month 필터`다. 정산월, 수업·학사 달력월, 시험일 범위의 날짜 계산·월 경계·setter를 먼저 고정한 뒤 이전/다음·현재값·결과 범위를 같은 구조로 정리한다. 저장·발송·삭제 행동은 실행하지 않고, 검색 입력의 지우기·0건 상태는 UI-3D에 남긴다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
