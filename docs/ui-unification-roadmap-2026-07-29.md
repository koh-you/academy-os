...
다음은 `UI-3B 화면 내부 보조 탭`이다. 모달 밖의 보조 탭을 먼저 inventory하고 상위 `WorkspaceTabs`와 다른 compact 위계·현재 위치 표현을 정한다. 선택 state/setter와 탭별 렌더 조건은 그대로 유지하고 실제 저장·발송·삭제 행동은 실행하지 않는다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
