...
| --- | --- | --- | --- |
| UI-2C | 월간 수업 달력, 수업일지 | 이전/다음 탐색, 날짜·수업 맥락, 상태, 수정·취소 행동을 탐색형 header로 분리 | 탐색·수정·취소 callback 불변. 실제 취소 조작 없음 |
| UI-2D | 학사일정, 학생관리, 시험관리, 정산 | 도메인별 상단 제목·요약·주요 행동을 공통 구조로 이관 | 저장 callback과 계산값 불변 |
| UI-2E | 모든 `sectionHeader`, `Toolbar`, `*Actions` | 섹션 제목/설명/건수/행동 순서와 compact/slim 규칙 통일 | 화면별 텍스트·callback snapshot 정적검사 |

### UI-3. 탭·필터·검색·선택 도구

| 단위 | 화면/대상 | 목표 | AI 검수 및 gate |
| --- | --- | --- | --- |
| UI-3A | 알림·시험·설정·학생·정산 상위 탭 | 공통 탭 높이, active 상태, keyboard focus, 좁은 화면 가로 스크롤 | 선택 state/setter 불변 |
| UI-3B | 화면 내부 보조 탭 | 주/보조 탭 위계를 구분하고 중첩 탭의 현재 위치를 명확히 표시 | 탭별 렌더 조건 불변 |
| UI-3C | 학교·학년·반·상태·날짜 필터 | label, select/button 배치, 초기화 행동, 결과 건수 위치 통일 | 필터 함수와 대상 집합 대조 |
...
다음은 `UI-2C 탐색형 머리말`이다. 월간 수업 달력과 수업일지의 현재 JSX/CSS, 이전/다음·뒤로가기·수정·취소 callback을 inventory한 뒤, 동작을 바꾸지 않는 전용 header primitive 범위를 확정한다. 다만 현재 main worktree에 별도 Solapi 작업의 미커밋 변경이 있으므로 `App.jsx`를 겹쳐 수정하지 않는다. 해당 변경이 정리되거나 별도 안전한 UI worktree가 준비된 뒤 구현한다.
