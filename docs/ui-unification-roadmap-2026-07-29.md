...
| UI-3C | 학교·학년·반·상태·날짜 필터 | label, select/button 배치, 초기화 행동, 결과 건수 위치 통일 | 진행 — 3C-1 button filter 완료, select/chip·날짜 필터 남음 |
...
다음은 `UI-3C-2 select/chip 복합 필터`다. 알림 대상 반, 시험 반·고사, 오답 학년·학생, 학사 학교 필터의 option value·normalization·대상 집합과 선택 setter를 고정한 뒤 label·controls·건수/초기화 위치를 공통화한다. 검색 입력은 UI-3D로 남기고 실제 저장·발송·삭제 행동은 실행하지 않는다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
