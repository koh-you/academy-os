...
> 최신 상태: `UI-4` 전체와 `UI-5A`, `UI-5B-1~2`, `UI-5C-1~3`을 완료했다. 현재 다음 작은 목표는 `UI-5D-1 삭제·취소 확인 모달 위험 행동 inventory`이며, 실제 저장·발송·삭제·출결 실행은 하지 않는다.

`docs/ui-modal-inventory-2026-07-29.md`에서 삭제·취소 확인창의 영향 범위, 보존 원천, 취소/최종 실행 순서, disabled·진행 상태를 비교한다. 가장 단순한 단일 원천 확인창부터 공통 위험 footer 후보를 정하되 callback과 문구를 먼저 정적 계약으로 고정한다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
