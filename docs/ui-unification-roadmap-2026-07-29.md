...
> 최신 상태: `UI-4C-3A` 시험분석·예약·테스트 표 shell은 완료했다. 현재 다음 작은 목표는 `UI-4C-3B 오답관리·학생 성적·학원시험 표 shell`이며, record key·입력/수정/삭제 draft와 callback을 유지해 가로스크롤 region만 공통화한다.
