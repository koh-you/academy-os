...
다음은 `UI-3C 필터`다. 학교·학년·반·상태·날짜 필터를 먼저 inventory하고 label·select/button·초기화·결과 건수의 공통 구조를 정한다. 필터 normalization, 결과 집합, 선택 setter는 그대로 유지하고 실제 저장·발송·삭제 행동은 실행하지 않는다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
