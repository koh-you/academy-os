...
| UI-4B | 학생·수업·알림·보충 목록 카드 | 제목, 메타, 상태, 행동 위치와 카드 밀도 통일 | 진행 — 4B-1 대표 shell 완료, 포털 읽기 카드부터 계속 |
...
> 최신 상태: `UI-4B-3` 선택형 행은 완료했다. 현재 다음 작은 목표는 `UI-4B-4 운영 행동 행`이며, 저장·완료·복귀·취소·삭제 callback과 실패 draft 계약을 목록별로 고정한 뒤 실제 동작을 실행하지 않고 shell만 하나씩 이관한다.
