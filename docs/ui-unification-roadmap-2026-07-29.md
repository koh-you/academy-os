...
다음은 `UI-2D 도메인 페이지 머리말`이다. 학사일정·학생관리·시험관리·정산 화면의 상단 제목·요약·주요 행동을 inventory하고, 저장 callback과 계산값을 바꾸지 않는 범위에서 `PageHeader` 적용 대상을 작은 화면군으로 나눈다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
