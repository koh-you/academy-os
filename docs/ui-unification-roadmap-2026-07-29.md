...
> 최신 상태: `UI-4` 전체와 `UI-5A`, `UI-5B-1~2`, `UI-5C-1~3`, `UI-5D-1~3`을 완료했다. 현재 다음 작은 목표는 `UI-5D-4 native confirm 위험 흐름 inventory`이며, 실제 audit·삭제·저장·발송·출결 실행은 하지 않는다.

`window.confirm`을 쓰는 시험정보, 시험분석/PDF, 알림 기록·Solapi 예약, 학사일정, 학생 기록, 특강 삭제·취소 흐름의 원천, 영향 범위, 진행 상태를 표로 분류한다. 이번 UI 프로그램에서 callback 이동 없이 안전하게 모달화할 수 있는 범위와 저장 신뢰성/독립 gate로 남길 범위를 정한다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
