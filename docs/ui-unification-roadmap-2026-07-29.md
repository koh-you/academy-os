...
> 최신 상태: `UI-4` 전체와 `UI-5A`, `UI-5B-1~2`, `UI-5C-1~2`를 완료했다. 현재 다음 작은 목표는 `UI-5C-3 단일 원천·진입형 모달 footer`이며, 저장·발송·삭제·출결 실행부는 이 단위에서 제외한다.

`docs/ui-modal-inventory-2026-07-29.md`에서 학사일정 날짜 조회, 시험대비 상세, 최근 보충 이력의 모달 전체 닫기와 읽기 전용 진입 행동을 비교한다. 행 안의 등록·수정·삭제·완료 복귀 callback은 footer로 이동하지 않고 UI-5D/UI-6에 남긴다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
