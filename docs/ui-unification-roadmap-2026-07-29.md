...
| UI-2E | 진행 | 2E-1/2/3A 완료. 2E-3B 실제 side-effect 작업 바를 두 묶음으로 계속 |
...
| UI-2E | 모든 `sectionHeader`, `Toolbar`, `*Actions` | 섹션 제목/설명/건수/행동 순서와 compact/slim 규칙 통일 | 진행 — 2E-1/2/3A 완료. 2E-3B side-effect 행동만 남음 |
...
다음은 `UI-2E-3B side-effect 작업 바`다. 남은 28개를 시험·AI 생성/다운로드와 운영 저장·삭제·발송 두 묶음으로 나누고, callback·인자·disabled·상태 표현을 그대로 `SectionHeader` slot에 전달한다. 실제 버튼 클릭이나 운영 원천 변경은 하지 않는다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
