...
> 최신 상태: `UI-4`와 `UI-5` 전체, `UI-6A`, `UI-6B-1`을 완료했다. 현재 다음 작은 목표는 `UI-6B-2 저장 주의 공통 톤`이며, 실제 저장·삭제·발송·출결 실행은 하지 않는다.

현재 7개 알림은 모두 미해결 위험과 연결돼 있어 삭제하지 않는다. 공통 `AutosaveRiskNotice`를 실제 실패·삭제의 danger 톤과 구분되는 `저장 주의` 톤으로 정리하되 저장 원천·중단 조건·권장 방향의 접힘 상세와 모든 callback을 보존한다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
