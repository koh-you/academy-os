...
다음은 `UI-3E 선택 행 작업 바`다. 학생·알림·특강·수업 명단처럼 체크/선택 뒤 행동하는 화면의 선택 state, 선택 해제, 건수, callback·disabled·위험 행동을 inventory로 고정한 뒤 공통 배치만 적용한다. 실제 일괄 저장·삭제·발송·등록은 실행하지 않는다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
