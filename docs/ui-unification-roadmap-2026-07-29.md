...
> 최신 상태: `UI-4C`와 `UI-4D`를 완료했다. 현재 다음 작은 목표는 `UI-4E 접힘/펼치기 영역`이며, 기존 open state·toggle callback·저장 draft는 바꾸지 않는다.

접힘/펼치기 영역의 요약 내용, toggle 위치, 기본 open 상태와 접근성 속성을 먼저 inventory한다. 읽기 전용 영역부터 작은 단위로 이관하고, 접힘 안에 저장·발송·삭제 행동이 있더라도 callback과 mount/unmount 의미는 변경하지 않는다. 별도 App.jsx 리팩터링은 UI 프로그램 완료까지 중단한다.
