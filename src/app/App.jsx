...
import { createExamPrepRowIdScope } from "../domains/exams/examPrepRowIdScope.js";
...
export function App() {
  ...
  const examPrepAutoRowClassTemplateId = classTemplates.some((template) => template.classTemplateId === "template_mwf_7_10")
    ? "template_mwf_7_10"
    : "";
  // 자동 생성 id 의 tenant 조각. 보고 있는 tenant(원장의 "다른 선생님으로 보기" 포함) 기준.
  const examPrepRowIdScope = createExamPrepRowIdScope(activeViewTenantId || session?.tenantId);
  useEffect(() => {
    // 반 목록이 아직 없으면(첫 로딩·캐시 초기화 직후) 기다린다 — 이때 돌리면 원장 학생 전원의
    // 행이 생겨 버린다.
    if (classTemplates.length === 0) return;
    setExamPrepRows((current) => {
      const nextRowsToAdd = buildExamPrepRowsFromStudents(students, currentExamCycle, examPrepAutoRowClassTemplateId, current, examPrepRowIdScope);
  ...
  }, [classTemplates.length, examPrepAutoRowClassTemplateId, examPrepRowIdScope, setExamPrepRows, students]);
  ...
  function handleEnsureExamCycleRows(examCycle, classTemplateId) {
    setExamPrepRows((current) => {
      const nextRowsToAdd = buildExamPrepRowsFromStudents(students, examCycle, classTemplateId, current, examPrepRowIdScope);
      const nextRows = mergeById(current, nextRowsToAdd, "examPrepId");
      const addedRows = nextRows.filter((row) => !current.some((item) => item.examPrepId === row.examPrepId));
