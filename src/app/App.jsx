...
export function App() {
  ...
  const calendarLessons = useMemo(
    () => {
      const autoDisplayKeys = new Set(autoGeneratedDisplayLessons.map((lesson) => getGeneratedLessonKey(lesson) || lesson.generatedKey));
      const baseLessons = lessons.filter((lesson) => !autoDisplayKeys.has(getGeneratedLessonKey(lesson)));
      ...
      <div className="schoolCalendarLayout">
        ...
        <section className="panel generatedLessonPreviewPanel">
          <div className="sectionHeader slim">
            <div>
              <h2>시험관리 자동 수업</h2>
              <p className="muted">시험관리의 수학시험 날짜와 시험기간을 기준으로 직전수업과 일요시험보강이 수업일지에 자동 표시됩니다.</p>
            </div>
          </div>
          <div className="generatedLessonSummary">
            <span>자동표시 {generatedPlanCounts.create ?? 0}</span>
            <span>저장됨 {generatedPlanCounts.update ?? 0}</span>
            <span>수정됨 {generatedPlanCounts.protected ?? 0}</span>
            <span>숨김 {generatedPlanCounts.skipped ?? 0}</span>
          </div>
          <div className="emptyHomeworkBox">
            시험관리 입력값이 본데이터입니다. 자동 표시된 수업을 수업일지에서 수정하면 실제 수업으로 저장되고, 삭제하면 해당 자동 수업만 숨김 처리됩니다.
          </div>
        </section>
