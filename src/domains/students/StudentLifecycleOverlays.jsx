...
export function StudentLifecycleOverlays({
  ...
  return (
    <>
      ...
      {handoverStudent ? (
        <ModalComponent
          className="studentHandoverModal"
          onClose={() => setHandoverStudentId("")}
          scrollable
          title={`${handoverStudent.name} ${getStudentHandoverTitle(handoverStudent)}`}
          titleAdornment={(
            <HelpTip
              label={getStudentHandoverTitle(handoverStudent)}
              text="수업·출결·숙제와 Tally 접수정보를 읽어 PDF 인쇄 창을 엽니다. 원천 데이터는 수정하지 않습니다."
            />
          )}
        >
          <div className="deleteConfirmBody">
