...
export function buildStudentMonthlyReportText(model = {}, { audience = "parent", note = "" } = {}) {
  ...
  const lines = [
    ...
    `예정 ${model.summary?.planned ?? 0}회 · 출결 확인 ${model.summary?.actual ?? 0}회 · 변동 ${model.summary?.changes ?? 0}건`,
    "",
    "■ 예정 수업",
    ...(model.plannedRows?.length
      ? model.plannedRows.map((row) => `${formatReportRow(row, { includeClassName: director })}${director ? ` · ${row.attendance}` : ""}`)
      : ["예정된 수업이 없습니다."]),
    "",
    "■ 실제 출결",
    ...(model.actualRows?.length
      ? model.actualRows.map((row) => `${formatReportRow(row, { includeClassName: director })} · ${row.attendance}`)
      : ["확인할 출결이 없습니다."]),
    "",
    "■ 변동사항",
...
function rowsToHtml(rows = [], { emptyText = "내역이 없습니다.", includeAttendance = false, includeClassName = false, includeReason = false } = {}) {
  ...
  return rows.map((row) => `<tr><td>${escapeHtml(row.dateLabel)}</td><td>${escapeHtml(row.timeLabel)}</td><td>${escapeHtml(`${row.typeLabel}${includeClassName && row.className !== row.typeLabel ? ` · ${row.className}` : ""}`)}</td>${includeAttendance ? `<td>${escapeHtml(row.attendance)}</td>` : ""}${includeReason ? `<td>${escapeHtml(row.changeReason)}</td>` : ""}</tr>`).join("");
}

export function openStudentMonthlyReportPdf(model = {}, { audience = "parent", note = "" } = {}) {
  const popup = window.open("", "_blank", "width=960,height=760");
  if (!popup) throw new Error("PDF 인쇄 창을 열지 못했습니다. 팝업 차단을 해제한 뒤 다시 시도해 주세요.");
  popup.opener = null;
  const director = audience === "director";
  const audienceLabel = director ? "원장님용 상세본" : "학부모용 간단본";
  const studentMeta = [model.student?.schoolName, model.student?.grade].filter(Boolean).join(" · ") || "학교·학년 미입력";
  const noteHtml = text(note) ? `<section><h2>${director ? "원장님 공유 메모" : "안내 메모"}</h2><div class="note">${escapeHtml(note)}</div></section>` : "";
  popup.document.write(`<!doctype html><html lang="ko"><head><meta charset="utf-8"><title>${escapeHtml(model.student?.name)} ${escapeHtml(model.monthLabel)} 월간 수업 안내</title><style>@page{size:A4;margin:15mm}body{font-family:Arial,'Malgun Gothic',sans-serif;color:#172554;font-size:11px;line-height:1.5}h1{font-size:22px;margin:0 0 4px}.subtitle{color:#475569;margin:0 0 14px}.meta,.summary{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.meta{grid-template-columns:repeat(2,1fr);background:#f8fbff;border:1px solid #dbeafe;padding:10px}.meta b{color:#1d4ed8}.summary{margin:10px 0}.summary div{background:#eff6ff;border:1px solid #bfdbfe;padding:9px;text-align:center}.summary strong{display:block;font-size:17px;color:#1d4ed8}h2{font-size:14px;border-bottom:1px solid #bfdbfe;padding-bottom:4px;margin:18px 0 7px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #cbd5e1;padding:6px;vertical-align:top}th{background:#eff6ff}section{break-inside:avoid}.note{white-space:pre-wrap;background:#fffbeb;border:1px solid #fde68a;padding:10px}</style></head><body><h1>${escapeHtml(model.student?.name)} ${escapeHtml(model.monthLabel)} 월간 수업 안내</h1><p class="subtitle">${audienceLabel}</p><div class="meta"><div><b>학생</b> ${escapeHtml(model.student?.name)}</div><div><b>학교·학년</b> ${escapeHtml(studentMeta)}</div>${director ? `<div><b>개별 스케줄</b> ${escapeHtml(model.student?.schedule)}</div><div><b>출결 집계</b> 출석 ${model.attendance?.present ?? 0} · 지각 ${model.attendance?.late ?? 0} · 결석 ${model.attendance?.absent ?? 0} · 미입력 ${model.attendance?.pending ?? 0}</div>` : ""}</div><div class="summary"><div><strong>${model.summary?.planned ?? 0}회</strong>예정 수업</div><div><strong>${model.summary?.actual ?? 0}회</strong>출결 확인</div><div><strong>${model.summary?.changes ?? 0}건</strong>변동사항</div></div><section><h2>예정 수업</h2><table><thead><tr><th>날짜</th><th>시간</th><th>수업</th>${director ? "<th>출결</th>" : ""}</tr></thead><tbody>${rowsToHtml(model.plannedRows, { emptyText: "예정된 수업이 없습니다.", includeAttendance: director, includeClassName: director })}</tbody></table></section><section><h2>실제 출결</h2><table><thead><tr><th>날짜</th><th>시간</th><th>수업</th><th>출결</th></tr></thead><tbody>${rowsToHtml(model.actualRows, { emptyText: "확인할 출결이 없습니다.", includeAttendance: true, includeClassName: director })}</tbody></table></section><section><h2>변동사항</h2><table><thead><tr><th>날짜</th><th>시간</th><th>수업</th><th>사유</th></tr></thead><tbody>${rowsToHtml(model.changeRows, { emptyText: "별도 변동사항이 없습니다.", includeClassName: director, includeReason: true })}</tbody></table></section>${noteHtml}<script>window.onload=()=>window.print()</script></body></html>`);
