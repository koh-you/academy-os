...
export function openExamPeriodSundaySchedulePdf(model = {}, { title = "일요시험대비 일정표" } = {}) {
  ...
  const dateSectionsHtml = model.dates?.length
    ? model.dates.map((day) => `<section><h2>${escapeHtml(day.dateLabel)} · ${day.studentCount}명</h2>${day.lessons.map((lesson) => `<div class="lessonBlock"><div class="lessonMeta">${escapeHtml(lesson.sourceLabel)} · ${lesson.studentCount}명</div><table><thead><tr><th>학교</th><th>학생</th><th>시간</th></tr></thead><tbody>${studentRowsToHtml(lesson.students)}</tbody></table></div>`).join("")}</section>`).join("")
    : '<p class="empty">등록된 일요시험대비 일정이 없습니다.</p>';
  popup.document.open();
  popup.document.write(`<!doctype html><html lang="ko"><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>@page{size:A4;margin:12mm}body{font-family:Arial,'Malgun Gothic',sans-serif;color:#172554;font-size:10px;line-height:1.45}h1{font-size:22px;margin:0 0 12px}.printActions{display:flex;justify-content:flex-end;margin:0 0 12px}.printActions button{background:#1d4ed8;border:0;border-radius:6px;color:#fff;font:inherit;font-weight:700;padding:8px 14px}.meta{background:#f8fbff;border:1px solid #dbeafe;color:#1d4ed8;font-weight:800;padding:8px 10px;margin:0 0 12px}h2{font-size:14px;border-bottom:1px solid #bfdbfe;padding-bottom:4px;margin:18px 0 7px}.lessonMeta{color:#1d4ed8;font-weight:800;margin:0 0 4px}table{width:100%;border-collapse:collapse;margin:0 0 10px}th,td{border:1px solid #cbd5e1;padding:6px;vertical-align:top}th{background:#eff6ff}th:last-child,td:last-child{width:90px}section{break-inside:avoid}.empty{color:#64748b}@media print{.printActions{display:none}}</style></head><body><div class="printActions"><button id="exam-period-sunday-schedule-print" type="button">인쇄하기</button></div><h1>${escapeHtml(title)}</h1><div class="meta">총 ${model.totalSessions ?? 0}회 · 연인원 ${model.totalStudents ?? 0}명</div>${dateSectionsHtml}</body></html>`);
