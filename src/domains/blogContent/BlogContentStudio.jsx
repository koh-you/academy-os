...
export function BlogContentStudio({ students = [], scoreRecords = [], postAppState }) {
  ...
  return <main className="blogStudio studioPage">
    <PageHeader title="SNS 스튜디오" eyebrow={getSessionBrandName()} description="블로그·인스타그램 글과 카드 편집" actions={<button className="softButton" disabled={!store.ready || store.busy} onClick={create} type="button">+ 새 콘텐츠</button>} />
    ...
    {draft && <section className="blogEditor" aria-label="콘텐츠 편집">
      ...
      {step === 1 && <div className="studioChatHandoff"><section className="studioTaskGuide"><h2>Chat으로 작업 이어가기</h2><p>자료를 복사해서 원하는 Chat에 붙여 넣으세요. 사진 파일은 Chat에 따로 첨부하고, 글과 디자인을 대화로 완성하세요.</p>{blocker && <p className="blogNotice">{blocker}</p>}<button className="ghostButton" disabled={Boolean(blocker)} onClick={() => copy(prompt)} type="button">Chat 작업용 복사</button><p>Chat에서 최종 JSON을 요청하면 완성본 탭에서 한 번에 가져올 수 있습니다.</p></section><Field label="Chat 전달 자료 미리보기" value={prompt} onChange={() => {}} readOnly multiline rows={20} /></div>}
