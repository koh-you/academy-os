...
export function createSupplementTaskCardViewModel({
  ...
  const isScheduleChangeMode = sourceReconcile
    ? sourceReconcile.isScheduleChangeMode
    : Boolean(task.linkedLessonId);
  const taskMetaParts = [
    task.reason,
    showMethodOptions ? getMethodLabel({ ...task, supplementMethod: draftValues.supplementMethod }) : "",
    `배정 ${task.attemptCount ?? 0}회`
  ].filter(Boolean);
  const scheduleGateBody = sourceReconcile?.scheduleBlockReason || (isScheduleChangeMode
    ? hasScheduleDiff
  ...
  return {
    ...
    scheduleActionProps: {
      isScheduleBlocked: Boolean(sourceReconcile && !sourceReconcile.canApplySchedule),
      scheduleActionLabel: sourceReconcile?.scheduleActionLabel || "",
      scheduleBlockReason: sourceReconcile?.scheduleBlockReason || ""
    },
    saveSummaryProps: {
      draftDiff,
      lessonStatus: sourceReconcile && !sourceReconcile.canApplySchedule
        ? sourceReconcile.lessonStatus
        : saveStatus.lesson || (
            hasScheduleDiff ? "changed" : sourceReconcile?.lessonStatus || (task.linkedLessonId ? "synced" : hasScheduleDraft ? "ready" : "empty")
          ),
    ...
    scheduleGateProps: {
      ...
      ...(sourceReconcile ? { isSourceBlocked: !sourceReconcile.canApplySchedule } : {}),
    ...
    sourceStatusProps: sourceReconcile
      ? {
          detail: sourceReconcile.detail,
          label: sourceReconcile.statusLabel,
          tone: sourceReconcile.statusTone
        }
      : null,
