...
이 파일의 주 용도는 숫자 변형·창의 변형의 원본이다. 그래서 모든 블록에 `mechanism_primary`(풀이 골조 한 줄) · `insights[]`(통찰 유형·depth·근거) · `variation_notes`(바꿔도 되는 수와 제약 · 골조를 유지한 창의 변형과 ★ 변동 지점)를 채웠다. 그림 문항은 12문이며 도로망·색칠·정육면체 그림이다.
...
- id: GN-CM1-236-512
  ...
  premium: false
  rationale: |
    500으로 약분해 x+2y+4z=20 (x,y,z ≥ 1) 로 만든 뒤 z 를 1부터 고정해 각 분기의 y 범위를 센다. 분기가 4개라 단계가 7개 이상(Mₛ 3)이고 「적어도 한 장씩」 하한(T-범위)과 각 분기 상한(T-경계)이 함정. 약분은 절차라 통찰로 세지 않았고, 확인체크 출발 ★2 에서 노동량으로 +1 → ★3.
  tier: star_3
...
- 통찰 유형 분포: I-EQV 9(서로소·지수 하한·단위 환산·동색 묶기) · I-MI 6(경유·동색·경로 길이 분기) · I-BW 2(약수 개수 역추적 · 추가 도로 수) · I-XU 1(판별식) · I-SYM 1(정육면체 대칭). I-SC·I-VF·I-PD·I-CON·I-RT 는 이 범위에 없다.
- M_total: 4~5 가 6문 · 6~7 이 22문 · 8 이 12문 · 9 가 1문. Mₖ 는 대부분 1~2 로, 이 단원의 변별은 계산량이 아니라 **분기를 빠짐없이 나누는가**와 **중복을 제거하는가**에 있다.
- 함정: T-범위(자연수·0 포함·눈의 수 상한)가 32문, T-경계(부등식 상한·중근 등호)가 20문, T-표기(공배수 중복·동류항·서로소)가 14문에 걸쳐 있다.
...
- 대상층: 하위권 7 · 중하위권 17 · 중위권 10 · 중상위권 6 · 상위권 1
