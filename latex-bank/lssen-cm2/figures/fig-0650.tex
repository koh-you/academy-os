% 0650(본책 95쪽) — 세 집합 A·B·C 의 벤 다이어그램 6개(좌변·우변 각 3개, 분배법칙 A∪(B∩C)=(A∪B)∩(A∪C)). 스캔 그림이라 TikZ 로 다시 그림.
% 각 행의 앞 두 그림에만 색칠(A·(B∩C) / (A∪B)·(A∪C)), 세 번째(결과) 그림은 빈칸으로 둠 — 원문 스캔은 세 번째까지 색칠돼 있으나(답) 0649 와 맞춰 비움. 답은 적지 않는다.
\begin{tikzpicture}[scale=0.6, line width=0.5pt, every node/.style={font=\small}]
  ...
  \begin{scope}[shift={(4.5,-4.6)}]
    ...
    \draw (0,0.75) circle (1); \draw (-0.65,-0.4) circle (1); \draw (0.65,-0.4) circle (1);
    \node at (0,1.95) {$A$}; \node at (-1.5,-1.35) {$B$}; \node at (1.5,-1.35) {$C$};
    \node at (0,-2.05) {$(A\cup C)$};
  \end{scope}
  \begin{scope}[shift={(9.0,-4.6)}]
    \draw (0,0.75) circle (1); \draw (-0.65,-0.4) circle (1); \draw (0.65,-0.4) circle (1);
    \node at (0,1.95) {$A$}; \node at (-1.5,-1.35) {$B$}; \node at (1.5,-1.35) {$C$};
    \node at (0,-2.05) {$=(A\cup B)\cap(A\cup C)$};
  \end{scope}
\end{tikzpicture}
