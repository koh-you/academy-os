...
create table if not exists lesson_student_records (
  ...
  homework_followup_method text check (homework_followup_method is null or homework_followup_method in ('next_lesson', 'stay_after')),
  homework_followup_text text,
  homework_followup_source_homework_id text,
  preparation_memo text,
  prep_memo_checked_at timestamptz,
  prep_memo_checked_source_date text,
  prep_memo_checked_source_record_id text,
...
create index if not exists idx_academy_reminders_date_status on academy_reminders(reminder_date, status);
create index if not exists idx_academy_reminders_student_date on academy_reminders(student_id, reminder_date);
create index if not exists idx_academy_reminders_lesson on academy_reminders(lesson_id);
