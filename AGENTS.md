...
- 시험분석·SNS 스튜디오는 사용자 요청(2026-09-07)에 따라 **데스크톱 사용을 설계·검증 기준**으로 삼는다. 넓은 모니터의 작업 배치·가독성·조작성을 우선하며, 모바일 최적화·별도 모바일 검증을 필수 완료 조건으로 추가하지 않는다. 기존 반응형 동작은 불필요하게 제거하지 않는다. 학생·학부모 화면의 기기 우선순위와는 구분한다.
