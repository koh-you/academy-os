/**
 * 회차대조 맹검지 v14 — 재현-1회 ↔ 재현-2회를 맞대어 잰다
 *
...
const 모음 = [...일회.map(([id, t]) => ({ id, t, 갈래: '1회' })),
  ...
  .sort((a, b) => 해시(a.id + 'v14-20260805-after-redesign') - 해시(b.id + 'V14'));

let 맹검 = `---
name: 회차대조-맹검지-34문-v14
description: 우리가 만든 두 회차 34문의 발문 전문. 배점·문항 번호·출처를 지웠다
version: v14 (2026-08-05 세션 115 · 다섯 자리 재설계 뒤 재판정)
...
fs.writeFileSync(R('bank/measurements/회차대조-맹검지-34문-v14.md'), 맹검, 'utf8');
fs.writeFileSync(R('bank/measurements/회차대조-봉인표-34문-v14.md'), 봉인, 'utf8');
console.log(`맹검지 v14 — ${모음.length}문 (1회 ${일회.length} + 2회 ${이회.length})`);
