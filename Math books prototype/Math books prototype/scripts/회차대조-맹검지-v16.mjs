/**
 * 회차대조 맹검지 v16 — 재현-1회 ↔ 재현-2회를 맞대어 잰다
 *
...
const 모음 = [...일회.map(([id, t]) => ({ id, t, 갈래: '1회' })),
  ...
  .sort((a, b) => 해시(a.id + 'v16-20260805-no-proof') - 해시(b.id + 'V16'));

let 맹검 = `---
name: 회차대조-맹검지-34문-v16
description: 우리가 만든 두 회차 34문의 발문 전문. 배점·문항 번호·출처를 지웠다
version: v16 (2026-08-05 세션 115 · 네 번째 판정)
...
fs.writeFileSync(R('bank/measurements/회차대조-맹검지-34문-v16.md'), 맹검, 'utf8');
fs.writeFileSync(R('bank/measurements/회차대조-봉인표-34문-v16.md'), 봉인, 'utf8');
console.log(`맹검지 v16 — ${모음.length}문 (1회 ${일회.length} + 2회 ${이회.length})`);
