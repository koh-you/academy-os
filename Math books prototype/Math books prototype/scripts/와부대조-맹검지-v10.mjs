/**
 * 와부대조 맹검지 v10 — 우리 회차를 .tex 에서 바로 뽑는다
 *
...
const 모음 = [...실기출.map(([id, t]) => ({ id, t, 갈래: '실기출' })),
  ...
  .sort((a, b) => 해시(a.id + 'v10-20260803-d') - 해시(b.id + 'V5'));
...
fs.writeFileSync(R('bank/measurements/와부대조-맹검지-34문-v10.md'), 맹검, 'utf8');
fs.writeFileSync(R('bank/measurements/와부대조-봉인표-34문-v10.md'), 봉인, 'utf8');
console.log(`맹검지 v10 — ${모음.length}문 (실기출 ${실기출.length} + 우리 ${우리.length})`);
