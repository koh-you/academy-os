/**
 * 와부고 실기출 ↔ 우리 재현-2회 — 난이도 급 분포 대조 (v12)
 *
 * v11 과 다른 것은 셋뿐이다.
 *   ① 봉인표가 v12
 *   ② 판정 파일이 `와부대조-판정-2회-V1|V2|V3.md`
 *   ③ 제목이 재현-2회
 *
 * 🔴 칸은 「견준 앵커」로 정한다. 값의 구간으로 칸을 정하지 않는다.
 *    사다리 `bank/anchors/깊이-앵커-사다리-v1.md` — 「점수를 매기지 않는다」.
 *    정본은 앵커다. 값은 곁들이다.
 *
 * 쓰는 법: node scripts/와부대조-집계-v12.mjs
 */
...
const 판정자 = ['V1', 'V2', 'V3']
  .map((n) => [n, `bank/measurements/와부대조-판정-2회-${n}.md`])
...
for (const L of 읽기('bank/measurements/와부대조-봉인표-34문-v12.md').split(/\r?\n/)) {
  const r = L.match(/^\|\s*(W\d\d)\s*\|\s*(실기출|우리)\s*\|\s*([RO]\d\d)\s*\|/);
  if (r) { 갈래.set(r[1], r[2]); 원번호.set(r[1], r[3]); }
}
const Ws = [...갈래.keys()].sort();

// 🔴 판정표에 빠진 줄이 있으면 먼저 알린다 — 조용히 빼고 평균을 내면 값이 어긋난다
...
console.log(`\n실기출 ↔ 재현-2회 난이도 대조 (맹검지 v12 · 판정자 ${판정자.length})`);
