/**
 * 회차대조 맹검지 v17 — 재현-1회 ↔ 재현-2회를 맞대어 잰다
 *
...
const 모음 = [...일회.map(([id, t]) => ({ id, t, 갈래: '1회' })),
  ...
  .sort((a, b) => 해시(a.id + 'v17-20260805-after-review') - 해시(b.id + 'V17'));
...
fs.writeFileSync(R('bank/measurements/회차대조-맹검지-34문-v17.md'), 맹검, 'utf8');
fs.writeFileSync(R('bank/measurements/회차대조-봉인표-34문-v17.md'), 봉인, 'utf8');
console.log(`맹검지 v17 — ${모음.length}문 (1회 ${일회.length} + 2회 ${이회.length})`);
