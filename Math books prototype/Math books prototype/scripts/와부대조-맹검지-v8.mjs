/**
 * 와부대조 맹검지 v8 — 우리 회차를 .tex 에서 바로 뽑는다
 *
...
const 모음 = [...실기출.map(([id, t]) => ({ id, t, 갈래: '실기출' })),
  ...
  .sort((a, b) => 해시(a.id + 'V8-20260803-c') - 해시(b.id + 'V5'));

let 맹검 = `---
name: 대조-맹검지-34문-v8
description: 학교 기출 두 회차 34문의 발문 전문. 배점·문항 번호·출처를 지웠다
version: v8 (2026-08-03 · 15번 v2 반영 뒤 재판정)
...
let 봉인 = `---
name: 대조-봉인표-34문-v8
description: 맹검지 v8 의 W번호 ↔ 원래 회차·문항 번호. 🔴 판정 전에 열지 마라
version: v8 (2026-08-03 · 15번 v2 반영 뒤 재판정)
---
...
fs.writeFileSync(R('bank/measurements/와부대조-맹검지-34문-v8.md'), 맹검, 'utf8');
fs.writeFileSync(R('bank/measurements/와부대조-봉인표-34문-v8.md'), 봉인, 'utf8');
console.log(`맹검지 v8 — ${모음.length}문 (실기출 ${실기출.length} + 우리 ${우리.length})`);
